...
PREREG = (
    ROOT
    / "outputs/analysis/winner_v16_support_action_direction_diagnostic_v2_preregistration.json"
)


def test_v2_preregistration_is_frozen_when_present() -> None:
    ...
    assert value["baseline_reproduction"] == {
        "derived_float_fields": ["terminal", "episode"],
        "finite_absolute_tolerance": 1.0e-12,
...
def test_v2_workflow_is_dormant_until_new_preregistration_exists() -> None:
    workflow = (
        ROOT
    ...
    assert "run_winner_v16_support_action_direction_diagnostic_v2.py" in source
