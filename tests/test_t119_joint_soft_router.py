...
from tools import build_t119_joint_soft_router_cpu_preregistration as build
...
def test_t119_preregistration_when_present() -> None:
    path = ANALYSIS / "t119_joint_soft_router_cpu_preregistration.json"
    if not path.exists():
        return
...
def test_t119_result_when_present() -> None:
    path = ANALYSIS / "t119_joint_soft_router_cpu_result.json"
    if not path.exists():
        return
    value = json.loads(path.read_text(encoding="utf-8"))
    basis = {
