...
def test_t217b_contract_source() -> None:
    builder = (
        ROOT
        / "tools/build_t217b_cost_init_backend_recovery_preregistration.py"
    ).read_text(encoding="utf-8")
    runner = (
        ROOT / "tools/run_t217b_cost_init_backend_recovery.py"
    ).read_text(encoding="utf-8")
...
def test_t217b_result_when_present() -> None:
    path = ANALYSIS / "t217b_cost_init_backend_recovery_result.json"
    if not path.exists():
        return
    value = json.loads(path.read_text(encoding="utf-8"))
    assert value["status"] == "PASS_T217B_COST_INIT_BACKEND_RECOVERY"
