...
def test_t114b_preregistration_when_present() -> None:
    path = ANALYSIS / "t114b_step_zero_attribution_preregistration.json"
    if not path.exists():
        return
...
def test_t114b_result_when_present() -> None:
    path = ANALYSIS / "t114b_step_zero_attribution_result.json"
    if not path.exists():
        return
    value = json.loads(path.read_text(encoding="utf-8"))
    basis = {
