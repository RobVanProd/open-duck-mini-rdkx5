...
PREREG = ROOT / "outputs/analysis/winner_v36_support_oracle_shooting_feasibility_preregistration.json"
...
def test_preregistration_is_exact_when_present() -> None:
    ...
    assert value["status"] == "PREREGISTERED_WINNER_V36_SUPPORT_ORACLE_SHOOTING_FEASIBILITY"
    assert value["decision"] == "AUTHORIZE_ONE_CPU_ONLY_COM_X_NEG_SHOOTING_SCREEN"
    screen = value["screen"]
...
def test_preregistration_executes_nothing_when_built() -> None:
    source = (ROOT / "tools/build_winner_v36_support_oracle_shooting_feasibility_preregistration.py").read_text(
        encoding="utf-8"
    )
