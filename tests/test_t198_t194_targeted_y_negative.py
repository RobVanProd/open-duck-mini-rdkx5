...
def test_t198_preregistration_when_present() -> None:
    path = ANALYSIS / "t198_t194_targeted_y_negative_preregistration.json"
    if not path.exists():
        return
...
def test_t198_result_when_present() -> None:
    path = ANALYSIS / "t198_t194_targeted_y_negative_result.json"
    if not path.exists():
        return
    value = json.loads(path.read_text(encoding="utf-8"))
    passed = value["status"] == "PASS_T198_T194_TARGETED_Y_NEGATIVE"
    assert value["status"] in {
        "PASS_T198_T194_TARGETED_Y_NEGATIVE",
        "HOLD_T198_T194_TARGETED_Y_NEGATIVE",
    }
    assert value["decision"] == (
        "EARN_T199_T194_FULL_R2_PREREGISTRATION_ONLY"
        if passed
        else "CLOSE_T194_CORRECTED_DYNAMIC_REFERENCE_SUPPORT_CONTINUATION"
    )
