...
def test_v119_preregistration_freezes_one_earned_continuation() -> None:
    value = load("winner_v119_hosted_preregistration.json")
    assert (
        value["status"]
    ...
    assert value["training"]["source"] == "exact_v114_final_checkpoint"
    assert value["training"]["transition_match_enabled"] is True
...
def test_v119_package_is_hash_frozen_and_offline() -> None:
    value = load("winner_v119_hosted_package_contract.json")
    assert value["status"] == "PASS_WINNER_V119_HOSTED_PACKAGE"
    assert value["failed_checks"] == []
    assert all(value["checks"].values())
...
def test_v119_launch_contract_allows_one_l4_session_only() -> None:
    value = load("winner_v119_colab_cli_launch_contract.json")
    assert (
        value["status"]
