...
def test_t50_preregisters_distinct_uniform_half_core_endpoints() -> None:
    value = load(
        "t50_uniform_half_core_qualification_preregistration.json"
    )
    assert value["status"] == (
...
def test_t50_result_requires_all_64_cells_if_present() -> None:
    path = ANALYSIS / "t50_uniform_half_core_qualification_result.json"
    if not path.is_file():
        pytest.skip("formal T50 qualification has not run")
