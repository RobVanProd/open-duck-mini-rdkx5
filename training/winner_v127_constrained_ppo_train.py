...
def winner_v127_dual_update(
    dual_lambda: jax.Array,
    dual_eta: jax.Array,
    dual_initial_cost: jax.Array,
    dual_initialized: jax.Array,
    batch_cost: jax.Array,
    quarter_iterations: int,
) -> tuple[jax.Array, jax.Array, jax.Array, jax.Array]:
  """Initialize on the first violating batch, then apply the frozen update."""
  first_positive = jnp.logical_and(
      jnp.logical_not(dual_initialized), batch_cost > 0.0
  )
  safe_initial_cost = jnp.where(first_positive, batch_cost, 1.0)
  derived_eta = 1.0 / (
      jnp.asarray(quarter_iterations, dtype=batch_cost.dtype)
      * safe_initial_cost
  )
  new_initial_cost = jnp.where(
      first_positive, batch_cost, dual_initial_cost
  )
  new_eta = jnp.where(first_positive, derived_eta, dual_eta)
  new_initialized = jnp.logical_or(dual_initialized, first_positive)
  new_lambda = jnp.maximum(0.0, dual_lambda + new_eta * batch_cost)
  return new_lambda, new_eta, new_initial_cost, new_initialized
...
def train(
    ...
    constrained_state_fn: Callable[..., None] = lambda *args: None,
    # checkpointing
    ...
    run_evals: bool = True,
    winner_v127_constrained_cost: bool = False,
):
  """PPO training.
  ...
  dual_quarter_iterations = int(np.ceil(total_training_iterations / 4))
  ...
  if not winner_v127_constrained_cost:
    raise ValueError('winner-v127 constrained train requires its cost channel')

  loss_fn = functools.partial(
      ppo_losses.compute_constrained_ppo_loss,
  ...
  def training_step(
    ...
    dense_cost = data.extras['state_extras'][
        'winner_v127_dense_torque_exceedance_cost'
    ]
    ...
    (
        ...
        training_state.dual_eta,
        training_state.dual_initial_cost,
        training_state.dual_initialized,
        batch_cost,
        dual_quarter_iterations,
    )
    ...
    metrics['winner_v127_dual_lambda_after_update'] = dual_lambda
    metrics['winner_v127_dual_initial_cost'] = dual_initial_cost
  ...
  init_params = ppo_losses.PPONetworkParams(
      ...
      cost_value=ppo_network.value_network.init(key_cost_value),
  )
  ...
  constrained_state_fn(
      current_step,
      _unpmap(training_state.params.cost_value),
      {
          'lambda': float(_unpmap(training_state.dual_lambda)),
          'eta': float(_unpmap(training_state.dual_eta)),
          'initial_cost': float(
              _unpmap(training_state.dual_initial_cost)
          ),
          'initialized': bool(
              _unpmap(training_state.dual_initialized)
          ),
