...
WALKER_SHA="b47f31c2cbb3167dfe0b6411e3c7e914d0189c8c194fa1c59a00734511faa84f"
...
KNEE_KP="34"
SNAPSHOT=""
OUTPUT_DIR="outputs/first_evidence/$(date -u +%Y%m%dT%H%M%SZ)_left_pitch_gain_x0"
RUN=0
PRESENT=0
...
cat <<EOF
Suspended left pitch-chain gain trial:
  command: x=0.00 only
  duration: 15 seconds
  left_hip_pitch P: 31
  left_knee P: 34
  all other runtime gains unchanged
...
REMOTE_JSONL="/home/sunrise/duck_logs/left_pitch_gain_x0.jsonl"
REMOTE_TERMINAL="/home/sunrise/duck_logs/left_pitch_gain_x0_terminal.log"
...
read -r -p "Type RUN_SUSPENDED_X0_GAIN_31_34 to continue: " confirmation
[[ "$confirmation" == "RUN_SUSPENDED_X0_GAIN_31_34" ]] || { echo "Aborted."; exit 1; }
mkdir -p "$OUTPUT_DIR"
cleanup() {
...
remote_run=$(printf 'rm -f %q %q && cd %q && set -o pipefail && printf "\\n" | %q sim2real_diagnostics.py suspended_policy_replay --onnx_model_path %q --command-x 0.0 --duration 15 --action_scale 0.25 --left-hip-pitch-kp %q --left-knee-kp %q --telemetry-path %q --telemetry-every-n 1 --i-understand-this-moves-the-robot 2>&1 | tee %q' \
  "$REMOTE_JSONL" "$REMOTE_TERMINAL" "$RUNTIME/scripts" "$PYTHON" "$POLICY" \
  "$HIP_KP" "$KNEE_KP" "$REMOTE_JSONL" "$REMOTE_TERMINAL")
...
python3 tools/analyze_suspended_replay.py "$LOCAL_JSONL" \
  --terminal-log "$LOCAL_TERMINAL" --output "$OUTPUT_DIR/suspended_x0_gate.md"
cleanup
trap - EXIT
echo "Gain x=0 gate captured. Stop for review; x=0.08 remains unauthorized."
