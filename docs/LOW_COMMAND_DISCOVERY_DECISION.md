...
## Contact-Synchronized Reference Rollout

A stronger stateful phase-retiming diagnostic was then tested:

```text
mode: contact_synchronized_projected
rule: choose the projected reference phase whose expected contacts best match
      the current simulated contacts, with a forward phase-distance tie-break
seeds: 0-7
command: x=0.04
```
...
```text
Contact synchronization solves most of the contact-label mismatch, but it does
not create forward locomotion. The policy/reference problem is therefore not
just "wrong reference phase" or "contact-bit mismatch." Once the reference clock
is retimed to match realized contacts, the motion collapses into mostly
double-support/low-or-negative-forward behavior. The next target-generation
strategy must include forward progress and contact transitions together, not
optimize contact matching alone.
```
