"""Command-aware universal-response prefix for the T10 V121 continuation."""
...
def load_calibrator_onnx(path: str | Path) -> dict[str, np.ndarray]:
    ...
    if hashlib.sha256(source.read_bytes()).hexdigest() != (
        ...
        raise ValueError("T10 response calibrator SHA-256 changed")
    ...
    if [item.name for item in model.graph.input] != [
        ...
        raise ValueError("T10 response calibrator input ABI changed")
    if [item.name for item in model.graph.output] != [
        "calibration_actions",
        "previous_action_out",
        "h_out",
    ]:
        raise ValueError("T10 response calibrator output ABI changed")
    values = {
