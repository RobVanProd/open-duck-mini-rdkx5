"""V121 recurrent actor extended by a zero-initialized response context.

The training actor is the V119/V121 reference-residual recurrent adapter with
two additive context paths.  Both new kernels initialize to exact zero, so an
expanded V121-half checkpoint is action- and state-equivalent before the first
optimizer update.  The exporter owns the complete V121 deployment hierarchy:
...
class ResponseConditionedV121Policy(linen.Module):
    """V121 actor plus zero-initialized context-to-state/action paths."""
    ...
    def __call__(
        ...
        context_hidden = linen.Dense(
            self.recurrent_hidden_size,
            use_bias=False,
            kernel_init=jax.nn.initializers.zeros,
            name="context_hidden_projection",
        )(calibration_context)
...
def export_response_conditioned_v121_onnx(
    ...
    _array(
        ...
        np.asarray(
            p["context_hidden_projection"]["kernel"],
            dtype=np.float32,
        ),
    )
    _array(
    ...
    nodes.extend(
        [
            ...
            helper.make_node(
                "Add",
                ["adapter_hidden_sum", "context_hidden_projected"],
                ["adapter_hidden_context_sum"],
            ),
            helper.make_node(
                "Add",
                ["adapter_hidden_context_sum", "adapter_hidden_bias"],
                ["adapter_hidden_pre"],
            ),
            helper.make_node(
                "Tanh",
            ...
            helper.make_node(
                ...
                ["recurrent_anchored_location"],
            ),
            helper.make_node(
                "Add",
                ["recurrent_anchored_location", "context_location"],
                ["anchored_location"],
            ),
            helper.make_node(
                "Tanh",
