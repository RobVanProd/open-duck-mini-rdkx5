...
def load_hidden_gate_asset(path: str | Path) -> dict[str, np.ndarray | float]:
    """Load and validate the immutable T98 live-hidden gate."""
    path = Path(path)
    value = json.loads(path.read_text(encoding="utf-8"))
    basis = {
        key: item for key, item in value.items() if key != "asset_sha256"
    }
    if (
        value.get("status") != "FROZEN_T98_HIDDEN_GATE_ASSET"
        or value.get("failed_checks")
        or _canonical_sha256(basis) != value.get("asset_sha256")
    ):
        raise ValueError("T98 hidden gate asset identity changed")
    model = value["model"]
    output = {
    ...
    if (
        output["mean"].shape != (64,)
        or output["scale"].shape != (64,)
        or output["coefficient"].shape != (64,)
        or not all(
        ...
        or not np.isfinite(output["intercept"])
        or np.any(output["scale"] <= 0.0)
    ):
        raise ValueError("T98 hidden gate asset values are invalid")
    return output


class T98HiddenExpertPolicy(linen.Module):
    """Protected mature actor plus a gated zero-initialized correction head."""

    action_size: int
    hidden_layer_sizes: Sequence[int]
    recurrent_hidden_size: int
    gate_mean: Sequence[float]
    gate_scale: Sequence[float]
    gate_coefficient: Sequence[float]
    ...
    def __call__(self, normalized_obs, raw_reference_action, hidden_in):
        ...
        negative_adapter_location = linen.Dense(
            self.action_size,
            kernel_init=jax.nn.initializers.zeros,
            bias_init=jax.nn.initializers.zeros,
...
def make_t98_hidden_expert_ppo_networks(
    ...
    hidden_gate_asset_path: str | Path | None = None,
    **kwargs,
    ...
    if hidden_gate_asset_path is None:
        raise ValueError("T98 hidden expert requires its frozen gate asset")
    gate = load_hidden_gate_asset(hidden_gate_asset_path)
    module = T98HiddenExpertPolicy(
        action_size=action_size,
        hidden_layer_sizes=policy_hidden_layer_sizes,
        recurrent_hidden_size=recurrent_hidden_size,
        gate_mean=tuple(float(value) for value in gate["mean"]),
        gate_scale=tuple(float(value) for value in gate["scale"]),
        gate_coefficient=tuple(float(value) for value in gate["coefficient"]),
...
def make_t98_hidden_expert_inference_fn(
...
def install_t98_hidden_expert_collector_hooks() -> None:
    from brax.training import acting

    acting.actor_step = recurrent_adapter_actor_step
    ppo_networks.make_inference_fn = make_t98_hidden_expert_inference_fn
...
def export_t98_hidden_expert_onnx(
    params,
    action_size: int,
    obs_size: int,
    hidden_size: int,
    ...
    hidden_layer_sizes: Sequence[int],
    hidden_gate_asset_path: str | Path,
    ...
    initializers.extend([
        ...
        numpy_helper.from_array(np.asarray(p["negative_adapter_location"]["kernel"], np.float32), name="negative_adapter_weight"),
        numpy_helper.from_array(np.asarray(p["negative_adapter_location"]["bias"], np.float32), name="negative_adapter_bias"),
        numpy_helper.from_array(np.asarray(gate["mean"], np.float32), name="hidden_gate_mean"),
        numpy_helper.from_array(np.asarray(gate["scale"], np.float32), name="hidden_gate_scale"),
        numpy_helper.from_array(np.asarray(gate["coefficient"], np.float32).reshape(64, 1), name="hidden_gate_coefficient"),
        numpy_helper.from_array(np.asarray([gate["intercept"]], np.float32), name="hidden_gate_intercept"),
        numpy_helper.from_array(np.asarray([0.0], np.float32), name="hidden_gate_zero"),
        numpy_helper.from_array(np.zeros((1, action_size), np.float32), name="zero_adapter_location"),
    ])
    nodes.extend([
        ...
        helper.make_node("Sub", ["h_out", "hidden_gate_mean"], ["hidden_gate_centered"]),
        helper.make_node("Div", ["hidden_gate_centered", "hidden_gate_scale"], ["hidden_gate_standardized"]),
        helper.make_node("MatMul", ["hidden_gate_standardized", "hidden_gate_coefficient"], ["hidden_gate_linear"]),
        helper.make_node("Add", ["hidden_gate_linear", "hidden_gate_intercept"], ["hidden_gate_score"]),
        helper.make_node("GreaterOrEqual", ["hidden_gate_score", "hidden_gate_zero"], ["negative_com_gate"]),
        ...
        helper.make_node("Add", ["base_anchored_location", "adapter_location"], ["shared_anchored_location"]),
        helper.make_node("Add", ["shared_anchored_location", "conditional_adapter_location"], ["anchored_location"]),
    ...
    graph = helper.make_graph(nodes, "open_duck_t98_hidden_expert", inputs, outputs, initializer=initializers)
    ...
    network = make_t98_hidden_expert_ppo_networks(
        ...
        hidden_gate_asset_path=hidden_gate_asset_path,
    )
