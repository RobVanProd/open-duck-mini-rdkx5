#!/usr/bin/env python3
"""Freeze corrected read-only attribution of T129's step-zero mismatch."""
...
T130B = ANALYSIS / "t130b_step_zero_serialization_preregistration.json"
...
T130B_RESULT = ANALYSIS / "t130b_step_zero_serialization_result.json"
T130B_WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t130b_step_zero_serialization_recovery"
)
OUTPUT = ANALYSIS / "t130c_step_zero_serialization_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T130C_STEP_ZERO_SERIALIZATION_PREREGISTRATION_20260729.md"
)
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t130c_step_zero_serialization_recovery.py"
TEST = ROOT / "tests" / "test_t130c_step_zero_serialization.py"
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T130C: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T130C preregistration requires clean worktree")

    t130 = json.loads(T130.read_text(encoding="utf-8"))
    t130b = json.loads(T130B.read_text(encoding="utf-8"))
    ...
    checks = {
        "t130_single_hold_exact": (
            ...
            and t130["failed_checks"] == [
                "step_zero_raw_onnx_byte_exact"
            ]
        ),
        "t130b_precomparison_schema_failure_preserved": (
            t130b["status"]
            == "PREREGISTERED_T130B_STEP_ZERO_SERIALIZATION_RECOVERY"
            and not T130B_RESULT.exists()
            and not T130B_WORK.exists()
            and all(
                "kind" not in item
                for item in (
                    *t130b["sources"].values(),
                    *t130b["policies"].values(),
                )
            )
        ),
    ...
    if failed:
        raise RuntimeError(f"T130C preregistration checks failed: {failed}")

    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t130c_step_zero_serialization_preregistration.v1"
        ),
        "status": (
        ...
        "correction_from_t130b": {
            "cause": (
                "The T130B verifier called a receipt helper requiring a "
                "kind field, but T130B froze plain file receipts."
            ),
            "precomparison": True,
        ...
        "authority": {
            "one_corrected_read_only_recovery": True,
    ...
    MARKDOWN.write_text(
        "# T130C step-zero serialization preregistration\n\n"
        f"- Status: `{value['status']}`\n"
        "- Correction: file-receipt verifier only; hypothesis unchanged\n"
