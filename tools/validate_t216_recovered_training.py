#!/usr/bin/env python3
"""Validate recovered T216 training on CPU before any behavior."""
...
PREREGISTRATION = (
    ANALYSIS / "t216_axis_complete_tilt_hosted_preregistration.json"
)
...
LAUNCH_CONTRACT = ANALYSIS / "t216_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t217_t216_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T217_T216_RECOVERED_TRAINING_VALIDATION_20260730.md"
...
EVAL_TAGS = (
    ...
    "eval/episode_reward",
    "eval/episode_t215b/original_clipped_reward",
    "eval/episode_t215b/predicted_pitch_risk_rad",
    "eval/episode_t215b/predicted_roll_risk_rad",
    "eval/episode_t215b/tilt_box_excess",
    "eval/episode_t215b/tilt_box_score",
)
TRAINING_TAGS = (
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T217: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T217 validation requires clean worktree")

    recovery_root = args.recovery_root.resolve()
    work = args.extracted_root.resolve() / "t216_axis_complete_tilt_continuation"
    training = work / "training"
    paths = {
        "hosted_result": recovery_root / "t216_result.json",
        "launch_receipt": recovery_root / "t216_launch_receipt.json",
        "recovery_archive": recovery_root / "t216_artifacts.tar.gz",
    }
    ...
    t214b = json.loads(T214B_SELECTION.read_text(encoding="utf-8"))
    ...
    checks = {
        "t216_preregistration_green": (
            prereg["status"]
            == "PREREGISTERED_T216_AXIS_COMPLETE_TILT_HOSTED_CONTINUATION"
            and prereg["decision"]
            == "AUTHORIZE_ONE_HASH_FROZEN_T216_L4_CONTINUATION"
            and prereg["failed_checks"] == []
        ),
        "t215b_cpu_authority_exact": (
        ...
        "t204_and_t214b_authorities_exact": (
            t204["status"]
            ...
            and hosted["validation"]["input_hashes"]["t214b_selection"]
            == sha256(T214B_SELECTION)
        ),
        "package_and_launch_contracts_green": (
            package["status"] == "PASS_T216_AXIS_COMPLETE_TILT_HOSTED_PACKAGE"
            and package["failed_checks"] == []
            and launch["status"] == "PASS_T216_COLAB_CLI_LAUNCH_CONTRACT"
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T216_COLAB_LAUNCH"
    ...
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t217_t216_recovered_training_validation.v1"
        ),
        "status": (
            "PASS_T217_T216_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T217_T216_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
        ...
        "input_hashes": {
            **input_hashes,
            "t216_preregistration": sha256(PREREGISTRATION),
            "t215b_cpu_result": sha256(CPU_RESULT),
            "t204_validation": sha256(T204_VALIDATION),
            "t214b_selection": sha256(T214B_SELECTION),
    ...
    MARKDOWN.write_text(
        "# T217 T216 recovered training validation\n\n"
