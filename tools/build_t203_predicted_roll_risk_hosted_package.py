#!/usr/bin/env python3
"""Build T203's hash-frozen hosted continuation bundle."""
...
MARKDOWN = (
    ANALYSIS / "T203_PREDICTED_ROLL_RISK_HOSTED_PACKAGE_20260730.md"
)
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T203 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T203 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T203 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T203 playground changed")
    if (
        ...
        raise RuntimeError("copied T203 assets changed")
    ...
    preflight_source = "\n".join(
        (
            ...
            "    print('PASS_T203_FULL_IDENTITY_PREFLIGHT')",
            "else:",
    ...
    if (
        ...
        or "PASS_T203_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T203 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t203_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t203_hosted_package.v1",
        "status": "PASS_T203_PREDICTED_ROLL_RISK_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            "preregistration_exact": True,
            "cpu_and_recovery_evidence_exact": True,
            ...
            "predicted_roll_readback_frozen": True,
            "support_objective_absent": True,
    ...
    MARKDOWN.write_text(
        "# T203 predicted-roll-risk hosted package\n\n"
