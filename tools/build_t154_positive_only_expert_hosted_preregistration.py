#!/usr/bin/env python3
"""Preregister one T154 exact-positive-only expert continuation."""
...
DRIVER = ROOT / "tools" / "colab_t154_positive_only_expert.py"
...
STEP_ZERO = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t153_positive_only_expert_cpu_v1/"
    "t100c_half_expected_always_on.onnx"
)
REFERENCE = ANALYSIS / "ground_up_projected_reference_feature_table.npz"
GATE = ANALYSIS / "t98_hidden_gate_asset.json"
OUTPUT = ANALYSIS / "t154_positive_only_expert_hosted_preregistration.json"
MARKDOWN = (
    ANALYSIS
    / "T154_POSITIVE_ONLY_EXPERT_HOSTED_PREREGISTRATION_20260729.md"
)
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T154 prereg: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T154 preregistration requires clean worktree")
    ...
    checks = {
        "cpu_contract_passed_exactly": (
            cpu["status"] == "PASS_T153_POSITIVE_ONLY_EXPERT_CPU_CONTRACT"
    ...
    if failed:
        raise RuntimeError(f"T154 preregistration checks failed: {failed}")

    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t154_positive_only_expert_hosted_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T154_POSITIVE_ONLY_EXPERT_HOSTED_CONTINUATION"
        ),
        "decision": "AUTHORIZE_ONE_HASH_FROZEN_T154_L4_CONTINUATION",
        "repository_commit": subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True
        ).strip(),
        "question": (
            "Does isolating the existing linear expert on exact positive COM "
            "produce two persistent positive-specialist checkpoints that can "
            "be composed without changing the frozen nominal/negative paths?"
        ),
        "causal_basis": {
            "t151": (
                "the first full-R2 failure is exact +0.05 m torso COM for "
                "every moving command, checkpoint, and measured actuator fit"
            ),
            "t153": (
                "the exact-positive-only 1024-step contract passed every "
        ...
        "training": {
            ...
            "training_slot_role": "positive_adapter_location",
        ...
        "post_training": {
            ...
            "extract_only_two_positive_adapter_tensors": True,
            "compose_with_frozen_t143c_nominal_negative_router": True,
            "evaluate_both_new_checkpoints": True,
            "nominal_negative_identity_before_behavior": True,
            "positive_com_endpoint_matrix_first": True,
    ...
    MARKDOWN.write_text(
        "# T154 exact-positive-only expert hosted preregistration\n\n"
