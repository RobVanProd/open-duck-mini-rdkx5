#!/usr/bin/env python3
"""Compose T216 nominal heads into the frozen T164-final graph."""
...
RESULT = ANALYSIS / "t218_t216_postexport_composition_result.json"
MARKDOWN = ANALYSIS / "T218_T216_POSTEXPORT_COMPOSITION_RESULT_20260730.md"
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t218_t216_postexport_composition_v1"
)
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or WORK.exists():
        raise FileExistsError("refusing to overwrite T218 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T218 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T218_T216_POSTEXPORT_COMPOSITION"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T218 preregistration changed")
    ...
    for graph in prereg["graphs"]:
        ...
        destination = WORK / str(step_value) / "t216_composed.onnx"
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t218_t216_postexport_composition_result.v1"
        ),
        "status": (
            "PASS_T218_T216_POSTEXPORT_COMPOSITION"
            if not failed
            else "HOLD_T218_T216_POSTEXPORT_COMPOSITION"
        ),
        "decision": (
            "EARN_T219_T216_NOMINAL_BEHAVIOR_MATRIX_"
            "PREREGISTRATION_ONLY"
            if not failed
            else "HOLD_T216_BEHAVIOR_AND_AUDIT_COMPOSITION"
        ),
    ...
    MARKDOWN.write_text(
        "# T218 T216 post-export composition result\n\n"
