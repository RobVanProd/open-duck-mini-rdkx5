#!/usr/bin/env python3
"""Apply the exact V119 trained-delta deployment hierarchy."""
...
PREREG = ANALYSIS / "winner_v121_deployment_transform_preregistration.json"
CONTRACT = ANALYSIS / "winner_v121_deployment_transform_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V121_DEPLOYMENT_TRANSFORM_CONTRACT_20260724.md"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("V121 transform preregistration changed")
        ...
        rows.append(
            {
                ...
                "final_projection_delta_exact": np.array_equal(
    ...
    value = {
        "schema_version": "winner_v121.deployment_transform_contract.v1",
        "status": (
            "PASS_WINNER_V121_DEPLOYMENT_TRANSFORM_CONTRACT"
            if not failed
            else "HOLD_WINNER_V121_DEPLOYMENT_TRANSFORM_CONTRACT"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner-v121 deployment transform contract\n\n"
        f"Status: `{value['status']}`\n\n"
        "Both V119 post-update actor graphs preserve trained nodes and use "
        "the exact shared trained float32 delta for the final post-G3 rate "
        "projection. No behavior, Gate 5, RDK-X5, or robot authority.\n",
        encoding="utf-8",
    )
    print(value["status"])
    for row in rows:
