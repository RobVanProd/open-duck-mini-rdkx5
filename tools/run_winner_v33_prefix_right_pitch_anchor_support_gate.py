#!/usr/bin/env python3
"""Bind the unchanged reviewed support gate to Winner-v32 half/final artifacts."""
...
SOURCE_SNAPSHOT_SHA256 = "405b61a905a47b4460b849b4329f218c9350b4522dfe03e9a46a6941105b7d2d"
...
def _validate_snapshot(snapshot: Mapping[str, Any], *, expected_stage: str) -> None:
    if smoke is None or training is None or v21 is None or _TRAINING_PREREG is None:
        raise AssertionError("Winner-v33 gate dependencies were not loaded")
    if set(snapshot) != {"parameters", "optimizer", "metadata", "target_mean", "target_std"}:
        raise ValueError("Winner-v33 checkpoint state schema changed")
    ...
    if (
        ...
        or update not in {251, 301}
        or metadata.get("source_completed_updates") != 201
        ...
        or metadata.get("anchor_scale") != 197.3112030029297
        ...
        raise ValueError("Winner-v33 checkpoint metadata changed")
    for name in ("target_mean", "target_std"):
        ...
        if (
            ...
            raise ValueError(f"Winner-v33 {name} schema changed")
    if bool(np.any(np.asarray(snapshot["target_std"]) <= 0.0)):
        raise ValueError("Winner-v33 target_std is not strictly positive")
    for tree in (
        snapshot["parameters"],
        ...
        if not all(
            ...
            raise ValueError("Winner-v33 checkpoint contains nonfinite arrays")


def load_snapshot_for_reviewed_gate(path: Path) -> dict[str, Any]:
    if v22v2 is None or normalized_support is None:
        raise AssertionError("Winner-v33 mechanics were not loaded")
    exact = v22v2.load_snapshot(path)
    _validate_snapshot(exact, expected_stage="prefix_right_pitch_anchor_joint_stage2")
    adapted = dict(exact)
...
def checkpoint_paths(work_root: Path, label: str) -> tuple[Path, Path]:
    if _TRAINING is None or smoke is None:
        raise AssertionError("Winner-v33 training result was not loaded")
    mapping = {"half": 251, "final": 301}
    if label not in mapping:
        raise ValueError(f"unknown Winner-v33 checkpoint: {label}")
    update = mapping[label]
    snapshot = (
        work_root / "snapshots" / f"snapshot_prefix_anchor_update_{update:03d}.npz"
    )
    graph = work_root / "graphs" / f"winner_v32_{label}.onnx"
    snapshot_receipt = next(
        row
        for row in _TRAINING["snapshot_manifest"]
    ...
    for path, receipt in ((snapshot, snapshot_receipt), (graph, graph_receipt)):
        if (
            ...
            raise ValueError(f"Winner-v33 {label} artifact bytes changed: {path.name}")
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "winner_v33.prefix_right_pitch_anchor_support_gate_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V33_PREFIX_RIGHT_PITCH_ANCHOR_SUPPORT_GATE"
        or value.get("decision")
        != "AUTHORIZE_ONE_FROZEN_WINNER_V33_248_CELL_GATE_ONLY"
        ...
        raise ValueError("Winner-v33 gate preregistration changed")
    ...
    if (
        ...
        raise ValueError("Winner-v33 gate dimensions changed")
    snapshots = {
    ...
    if value.get("training_artifact") != {
        ...
        "half": {
            "snapshot": snapshots[251],
            "graph": checkpoints["half"]["graph"],
        },
        "final": {
            "snapshot": snapshots[301],
            "graph": checkpoints["final"]["graph"],
        },
    }:
        raise ValueError("Winner-v33 gate training-artifact binding changed")
    sources = value.get("sources")
    if not isinstance(sources, dict) or set(sources) != set(builder.SOURCES):
        raise ValueError("Winner-v33 gate sources are absent")
    for name, item in sources.items():
        ...
        if (
            ...
            raise ValueError(f"Winner-v33 gate source changed: {name}")
    if reviewed_gate.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v33 gate source manifest changed")


def main() -> int:
    global reviewed_gate, smoke, training, v21, v22v2, normalized_support
    global builder, _TRAINING, _TRAINING_PREREG
    import build_winner_v33_prefix_right_pitch_anchor_support_gate_preregistration as builder_module
    ...
    if (
        ...
        raise ValueError("Winner-v32 training does not authorize the gate")
    ...
    reviewed_gate.CHECKPOINTS = (("half", 251), ("final", 301))
    ...
    reviewed["sources"] = {
        ...
        "training_preregistration_lf_sha256": smoke.lf_sha256(
