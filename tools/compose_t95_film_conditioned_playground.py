#!/usr/bin/env python3
"""Compose the default-off T95 calibration-FiLM V121 training source."""
...
def replace_once(
    ...
    *,
    ...
    if count != 1:
        raise RuntimeError(
            f"T95 composition {label} expected one match, observed {count}"
        )
...
""",
            """        if self.args.policy_architecture == "response_conditioned_v121":
            from playground.common.t95_film_only_updates import (
                film_only_updates,
            )

            with film_only_updates():
                _, params, _ = train_fn(
                    environment=self.env,
                    eval_env=self.eval_env,
                    wrap_env_fn=wrapper.wrap_for_brax_training,
...
def main() -> int:
    ...
    if not args.execute:
        raise SystemExit("T95 composition requires --execute")
    base = args.base_root.resolve()
    output = args.output_root.resolve()
    if output.exists():
        raise FileExistsError(f"refusing to reuse T95 source: {output}")
    ...
    update_mask_target = common / "t95_film_only_updates.py"
    shutil.copy2(NETWORK_SOURCE, network_target)
    shutil.copy2(WRAPPER_SOURCE, wrapper_target)
    shutil.copy2(UPDATE_MASK_SOURCE, update_mask_target)
    robot_runner = (
    ...
    basis = {
        "schema_version": "open_duck.t95_film_composed_source.v1",
        "base": {
            "path": str(base),
        ...
        "repository_sources": {
            ...
            "update_mask": receipt(UPDATE_MASK_SOURCE),
        },
        "composition": {
            "patch_count": patch_count,
            "default_off": (
                "new calibration-FiLM architecture, response wrapper, and "
                "FiLM-only update mask are inactive unless selected"
            ),
        },
        "final_sources": {
            "network": receipt(network_target),
            "wrapper": receipt(wrapper_target),
            "update_mask": receipt(update_mask_target),
    ...
    manifest_path = output / "T95_COMPOSED_SOURCE_MANIFEST.json"
    ...
    print("PASS_T95_FILM_COMPOSED_SOURCE")
