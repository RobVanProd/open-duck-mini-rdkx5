#!/usr/bin/env python3
"""Build the hash-frozen T32 hosted continuation bundle."""
...
MARKDOWN = (
    ANALYSIS / "T32_ACTION_MARGIN_TRAINTHROUGH_HOSTED_PACKAGE_20260727.md"
)
BUNDLE_NAME = "t32_action_margin_trainthrough_bundle"
MANIFEST_NAME = "t32_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("T32 preregistration changed")
    ...
    if (
        ...
        raise ValueError("T32 package inputs changed")
    ...
    if copied_inventory != observed_inventory:
        raise ValueError("copied T32 playground changed")
    if (
        directory_sha256(assets / SOURCE_CHECKPOINT_NAME)
        != observed["source_checkpoint"]
    ):
        raise ValueError("copied T32 source checkpoint changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T32_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T32_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T32 isolated import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t32_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t32_hosted_package.v1",
        "status": "PASS_T32_ACTION_MARGIN_TRAINTHROUGH_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T32 action-margin train-through hosted package",
                "",
