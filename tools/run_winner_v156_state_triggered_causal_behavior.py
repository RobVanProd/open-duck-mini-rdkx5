#!/usr/bin/env python3
"""Run V156's one-cell state-triggered causal behavior test."""
...
MARKDOWN = (
    ANALYSIS
    / "WINNER_V156_STATE_TRIGGERED_CAUSAL_BEHAVIOR_RESULT_20260725.md"
)
RIGHT_ANKLE = 13
...
def gate_from_graph(path: Path, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    velocity_index = int(
        np.asarray(values["v155_velocity_index"]).reshape(-1)[0]
    )
    threshold = float(
        np.asarray(values["v155_velocity_threshold"]).reshape(-1)[0]
    )
    ...
    return {
        ...
        "active_velocity": obs[active, velocity_index].tolist(),
...
def causal_contract(
    *,
    ...
    checks = {
        "both_traces_complete_600": len(source) == len(candidate) == 600,
        "active_ticks_exact_394_583": active == [394, 583],
        ...
        "action_prefix_before_first_activation_bit_exact": np.array_equal(
            source_action[:first_tick], candidate_action[:first_tick]
        ),
        ...
        "both_activation_velocities_exceed_frozen_threshold": (
            len(gate["active_velocity"]) == 2
            and all(
                value > gate["velocity_threshold"]
                for value in gate["active_velocity"]
            )
        ),
    }
...
def main() -> int:
    ...
    payload = {
        "schema_version": (
            "winner_v156.state_triggered_causal_behavior_result.v1"
        ),
        "status": (
    ...
    MARKDOWN.write_text(
        "# Winner V156 state-triggered causal behavior\n\n"
