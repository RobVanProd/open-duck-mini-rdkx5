#!/usr/bin/env python3
"""Run T222B's frozen nominal/default persistence matrix."""
...
PREREG = ANALYSIS / "t223_global_plateau_nominal_matrix_preregistration.json"
RESULT = ANALYSIS / "t223_global_plateau_nominal_matrix_result.json"
MARKDOWN = ANALYSIS / "T223_GLOBAL_PLATEAU_NOMINAL_MATRIX_RESULT_20260730.md"
CACHE = Path(
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T223 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T223 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T223 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T223 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t223_global_plateau_nominal_matrix_result.v1"
        ),
        "status": (
            "PASS_T223_GLOBAL_PLATEAU_NOMINAL_MATRIX"
            if passed
            else "HOLD_T223_GLOBAL_PLATEAU_NOMINAL_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T223 global plateau nominal matrix result\n\n"
