#!/usr/bin/env python3
"""Run one frozen T32 action-margin train-through GPU continuation."""
...
CPU_RESULT_NAME = "t31_action_margin_trainthrough_cpu_result.json"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T32 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T32 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T32 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(work, arcname="t32_action_margin_trainthrough_continuation")
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t32_hosted_result.v1",
        "status": "RUNNING_T32_ACTION_MARGIN_TRAINTHROUGH_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"T32 runner returned {completed.returncode}")
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T32_ACTION_MARGIN_TRAINTHROUGH_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"T32 artifact checks failed: {failed}")
        ...
        payload["status"] = "HOLD_T32_ACTION_MARGIN_TRAINTHROUGH_CONTINUATION"
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T32 requires hosted GPU authorization")
    return run(
