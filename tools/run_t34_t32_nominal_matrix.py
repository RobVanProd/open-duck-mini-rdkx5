#!/usr/bin/env python3
"""Run T32's preregistered 16-cell corrected nominal matrix."""
...
PREREG = ANALYSIS / "t34_t32_nominal_matrix_preregistration.json"
RESULT = ANALYSIS / "t34_t32_nominal_matrix_result.json"
RESULT_MD = ANALYSIS / "T34_T32_NOMINAL_MATRIX_RESULT_20260727.md"
CACHE_ROOT = (
    Path(r"D:\CodexArtifacts\open-duck-policy")
    / "t34_t32_nominal_matrix_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T34 preregistration")
    if prereg["status"] != "PREREGISTERED_T34_T32_NOMINAL_MATRIX":
        raise ValueError("T34 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T34 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T34 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T34 execution requires a clean worktree")
    ...
    RESULT_MD.write_text(
        "\n".join(
            [
                ...
                "",
                (
                    "A pass authorizes only preregistration of the frozen R2 "
                    "robustness ladder. Gate 5 and robot access remain closed."
                ),
                "",
            ]
        ),
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t34_t32_nominal_matrix_result.v1",
        "status": (
            "PASS_T34_T32_NOMINAL_MATRIX"
            if passed
            else "HOLD_T34_T32_NOMINAL_MATRIX"
        ),
        "decision": (
