#!/usr/bin/env python3
"""Preregister T56's exact transfer-stage deployment transforms."""
...
VALIDATION = ANALYSIS / "t56_recovered_training_validation.json"
T31_PREREGISTRATION = (
...
OUTPUT = ANALYSIS / "t58_t56_postexport_preregistration.json"
MARKDOWN = ANALYSIS / "T58_T56_POSTEXPORT_PREREGISTRATION_20260728.md"
RAW_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t56_extracted_20260728/"
    "t56_dynamic_single_support_continuation/transfer_training"
)
EXPECTED_STEPS = [0, 1_003_520, 2_007_040]
SOURCE_PATHS = (
    "tools/run_t58_t56_postexport_transform.py",
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T58 preregistration requires a clean worktree")
    ...
    checks = {
        "t56_recovered_training_validation_green": (
            validation["status"]
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t58_t56_postexport_preregistration.v1"
        ),
        "status": (
        ...
        "checks": checks,
        "input_hashes": {
            "t56_validation": sha256(VALIDATION),
        ...
        "golden_requirement": (
            ...
            "every deployment sub-contract on every T56 transfer export"
        ),
        "postupdate_steps": EXPECTED_STEPS[1:],
        "decision": (
            "RUN_ONE_CPU_ONLY_T58_POSTEXPORT_TRANSFORM"
            if not failed
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T58 T56 post-export preregistration",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                "- Three transfer exports are hash-exact to T56 CPU validation",
