#!/usr/bin/env python3
"""Apply T120's frozen deployment chain to all three exports."""
...
PREREGISTRATION = ANALYSIS / "t122_t120_postexport_preregistration.json"
RESULT = ANALYSIS / "t122_t120_postexport_result.json"
MARKDOWN = ANALYSIS / "T122_T120_POSTEXPORT_RESULT_20260729.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T122 exact transform requires --execute")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T122 transform requires clean worktree")
    ...
    if (
        prereg["status"] != "PREREGISTERED_T122_T120_POSTEXPORT_TRANSFORM"
        or prereg["failed_checks"]
        or t20.canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise ValueError("T122 preregistration changed")
    for name, expected in prereg["source_hashes"].items():
        if t20.sha256(ROOT / name) != expected:
            raise ValueError(f"T122 source changed: {name}")
    ...
    if raw_steps != EXPECTED_STEPS or raw_hashes != prereg["raw_graphs"]:
        raise ValueError("T122 raw export inventory changed")
    ...
    value = {
        "schema_version": "open_duck.t122_t120_postexport_result.v1",
        "status": (
        ...
        "decision": (
            "EARN_T123_JOINT_SOFT_ROUTER_NOMINAL_PREREGISTRATION_ONLY"
            if not failed
