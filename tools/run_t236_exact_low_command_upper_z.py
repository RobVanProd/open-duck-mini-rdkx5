#!/usr/bin/env python3
"""Run T234B's frozen upper-Z persistence matrix."""
...
PREREG = ANALYSIS / "t236_exact_low_command_upper_z_preregistration.json"
RESULT = ANALYSIS / "t236_exact_low_command_upper_z_result.json"
MARKDOWN = ANALYSIS / "T236_EXACT_LOW_COMMAND_UPPER_Z_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t236_exact_low_command_upper_z_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T236 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T236 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T236 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T236_EXACT_LOW_COMMAND_UPPER_Z"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T236 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t236_exact_low_command_upper_z_result.v1"
        ),
        "status": (
            "PASS_T236_EXACT_LOW_COMMAND_UPPER_Z"
            if passed
            else "HOLD_T236_EXACT_LOW_COMMAND_UPPER_Z"
        ),
        "decision": (
        ...
        "authority": {
            "full_r2_restart_preregistration": passed,
            "training": False,
            "hosted": False,
            "gate5": False,
    ...
    MARKDOWN.write_text(
        "# T236 exact low-command upper-Z result\n\n"
