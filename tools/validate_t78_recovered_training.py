#!/usr/bin/env python3
"""Validate recovered T78 training on CPU before any behavior evaluation."""
...
LAUNCH_CONTRACT = ANALYSIS / "t78_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t78_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T78_RECOVERED_TRAINING_VALIDATION_20260728.md"
...
def main() -> int:
    ...
    paths = {
        "hosted_result": recovery / "t78_result.json",
        "launch_receipt": recovery / "t78_launch_receipt.json",
        "recovery_archive": recovery / "t78_artifacts.tar.gz",
    }
    ...
    checks = {
        ...
        "package_and_launch_contracts_green": (
            ...
            and launch["status"] == "PASS_T78_COLAB_CLI_LAUNCH_CONTRACT"
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T78_COLAB_LAUNCH"
        ...
        "both_exports_only_update_complete_adapter_and_critic": all(
            row["structure_exact"]
            and row["tree_finite"]
    ...
    result = {
        "schema_version": (
            "open_duck.t78_recovered_training_validation.v1"
        ),
        "status": (
            "PASS_T78_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T78_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T79_EXACT_DEPLOYMENT_TRANSFORM_PREREGISTRATION"
            if not failed
        ...
        "classification": {
            ...
            "behavior_cells": 0,
            "actor_update_scope": "complete_recurrent_adapter",
            "endpoint_strata": 8,
            "correction_method": (
                "read-only CPU topology remap using frozen T77 template"
            ),
        },
        "authority": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T78 recovered training validation",
                "",
                f"- Status: `{result['status']}`",
                f"- Decision: `{result['decision']}`",
                f"- Failed checks: `{failed}`",
                "- Exports: `0 / 1,003,520 / 2,007,040`",
                "- Actor scope: `complete recurrent adapter`",
                "- Base actor / normalizer: `bit-exact / bit-exact`",
