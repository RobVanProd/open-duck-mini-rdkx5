#!/usr/bin/env python3
"""Run T194's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t198_t194_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t198_t194_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T198_T194_TARGETED_Y_NEGATIVE_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t198_t194_targeted_y_negative_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T198 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T198 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T198 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T198_T194_TARGETED_Y_NEGATIVE"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T198 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t198_t194_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T198_T194_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T198_T194_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T198 T194 targeted Y-negative result\n\n"
