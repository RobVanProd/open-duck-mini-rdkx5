#!/usr/bin/env python3
"""Run T139's frozen negative-COM endpoint matrix."""
...
RESULT = ANALYSIS / "t141_expert_bank_negative_endpoint_result.json"
MARKDOWN = (
    ANALYSIS / "T141_EXPERT_BANK_NEGATIVE_ENDPOINT_RESULT_20260729.md"
)
CACHE = Path(
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T141 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T141 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T141 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T141 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t141_expert_bank_negative_endpoint_result.v1"
        ),
        "status": (
            "PASS_T141_EXPERT_BANK_NEGATIVE_ENDPOINT_MATRIX"
            if passed
            else "HOLD_T141_EXPERT_BANK_NEGATIVE_ENDPOINT_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T141 expert-bank negative endpoint result\n\n"
