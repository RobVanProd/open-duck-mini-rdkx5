#!/usr/bin/env python3
"""Run T210's one frozen dual roll-cost continuation."""
...
PREREGISTRATION_NAME = "t210_dual_roll_cost_hosted_preregistration.json"
CPU_RESULT_NAME = "t209_dual_roll_cost_cpu_result.json"
T204_VALIDATION_NAME = "t204_t203_recovered_training_validation.json"
T208_SELECTION_NAME = "t208_t203_persistence_autopsy_result.json"
...
def runner_command(
    ...
    """Add only T209's frozen separate-cost mechanism to T170."""
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    t208_path = bundle / T208_SELECTION_NAME
    ...
    t208 = json.loads(t208_path.read_text(encoding="utf-8"))
    basis = {
        key: value
    ...
    if (
        ...
        or t208.get("decision")
        != "EARN_T209_DUAL_ROLL_COST_CPU_CONTRACT_PREREGISTRATION_ONLY"
    ):
        raise ValueError("T210 prerequisite status or identity changed")
    ...
    observed = {
        ...
        "t208_selection": sha256(t208_path),
    ...
    if (
        ...
        raise ValueError("T210 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T210 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(work, arcname="t210_dual_roll_cost_continuation")
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t210_dual_roll_cost_hosted_result.v1",
        "status": "RUNNING_T210_DUAL_ROLL_COST_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"T210 runner returned {completed.returncode}")
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T210_DUAL_ROLL_COST_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"T210 artifact checks failed: {failed}")
        ...
        payload["status"] = "HOLD_T210_DUAL_ROLL_COST_CONTINUATION"
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T210 requires hosted GPU authorization")
    return run(
