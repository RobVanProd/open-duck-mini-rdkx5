#!/usr/bin/env python3
"""Run T78's preregistered 16-cell corrected nominal matrix."""
...
PREREG = ANALYSIS / "t80_t78_nominal_matrix_preregistration.json"
RESULT = ANALYSIS / "t80_t78_nominal_matrix_result.json"
RESULT_MD = ANALYSIS / "T80_T78_NOMINAL_MATRIX_RESULT_20260728.md"
CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t80_t78_nominal_matrix_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T80 preregistration")
    if prereg["status"] != "PREREGISTERED_T80_T78_NOMINAL_MATRIX":
        raise ValueError("T80 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T80 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T80 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T80 execution requires a clean worktree")
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t80_t78_nominal_matrix_result.v1",
        "status": (
            "PASS_T80_T78_NOMINAL_MATRIX"
            if passed
            else "HOLD_T80_T78_NOMINAL_MATRIX"
        ),
        "decision": (
