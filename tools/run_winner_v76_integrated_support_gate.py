#!/usr/bin/env python3
"""Run the frozen Winner-v76 half/final offline support gate."""
...
import build_winner_v76_integrated_support_gate_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v76_integrated_support_gate_preregistration.json"


def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        value.get("schema_version")
        != "winner_v76.integrated_support_gate_preregistration.v1"
        or value.get("status") != "PREREGISTERED_WINNER_V76_INTEGRATED_SUPPORT_GATE"
        or value.get("decision")
        != "AUTHORIZE_ONE_FROZEN_WINNER_V76_248_CELL_GATE_ONLY"
        ...
        raise ValueError("Winner-v76 contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v76 source manifest absent")
    for name, item in sources.items():
        if (
            item.get("hash_mode") != "lf"
            or spec.lf_sha256(ROOT / item["path"]) != item.get("sha256")
        ):
            raise ValueError(f"Winner-v76 source changed: {name}")
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v76 source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v76 transformed source changed")
    return transformed
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.formal_gate_authorized:
        raise PermissionError(
            "Winner-v76 requires --offline-cpu-only --formal-gate-authorized"
        )
    if args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v76 evidence")
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    transformed = validate_contract(contract)
    namespace: dict[str, Any] = {
        "__file__": str(ROOT / "tools/run_winner_v76_integrated_support_gate.py"),
        "__name__": "winner_v76_transformed_support_gate",
    }
