#!/usr/bin/env python3
"""Execute T62's exact bundle in one fresh Colab L4 session."""
...
BUNDLE_NAME = "t62_midpoint_gait_transfer_bundle"
...
PACKAGE_MANIFEST_SHA256 = (
    "07a786aab556524265c7514937d91bf303fa3b692da14cd4573aa1100dd56fee"
)
DRIVER_SHA256 = (
    "efd394db9a76f0d265dcc1ad233acb12150530343f8b782677faad309d19a58c"
)
DRIVER_HELPER_SHA256 = (
    "09bbe4ab7ee42d7f77bb302d7fa8cab15b8734be97f5db80f912ad6619dd808f"
)
WORK = Path("/content/t62_work")
EXTRACT = Path("/content/t62_launch")
OUTPUT_JSON = Path("/content/t62_result.json")
OUTPUT_ARCHIVE = Path("/content/t62_artifacts.tar.gz")
RECEIPT = Path("/content/t62_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T62 requires L4: {gpu.stdout}{gpu.stderr}")
    ...
    if (
        ...
        raise ValueError("T62 uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T62 package contains playground/.tmp")
        archive.extractall(EXTRACT, filter="data")
    bundle = EXTRACT / BUNDLE_NAME
    preregistration = (
        bundle / "t62_midpoint_gait_transfer_hosted_preregistration.json"
    )
    cpu_result = bundle / "t61_midpoint_gait_transfer_cpu_result.json"
    manifest = bundle / "t62_package_manifest.json"
    driver = bundle / "colab_t62_midpoint_gait_transfer_continuation.py"
    ...
    if (
        ...
        raise ValueError("T62 extracted package identity changed")
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"T62 pinned install failed: {completed.returncode}")
    observed_versions = {
        name: importlib.metadata.version(name) for name in EXPECTED_VERSIONS
    }
    if observed_versions != EXPECTED_VERSIONS:
        raise ValueError(f"T62 software versions changed: {observed_versions}")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T62_HOSTED_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T62_HOSTED_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T62 hosted import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    command = [
        sys.executable,
    ...
    receipt = {
        "schema_version": "open_duck.t62_colab_launch_receipt.v1",
        "status": "STARTING_T62_HASH_FROZEN_GPU_CONTINUATION",
        ...
        "hosted_import_preflight": "PASS_T62_HOSTED_IMPORT Joystick",
    ...
    receipt["status"] = (
        "COMPLETED_T62_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T62_COLAB_LAUNCH"
    )
