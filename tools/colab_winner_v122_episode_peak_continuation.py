#!/usr/bin/env python3
"""Run the one frozen Winner-v122 episode-peak GPU continuation."""
...
PREREGISTRATION_NAME = "winner_v122_hosted_preregistration.json"
CPU_RESULT_NAME = "winner_v122_episode_peak_cpu_result.json"
REWARD_MASS_NAME = "winner_v122_cpu_reward_mass_attribution.json"
COMPOSED_MANIFEST_NAME = "WINNER_V122_COMPOSED_SOURCE_MANIFEST.json"
REFERENCE_NAME = "ground_up_projected_reference_feature_table.npz"
SOURCE_CHECKPOINT_NAME = "source_checkpoint"
DRIVER_HELPER_NAME = "colab_winner_v114_linear_torque_continuation.py"
OBJECTIVE_PATCH_NAME = "winner_v122_episode_peak_torque_increment.patch"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    reward_mass_path = bundle / REWARD_MASS_NAME
    ...
    reward_mass = json.loads(
        reward_mass_path.read_text(encoding="utf-8")
    )
    if (
        prereg.get("status")
        ...
        or reward_mass.get("status")
        != "PASS_WINNER_V122_CPU_REWARD_MASS_ATTRIBUTION"
        or reward_mass.get("decision")
        != "PREREGISTER_ONE_V122_HOSTED_CONTINUATION"
    ):
        raise ValueError("Winner-v122 prerequisite status changed")
    observed = {
        "driver": sha256(Path(__file__).resolve()),
        "driver_helper": sha256(
            Path(__file__).resolve().with_name(DRIVER_HELPER_NAME)
        ),
        "objective_patch": sha256(bundle / OBJECTIVE_PATCH_NAME),
        "cpu_result": sha256(cpu_result_path),
        "reward_mass_attribution": sha256(reward_mass_path),
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError(
            f"Winner-v122 inputs changed: {observed} != "
    ...
    if (
        ...
        raise RuntimeError(f"Winner-v122 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(
            work, arcname="winner_v122_episode_peak_continuation"
        )
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "winner_v122.hosted_result.v1",
        "status": "RUNNING_WINNER_V122_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"Winner-v122 runner returned {completed.returncode}"
            )
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_WINNER_V122_HOSTED_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"Winner-v122 artifact checks failed: {failed}")
        ...
        payload["status"] = "HOLD_WINNER_V122_HOSTED_CONTINUATION"
