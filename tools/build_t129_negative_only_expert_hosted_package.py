#!/usr/bin/env python3
"""Build the hash-frozen T129 exact-negative expert hosted bundle."""
...
PREREG = ANALYSIS / "t129_negative_only_expert_hosted_preregistration.json"
...
MARKDOWN = (
    ANALYSIS / "T129_NEGATIVE_ONLY_EXPERT_HOSTED_PACKAGE_20260729.md"
)
BUNDLE_NAME = "t129_negative_only_expert_bundle"
MANIFEST_NAME = "t129_package_manifest.json"
...
def main() -> int:
    ...
    for path in (staging, archive_path, CONTRACT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T129 package: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T129 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T129 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T129 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T129 playground changed")
    if (
        ...
        raise RuntimeError("copied T129 assets changed")
        ...
        [
            ...
            "\n".join(
                (
                    ...
                    "    print('PASS_T129_FULL_IDENTITY_PREFLIGHT')",
                    "else:",
    ...
    if (
        ...
        or "PASS_T129_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T129 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t129_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t129_hosted_package.v1",
        "status": "PASS_T129_NEGATIVE_ONLY_EXPERT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            "preregistration_exact": True,
            "cpu_contract_exact": True,
            ...
            "exact_negative_randomizer_readback_frozen": True,
    ...
    MARKDOWN.write_text(
        "# T129 exact-negative-only expert hosted package\n\n"
