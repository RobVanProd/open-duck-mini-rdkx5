#!/usr/bin/env python3
"""Run T175's frozen Y-negative prefix-mean behavior matrix."""
...
PREREG = ANALYSIS / "t176_head_prefix_mean_targeted_preregistration.json"
RESULT = ANALYSIS / "t176_head_prefix_mean_targeted_result.json"
MARKDOWN = ANALYSIS / "T176_HEAD_PREFIX_MEAN_TARGETED_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t176_head_prefix_mean_targeted_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T176 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T176 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T176 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T176 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t176_head_prefix_mean_targeted_result.v1"
        ),
        "status": (
            "PASS_T176_HEAD_PREFIX_MEAN_TARGETED"
            if passed
            else "HOLD_T176_HEAD_PREFIX_MEAN_TARGETED"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T176 head-prefix-mean targeted result\n\n"
