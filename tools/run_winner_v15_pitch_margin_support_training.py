#!/usr/bin/env python3
"""Run the one preregistered Winner-v15 pitch-margin support training arm."""
...
def validate_source_manifest(value: Mapping[str, Any]) -> None:
    ...
    if not isinstance(sources, dict) or not sources:
        raise ValueError("Winner-v15 support-training source manifest is absent")
    for name, item in sources.items():
        ...
        if (
            ...
            raise ValueError(f"Winner-v15 support-training source changed: {name}")
    if canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v15 support-training source-manifest digest changed")


def validate_preregistration(value: Mapping[str, Any]) -> None:
    if (
        value.get("schema_version")
        != "winner_v15.pitch_margin_support_training_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V15_PITCH_MARGIN_SUPPORT_TRAINING"
        or value.get("decision")
        != "AUTHORIZE_ONE_100_UPDATE_PITCH_MARGIN_SUPPORT_RUN_ONLY"
        ...
        raise ValueError("Winner-v15 support-training preregistration changed")
    ...
    if frozen != {
        ...
        raise ValueError("Winner-v15 support-training constants changed")
...
def graph_receipt(
    ...
    if not all(contract[name] for name in required):
        raise ValueError(f"Winner-v15 pitch-margin {label} ONNX contract failed")
...
def main() -> int:
    ...
    parser.add_argument("--pitch-margin-support-training-authorized", action="store_true")
    args = parser.parse_args()
    if not args.offline_cpu_only or not args.pitch_margin_support_training_authorized:
        raise PermissionError(
            "pitch-margin support training requires --offline-cpu-only "
            "--pitch-margin-support-training-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite pitch-margin training evidence")
    ...
    if jax.default_backend() != "cpu" or any(
        ...
        raise ValueError("Winner-v15 pitch-margin training requires CPU-only JAX")
    preregistration = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    validate_preregistration(preregistration)
    cpu_result = json.loads(CPU_RESULT.read_text(encoding="utf-8"))
    if (
        cpu_result.get("status") != "PASS_WINNER_V15_PITCH_MARGIN_CPU_CONTRACT"
        ...
        or cpu_result.get("proof", {}).get("reward", {}).get(
            "reward_formula_bit_exact"
        )
        is not True
    ):
        raise ValueError("Winner-v15 pitch-margin CPU result changed")
    ...
    if len(population) != 80:
        raise ValueError("Winner-v15 pitch-margin training population changed")
    ...
    for update_index in range(UPDATES):
        batch_np, episodes, observations = pitch_margin.stage2_rollout(
            smoke=smoke,
            full=full,
            training=training,
            mujoco=mujoco,
        ...
        reward = pitch_margin.reward_evidence(batch_np)
        if not (
            ...
            and reward["all_rewards_nonnegative"]
            and reward["all_penalties_in_unit_interval"]
        ):
            raise ValueError(f"pitch-margin transition changed at {update_index + 1}")
        if not training.finite_tree({"batch": batch_np, "observations": observations}):
            raise FloatingPointError(f"nonfinite pitch-margin rollout {update_index + 1}")
        ...
        if not training.finite_tree(
            ...
            raise FloatingPointError(f"nonfinite pitch-margin update {update_index + 1}")
        sampled = int(np.sum(batch_np["valid_mask"]))
        valid = int(np.sum(batch_np["valid_transition_mask"]))
        if sampled < valid or valid <= 0:
            raise ValueError(f"invalid pitch-margin sample count at {update_index + 1}")
        ...
        metadata = {
            "schema_version": "winner_v15.pitch_margin_support_training_snapshot.v1",
            "stage": "stage2",
        ...
        if not (
            ...
            raise ValueError(f"pitch-margin snapshot readback failed at {update_index + 1}")
    ...
    result = {
        "schema_version": "winner_v15.pitch_margin_support_training_result.v1",
        "status": (
            "PASS_WINNER_V15_PITCH_MARGIN_SUPPORT_TRAINING_ARTIFACT"
            if not failed
            else "HOLD_WINNER_V15_PITCH_MARGIN_SUPPORT_TRAINING_ARTIFACT"
        ),
        "decision": (
            "AUTHORIZE_SEPARATE_UNCHANGED_124_CELL_SUPPORT_GATE_PREREGISTRATION_ONLY"
            if not failed
            else "DO_NOT_EVALUATE_PITCH_MARGIN_SUPPORT_CONTROLLER"
        ),
        "checks": checks,
