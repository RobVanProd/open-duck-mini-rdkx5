#!/usr/bin/env python3
"""Run T62's preregistered 16-cell corrected nominal matrix."""
...
RESULT_MD = ANALYSIS / "T64_T62_NOMINAL_MATRIX_RESULT_20260728.md"
CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t64_t62_nominal_matrix_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T64 preregistration")
    if prereg["status"] != "PREREGISTERED_T64_T62_NOMINAL_MATRIX":
        raise ValueError("T64 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T64 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T64 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T64 execution requires a clean worktree")
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t64_t62_nominal_matrix_result.v1",
        "status": (
            "PASS_T64_T62_NOMINAL_MATRIX"
            if passed
            else "HOLD_T64_T62_NOMINAL_MATRIX"
        ),
        "decision": (
