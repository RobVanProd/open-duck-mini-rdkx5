#!/usr/bin/env python3
"""Build the hash-frozen T120 joint soft-router hosted bundle."""
...
MARKDOWN = ANALYSIS / "T120_JOINT_SOFT_ROUTER_HOSTED_PACKAGE_20260729.md"
BUNDLE_NAME = "t120_joint_soft_router_bundle"
MANIFEST_NAME = "t120_package_manifest.json"
...
def main() -> int:
    ...
    for path in (staging, archive_path, CONTRACT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T120 package: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T120 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T120 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T120 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T120 playground changed")
    if (
        ...
        raise RuntimeError("copied T120 assets changed")
        ...
        [
            ...
            "\n".join(
                (
                    ...
                    "    print('PASS_T120_FULL_IDENTITY_PREFLIGHT')",
                    "else:",
    ...
    if (
        ...
        or "PASS_T120_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T120 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t120_hosted_bundle_manifest.v1",
        "preregistration_sha256": sha256(PREREG),
        "cpu_result_sha256": sha256(CPU_RESULT),
        "recovery_result_sha256": sha256(RECOVERY),
    ...
    contract = {
        "schema_version": "open_duck.t120_hosted_package.v1",
        "status": "PASS_T120_JOINT_SOFT_ROUTER_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            "preregistration_exact": True,
            "cpu_contract_and_recovery_exact": True,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T120 joint soft-router hosted package",
                "",
