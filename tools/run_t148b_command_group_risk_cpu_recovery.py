#!/usr/bin/env python3
"""Run T148B's population-corrected command-group-risk CPU recovery."""
...
RESULT = ANALYSIS / "t148b_command_group_risk_cpu_recovery_result.json"
MARKDOWN = (
    ANALYSIS / "T148B_COMMAND_GROUP_RISK_CPU_RECOVERY_RESULT_20260729.md"
)
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t148b_command_group_risk_cpu_recovery_v1"
)
...
def validate_prereg(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("T148B preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T148B composed source inventory changed")
...
def main() -> int:
    ...
    if not args.cpu_contract_authorized:
        raise PermissionError("T148B requires --cpu-contract-authorized")
    for path in (RESULT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T148B: {path}")
    if WORK.exists():
        raise FileExistsError(f"refusing to reuse T148B work root: {WORK}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T148B execution requires a clean worktree")
    ...
    model_contract = t128.mechanism_contract(playground, 32)
    ...
    command[command.index("--ppo_num_envs") + 1] = "32"
    command[command.index("--ppo_batch_size") + 1] = "32"
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T148B CPU training failed rc={completed.returncode}\n"
    ...
    if sorted(checkpoints) != [0, 1024] or sorted(graphs) != [0, 1024]:
        raise RuntimeError("T148B export steps changed")
    event_files = list(OUTPUT.glob("events.out.tfevents*"))
    if len(event_files) != 1:
        raise RuntimeError(
            f"T148B expected one TensorBoard event file: {event_files}"
        )
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t148b_command_group_risk_cpu_recovery_result.v1"
        ),
        "status": (
            "PASS_T148B_COMMAND_GROUP_RISK_CPU_RECOVERY"
            if passed
            else "HOLD_T148B_COMMAND_GROUP_RISK_CPU_RECOVERY"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T148B command-group risk CPU recovery result\n\n"
