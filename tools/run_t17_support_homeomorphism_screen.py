#!/usr/bin/env python3
"""Run T17's preregistered bounded support-homeomorphism screen."""
...
MARKDOWN = ANALYSIS / "T17_SUPPORT_HOMEOMORPHISM_RESULT_20260726.md"
...
def verify_receipt(value: dict[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T17 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        or value["status"]
        != "PREREGISTERED_T17_SUPPORT_HOMEOMORPHISM_SCREEN"
        or canonical_sha256(basis)
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T17 preregistration identity changed")
    ...
    for relative, expected in value["playground"][
        ...
        if sha256(playground / relative) != expected:
            raise RuntimeError(f"T17 playground changed: {relative}")
    return value
...
def main() -> int:
    ...
    if not args.execute:
        raise SystemExit("T17 requires --execute")
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T17 result")
    root = args.cache_root.resolve()
    if root.exists():
        raise FileExistsError(f"refusing to reuse T17 cache: {root}")
    root.mkdir(parents=True)
    prereg = load_preregistration()
    if prereg["transform"]["contract_tolerance"] != CONTRACT_TOLERANCE:
        raise RuntimeError("T17 float32 contract tolerance changed")
    ...
    for checkpoint in prereg["candidate"]["checkpoints"]:
        wrapped = (
            ...
            / "support_homeomorphism.onnx"
        )
        wrapper = wrap_support_homeomorphism(
    ...
    wrapper_contract_green = all(
        ...
        <= CONTRACT_TOLERANCE
        and row["verification"]["maximum_observation_roundtrip_error"]
        <= CONTRACT_TOLERANCE
        and row["verification"]["maximum_action_roundtrip_error"]
        <= CONTRACT_TOLERANCE
    ...
    basis = {
        "schema_version": (
            "open_duck.t17_support_homeomorphism_result.v1"
        ),
        "status": (
            "PASS_T17_SUPPORT_HOMEOMORPHISM_SCREEN"
            if passed
            else "HOLD_T17_SUPPORT_HOMEOMORPHISM_SCREEN"
        ),
        "decision": (
            "EARN_T17_FULL_R2_PREREGISTRATION"
            if passed
            else "CLOSE_BOUNDED_SUPPORT_HOMEOMORPHISM"
        ),
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T17 bounded support-homeomorphism result",
                "",
