#!/usr/bin/env python3
"""Build the hash-frozen T67 hosted continuation bundle."""
...
from colab_t67_endpoint_core_continuation import (
    CPU_RESULT_NAME,
...
CONTRACT = ANALYSIS / "t67_endpoint_core_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T67_ENDPOINT_CORE_HOSTED_PACKAGE_20260728.md"
BUNDLE_NAME = "t67_endpoint_core_bundle"
MANIFEST_NAME = "t67_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("T67 preregistration changed")
    ...
    if (
        ...
        raise ValueError("T67 package inputs changed")
    ...
    if copied_inventory != observed_inventory:
        raise ValueError("copied T67 playground changed")
    if (
        ...
        raise ValueError("copied T67 source assets changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T67_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T67_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T67 isolated import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t67_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t67_hosted_package.v1",
        "status": "PASS_T67_ENDPOINT_CORE_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T67 endpoint-core hosted package",
                "",
