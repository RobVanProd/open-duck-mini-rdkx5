#!/usr/bin/env python3
"""Execute the frozen Winner-v58 integrated first-tick continuation."""
...
import build_winner_v58_integrated_first_tick_training_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v58_integrated_first_tick_training_preregistration.json"
...
def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        ...
        or value.get("frozen_training", {}).get("continuation_optimizer_updates") != 100
        or value.get("frozen_training", {}).get("persistent_checkpoints")
        != {"half": 504, "final": 554}
        or value.get("authority", {}).get("training_authorized") is not True
        or value.get("authority", {}).get("formal_support_gate_authorized") is not False
    ):
        raise ValueError("Winner-v58 contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v58 source manifest absent")
    for name, item in sources.items():
        if (
            item.get("hash_mode") != "lf"
            or spec.lf_sha256(ROOT / item["path"]) != item.get("sha256")
        ):
            raise ValueError(f"Winner-v58 source changed: {name}")
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v58 source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v58 transformed source changed")
    return transformed
...
def main() -> int:
    ...
    if (
        ...
        raise PermissionError(
            "Winner-v58 requires --offline-cpu-only "
            "--integrated-first-tick-teacher-training-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v58 evidence")
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    transformed = validate_contract(contract)
    namespace: dict[str, Any] = {
        "__file__": str(spec.ROOT / "tools/run_winner_v46_static_target_teacher_training.py"),
        "__name__": "winner_v58_transformed_training",
    }
