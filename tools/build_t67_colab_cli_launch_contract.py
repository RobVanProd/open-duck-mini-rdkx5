#!/usr/bin/env python3
"""Freeze T67's one-session Colab CLI launch contract."""
...
PREREGISTRATION = (
    ANALYSIS / "t67_endpoint_core_hosted_preregistration.json"
)
PACKAGE_CONTRACT = (
...
PACKAGE = Path(
    ...
    "t67-endpoint-core-hosted-20260728.tar.gz"
)
EXECUTOR = ROOT / "tools" / "execute_t67_colab_cli.py"
OUTPUT = ANALYSIS / "t67_colab_cli_launch_contract.json"
MARKDOWN = ANALYSIS / "T67_COLAB_CLI_LAUNCH_CONTRACT_20260728.md"
SESSION = "t67-endpoint-core-20260728"
PACKAGE_SHA256 = (
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T67 launch: {path}")
    ...
    checks = {
        ...
        "package_contract_green": (
            package.get("status")
            == "PASS_T67_ENDPOINT_CORE_HOSTED_PACKAGE"
        ...
        "executor_single_t67_driver_exact": (
            "colab_t67_endpoint_core_continuation.py" in executor_text
        ),
    ...
    commands = [
        ...
        [
            ...
            SESSION,
            str(PACKAGE),
            "/content/t67-endpoint-core-hosted-20260728.tar.gz",
        ],
        [
            "colab",
            "exec",
        ...
        [
            ...
            "--session",
            SESSION,
            "/content/t67_launch_receipt.json",
            "<LOCAL_RECEIPT>",
        ],
        ["colab", "stop", "--session", SESSION],
    ]
    value = {
        "schema_version": "open_duck.t67_colab_cli_launch_contract.v1",
        "status": (
            "PASS_T67_COLAB_CLI_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_T67_COLAB_CLI_LAUNCH_CONTRACT"
        ),
        "checks": checks,
