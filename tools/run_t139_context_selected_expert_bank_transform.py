#!/usr/bin/env python3
"""Build the exact calibration-selected T100C/T129 expert bank."""
...
RESULT = ANALYSIS / "t139_context_selected_expert_bank_result.json"
MARKDOWN = (
    ANALYSIS / "T139_CONTEXT_SELECTED_EXPERT_BANK_RESULT_20260729.md"
)
WORK = Path(
...
def transform(
    ...
    if differences != {
        ...
        raise RuntimeError(f"T139 source difference changed: {differences}")
    expert_nodes = [
        (index, node)
    ...
    if (
        len(expert_nodes) != 1
        or len(gate_nodes) != 1
        or expert_nodes[0][0] + 1 != gate_nodes[0][0]
    ):
        raise RuntimeError("T139 expert/dynamic-gate adjacency changed")
    expert_index, expert_node = expert_nodes[0]
    gate_index, _ = gate_nodes[0]
    ...
    additions = {
        ...
        "expert_bank_router_coefficient": coefficient.reshape(64, 1),
        "expert_bank_router_intercept": np.asarray(
            [intercept], dtype=np.float32
        ),
        "expert_bank_router_zero": np.asarray([0.0], dtype=np.float32),
    }
    ...
    inserted = [
        helper.make_node(
            ...
            name="t139_nominal_expert",
        ),
        helper.make_node(
            "MatMul",
            ["calibration_context", "expert_bank_router_coefficient"],
            ["expert_bank_router_linear"],
            name="t139_expert_bank_router_matmul",
        ),
        helper.make_node(
            "Add",
            ["expert_bank_router_linear", "expert_bank_router_intercept"],
            ["expert_bank_router_score"],
            name="t139_expert_bank_router_add",
        ),
        helper.make_node(
            "GreaterOrEqual",
            ["expert_bank_router_score", "expert_bank_router_zero"],
            ["expert_bank_negative_condition"],
            name="t139_expert_bank_router_gate",
        ),
        helper.make_node(
            "Where",
            [
            ...
            ["negative_adapter_location"],
            name="t139_select_expert_bank",
        ),
    ]
    for offset, node in enumerate(inserted):
        model.graph.node.insert(gate_index + offset, node)
    ...
    unchanged_nodes = 0
    expert_rewire_exact = False
    old_index = 0
    for new_index, node in enumerate(new_nodes):
        if gate_index <= new_index < gate_index + len(inserted):
            continue
    ...
    return {
        ...
        "expert_output_rewire_exact": expert_rewire_exact,
        "all_other_nodes_byte_exact": (
            unchanged_nodes == len(old_nodes) - 1
        ),
        ...
        "dynamic_gate_unchanged_and_single": len(dynamic_gate) == 1,
...
def equivalence_contract(
    ...
    negative_session = ort.InferenceSession(
        str(negative_source), providers=["CPUExecutionProvider"]
    )
        ...
        rows.append(
            {
                ...
                "selected_source": (
                    "T129_negative_expert"
                    if negative_condition
                    else "T100C_nominal_expert"
                ),
                "samples": samples,
    ...
    return {
        ...
        "cpu_only": (
            nominal_session.get_providers()[0] == "CPUExecutionProvider"
            and negative_session.get_providers()[0]
            == "CPUExecutionProvider"
            and transformed_session.get_providers()[0]
            == "CPUExecutionProvider"
        ),
    }
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T139 requires --execute")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T139 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T139 preregistration changed")
    ...
    for step in sorted(prereg["nominal_graphs"]):
        ...
        destination = WORK / step / "context_selected_expert_bank.onnx"
        graphs[step] = transform(
            nominal, negative, destination, coefficient, intercept
        )
        contracts[step] = equivalence_contract(
            nominal, negative, destination, t135b["runs"]
        )
    checks = {
        "all_graph_transforms_exact": all(
            all(
                ...
                for name in (
                    ...
                    "five_initializers_added",
                    "dynamic_gate_unchanged_and_single",
                    "abi_exact",
                )
            )
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t139_context_selected_expert_bank_result.v1"
        ),
        "status": (
            "PASS_T139_CONTEXT_SELECTED_EXPERT_BANK_TRANSFORM"
            if passed
            else "HOLD_T139_CONTEXT_SELECTED_EXPERT_BANK_TRANSFORM"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T139 context-selected expert bank\n\n"
        f"- Status: `{value['status']}`\n"
        f"- Decision: `{value['decision']}`\n"
        "- Nominal context is bit-exact to T100C; negative to T129\n"
        "- Dynamic hidden-state gate and full deployment chain unchanged\n"
