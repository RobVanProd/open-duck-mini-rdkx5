#!/usr/bin/env python3
"""Freeze T203's 16-cell nominal/default behavior matrix."""
...
OUTPUT = ANALYSIS / "t206_t203_nominal_matrix_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T206_T203_NOMINAL_MATRIX_PREREGISTRATION_20260730.md"
)
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t206_t203_nominal_matrix.py"
TEST = ROOT / "tests" / "test_t206_t203_nominal_matrix.py"
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T206: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T206 preregistration requires clean worktree")
    ...
    if failed:
        raise RuntimeError(f"T206 preregistration checks failed: {failed}")
    copied = {
        name: basis[name]
        for name in (
            "fits",
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t206_t203_nominal_matrix_preregistration.v1"
        ),
        "status": "PREREGISTERED_T206_T203_NOMINAL_MATRIX",
        **copied,
        "condition": condition,
        "policies": policies,
        "matrix": {
    ...
    MARKDOWN.write_text(
        "# T206 T203 nominal matrix preregistration\n\n"
