#!/usr/bin/env python3
"""Run T210's sequential full R2 matrix one condition per invocation."""
...
PREREG = ANALYSIS / "t215_t210_full_r2_preregistration.json"
RESULT = ANALYSIS / "t215_t210_full_r2_result.json"
MARKDOWN = ANALYSIS / "T215_T210_FULL_R2_RESULT_20260730.md"
CACHE = Path("D:/CodexArtifacts/open-duck-policy/t215_t210_full_r2_v1")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        value.get("status") != "PREREGISTERED_T215_T210_FULL_R2"
        or value.get("failed_checks")
        or canonical_sha256(basis)
        != value.get("preregistered_contract_sha256")
    ):
        raise RuntimeError("T215 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T215 matrix plan changed")
    return value
...
def write_progress(
    ...
    *,
    ...
    value = {
        "schema_version": "open_duck.t215_t210_full_r2_progress.v1",
        "status": "IN_PROGRESS_T215_T210_FULL_R2",
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T215 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T215 execution requires clean worktree")
    ...
    if paused and first_failed is None:
        ...
        print("IN_PROGRESS_T215_T210_FULL_R2")
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t215_t210_full_r2_result.v1",
        "status": (
            "PASS_T215_T210_FULL_R2"
            if all_green
            else "HOLD_T215_T210_FULL_R2"
        ),
        "decision": (
