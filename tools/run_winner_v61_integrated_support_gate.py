#!/usr/bin/env python3
"""Run the frozen Winner-v61 half/final offline support gate."""
...
import build_winner_v61_integrated_support_gate_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v61_integrated_support_gate_preregistration.json"
...
def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        ...
        or value.get("status") != "PREREGISTERED_WINNER_V61_INTEGRATED_SUPPORT_GATE"
        or value.get("decision")
        != "AUTHORIZE_ONE_FROZEN_WINNER_V61_248_CELL_GATE_ONLY"
        or value.get("selection_rule", {}).get("selected_update_if_pass") != 554
        ...
        raise ValueError("Winner-v61 contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v61 source manifest absent")
    for name, item in sources.items():
        if (
            item.get("hash_mode") != "lf"
            or spec.lf_sha256(ROOT / item["path"]) != item.get("sha256")
        ):
            raise ValueError(f"Winner-v61 source changed: {name}")
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v61 source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v61 transformed source changed")
    return transformed
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.formal_gate_authorized:
        raise PermissionError(
            "Winner-v61 requires --offline-cpu-only --formal-gate-authorized"
        )
    if args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v61 evidence")
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    transformed = validate_contract(contract)
    namespace: dict[str, Any] = {
        "__file__": str(ROOT / "tools/run_winner_v61_integrated_support_gate.py"),
        "__name__": "winner_v61_transformed_support_gate",
    }
    ...
    try:
        sys.argv = [
            namespace["__file__"],
            "--training-work-root", str(args.training_work_root),
