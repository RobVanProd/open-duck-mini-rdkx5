#!/usr/bin/env python3
"""Freeze T164's sequential 20-condition R2 revalidation."""
...
OUTPUT = ANALYSIS / "t165_composed_full_r2_preregistration.json"
MARKDOWN = ANALYSIS / "T165_COMPOSED_FULL_R2_PREREGISTRATION_20260729.md"
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t165_composed_full_r2.py"
TEST = ROOT / "tests" / "test_t165_composed_full_r2.py"


def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T165: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T165 preregistration requires clean worktree")
    t163 = json.loads(T163.read_text(encoding="utf-8"))
    ...
    graph_by_step = {
        ...
        for row in t164["graphs"]
    }
    policies = [
        {
            "checkpoint_id": f"T164_COMPOSED_{label}",
            "step": int(step),
            **graph_by_step[step],
        }
    ...
    frozen = {
        ...
        "runner": RUNNER,
        "test": TEST,
        "t163_positive_matrix": T163,
        "t164_composition": T164,
        "r2_basis": BASIS,
    }
    checks = {
        "t163_positive_endpoint_green": (
            t163["status"]
            == "PASS_T163_COMMAND_ENDPOINT_POSITIVE_MATRIX"
            and t163["condition"]["green_cells"] == 16
        ),
        "t164_composition_green": (
            t164["status"] == "PASS_T164_PRIOR_REPAIR_COMPOSITION"
            and not t164["failed_checks"]
            and all(t164["checks"].values())
        ),
        "t164_graphs_exact": (
            len(policies) == 2
    ...
    if failed:
        raise RuntimeError(f"T165 preregistration checks failed: {failed}")
    copied = {
        name: basis[name]
        for name in (
            "conditions",
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t165_composed_full_r2_preregistration.v1"
        ),
        "status": "PREREGISTERED_T165_COMPOSED_FULL_R2",
        **copied,
        "policies": policies,
        "matrix": {
            "conditions": 20,
        ...
        "decision_rule": {
            "pass_decision": (
                "EARN_T166_OFFLINE_DEPLOYMENT_CONTRACT_AUDIT_"
                "PREREGISTRATION_ONLY"
            ),
            "fail_decision": (
                "CLOSE_COMPOSED_POLICY_AT_FIRST_FAILED_R2_CONDITION"
            ),
            "no_retry": True,
    ...
    MARKDOWN.write_text(
        "# T165 composed full R2 preregistration\n\n"
