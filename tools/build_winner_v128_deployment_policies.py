#!/usr/bin/env python3
"""Apply the exact V121 hierarchy to both V127 checkpoints."""
...
PREREG = ANALYSIS / "winner_v128_deployment_transform_preregistration.json"
CONTRACT = ANALYSIS / "winner_v128_deployment_transform_contract.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V128_DEPLOYMENT_TRANSFORM_CONTRACT_20260724.md"
)
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("V128 transform preregistration changed")
    ...
    for spec in prereg["sources"]:
        ...
        if (
            ...
            raise ValueError(f"raw V127 source changed: {spec['id']}")
    ...
    payload = {
        "schema_version": "winner_v128.deployment_transform_contract.v1",
        "status": (
            "PASS_WINNER_V128_DEPLOYMENT_TRANSFORM_CONTRACT"
            if not failed
            else "HOLD_WINNER_V128_DEPLOYMENT_TRANSFORM_CONTRACT"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner V128 deployment transform\n\n"
        f"- Status: `{payload['status']}`\n"
        "- Both V127 post-update graphs use the exact V121 hierarchy.\n"
