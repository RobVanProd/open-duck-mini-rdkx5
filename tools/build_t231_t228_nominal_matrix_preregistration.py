#!/usr/bin/env python3
"""Freeze T228's 16-cell nominal/default behavior matrix."""
...
OUTPUT = ANALYSIS / "t231_t228_nominal_matrix_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T231_T228_NOMINAL_MATRIX_PREREGISTRATION_20260730.md"
)
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools/run_t231_t228_nominal_matrix.py"


def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T231: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T231 preregistration requires clean worktree")
    t230 = json.loads(T230.read_text(encoding="utf-8"))
    basis = json.loads(BASIS.read_text(encoding="utf-8"))
    graph_by_step = {
        int(row["step"]): row["plateau"] for row in t230["graphs"]
    }
    policies = [
        {
            "checkpoint_id": f"T228_COMMAND_ATOM_{label}",
            "step": step,
            **graph_by_step[step],
        }
        for label, step in (
    ...
    checks = {
        "t230_composition_green": (
            t230["status"] == "PASS_T230_T228_DEPLOYMENT_COMPOSITION"
            and t230["failed_checks"] == []
            and t230["decision"]
            == (
    ...
    if failed:
        raise RuntimeError(f"T231 preregistration checks failed: {failed}")
    copied = {
        name: basis[name]
        for name in (
            "fits",
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t231_t228_nominal_matrix_preregistration.v1"
        ),
        "status": "PREREGISTERED_T231_T228_NOMINAL_MATRIX",
        **copied,
        "condition": condition,
        "policies": policies,
        "matrix": {
        ...
        "decision_rule": {
            "pass_decision": (
                "EARN_T232_T228_TARGETED_Y_NEGATIVE_MATRIX_"
                "PREREGISTRATION_ONLY"
            ),
            "fail_decision": "CLOSE_T228_COMMAND_ATOM_CONTINUATION",
            "no_retry": True,
    ...
    MARKDOWN.write_text(
        "# T231 T228 nominal matrix preregistration\n\n"
