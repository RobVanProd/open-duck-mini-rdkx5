#!/usr/bin/env python3
"""CPU worker for T209 default-off and enabled environment contracts."""
...
def configure(joystick, reference: Path, *, t209_enabled: bool):
    ...
    if hasattr(config, "winner_t202_predicted_roll_risk"):
        config.winner_t202_predicted_roll_risk = False
    if hasattr(config, "winner_v127_constrained_cost"):
        config.winner_v127_constrained_cost = t209_enabled
    elif t209_enabled:
        raise ValueError("enabled T209 worker loaded a pre-V127 playground")
    if hasattr(config, "winner_t209_dual_roll_cost"):
        config.winner_t209_dual_roll_cost = t209_enabled
    elif t209_enabled:
        raise ValueError("enabled T209 worker loaded a non-T209 playground")
    return config
...
def default_off(playground: Path, reference: Path) -> dict[str, Any]:
    ...
    env = joystick.Joystick(
        ...
        config=configure(joystick, reference, t209_enabled=False),
    )
    state = jax.jit(env.reset)(jax.random.PRNGKey(20900))
    digests = [tree_digest(state, prefix="t209_default_off")]
    actions = np.linspace(
    ...
    for action in actions:
        ...
        digests.append(tree_digest(state, prefix="t209_default_off"))
...
def metric_errors(env, state, t202) -> dict[str, float]:
    ...
    return {
        ...
        "cost_collector": float(jnp.abs(recorded_cost - expected_cost)),
        "reward": float(
            jnp.abs(
                state.reward
                - state.metrics["t209/original_clipped_reward"]
            )
        ),
    }
...
def enabled(
    ...
    *,
    ...
    env = joystick.Joystick(
        task="flat_terrain_backlash",
        config=configure(joystick, reference, t209_enabled=True),
    )
    ...
    for tick in range(ticks):
        ...
        maximum_risk = max(
            maximum_risk,
            float(state.metrics["t209/predicted_roll_risk_rad"]),
        )
        maximum_cost = max(
            maximum_cost,
            float(state.metrics["cost/t209_predicted_roll_risk"]),
        )
    ...
    base_qvel = int(env._floating_base_qvel_addr)
    qvel = synthetic.data.qvel.at[base_qvel + 3].set(jnp.float32(10.0))
    ...
    synthetic_risk = float(
        synthetic.metrics["t209/predicted_roll_risk_rad"]
    )
    synthetic_excess = float(
        synthetic.metrics["t209/roll_risk_excess_rad"]
    )
    synthetic_cost = float(
        synthetic.metrics["cost/t209_predicted_roll_risk"]
    )
    synthetic_original = float(
        synthetic.metrics["t209/original_clipped_reward"]
    )
    ...
    checks = {
        ...
        "synthetic_path_exceeds_envelope": (
            ...
            and synthetic_excess > 0.0
            and synthetic_cost > 0.0
        ),
        "cost_is_unscaled_squared_excess": math.isclose(
            synthetic_cost,
            synthetic_excess * synthetic_excess,
            rel_tol=1.0e-6,
            abs_tol=1.0e-7,
        ),
        "cost_is_outside_and_does_not_change_reward": (
            synthetic_reward == synthetic_original
        ),
