#!/usr/bin/env python3
"""Run the always-on expert's frozen nominal matrix."""
...
PREREG = ANALYSIS / "t110_always_on_nominal_preregistration.json"
RESULT = ANALYSIS / "t110_always_on_nominal_result.json"
MARKDOWN = ANALYSIS / "T110_ALWAYS_ON_NOMINAL_RESULT_20260729.md"
CACHE = Path("D:/CodexArtifacts/open-duck-policy/t110_always_on_nominal_v1")
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T110 input: {path}")


def load() -> dict[str, Any]:
    value = json.loads(PREREG.read_text(encoding="utf-8"))
    if (
        value["status"] != "PREREGISTERED_T110_ALWAYS_ON_NOMINAL_MATRIX"
        or value["failed_checks"]
        or canonical_sha256(value, "preregistered_contract_sha256")
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T110 preregistration changed")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T110 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T110 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t110_always_on_nominal_result.v1",
        "status": (
            "PASS_T110_ALWAYS_ON_NOMINAL_MATRIX"
            if passed
            else "HOLD_T110_ALWAYS_ON_NOMINAL_MATRIX"
        ),
        "decision": (
        ...
        "authority": {
            "t111_negative_endpoint_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
