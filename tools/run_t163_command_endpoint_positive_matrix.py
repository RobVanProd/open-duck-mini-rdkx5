#!/usr/bin/env python3
"""Run T162's frozen command-endpoint +COM behavior matrix."""
...
PREREG = ANALYSIS / "t163_command_endpoint_positive_matrix_preregistration.json"
RESULT = ANALYSIS / "t163_command_endpoint_positive_matrix_result.json"
MARKDOWN = ANALYSIS / "T163_COMMAND_ENDPOINT_POSITIVE_MATRIX_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t163_command_endpoint_positive_matrix_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T163 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T163 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T163 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T163 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t163_command_endpoint_positive_matrix_result.v1"
        ),
        "status": (
            "PASS_T163_COMMAND_ENDPOINT_POSITIVE_MATRIX"
            if passed
            else "HOLD_T163_COMMAND_ENDPOINT_POSITIVE_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T163 command-endpoint positive matrix result\n\n"
