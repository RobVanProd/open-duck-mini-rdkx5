#!/usr/bin/env python3
"""Bind the unchanged reviewed support gate to Winner-v46 half/final artifacts."""
...
SOURCE_SNAPSHOT_SHA256 = "72c2e0bf895e8968a0d851ee1079435daca6125a572943b557fe766fc6e4832b"
...
def _validate_snapshot(snapshot: Mapping[str, Any], *, expected_stage: str) -> None:
    if smoke is None or training is None or v21 is None or _TRAINING_PREREG is None:
        raise AssertionError("Winner-v47 dependencies were not loaded")
    if set(snapshot) != {"parameters", "optimizer", "metadata", "target_mean", "target_std"}:
        raise ValueError("Winner-v47 checkpoint state schema changed")
    ...
    if (
        ...
        or update not in {302, 352}
        or metadata.get("source_completed_updates") != 252
        or metadata.get("source_snapshot_sha256") != SOURCE_SNAPSHOT_SHA256
        or metadata.get("teacher_snapshot_sha256") != TEACHER_SNAPSHOT_SHA256
        ...
        or metadata.get("static_target_teacher_scale") != 58.436370849609375
        ...
        raise ValueError("Winner-v47 checkpoint metadata changed")
    for name in ("target_mean", "target_std"):
        ...
        if value.shape != (50,) or value.dtype != np.dtype(np.float32) or not np.all(np.isfinite(value)):
            raise ValueError(f"Winner-v47 {name} schema changed")
    if np.any(np.asarray(snapshot["target_std"]) <= 0.0):
        raise ValueError("Winner-v47 target_std is not positive")
    for tree in (snapshot["parameters"], snapshot["optimizer"]["m"], snapshot["optimizer"]["v"]):
        if not all(np.all(np.isfinite(np.asarray(value))) for value in tree.values()):
            raise ValueError("Winner-v47 checkpoint contains nonfinite arrays")


def load_snapshot_for_reviewed_gate(path: Path) -> dict[str, Any]:
    if v22v2 is None or normalized_support is None:
        raise AssertionError("Winner-v47 mechanics were not loaded")
    exact = v22v2.load_snapshot(path)
    _validate_snapshot(exact, expected_stage="static_target_teacher_joint_stage2")
    adapted = dict(exact)
...
def checkpoint_paths(work_root: Path, label: str) -> tuple[Path, Path]:
    if _TRAINING is None or smoke is None:
        raise AssertionError("Winner-v47 training result was not loaded")
    mapping = {"half": 302, "final": 352}
    if label not in mapping:
        raise ValueError(f"unknown Winner-v47 checkpoint: {label}")
    update = mapping[label]
    snapshot = work_root / "snapshots" / f"snapshot_static_target_teacher_update_{update:03d}.npz"
    graph = work_root / "graphs" / f"winner_v46_{label}.onnx"
    snapshot_receipt = next(row for row in _TRAINING["snapshot_manifest"] if row["completed_updates"] == update)
    graph_receipt = next(
    ...
    for path, receipt in ((snapshot, snapshot_receipt), (graph, graph_receipt)):
        if not path.is_file() or smoke.sha256(path) != receipt["sha256"] or path.stat().st_size != receipt["bytes"]:
            raise ValueError(f"Winner-v47 {label} artifact bytes changed: {path.name}")
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    if smoke is None or reviewed_gate is None or builder is None or _TRAINING is None:
        raise AssertionError("Winner-v47 gate dependencies were not loaded")
    if (
        value.get("schema_version") != "winner_v47.static_target_teacher_support_gate_preregistration.v1"
        or value.get("status") != "PREREGISTERED_WINNER_V47_STATIC_TARGET_TEACHER_SUPPORT_GATE"
        or value.get("decision") != "AUTHORIZE_ONE_FROZEN_WINNER_V47_248_CELL_GATE_ONLY"
        or value.get("execution_now") != {
            "formal_support_cells": 0, "heldout_repeat_cells": 0,
            "locomotion_steps": 0, "robot_or_rdk_access": 0,
        }
    ):
        raise ValueError("Winner-v47 preregistration identity changed")
    ...
    if (
        ...
        raise ValueError("Winner-v47 gate dimensions changed")
    snapshots = {row["completed_updates"]: row for row in _TRAINING["snapshot_manifest"]}
    checkpoints = {row["label"]: row for row in _TRAINING["persistent_checkpoints"]}
    if value.get("training_artifact") != {
        ...
        "half": {"snapshot": snapshots[302], "graph": checkpoints["half"]["graph"]},
        "final": {"snapshot": snapshots[352], "graph": checkpoints["final"]["graph"]},
    }:
        raise ValueError("Winner-v47 training-artifact binding changed")
    sources = value.get("sources")
    if not isinstance(sources, dict) or set(sources) != set(builder.SOURCES):
        raise ValueError("Winner-v47 sources are absent")
    for name, item in sources.items():
        path = ROOT / item["path"]
        if set(item) != {"hash_mode", "path", "sha256"} or item["hash_mode"] != "lf" or smoke.lf_sha256(path) != item["sha256"]:
            raise ValueError(f"Winner-v47 source changed: {name}")
    if reviewed_gate.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v47 source manifest changed")


def main() -> int:
    global reviewed_gate, smoke, training, v21, v22v2, normalized_support
    global builder, _TRAINING, _TRAINING_PREREG
    import build_winner_v47_static_target_teacher_support_gate_preregistration as builder_module
    ...
    if not args.offline_cpu_only or not args.formal_gate_authorized:
        raise PermissionError("Winner-v47 requires --offline-cpu-only --formal-gate-authorized")
    if args.output.exists():
        raise FileExistsError(f"refusing to overwrite Winner-v47 result: {args.output}")
    ...
    if (
        training_result.get("status") != "PASS_WINNER_V46_STATIC_TARGET_TEACHER_TRAINING_ARTIFACT"
        or training_result.get("decision") != "AUTHORIZE_STATIC_TARGET_TEACHER_SUPPORT_GATE_PREREGISTRATION_ONLY"
        or training_result.get("failed_checks") != []
        or training_result.get("execution", {}).get("formal_support_cells") != 0
    ):
        raise ValueError("Winner-v46 does not authorize the gate")
    ...
    reviewed_gate.CHECKPOINTS = (("half", 302), ("final", 352))
    ...
    try:
        sys.argv = [
            ...
            "--playground-root", str(args.playground_root), "--canonical-fit", str(args.canonical_fit),
            "--output", str(pending), "--offline-cpu-only", "--formal-gate-authorized",
        ]
    ...
    reviewed["schema_version"] = "winner_v47.static_target_teacher_support_gate_result.v1"
    reviewed["status"] = (
    ...
    args.output.write_text(json.dumps(reviewed, allow_nan=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
