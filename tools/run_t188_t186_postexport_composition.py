#!/usr/bin/env python3
"""Compose T186 nominal heads into the frozen T164-final graph."""
...
PREREG = ANALYSIS / "t188_t186_postexport_composition_preregistration.json"
RESULT = ANALYSIS / "t188_t186_postexport_composition_result.json"
MARKDOWN = ANALYSIS / "T188_T186_POSTEXPORT_COMPOSITION_RESULT_20260730.md"
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t188_t186_postexport_composition_v1"
)
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or WORK.exists():
        raise FileExistsError("refusing to overwrite T188 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T188 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T188_T186_POSTEXPORT_COMPOSITION"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T188 preregistration changed")
    ...
    for graph in prereg["graphs"]:
        ...
        destination = WORK / str(step_value) / "t186_composed.onnx"
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t188_t186_postexport_composition_result.v1"
        ),
        "status": (
            "PASS_T188_T186_POSTEXPORT_COMPOSITION"
            if not failed
            else "HOLD_T188_T186_POSTEXPORT_COMPOSITION"
        ),
        "decision": (
            "EARN_T189_T186_NOMINAL_BEHAVIOR_MATRIX_PREREGISTRATION_ONLY"
            if not failed
            else "HOLD_T186_BEHAVIOR_AND_AUDIT_COMPOSITION"
        ),
    ...
    MARKDOWN.write_text(
        "# T188 T186 post-export composition result\n\n"
