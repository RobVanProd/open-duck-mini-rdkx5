#!/usr/bin/env python3
"""Run T66's preregistered endpoint-bank recurrent-core CPU contract."""
...
MARKDOWN = ANALYSIS / "T66_ENDPOINT_CORE_CPU_RESULT_20260728.md"
WORK = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t66_endpoint_core_cpu_v1"
)
OUTPUT = WORK / "smoke"
...
def validate_prereg(value: dict[str, Any]) -> None:
    ...
    if (
        value.get("status")
        != "PREREGISTERED_T66_ENDPOINT_CORE_CPU_CONTRACT"
        or value.get("failed_checks") != []
        or t20.canonical_sha256(basis)
        != value.get("preregistered_contract_sha256")
    ):
        raise RuntimeError("T66 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T66 composed source inventory changed")
...
def training_command(
    *,
    ...
    try:
        ...
        t20.VELOCITY_LIMITS = prior
    replace_argument(command, "--ppo_num_envs", "8")
    replace_argument(command, "--ppo_batch_size", "8")
...
def main() -> int:
    for path in (RESULT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T66 result: {path}")
    if WORK.exists():
        raise FileExistsError(f"refusing to reuse T66 work root: {WORK}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"],
        cwd=ROOT,
        text=True,
    ).strip():
        raise RuntimeError("formal T66 execution requires a clean worktree")
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T66 CPU training failed rc={completed.returncode}\n"
    ...
    if sorted(checkpoints) != [0, 1024] or sorted(graphs) != [0, 1024]:
        raise RuntimeError(
            f"T66 export steps changed: {sorted(checkpoints)}, {sorted(graphs)}"
        )
    ...
    checks = {
        "command_exact_flags": (
            ...
            and "--winner_t66_endpoint_core_continuation" in command
        ...
        "all_three_core_groups_present": (
            all(
                any(group in name for name in core_deltas)
                for group in prereg["mechanism"]["trainable_actor_groups"]
            )
        ),
        "every_core_leaf_updated": (
            bool(core_deltas)
            and all(delta > 0.0 for delta in core_deltas.values())
        ),
        "every_other_actor_leaf_bit_exact": (
            bool(frozen_deltas)
    ...
    passed = not failed
    result: dict[str, Any] = {
        "schema_version": "open_duck.t66_endpoint_core_cpu_result.v1",
        "status": (
            "PASS_T66_ENDPOINT_CORE_CPU_CONTRACT"
            if passed
            else "HOLD_T66_ENDPOINT_CORE_CPU_CONTRACT"
        ),
        "decision": (
        ...
        "tree_contract": {
            ...
            "core_actor_leaf_deltas": core_deltas,
            "frozen_actor_leaf_deltas": frozen_deltas,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T66 endpoint-core CPU result",
                "",
                f"- Status: `{result['status']}`",
                f"- Decision: `{result['decision']}`",
                f"- Failed checks: `{failed}`",
                f"- Core actor leaves: `{len(core_deltas)}`",
                f"- Frozen actor leaves: `{len(frozen_deltas)}`",
