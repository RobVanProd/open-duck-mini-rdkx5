#!/usr/bin/env python3
"""Execute T228's exact package in one fresh Colab L4 session."""
...
PACKAGE = Path("/content/t228-command-atom-20260730.tar.gz")
PACKAGE_BYTES = 4_360_846
PACKAGE_SHA256 = (
    "8999a2855a0c976f1d384c034906698fdb09699b9029d8f59af6fbad49b0486c"
)
BUNDLE_NAME = "t228_command_atom_bundle"
MANIFEST_NAME = "t228_package_manifest.json"
MANIFEST_SHA256 = (
    "d898deae1e7c183225dd4cf766ae87d88af6fde20f7fff4141d7c0e75486d4ba"
)
WORK = Path("/content/t228_work")
EXTRACT = Path("/content/t228_launch")
OUTPUT_JSON = Path("/content/t228_result.json")
OUTPUT_ARCHIVE = Path("/content/t228_artifacts.tar.gz")
RECEIPT = Path("/content/t228_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def verify_manifest(bundle: Path) -> dict[str, Any]:
    ...
    if sha256(manifest_path) != MANIFEST_SHA256:
        raise ValueError("T228 extracted manifest changed")
    ...
    if (
        ...
        raise ValueError("T228 manifest identity changed")
    for relative, expected in members.items():
        ...
        if (
            ...
            raise ValueError(f"T228 bundle member changed: {relative}")
    return manifest
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T228 requires L4: {gpu.stdout}{gpu.stderr}")
    for path in (WORK, EXTRACT, OUTPUT_JSON, OUTPUT_ARCHIVE, RECEIPT):
        if path.exists():
            raise FileExistsError(f"T228 no-retry path exists: {path}")
    if (
        PACKAGE.stat().st_size != PACKAGE_BYTES
        or sha256(PACKAGE) != PACKAGE_SHA256
    ):
        raise ValueError("T228 uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T228 package contains playground/.tmp")
    ...
    if install.returncode != 0:
        raise RuntimeError(f"T228 pinned install failed: {install.returncode}")
    versions = {
        name: importlib.metadata.version(name)
        for name in EXPECTED_VERSIONS
    }
    if versions != EXPECTED_VERSIONS:
        raise ValueError(f"T228 software versions changed: {versions}")
    ...
    preflight_source = "\n".join(
        (
            "from pathlib import Path",
            "import colab_t228_command_atom_continuation as driver",
            ...
            ")",
            "assert cmd.count('--winner_t227_command_atom_bank') == 1",
            ...
            "module = Path('playground/playground/common/t227_command_atom_bank.py').read_text()",
            "losses = Path('playground/playground/common/winner_v127_constrained_ppo_losses.py').read_text()",
            "driver_text = Path('colab_t228_command_atom_continuation.py').read_text()",
            "assert 'winner_t227_command_atom_bank' in runner",
            "assert 'T227_COMMAND_ATOM_BANK=' in module",
            "assert 'CONFIGURATION_STRATA = 8' in module",
            "assert 'COMMAND_STRATA = 4' in module",
            "assert 'mixed_advantages' in losses",
            "assert 't227_hosted_runner_readback_exact' in driver_text",
            "assert 'hosted_axis_complete_cost_exercised' in Path('colab_t216_axis_complete_tilt_continuation.py').read_text()",
            "print('PASS_T228_HOSTED_FULL_PREFLIGHT')",
        )
    )
    ...
    if (
        ...
        or "PASS_T228_HOSTED_FULL_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T228 hosted preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    driver = bundle / "colab_t228_command_atom_continuation.py"
    command = [
        sys.executable,
        str(driver),
        "--bundle-root",
    ...
    receipt = {
        "schema_version": "open_duck.t228_colab_launch_receipt.v1",
        "status": "STARTING_T228_HASH_FROZEN_GPU_CONTINUATION",
    ...
    receipt["status"] = (
        "COMPLETED_T228_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T228_COLAB_LAUNCH"
    )
    for name, path in (
