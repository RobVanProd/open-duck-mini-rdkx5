#!/usr/bin/env python3
"""Run the one frozen Winner-v119 transition-matched GPU continuation."""
...
PREREGISTRATION_NAME = "winner_v119_hosted_preregistration.json"
CPU_RESULT_NAME = "winner_v119_transition_cpu_result.json"
COMPOSED_MANIFEST_NAME = "WINNER_V119_COMPOSED_SOURCE_MANIFEST.json"
...
TRANSITION_PATCH_NAME = "winner_v119_train_transition_match.patch"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("Winner-v119 prerequisite status changed")
    observed = {
        "driver": sha256(Path(__file__).resolve()),
        "driver_helper": sha256(
            Path(__file__).resolve().with_name(DRIVER_HELPER_NAME)
        ),
        "transition_patch": sha256(bundle / TRANSITION_PATCH_NAME),
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError(
            f"Winner-v119 inputs changed: {observed} != "
    ...
    if (
        ...
        raise RuntimeError(f"Winner-v119 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(
            work, arcname="winner_v119_transition_continuation"
        )
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "winner_v119.hosted_result.v1",
        "status": "RUNNING_WINNER_V119_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"Winner-v119 runner returned {completed.returncode}"
            )
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_WINNER_V119_HOSTED_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"Winner-v119 artifact checks failed: {failed}")
        ...
        payload["status"] = "HOLD_WINNER_V119_HOSTED_CONTINUATION"
