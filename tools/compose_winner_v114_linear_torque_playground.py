#!/usr/bin/env python3
"""Compose V111 plus the default-off linear peak-torque objective."""
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to reuse V114 tree: {output}")
    base_manifest = base / "WINNER_V111_COMPOSED_SOURCE_MANIFEST.json"
    ...
    manifest = {
        "schema_version": "winner_v114.linear_torque_composed_source.v1",
        "base": {
            "path": str(base),
    ...
    manifest_path = output / "WINNER_V114_COMPOSED_SOURCE_MANIFEST.json"
