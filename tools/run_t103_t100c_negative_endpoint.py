#!/usr/bin/env python3
"""Run T100C's frozen COM-X-negative endpoint matrix."""
...
PREREG = ANALYSIS / "t103_t100c_negative_endpoint_preregistration.json"
RESULT = ANALYSIS / "t103_t100c_negative_endpoint_result.json"
MARKDOWN = ANALYSIS / "T103_T100C_NEGATIVE_ENDPOINT_RESULT_20260728.md"
DEFAULT_CACHE_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t103_t100c_negative_endpoint_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T103 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T103 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T103 matrix plan changed")
    return value
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T103 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T103 formal execution requires a clean worktree")
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t103_t100c_negative_endpoint_result.v1",
        "status": (
            "PASS_T103_T100C_NEGATIVE_ENDPOINT_MATRIX"
            if all_green
            else "HOLD_T103_T100C_NEGATIVE_ENDPOINT_MATRIX"
        ),
        "decision": (
        ...
        "interpretation": {
            "negative_com_endpoint_repaired": all_green,
            "both_checkpoints_persistent": all_green,
            "hosted_run_earned": False,
        },
        "cache_root": str(cache_root),
        "authority": {
            "t104_full_r2_preregistration": all_green,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T103 T100C negative-endpoint result",
                "",
                f"- Status: `{result['status']}`",
                f"- Decision: `{result['decision']}`",
                f"- Green cells: `{summary['green_cells']}/{summary['cells']}`",
                (
                    "- Worst tracking p95: "
