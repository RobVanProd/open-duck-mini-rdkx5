#!/usr/bin/env python3
"""Run T185's default-off, environment, and 1,024-step CPU contract."""
...
RESULT = (
    ANALYSIS / "t185_in_episode_single_support_cpu_result.json"
)
MARKDOWN = (
    ANALYSIS
    / "T185_IN_EPISODE_SINGLE_SUPPORT_CPU_RESULT_20260730.md"
)
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t185_in_episode_single_support_cpu_v1"
)
...
def validate_prereg(value: dict[str, Any]) -> None:
    if (
        ...
        raise RuntimeError("T185 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T185 composed playground changed")
...
def run_worker(
    *,
    ...
    command = [
        sys.executable,
        str(TOOLS / "run_t185_environment_contract_worker.py"),
        "--mode",
        mode,
        "--playground",
        str(playground),
    ...
    if mode == "enabled":
        command.extend(["--env-count", "256", "--seed", "185"])
...
def training_command(
    *,
    ...
    command.insert(
        index, "--winner_t185_in_episode_single_support_prefix"
    )
    return command
...
def main() -> int:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T185 CPU training failed rc={completed.returncode}\n"
    ...
    checks = {
        ...
        "training_metrics_finite_and_prefix_exercised": (
            ...
            and positive_finite_metric(
                events, "eval/episode_reward/t185/prefix_active"
            )
        ),
    ...
    MARKDOWN.write_text(
        ...
        "- Prefix: 27 frozen ticks, then same-episode locomotion\n"
        "- Optimizer / behavior / hosted / robot: `1024 / 0 / 0 / 0`\n",
        encoding="utf-8",
        newline="\n",
    )
    print(result["status"])
