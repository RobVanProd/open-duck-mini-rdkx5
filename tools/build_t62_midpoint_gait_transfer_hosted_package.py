#!/usr/bin/env python3
"""Build the hash-frozen T62 hosted continuation bundle."""
...
from colab_t62_midpoint_gait_transfer_continuation import (
    CPU_RESULT_NAME,
...
MARKDOWN = (
    ANALYSIS
    / "T62_MIDPOINT_GAIT_TRANSFER_HOSTED_PACKAGE_20260728.md"
)
BUNDLE_NAME = "t62_midpoint_gait_transfer_bundle"
MANIFEST_NAME = "t62_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("T62 preregistration changed")
    ...
    if (
        ...
        raise ValueError("T62 package inputs changed")
    ...
    if copied_inventory != observed_inventory:
        raise ValueError("copied T62 playground changed")
    if (
        directory_sha256(assets / SOURCE_CHECKPOINT_NAME)
        != observed["source_checkpoint"]
    ):
        raise ValueError("copied T62 source checkpoint changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T62_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T62_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T62 isolated import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t62_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t62_hosted_package.v1",
        "status": "PASS_T62_MIDPOINT_GAIT_TRANSFER_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T62 midpoint gait-transfer hosted package",
                "",
    ...
    print(f"manifest_sha256={contract['bundle_manifest']['sha256']}")
