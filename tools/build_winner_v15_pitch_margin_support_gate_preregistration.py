#!/usr/bin/env python3
"""Freeze the unchanged support gate for Winner-v15 half/final checkpoints."""
...
OUTPUT = ANALYSIS / "winner_v15_pitch_margin_support_gate_preregistration.json"
MARKDOWN = ANALYSIS / "WINNER_V15_PITCH_MARGIN_SUPPORT_GATE_PREREGISTRATION_20260721.md"
...
SOURCES = {
    "builder": Path("tools/build_winner_v15_pitch_margin_support_gate_preregistration.py"),
    "gate_runner": Path("tools/run_winner_v15_pitch_margin_support_gate.py"),
    "gate_tests": Path("tests/test_winner_v15_pitch_margin_support_gate.py"),
    "preregistration_tests": Path("tests/test_winner_v15_pitch_margin_support_gate_preregistration.py"),
    "workflow": Path(".github/workflows/winner-v15-pitch-margin-support-gate.yml"),
    "training_result": Path("outputs/analysis/winner_v15_pitch_margin_support_training_result.json"),
    "training_preregistration": Path("outputs/analysis/winner_v15_pitch_margin_support_training_preregistration.json"),
    "training_runner": Path("tools/run_winner_v15_pitch_margin_support_training.py"),
    "training_importer": Path("tools/import_winner_v15_pitch_margin_support_training.py"),
...
def main() -> int:
    ...
    if (
        ...
        or training.get("decision") != "AUTHORIZE_SEPARATE_UNCHANGED_124_CELL_SUPPORT_GATE_PREREGISTRATION_ONLY"
        or training.get("failed_checks") != []
        or training.get("execution", {}).get("formal_support_cells") != 0
    ):
        raise ValueError("Winner-v15 training does not authorize a gate")
    ...
    if (
        ...
        raise ValueError("Winner-v15 training attribution changed")
    ...
    payload = {
        "schema_version": "winner_v15.pitch_margin_support_gate_preregistration.v1",
        "status": "PREREGISTERED_WINNER_V15_PITCH_MARGIN_SUPPORT_GATE",
        "decision": "AUTHORIZE_ONE_FROZEN_UNCHANGED_248_CELL_GATE_ONLY",
        "binding_change_only": (
            "The reviewed Winner-v12 evaluator, populations, thresholds, seeds, and all-or-nothing "
            "half/final rule are unchanged. Only the independently verified Winner-v15 snapshot and graph paths are bound."
        ),
        "training_artifact": {
            "repository_attribution": attribution,
            "source_stage1_snapshot": training["source_stage1_snapshot"],
            "half": {"snapshot": training["snapshot_manifest"][49], "graph": checkpoints[0]["graph"]},
            "final": {"snapshot": training["snapshot_manifest"][99], "graph": checkpoints[1]["graph"]},
        },
        "future_frozen_support_gate": gate,
        "execution_now": {"formal_support_cells": 0, "heldout_repeat_cells": 0, "locomotion_steps": 0, "robot_or_rdk_access": 0},
        "pass_rule": (
            "All 124 main cells and 32 heldout repeats must pass independently at both half and final. "
            "No nearest checkpoint, reward selection, or partial promotion is allowed."
        ),
        "authority": {
            "robot_clearance": False,
            "response_conditioned_locomotion_training_authorized": False,
            "pass_authorizes_only": "a separate response-conditioned locomotion preregistration",
        },
        "sources": sources,
    ...
    args.markdown.write_text("\n".join([
        "# Winner-v15 pitch-margin support-gate preregistration", "",
        f"- Status: `{payload['status']}`", f"- Decision: `{payload['decision']}`",
        "- Population: `124` main cells at half plus `124` at final",
        "- Repeats: `32` heldout cells per checkpoint", "- Duration: `250` ticks per cell",
        ...
        "independently imported Winner-v15 half/final artifacts.", "",
    ]), encoding="utf-8")
