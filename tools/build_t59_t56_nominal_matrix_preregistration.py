#!/usr/bin/env python3
"""Freeze T56's 16-cell corrected nominal behavior matrix."""
...
TRANSFORM = ANALYSIS / "t58_t56_postexport_result.json"
T27_PREREG = ANALYSIS / "t27_t23_robustness_matrix_preregistration.json"
T27_RUNNER_CONTRACT = (
    ANALYSIS / "t27_t23_robustness_runner_contract.json"
)
RUNNER = ROOT / "tools" / "run_t59_t56_nominal_matrix.py"
WORKER = ROOT / "tools" / "evaluate_t27_t23_robustness_condition.py"
ADAPTER = ROOT / "tools" / "t27_state_coherent_eval_adapter.py"
TEST = ROOT / "tests" / "test_t59_t56_nominal_matrix.py"
OUTPUT = ANALYSIS / "t59_t56_nominal_matrix_preregistration.json"
OUTPUT_MD = (
    ANALYSIS / "T59_T56_NOMINAL_MATRIX_PREREGISTRATION_20260728.md"
)
...
def main() -> int:
    for path in (OUTPUT, OUTPUT_MD):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T59: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"],
        cwd=ROOT,
        text=True,
    ).strip():
        raise RuntimeError("T59 preregistration requires a clean worktree")
    ...
    checks = {
        "t58_transform_green": (
            transform["status"] == "PASS_T58_T56_POSTEXPORT_TRANSFORM"
    ...
    payload: dict[str, Any] = {
        "schema_version": (
            "open_duck.t59_t56_nominal_matrix_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T59_T56_NOMINAL_MATRIX"
            if not failed
            else "HOLD_T59_T56_NOMINAL_MATRIX_PREREGISTRATION"
        ),
        "question": (
            "Do both T56 gait-transfer checkpoints pass every command and "
        ...
        "frozen_inputs": {
            "t58_transform": receipt(TRANSFORM),
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# T59 T56 nominal matrix preregistration",
                "",
