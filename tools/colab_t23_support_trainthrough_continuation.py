#!/usr/bin/env python3
"""Run the one frozen T23 support-train-through GPU continuation."""
...
PREREGISTRATION_NAME = "t23_support_trainthrough_hosted_preregistration.json"
CPU_RESULT_NAME = "t22_corrected_one_update_cpu_result.json"
COMPOSED_MANIFEST_NAME = "T19_COMPOSED_SOURCE_MANIFEST.json"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T23 prerequisite status changed")
    observed = {
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError(
            f"T23 inputs changed: {observed} != {prereg['input_hashes']}"
        )
    manifest = json.loads(
    ...
    if (
        ...
        raise RuntimeError(f"T23 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(work, arcname="t23_support_trainthrough_continuation")
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t23_hosted_result.v1",
        "status": "RUNNING_T23_SUPPORT_TRAINTHROUGH_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"T23 runner returned {completed.returncode}")
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T23_SUPPORT_TRAINTHROUGH_HOSTED_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"T23 artifact checks failed: {failed}")
        ...
        payload["status"] = (
            "HOLD_T23_SUPPORT_TRAINTHROUGH_HOSTED_CONTINUATION"
        )
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T23 requires hosted GPU authorization")
    return run(
