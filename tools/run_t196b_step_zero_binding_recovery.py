#!/usr/bin/env python3
"""Re-evaluate T196's immutable composition result under T196B."""
...
RESULT = ANALYSIS / "t196b_step_zero_binding_recovery_result.json"
MARKDOWN = ANALYSIS / "T196B_STEP_ZERO_BINDING_RECOVERY_RESULT_20260730.md"
...
def main() -> int:
    ...
    if not args.saved_result_authorized:
        raise PermissionError("T196B requires --saved-result-authorized")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T196B execution requires clean worktree")
    prereg = load(PREREG)
    if (
        prereg["status"]
        ...
        raise RuntimeError("T196B preregistration changed")
    for name, item in prereg["frozen_inputs"].items():
        verify(item, name)
    source = load(
        Path(prereg["frozen_inputs"]["t196_result"]["path"])
    )
    substantive = {
        name: passed
    ...
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t196b_step_zero_binding_recovery_result.v1"
        ),
        "status": (
            "PASS_T196B_STEP_ZERO_BINDING_RECOVERY"
            if not failed
            else "HOLD_T196B_STEP_ZERO_BINDING_RECOVERY"
        ),
        "decision": (
        ...
        "preserved_t196_checks": substantive,
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# T196B step-zero binding recovery result\n\n"
