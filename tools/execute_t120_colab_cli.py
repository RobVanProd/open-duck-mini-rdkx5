#!/usr/bin/env python3
"""Execute T120's exact package in one fresh Colab L4 session."""
...
PACKAGE = Path("/content/t120-joint-soft-router-20260729.tar.gz")
PACKAGE_BYTES = 4_322_347
PACKAGE_SHA256 = (
    "596599321c2f8933c6344f25a44455fa11546d97a511a9c8eb9b866292577f00"
)
BUNDLE_NAME = "t120_joint_soft_router_bundle"
PREREGISTRATION_SHA256 = (
    "3689bf8a4b5ce3caef53e780f03df87c734065fbefe5e3b5678515df7d0f430f"
)
CPU_RESULT_SHA256 = (
    "a058d34e8d85870a7dd6a7900a983ba4696e51bef5015b290f1820c2b4e64001"
)
RECOVERY_RESULT_SHA256 = (
    "8c0585cf7e526023038318e90c7fb312d19ec7811f141ef98c04b58d24438474"
)
PACKAGE_MANIFEST_SHA256 = (
    "e5342a2434e06ec4d822bbce631bdf4627075e2c6c2c974481d4b63b8ffa4dd0"
)
DRIVER_SHA256 = (
    "6fe7e774f66297ffd9bb4d70bd8c0483eace35411a44553d56188ed35348e6d7"
)
BASE_DRIVER_SHA256 = (
...
WORK = Path("/content/t120_work")
EXTRACT = Path("/content/t120_launch")
OUTPUT_JSON = Path("/content/t120_result.json")
OUTPUT_ARCHIVE = Path("/content/t120_artifacts.tar.gz")
RECEIPT = Path("/content/t120_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T120 requires L4: {gpu.stdout}{gpu.stderr}")
    for path in (WORK, EXTRACT, OUTPUT_JSON, OUTPUT_ARCHIVE, RECEIPT):
        if path.exists():
            raise FileExistsError(f"T120 no-retry path exists: {path}")
    if (
        PACKAGE.stat().st_size != PACKAGE_BYTES
        or sha256(PACKAGE) != PACKAGE_SHA256
    ):
        raise ValueError("T120 uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T120 package contains playground/.tmp")
        archive.extractall(EXTRACT, filter="data")
    bundle = EXTRACT / BUNDLE_NAME
    paths = (
        bundle / "t120_joint_soft_router_hosted_preregistration.json",
        bundle / "t119_joint_soft_router_cpu_result.json",
        bundle / "t119b_random_binding_recovery_result.json",
        bundle / "t120_package_manifest.json",
        bundle / "colab_t120_joint_soft_router_trainthrough.py",
    ...
    if observed != expected:
        raise ValueError("T120 extracted package identity changed")
    ...
    if install.returncode != 0:
        raise RuntimeError(f"T120 pinned install failed: {install.returncode}")
    versions = {
        name: importlib.metadata.version(name) for name in EXPECTED_VERSIONS
    }
    if versions != EXPECTED_VERSIONS:
        raise ValueError(f"T120 software versions changed: {versions}")
    ...
    preflight_source = "\n".join(
        (
            "from pathlib import Path",
            "import colab_t120_joint_soft_router_trainthrough as driver",
            ...
            "print('PASS_T120_HOSTED_FULL_PREFLIGHT')",
        )
    )
    ...
    if (
        ...
        or "PASS_T120_HOSTED_FULL_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T120 hosted preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    ...
    receipt = {
        "schema_version": "open_duck.t120_colab_launch_receipt.v1",
        "status": "STARTING_T120_HASH_FROZEN_GPU_CONTINUATION",
    ...
    receipt["status"] = (
        "COMPLETED_T120_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T120_COLAB_LAUNCH"
    )
    for name, path in (
