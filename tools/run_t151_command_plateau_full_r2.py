#!/usr/bin/env python3
"""Run T149B's sequential full R2 matrix one condition per invocation."""
...
PREREG = ANALYSIS / "t151_command_plateau_full_r2_preregistration.json"
RESULT = ANALYSIS / "t151_command_plateau_full_r2_result.json"
MARKDOWN = ANALYSIS / "T151_COMMAND_PLATEAU_FULL_R2_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t151_command_plateau_full_r2_v1"
)
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T151 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T151 matrix plan changed")
    return value
...
def write_progress(
    ...
    *,
    ...
    value = {
        "schema_version": "open_duck.t151_full_r2_progress.v1",
        "status": "IN_PROGRESS_T151_COMMAND_PLATEAU_FULL_R2",
...
def write_markdown(value: dict[str, Any]) -> None:
    lines = [
        "# T151 command-plateau full R2 result",
        "",
        f"- Status: `{value['status']}`",
        f"- Decision: `{value['decision']}`",
        f"- Completed conditions: `{value['summary']['completed_conditions']}/20`",
        f"- Green cells: `{value['summary']['green_cells']}/{value['summary']['completed_cells']}`",
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T151 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T151 execution requires clean worktree")
    ...
    if paused and first_failed is None:
        ...
        print("IN_PROGRESS_T151_COMMAND_PLATEAU_FULL_R2")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t151_command_plateau_full_r2_result.v1"
        ),
        "status": (
            "PASS_T151_COMMAND_PLATEAU_FULL_R2"
            if all_green
            else "HOLD_T151_COMMAND_PLATEAU_FULL_R2"
        ),
        "decision": (
