#!/usr/bin/env python3
"""Validate recovered T186 training on CPU before behavior."""
...
CPU_SOURCE = ANALYSIS / "t185c_in_episode_single_support_cpu_result.json"
CPU_AUTHORITY = (
    ANALYSIS / "t185f_metric_runner_path_recovery_result.json"
)
PACKAGE_CONTRACT = (
    ANALYSIS
    / "t186_in_episode_single_support_hosted_package_contract.json"
)
LAUNCH_CONTRACT = ANALYSIS / "t186_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t187_t186_recovered_training_validation.json"
MARKDOWN = (
    ANALYSIS / "T187_T186_RECOVERED_TRAINING_VALIDATION_20260730.md"
)
...
PREFIX_TAG = "eval/episode_t185/prefix_active"
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T187: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T187 validation requires clean worktree")

    recovery_root = args.recovery_root.resolve()
    work = (
        args.extracted_root.resolve()
        / "t186_in_episode_single_support_continuation"
    )
    training = work / "training"
    paths = {
        "hosted_result": recovery_root / "t186_result.json",
        "launch_receipt": recovery_root / "t186_launch_receipt.json",
        "recovery_archive": recovery_root / "t186_artifacts.tar.gz",
    }
    ...
    t185_readback = (
        "T185_IN_EPISODE_SINGLE_SUPPORT_PREFIX="
        "ticks=27,left_phase=2,right_phase=15,"
        "side_probability=0.5,transition=same_episode"
    )
    t98_readback = (
    ...
    checks = {
        ...
        "package_and_launch_contracts_green": (
            package["status"]
            == "PASS_T186_IN_EPISODE_SINGLE_SUPPORT_HOSTED_PACKAGE"
            and package["failed_checks"] == []
            and launch["status"] == "PASS_T186_COLAB_CLI_LAUNCH_CONTRACT"
            and launch["failed_checks"] == []
        ),
        "hosted_artifact_passed_pending_cpu_validation": (
            hosted["status"]
            == "PASS_T186_TRAINING_ARTIFACT_PENDING_CPU_VALIDATION"
            and hosted["failed_checks"] == []
            and hosted["cpu_topology_validation_required"] is True
        ),
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T186_COLAB_LAUNCH"
        ...
        "preregistration_hash_matches_hosted_and_receipt": (
            hosted["validation"]["preregistration_sha256"]
            == sha256(PREREGISTRATION)
            and receipt["member_hashes"][
                "t186_in_episode_single_support_hosted_preregistration.json"
            ]
            == sha256(PREREGISTRATION)
        ),
        ...
        "exact_t185_and_t98_readbacks": (
            t185_readback in log_text and t98_readback in log_text
        ),
        "hosted_prefix_metrics_exercised_bilaterally": all(
            len(values) == 3 and min(values) > 0.0
            for values in support_values.values()
        ),
        "hosted_same_episode_resume_metrics_exact": (
            len(prefix_values) == 3
            and all(value == 0.0 for value in prefix_values)
            and len(original_values) == 3
            and min(original_values) > 0.0
        ),
    ...
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t187_t186_recovered_training_validation.v1"
        ),
        "status": (
            "PASS_T187_T186_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T187_T186_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T188_T186_POSTEXPORT_COMPOSITION_PREREGISTRATION_ONLY"
            if not failed
        ...
        "input_hashes": {
            **input_hashes,
            "t186_preregistration": sha256(PREREGISTRATION),
            "t185_cpu_authority": sha256(CPU_AUTHORITY),
            "t185_cpu_source": sha256(CPU_SOURCE),
        ...
        "prefix_metrics": {
            ...
            "terminal_prefix_active": prefix_values,
            "resumed_original_reward": original_values,
        },
        "classification": {
    ...
    MARKDOWN.write_text(
        "# T187 T186 recovered training validation\n\n"
        f"- Status: `{value['status']}`\n"
        f"- Decision: `{value['decision']}`\n"
        f"- Failed checks: `{failed}`\n"
        "- Exports: 0 / 1,003,520 / 2,007,040\n"
        "- T185 prefix metrics: bilateral, finite, positive\n"
