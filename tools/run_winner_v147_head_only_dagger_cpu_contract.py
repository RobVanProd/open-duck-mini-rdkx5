#!/usr/bin/env python3
"""Run the preregistered V147 action-head-only DAgger CPU contract."""
...
import run_winner_v145_on_policy_dagger_cpu_contract as v145_runner  # noqa: E402
...
import winner_v147_head_only_dagger as head  # noqa: E402


ANALYSIS = ROOT / "outputs/analysis"
PREREG = ANALYSIS / "winner_v147_head_only_dagger_cpu_preregistration.json"
RESULT = ANALYSIS / "winner_v147_head_only_dagger_cpu_result.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V147_HEAD_ONLY_DAGGER_CPU_RESULT_20260725.md"
)
...
def run_head_updates(
    *,
    ...
    for update_index in range(1, 3):
        ...
        grads = head.mask_head_gradients(raw_grads)
        norm = float(head.gradient_norm(grads))
        slope = norm * norm
        initial_step = 1.0 / max(1.0, norm)
        accepted = None
        for backtrack in range(head.STD_BACKTRACK_LIMIT + 1):
            step = initial_step * (0.5**backtrack)
            candidate = head.apply_gradient(policy, grads, step)
            candidate_loss, candidate_components = evaluate(candidate)
            if float(candidate_loss) <= (
                float(loss) - head.ARMIJO_FRACTION * step * slope
            ):
                accepted = (
                    candidate,
                    step,
                ...
                break
        if accepted is None:
            raise RuntimeError("V147 deterministic line search failed")
        (
            policy,
            step,
            backtrack,
...
def main() -> int:
    ...
    observed_hashes = {
        ...
        "v145_result": file_sha256(V145_RESULT),
        "v146_result": file_sha256(V146_RESULT),
        ...
        "v145_training_module": file_sha256(
            TRAINING / "winner_v145_on_policy_dagger.py"
        ),
        "training_module": file_sha256(
            TRAINING / "winner_v147_head_only_dagger.py"
        ),
        "v145_runner": file_sha256(
            TOOLS / "run_winner_v145_on_policy_dagger_cpu_contract.py"
        ),
    ...
    if (
        ...
        raise ValueError("V147 CPU preregistration changed")
    ...
    v134_policy, v134_updates, _, _ = v145_runner.run_two_updates(
        network=network,
    ...
    v140_policy = head.interpolate_policy(
        source_policy, v134_policy, alpha
    )
    shadow_source = head.load_shadow_dataset(shadow_trace)
    teacher_base_action, teacher_base_hidden = v145_runner.evaluate_policy(
        network, normalizer, v140_policy, teacher_source, transform
    )
    shadow_base_action, shadow_base_hidden = v145_runner.evaluate_policy(
        network, normalizer, v140_policy, shadow_source, transform
    )
    dataset = head.build_aggregated_dataset(
    ...
    updated_policy, updates, gradient_rows, final_loss = run_head_updates(
        network=network,
        normalizer=normalizer,
    ...
    final_action, final_hidden = v145_runner.evaluate_policy(
    ...
    hidden_linf = float(
        np.max(np.abs(final_hidden - dataset["target_h_out"]))
    )
    ...
    expected_changed = {
        ...
        "1/params/residual_location/bias",
        "1/params/residual_location/kernel",
    }
    labeled = np.asarray(head.LABELED_JOINTS, dtype=np.int64)
    unlabeled = np.asarray(
        [
            index
            for index in range(14)
            if index not in head.LABELED_JOINTS
        ],
        dtype=np.int64,
    )
    head_deltas = {}
    selected_delta_min = float("inf")
    unselected_delta_max = 0.0
    for name in head.TRAINABLE_HEADS:
        before = v140_policy["params"][name]
        after = updated_policy["params"][name]
        bias_delta = np.abs(
            np.asarray(after["bias"]) - np.asarray(before["bias"])
        )
        kernel_delta = np.abs(
            np.asarray(after["kernel"]) - np.asarray(before["kernel"])
        )
        selected_delta_min = min(
            selected_delta_min,
            float(np.max(bias_delta[labeled])),
            float(np.max(kernel_delta[:, labeled])),
        )
        unselected_delta_max = max(
            unselected_delta_max,
            float(np.max(bias_delta[unlabeled])),
            float(np.max(kernel_delta[:, unlabeled])),
        )
        head_deltas[name] = {
            "selected_bias_linf": float(np.max(bias_delta[labeled])),
            "selected_kernel_linf": float(
                np.max(kernel_delta[:, labeled])
            ),
            "unselected_bias_linf": float(np.max(bias_delta[unlabeled])),
            "unselected_kernel_linf": float(
                np.max(kernel_delta[:, unlabeled])
            ),
        }
    ...
    for value, params in ((0, v140_tree), (2, updated_tree)):
        raw_path = work / f"winner_v147_raw_{value}.onnx"
        deployed_path = work / f"winner_v147_deployed_{value}.onnx"
        export_reference_residual_recurrent_adapter_onnx(
            params,
            action_size=14,
            obs_size=115,
    ...
    checks = {
        ...
        "v140_reconstruction_exact": step_zero_error <= 1.0e-7,
        "dataset_exact_5400_21_rows_60_elements": (
            dataset["obs"].shape == (5_400, 115)
            and int(np.sum(dataset["corrected"])) == 21
            and int(np.sum(dataset["joint_mask"])) == 60
        ),
        "labeled_joint_set_exact": tuple(
            np.flatnonzero(np.any(dataset["joint_mask"], axis=0))
        )
        == head.LABELED_JOINTS,
        ...
        "both_updates_accepted": len(updates) == 2,
        "loss_decreases": (
        ...
        "hidden_output_bit_exact": hidden_linf == 0.0,
        "checkpoint_structure_exact": structure,
        "only_four_location_head_leaves_changed": (
            changed == expected_changed
        ),
        "every_selected_head_slice_changed": selected_delta_min > 0.0,
        "all_unselected_head_columns_bit_exact": unselected_delta_max == 0.0,
    ...
    payload = {
        "schema_version": "winner_v147.head_only_dagger_cpu_result.v1",
        "status": (
            "PASS_WINNER_V147_HEAD_ONLY_DAGGER_CPU_CONTRACT"
            if not failed
            else "HOLD_WINNER_V147_HEAD_ONLY_DAGGER_CPU_CONTRACT"
        ),
        ...
        "dataset": {
            "rows": 5_400,
            "corrected_rows": int(np.sum(dataset["corrected"])),
            "joint_labels": int(np.sum(dataset["joint_mask"])),
            "labeled_joints": list(head.LABELED_JOINTS),
        ...
        "decision": (
            "EARN_ONE_V148_HEAD_ONLY_DAGGER_CAUSAL_BEHAVIOR_PREREGISTRATION"
            if not failed
            else "CLOSE_HEAD_ONLY_DAGGER"
        ),
        "authority": {
            "v148_behavior_preregistration": not failed,
            "behavior": False,
    ...
    MARKDOWN.write_text(
        "# Winner V147 head-only DAgger CPU contract\n\n"
        f"- Status: `{payload['status']}`\n"
        "- Trainable: seven labeled columns of adapter_location and "
        "residual_location only.\n"
        f"- Corrected/preservation ratios: "
