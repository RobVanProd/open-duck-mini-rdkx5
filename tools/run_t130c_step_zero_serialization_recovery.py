#!/usr/bin/env python3
"""Run the corrected frozen read-only T130C step-zero recovery."""
...
PREREG = ANALYSIS / "t130c_step_zero_serialization_preregistration.json"
RESULT = ANALYSIS / "t130c_step_zero_serialization_result.json"
MARKDOWN = ANALYSIS / "T130C_STEP_ZERO_SERIALIZATION_RESULT_20260729.md"
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t130c_step_zero_serialization_recovery"
)
...
def verify_file_receipt(item: dict[str, Any], name: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T130C frozen input changed: {name}={path}")
...
def main() -> int:
    ...
    if not args.read_only_authorized:
        raise PermissionError("T130C requires --read-only-authorized")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T130C execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T130C preregistration changed")
    ...
    omitted = np.asarray(numpy_helper.to_array(omitted_tensor))
    ...
    facts = {
        ...
        "nodes_bit_exact": (
            node_bytes(expected_model) == node_bytes(hosted_model)
        ),
        "abi": {
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t130c_step_zero_serialization_result.v1"
        ),
        "status": (
            "PASS_T130C_STEP_ZERO_SERIALIZATION_RECOVERY"
            if passed
            else "HOLD_T130C_STEP_ZERO_SERIALIZATION_RECOVERY"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T130C step-zero serialization result\n\n"
