#!/usr/bin/env python3
"""Run the T129 negative-only expert frozen nominal matrix."""
...
PREREG = ANALYSIS / "t132_t129_nominal_preregistration.json"
RESULT = ANALYSIS / "t132_t129_nominal_result.json"
MARKDOWN = ANALYSIS / "T132_T129_NOMINAL_RESULT_20260729.md"
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T132 input: {path}")


def load() -> dict[str, Any]:
    value = json.loads(PREREG.read_text(encoding="utf-8"))
    if (
        value["status"] != "PREREGISTERED_T132_T129_NOMINAL_MATRIX"
        or value["failed_checks"]
        or canonical_sha256(value, "preregistered_contract_sha256")
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T132 preregistration changed")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T132 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T132 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t132_t129_nominal_result.v1",
        "status": (
            "PASS_T132_T129_NOMINAL_MATRIX"
            if passed
            else "HOLD_T132_T129_NOMINAL_MATRIX"
        ),
        "decision": (
