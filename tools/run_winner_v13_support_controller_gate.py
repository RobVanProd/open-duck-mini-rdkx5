#!/usr/bin/env python3
"""Bind the reviewed support/context gate to Winner-v13 training artifacts."""
...
def validate_snapshot(snapshot: Mapping[str, Any]) -> None:
    """Validate a v13 checkpoint before the reviewed evaluator consumes it."""

    if smoke is None or training is None:
        raise AssertionError("Winner-v13 support-gate dependencies were not loaded")
    if set(snapshot) != {
        "parameters",
        "optimizer",
        "metadata",
        ...
        raise ValueError("Winner-v13 support checkpoint state schema changed")
    metadata = snapshot["metadata"]
    update = metadata.get("completed_updates")
    if (
        metadata.get("schema_version")
        != "winner_v13.support_controller_training_snapshot.v1"
        or metadata.get("stage") != "stage2"
        or update not in {50, 100}
        or metadata.get("root_seed") != 120120
        or metadata.get("learning_rate") != 0.0001
        or metadata.get("source_stage1_snapshot_sha256")
        ...
        raise ValueError("Winner-v13 support checkpoint metadata changed")
    if _TRAINING is None:
        raise AssertionError("Winner-v13 training result was not loaded")
    expected_stage1 = _TRAINING["source_stage1_snapshot"][
        "frozen_stage1_tree_sha256"
    ]
    if (
        metadata.get("frozen_stage1_tree_sha256") != expected_stage1
        or smoke.tree_sha256(training.stage1_parameters(snapshot["parameters"]))
        != expected_stage1
    ):
        raise ValueError("Winner-v13 frozen Stage-1 tree changed")
    for name in ("target_mean", "target_std"):
        value = np.asarray(snapshot[name])
        if value.shape != (50,) or value.dtype != np.dtype(np.float32):
            raise ValueError(f"Winner-v13 {name} schema changed")
        if not bool(np.all(np.isfinite(value))):
            raise ValueError(f"Winner-v13 {name} contains nonfinite values")
    if bool(np.any(np.asarray(snapshot["target_std"]) <= 0.0)):
        raise ValueError("Winner-v13 target_std is not strictly positive")
...
def checkpoint_paths(work_root: Path, label: str) -> tuple[Path, Path]:
    ...
    if label not in mapping:
        raise ValueError(f"unknown Winner-v13 support checkpoint: {label}")
    ...
    for path, receipt in ((snapshot, snapshot_receipt), (graph, graph_receipt)):
        if (
            ...
            raise ValueError(f"Winner-v13 {label} artifact bytes changed: {path.name}")
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "winner_v13.support_controller_gate_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V13_SUPPORT_CONTROLLER_GATE"
        or value.get("decision") != "AUTHORIZE_ONE_FROZEN_248_CELL_GATE_ONLY"
        ...
        raise ValueError("Winner-v13 support-gate preregistration changed")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support-gate dimensions changed")
    sources = value.get("sources")
    if not isinstance(sources, dict) or not sources:
        raise ValueError("Winner-v13 support-gate sources are absent")
    for name, item in sources.items():
        ...
        if (
            ...
            raise ValueError(f"Winner-v13 support-gate source changed: {name}")
    if reviewed_gate.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v13 support-gate source manifest changed")
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("Winner-v13 support training does not authorize the gate")
    ...
    reviewed["schema_version"] = "winner_v13.support_controller_gate_result.v1"
    reviewed["status"] = (
