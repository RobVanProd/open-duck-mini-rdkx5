#!/usr/bin/env python3
"""Build the hash-frozen T78 hosted continuation bundle."""
...
MARKDOWN = (
    ANALYSIS / "T78_ENDPOINT_JOINT_ADAPTER_HOSTED_PACKAGE_20260728.md"
)
BUNDLE_NAME = "t78_endpoint_joint_adapter_bundle"
MANIFEST_NAME = "t78_package_manifest.json"
...
def main() -> int:
    ...
    step_zero = Path(prereg["paths"]["expected_step_zero_raw"]).resolve()
    ...
    if (
        ...
        raise ValueError("T78 preregistration changed")
    ...
    if (
        ...
        raise ValueError("T78 package inputs changed")
    ...
    shutil.copy2(step_zero, assets / STEP_ZERO_NAME)
    ...
    if copied_inventory != observed_inventory:
        raise ValueError("copied T78 playground changed")
    if (
        directory_sha256(assets / SOURCE_CHECKPOINT_NAME)
        != observed["source_checkpoint"]
        or sha256(assets / STEP_ZERO_NAME)
        != observed["expected_step_zero_raw"]
    ):
        raise ValueError("copied T78 source assets changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "assert 'winner_t77_endpoint_joint_adapter_continuation' "
                "in text; "
                "print('PASS_T78_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T78_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T78 isolated import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t78_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t78_hosted_package.v1",
        "status": "PASS_T78_ENDPOINT_JOINT_ADAPTER_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T78 endpoint joint-adapter hosted package",
                "",
