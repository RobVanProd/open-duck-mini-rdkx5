#!/usr/bin/env python3
"""Bind the unchanged reviewed support gate to Winner-v15 checkpoints."""
...
def validate_snapshot(snapshot: Mapping[str, Any]) -> None:
    if smoke is None or training is None:
        raise AssertionError("Winner-v15 gate dependencies were not loaded")
    if set(snapshot) != {"parameters", "optimizer", "metadata", "target_mean", "target_std"}:
        raise ValueError("Winner-v15 checkpoint state schema changed")
    metadata = snapshot["metadata"]
    update = metadata.get("completed_updates")
    if (
        metadata.get("schema_version") != "winner_v15.pitch_margin_support_training_snapshot.v1"
        or metadata.get("stage") != "stage2" or update not in {50, 100}
        or metadata.get("root_seed") != 120120 or metadata.get("learning_rate") != 0.0001
        ...
        or metadata.get("preregistration_lf_sha256") != smoke.lf_sha256(TRAINING_PREREGISTRATION)
        ...
        or len(metadata.get("metrics", [])) != update
    ):
        raise ValueError("Winner-v15 checkpoint metadata changed")
    if _TRAINING is None:
        raise AssertionError("Winner-v15 training result was not loaded")
    expected_stage1 = _TRAINING["source_stage1_snapshot"]["frozen_stage1_tree_sha256"]
    if (
        metadata.get("frozen_stage1_tree_sha256") != expected_stage1
        or smoke.tree_sha256(training.stage1_parameters(snapshot["parameters"])) != expected_stage1
    ):
        raise ValueError("Winner-v15 frozen Stage-1 tree changed")
    for name in ("target_mean", "target_std"):
        value = np.asarray(snapshot[name])
        if value.shape != (50,) or value.dtype != np.dtype(np.float32) or not bool(np.all(np.isfinite(value))):
            raise ValueError(f"Winner-v15 {name} schema changed")
    if bool(np.any(np.asarray(snapshot["target_std"]) <= 0.0)):
        raise ValueError("Winner-v15 target_std is not strictly positive")
...
def checkpoint_paths(work_root: Path, label: str) -> tuple[Path, Path]:
    ...
    if label not in mapping:
        raise ValueError(f"unknown Winner-v15 checkpoint: {label}")
    update = mapping[label]
    snapshot = work_root / "snapshots" / f"snapshot_stage2_update_{update:03d}.npz"
    graph = work_root / "graphs" / f"winner_v15_pitch_margin_{label}.onnx"
    ...
    for path, receipt in ((snapshot, snapshot_receipt), (graph, graph_receipt)):
        if not path.is_file() or smoke.sha256(path) != receipt["sha256"] or path.stat().st_size != receipt["bytes"]:
            raise ValueError(f"Winner-v15 {label} artifact bytes changed: {path.name}")
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    if smoke is None or reviewed_gate is None:
        raise AssertionError("Winner-v15 gate dependencies were not loaded")
    if (
        value.get("schema_version") != "winner_v15.pitch_margin_support_gate_preregistration.v1"
        or value.get("status") != "PREREGISTERED_WINNER_V15_PITCH_MARGIN_SUPPORT_GATE"
        or value.get("decision") != "AUTHORIZE_ONE_FROZEN_UNCHANGED_248_CELL_GATE_ONLY"
        or value.get("execution_now") != {
            "formal_support_cells": 0, "heldout_repeat_cells": 0,
            "locomotion_steps": 0, "robot_or_rdk_access": 0,
        }
    ):
        raise ValueError("Winner-v15 gate preregistration changed")
    ...
    if (
        ...
        raise ValueError("Winner-v15 gate dimensions changed")
    sources = value.get("sources")
    if not isinstance(sources, dict) or not sources:
        raise ValueError("Winner-v15 gate sources are absent")
    for name, item in sources.items():
        path = ROOT / item["path"]
        if set(item) != {"hash_mode", "path", "sha256"} or item["hash_mode"] != "lf" or smoke.lf_sha256(path) != item["sha256"]:
            raise ValueError(f"Winner-v15 gate source changed: {name}")
    if reviewed_gate.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v15 gate source manifest changed")


def main() -> int:
    global reviewed_gate, smoke, training, _TRAINING
    ...
    reviewed_gate, smoke, training = reviewed_gate_module, smoke_module, training_module
    ...
    if (
        training_result.get("status") != "PASS_WINNER_V15_PITCH_MARGIN_SUPPORT_TRAINING_ARTIFACT"
        or training_result.get("decision") != "AUTHORIZE_SEPARATE_UNCHANGED_124_CELL_SUPPORT_GATE_PREREGISTRATION_ONLY"
        or training_result.get("failed_checks") != []
        or training_result.get("execution", {}).get("formal_support_cells") != 0
    ):
        raise ValueError("Winner-v15 training does not authorize the gate")
    ...
    reviewed_gate.full_training.validate_resume = validate_snapshot
    ...
    reviewed["schema_version"] = "winner_v15.pitch_margin_support_gate_result.v1"
    reviewed["status"] = "PASS_WINNER_V15_PITCH_MARGIN_SUPPORT_GATE" if passed else "HOLD_WINNER_V15_PITCH_MARGIN_SUPPORT_GATE"
    reviewed["decision"] = (
