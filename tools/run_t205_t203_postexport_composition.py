#!/usr/bin/env python3
"""Compose T203 nominal heads into the frozen T164-final graph."""
...
RESULT = ANALYSIS / "t205_t203_postexport_composition_result.json"
MARKDOWN = ANALYSIS / "T205_T203_POSTEXPORT_COMPOSITION_RESULT_20260730.md"
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t205_t203_postexport_composition_v1"
)
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or WORK.exists():
        raise FileExistsError("refusing to overwrite T205 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T205 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T205 preregistration changed")
    ...
    for graph in prereg["graphs"]:
        ...
        destination = WORK / str(step_value) / "t203_composed.onnx"
    ...
    checks = {
        ...
        "both_y_negative_fits_exercise_changed_moving_action": all(
            row["inference"][
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t205_t203_postexport_composition_result.v1"
        ),
        "status": (
            "PASS_T205_T203_POSTEXPORT_COMPOSITION"
            if not failed
            else "HOLD_T205_T203_POSTEXPORT_COMPOSITION"
        ),
        "decision": (
            "EARN_T206_T203_NOMINAL_BEHAVIOR_MATRIX_"
            "PREREGISTRATION_ONLY"
            if not failed
            else "HOLD_T203_BEHAVIOR_AND_AUDIT_COMPOSITION"
        ),
    ...
    MARKDOWN.write_text(
        "# T205 T203 post-export composition result\n\n"
