#!/usr/bin/env python3
"""Freeze T216's exact 16-cell Y-negative persistence matrix."""
...
OUTPUT = ANALYSIS / "t220_t216_targeted_y_negative_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T220_T216_TARGETED_Y_NEGATIVE_PREREGISTRATION_20260730.md"
)
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t220_t216_targeted_y_negative.py"
TEST = ROOT / "tests" / "test_t220_t216_targeted_y_negative.py"


def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T220: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T220 preregistration requires clean worktree")
    ...
    graph_by_step = {
        ...
        for row in t218["graphs"]
    }
    policies = [
        {
            "checkpoint_id": f"T216_COMPOSED_{label}",
            "step": value,
            **graph_by_step[value],
        }
        for label, value in (
    ...
    checks = {
        "t219_nominal_matrix_green": (
            t219["status"] == "PASS_T219_T216_NOMINAL_MATRIX"
            and t219["condition"]["condition_green"]
            and t219["condition"]["green_cells"] == 16
            and t219["decision"]
            == "EARN_T220_T216_TARGETED_Y_NEGATIVE_MATRIX_"
            "PREREGISTRATION_ONLY"
        ),
        "t218_composition_green": (
            t218["status"] == "PASS_T218_T216_POSTEXPORT_COMPOSITION"
            and not t218["failed_checks"]
        ),
        "policies_match_t218": (
            len(policies) == 2
    ...
    if failed:
        raise RuntimeError(f"T220 preregistration checks failed: {failed}")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t220_t216_targeted_y_negative_preregistration.v1"
        ),
        "status": "PREREGISTERED_T220_T216_TARGETED_Y_NEGATIVE",
        **copied,
        "condition": condition,
        "policies": policies,
        "matrix": {
        ...
        "decision_rule": {
            ...
            "fail_decision": "CLOSE_T216_AXIS_COMPLETE_TILT_CONTINUATION",
            "no_retry": True,
        ...
        "frozen_inputs": {
            ...
            for name, path in {
                ...
                "runner": RUNNER,
                "test": TEST,
                "t218_composition": T218,
                "t219_nominal_result": T219,
    ...
    MARKDOWN.write_text(
        "# T220 T216 targeted Y-negative preregistration\n\n"
