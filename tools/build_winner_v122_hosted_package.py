#!/usr/bin/env python3
"""Build the hash-frozen Winner-v122 hosted continuation bundle."""
...
CONTRACT = ANALYSIS / "winner_v122_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V122_HOSTED_PACKAGE_CONTRACT_20260724.md"
PREREGISTRATION_SHA256 = (
    "fbbd796fdbf30d0e634ea712ceadd2946af035037bf6481f4ef3e1d0caed8c8e"
)
BUNDLE_NAME = "winner_v122_episode_peak_bundle"
MANIFEST_NAME = "winner_v122_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("Winner-v122 preregistration changed")
    observed = {
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError("Winner-v122 package inputs changed")
    ...
    composed_manifest = json.loads(
        (
            bundle / "playground/WINNER_V122_COMPOSED_SOURCE_MANIFEST.json"
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_WINNER_V122_PACKAGE_IMPORT')"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_WINNER_V122_PACKAGE_IMPORT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            "Winner-v122 isolated import failed: "
            f"{preflight.stdout}\n{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "winner_v122.hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "winner_v122.hosted_package.v1",
        "status": "PASS_WINNER_V122_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            ...
            "driver_dependency_hash_frozen": True,
    ...
    MARKDOWN.write_text(
        "# Winner-v122 hosted package contract\n\n"
        f"Status: `{contract['status']}`\n\n"
        f"- archive SHA-256: `{contract['archive']['sha256']}`\n"
        f"- archive bytes: `{contract['archive']['bytes']}`\n"
        f"- bundle files: `{len(members) + 1}`\n"
        "- isolated episode-peak import: `PASS`\n"
        "- credentials and robot access material: `ABSENT`\n\n"
        "This bundle authorizes one exact hosted continuation without "
        "retry. It grants no behavior selection, Gate 5, RDK-X5, robot, "
