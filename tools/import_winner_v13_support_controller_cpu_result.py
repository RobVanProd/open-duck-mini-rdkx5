#!/usr/bin/env python3
"""Safely import the Winner-v13 support-controller CPU contract artifact."""
...
OUTPUT_JSON = ANALYSIS / "winner_v13_support_controller_cpu_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V13_SUPPORT_CONTROLLER_CPU_RESULT_20260721.md"
RAW_RESULT = "winner-v13-support-controller-cpu-result.json"
RAW_RECEIPT = "winner-v13-support-controller-cpu-result.sha256"
WORK_PREFIX = "winner-v13-support-controller-cpu-work"
RAW_GRAPH = f"{WORK_PREFIX}/winner_v13_support_controller_one_update.onnx"
RAW_SNAPSHOT = f"{WORK_PREFIX}/winner_v13_support_controller_update_001.npz"
CHECKS = {
    ...
    "source_snapshot_hash_and_bytes_exact",
    "source_snapshot_stage1_update_100_exact",
    "source_stage1_result_passed",
...
def read_artifact(path: Path) -> dict[str, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v13 support CPU artifact inventory changed")
        if sum(item.file_size for item in infos) > 10_000_000:
            raise ValueError("Winner-v13 support CPU artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v13 support CPU artifact has unsafe member")
...
def validate_result(value: Mapping[str, Any]) -> None:
    if value.get("schema_version") != "winner_v13.support_controller_cpu_result.v1":
        raise ValueError("Winner-v13 support CPU result schema changed")
    if (
        value.get("status") != "PASS_WINNER_V13_SUPPORT_CONTROLLER_CPU_CONTRACT"
        or value.get("decision")
        != "AUTHORIZE_SUPPORT_CONTROLLER_TRAINING_PREREGISTRATION_ONLY"
        or value.get("failed_checks") != []
    ):
        raise ValueError("Winner-v13 support CPU contract did not pass")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support CPU checks changed")
    if value.get("execution") != {
        ...
        raise ValueError("Winner-v13 support CPU execution boundary changed")
    if value.get("sources") != {
        ...
        raise ValueError("Winner-v13 support CPU source identities changed")
    if value.get("authority") != {
        "robot_clearance": False,
        "rdkx5_robot_serial_gpio_i2c_torque_motion": False,
        "pass_authorizes_only": "a separate support-controller training preregistration",
    }:
        raise ValueError("Winner-v13 support CPU authority changed")
    proof = value.get("proof")
    if not isinstance(proof, dict):
        raise ValueError("Winner-v13 support CPU proof is absent")
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    source = contract["source_artifact"]
    if (
        proof.get("source_snapshot_sha256") != source["snapshot_sha256"]
        ...
        raise ValueError("Winner-v13 support CPU restored proof changed")
    for name in ("gradient_max_abs", "stage2_leaf_max_abs_delta"):
        ...
        if (
            ...
            raise ValueError(f"Winner-v13 support CPU {name} changed")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support CPU action boundary changed")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support CPU graph proof changed")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support CPU snapshot proof changed")
...
def verify_payloads(members: Mapping[str, bytes], value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("Winner-v13 support CPU graph payload changed")
    if (
        sha256_bytes(members[RAW_SNAPSHOT]) != snapshot["sha256"]
        or len(members[RAW_SNAPSHOT]) != snapshot["bytes"]
    ):
        raise ValueError("Winner-v13 support CPU snapshot payload changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v13 support CPU result is already imported")
    ...
    if (
        ...
        or args.artifact_name != f"winner-v13-support-controller-cpu-{args.run_id}"
        or args.artifact_digest != f"sha256:{zip_sha}"
    ):
        raise ValueError("Winner-v13 support CPU repository attribution changed")
    ...
    if members[RAW_RECEIPT] != f"{raw_sha}  /tmp/{RAW_RESULT}\n".encode():
        raise ValueError("Winner-v13 support CPU result receipt changed")
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                ...
                f"- Stage-1 snapshot SHA-256: `{proof['source_snapshot_sha256']}`",
                f"- Sampled / valid transitions: `{proof['sampled_count']} / {proof['valid_transition_count']}`",
                f"- JAX/ONNX maximum absolute error: `{proof['graph_contract']['jax_onnx_max_abs_error']}`",
