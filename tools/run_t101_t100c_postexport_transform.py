#!/usr/bin/env python3
"""Apply T100C's frozen deployment chain to all three exports."""
...
PREREGISTRATION = ANALYSIS / "t101_t100c_postexport_preregistration.json"
RESULT = ANALYSIS / "t101_t100c_postexport_result.json"
MARKDOWN = ANALYSIS / "T101_T100C_POSTEXPORT_RESULT_20260728.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T101 exact transform requires --execute")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T101 transform requires clean worktree")
    ...
    if raw_steps != EXPECTED_STEPS or raw_hashes != prereg["raw_graphs"]:
        raise ValueError("T101 raw export inventory changed")
    ...
    checks = {
        ...
        "both_postupdate_exports_transformed": all(
            str(step) in deployments for step in EXPECTED_STEPS[1:]
        ),
    ...
    value = {
        "schema_version": "open_duck.t101_t100c_postexport_result.v1",
        "status": (
            "PASS_T101_T100C_POSTEXPORT_TRANSFORM"
            if not failed
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T101 T100C post-export result",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                f"- Failed checks: `{failed}`",
                "- All three graphs use the identical T99-verified deployment chain",
