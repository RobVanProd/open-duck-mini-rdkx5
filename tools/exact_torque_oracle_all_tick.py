...
def project_action(
    *,
    ...
    if not np.all(np.isfinite(np.concatenate((action, low, high)))):
        raise ValueError("oracle projection arrays must be finite")
    if np.any(low > high) or np.any(horizons < 0):
        raise ValueError("supreme oracle interval or horizon is invalid")
    if (
    ...
    requested_base = action.copy()
    action = np.clip(action, low, high)
    base = action.copy()
    base_supreme_clip = action - requested_base
    base_supreme_clipped_joint_indices = np.flatnonzero(
        np.abs(base_supreme_clip) > 0.0
    ).astype(int)
    empty: set[int] = set()
    ...
    return action.astype(np.float32), {
        ...
        "base_action": requested_base.tolist(),
        "supreme_feasible_base_action": base.tolist(),
        "base_supreme_clip_linf": float(
            np.max(np.abs(base_supreme_clip))
        ),
        "base_supreme_clip_l2": float(
            np.linalg.norm(base_supreme_clip)
        ),
        "base_supreme_clipped_joint_indices": (
            base_supreme_clipped_joint_indices.tolist()
        ),
        ...
        "torque_projection_clip_linf": float(
            np.max(np.abs(action - base))
        ),
        "torque_projection_clip_l2": float(
            np.linalg.norm(action - base)
        ),
        "clip_linf": float(np.max(np.abs(action - requested_base))),
        "clip_l2": float(np.linalg.norm(action - requested_base)),
    }
