#!/usr/bin/env python3
"""Execute T23's exact cache-free bundle in one fresh Colab L4 session."""
...
PACKAGE_MANIFEST_SHA256 = (
    "7b27dd1d3da73ee7660f6d48f4b302c196da37f6d5db92aeb4f7ed1bfe89b7dd"
)
DRIVER_SHA256 = (
...
WORK = Path("/content/t23b_work")
EXTRACT = Path("/content/t23b_launch")
OUTPUT_JSON = Path("/content/t23b_result.json")
OUTPUT_ARCHIVE = Path("/content/t23b_artifacts.tar.gz")
RECEIPT = Path("/content/t23b_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T23 recovery requires L4: {gpu.stdout}{gpu.stderr}")
    for path in (WORK, EXTRACT, OUTPUT_JSON, OUTPUT_ARCHIVE, RECEIPT):
        if path.exists():
            raise FileExistsError(f"T23 recovery no-retry path exists: {path}")
    if (
        PACKAGE.stat().st_size != PACKAGE_BYTES
        or sha256(PACKAGE) != PACKAGE_SHA256
    ):
        raise ValueError("T23 recovery uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T23 recovery package contains playground/.tmp")
    ...
    if (
        ...
        raise ValueError("T23 recovery extracted package identity changed")
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T23 recovery pinned install failed: {completed.returncode}"
        )
    observed_versions = {
    ...
    if observed_versions != EXPECTED_VERSIONS:
        raise ValueError(
            f"T23 recovery software versions changed: {observed_versions}"
        )
    ...
    if (
        ...
        or "PASS_T23B_HOSTED_IMPORT 14"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T23 recovery hosted import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    command = [
        sys.executable,
    ...
    receipt = {
        "schema_version": "open_duck.t23b_colab_launch_receipt.v1",
        "status": "STARTING_T23B_HASH_FROZEN_GPU_CONTINUATION",
        ...
        "hosted_import_preflight": "PASS_T23B_HOSTED_IMPORT 14",
        "software_versions": observed_versions,
        "command": command,
        "retry": False,
        "resume": False,
        "prior_preexecution_transport_hold_decision_weight": 0,
    ...
    receipt["status"] = (
        "COMPLETED_T23B_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T23B_COLAB_LAUNCH"
    )
