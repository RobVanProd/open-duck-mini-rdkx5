#!/usr/bin/env python3
"""Freeze T186's one-session Colab CLI launch contract."""
...
PACKAGE = Path(
    ...
    "t186_in_episode_single_support_bundle_20260730.tar.gz"
)
EXECUTOR = ROOT / "tools/execute_t186_colab_cli.py"
OUTPUT = ANALYSIS / "t186_colab_cli_launch_contract.json"
MARKDOWN = ANALYSIS / "T186_COLAB_CLI_LAUNCH_CONTRACT_20260730.md"
SESSION = "t186-single-support-20260730"
PACKAGE_SHA256 = (
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T186 launch contract requires clean tree")
    ...
    checks = {
        ...
        "executor_uses_exact_driver_and_prefix": (
            "colab_t186_in_episode_single_support_continuation.py" in text
            and "PASS_T186_HOSTED_FULL_PREFLIGHT" in text
            and "--winner_t185_in_episode_single_support_prefix" in text
        ),
    ...
    commands = [
        ...
        [
            ...
            "--session",
            SESSION,
            "/content/t186_launch_receipt.json",
            "<LOCAL_RECEIPT>",
        ],
        ["colab", "stop", "--session", SESSION],
    ]
    value = {
        "schema_version": "open_duck.t186_colab_cli_launch_contract.v1",
        "status": (
            "PASS_T186_COLAB_CLI_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_T186_COLAB_CLI_LAUNCH_CONTRACT"
        ),
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# T186 Colab CLI launch contract\n\n"
