#!/usr/bin/env python3
"""Run T156B's frozen positive-COM endpoint matrix."""
...
PREREG = ANALYSIS / "t157_positive_com_endpoint_preregistration.json"
RESULT = ANALYSIS / "t157_positive_com_endpoint_result.json"
MARKDOWN = (
    ANALYSIS / "T157_POSITIVE_COM_ENDPOINT_RESULT_20260729.md"
)
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t157_positive_com_endpoint_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T157 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T157 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T157 execution requires clean worktree")
    ...
    if (
        prereg["status"] != "PREREGISTERED_T157_POSITIVE_COM_ENDPOINT"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T157 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t157_positive_com_endpoint_result.v1"
        ),
        "status": (
            "PASS_T157_POSITIVE_COM_ENDPOINT"
            if passed
            else "HOLD_T157_POSITIVE_COM_ENDPOINT"
        ),
        "decision": (
        ...
        "authority": {
            "resume_r2_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
    ...
    MARKDOWN.write_text(
        "# T157 positive-COM endpoint result\n\n"
