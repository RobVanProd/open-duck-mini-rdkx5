#!/usr/bin/env python3
"""Run T175's sequential full R2 matrix one condition per invocation."""
...
PREREG = ANALYSIS / "t177_head_prefix_mean_full_r2_preregistration.json"
RESULT = ANALYSIS / "t177_head_prefix_mean_full_r2_result.json"
MARKDOWN = ANALYSIS / "T177_HEAD_PREFIX_MEAN_FULL_R2_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t177_head_prefix_mean_full_r2_v1"
)
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T177 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T177 matrix plan changed")
    return value
...
def write_progress(
    ...
    *,
    ...
    value = {
        "schema_version": (
            "open_duck.t177_head_prefix_mean_full_r2_progress.v1"
        ),
        "status": "IN_PROGRESS_T177_HEAD_PREFIX_MEAN_FULL_R2",
...
def write_markdown(value: dict[str, Any]) -> None:
    lines = [
        "# T177 head-prefix-mean full R2 result",
        "",
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T177 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T177 execution requires clean worktree")
    ...
    if paused and first_failed is None:
        ...
        print("IN_PROGRESS_T177_HEAD_PREFIX_MEAN_FULL_R2")
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t177_head_prefix_mean_full_r2_result.v1",
        "status": (
            "PASS_T177_HEAD_PREFIX_MEAN_FULL_R2"
            if all_green
            else "HOLD_T177_HEAD_PREFIX_MEAN_FULL_R2"
        ),
        "decision": (
