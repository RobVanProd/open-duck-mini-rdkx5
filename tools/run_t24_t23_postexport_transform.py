#!/usr/bin/env python3
"""Apply T23's frozen deployment chain to all three exact exports."""
...
PREREGISTRATION = ANALYSIS / "t24_t23_postexport_preregistration.json"
T22_RESULT = ANALYSIS / "t22_corrected_one_update_cpu_result.json"
RESULT = ANALYSIS / "t24_t23_postexport_result.json"
MARKDOWN = ANALYSIS / "T24_T23_POSTEXPORT_RESULT_20260726.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T24 exact transform requires --execute")
    ...
    if (
        ...
        or raw_hashes != prereg["raw_graphs"]
    ):
        raise ValueError("T24 raw export inventory changed")

    work.mkdir(parents=True)
    deployments = {
        str(graph_step(path)): t22.deployment_graph(
            path,
            work / str(graph_step(path)),
        )
        for path in raw_graphs
    }
    t22_result = json.loads(T22_RESULT.read_text(encoding="utf-8"))
    golden = t22_result["deployments"]["0"]
    step_zero = deployments["0"]
    postupdate = [
        deployments[str(step)] for step in EXPECTED_STEPS[1:]
    ]
    checks = {
        "raw_inventory_exact": raw_steps == EXPECTED_STEPS
        and raw_hashes == prereg["raw_graphs"],
        "step_zero_raw_byte_exact": (
            step_zero["raw"]["sha256"] == golden["raw"]["sha256"]
        ),
        "step_zero_source_deployed_byte_exact": (
            step_zero["source_deployed"]["sha256"]
            == golden["source_deployed"]["sha256"]
        ),
        "step_zero_context_abi_byte_exact": (
            step_zero["context_abi"]["sha256"]
            == golden["context_abi"]["sha256"]
        ),
        "step_zero_final_wrapper_byte_exact": (
            step_zero["wrapped"]["sha256"] == golden["wrapped"]["sha256"]
        ),
    ...
    value = {
        "schema_version": "open_duck.t24_t23_postexport_result.v1",
        "status": (
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T24 T23 post-export transform result",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                f"- Failed checks: `{failed}`",
                "- Step-zero raw/source/context/final chain is byte-exact "
                "to the green T22 golden transform.",
                "- Both post-update graphs passed source deployment, context "
                "parity, and external physical-rate wrapper contracts.",
