#!/usr/bin/env python3
"""Execute the frozen zero-update Winner-v58 guard attribution."""
...
import build_winner_v58a_guard_failure_attribution_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v58a_guard_failure_attribution_preregistration.json"
...
def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        ...
        raise ValueError("Winner-v58a contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v58a source manifest absent")
    ...
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v58a source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v58a transformed source changed")
    return transformed
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.guard_failure_attribution_authorized:
        raise PermissionError(
            "Winner-v58a requires --offline-cpu-only "
            "--guard-failure-attribution-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v58a evidence")
    ...
    namespace: dict[str, Any] = {
        ...
        "__name__": "winner_v58a_transformed_attribution",
    }
