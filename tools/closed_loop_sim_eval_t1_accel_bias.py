...
class ClosedLoopConfig:
    ...
    observation_accel_x_bias_m_s2: float = 0.0
...
def run_closed_loop_sim(config: ClosedLoopConfig) -> dict:
    ...
    for mode in available_modes(config.bridge_mode):
        ...
        for tick in range(sim_steps):
            ...
            accel_x_pre_bias = float(obs[3])
            obs[3] = np.float32(
                obs[3] + np.float32(config.observation_accel_x_bias_m_s2)
            )
            accel_x_post_bias = float(obs[3])
            ...
            record = {
                ...
                "observation_accel_x_bias_m_s2": float(
                    config.observation_accel_x_bias_m_s2
                ),
                "accel_x_pre_bias_m_s2": accel_x_pre_bias,
                "accel_x_post_bias_m_s2": accel_x_post_bias,
                "command": [
                    config.command_x,
                    config.command_y,
                    config.command_yaw,
