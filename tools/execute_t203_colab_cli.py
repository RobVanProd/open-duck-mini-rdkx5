#!/usr/bin/env python3
"""Execute T203's exact package in one fresh Colab L4 session."""
...
PACKAGE = Path("/content/t203-predicted-roll-risk-20260730.tar.gz")
PACKAGE_BYTES = 4_340_024
PACKAGE_SHA256 = (
    "3a1eb154683173f9947465b14c9dcbd1ab7f5d512ae63406ad921f2de00b74e9"
)
BUNDLE_NAME = "t203_predicted_roll_risk_bundle"
MANIFEST_NAME = "t203_package_manifest.json"
MANIFEST_SHA256 = (
    "2105f10ecdd22f5903d99b81a62dc1a599a94a4ab9e15195c91bd3bde2a30ff3"
)
WORK = Path("/content/t203_work")
EXTRACT = Path("/content/t203_launch")
OUTPUT_JSON = Path("/content/t203_result.json")
OUTPUT_ARCHIVE = Path("/content/t203_artifacts.tar.gz")
RECEIPT = Path("/content/t203_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def verify_manifest(bundle: Path) -> dict[str, Any]:
    ...
    if sha256(manifest_path) != MANIFEST_SHA256:
        raise ValueError("T203 extracted manifest changed")
    ...
    if (
        ...
        raise ValueError("T203 manifest identity changed")
    for relative, expected in members.items():
        ...
        if (
            ...
            raise ValueError(f"T203 bundle member changed: {relative}")
    return manifest
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T203 requires L4: {gpu.stdout}{gpu.stderr}")
    for path in (WORK, EXTRACT, OUTPUT_JSON, OUTPUT_ARCHIVE, RECEIPT):
        if path.exists():
            raise FileExistsError(f"T203 no-retry path exists: {path}")
    if (
        PACKAGE.stat().st_size != PACKAGE_BYTES
        or sha256(PACKAGE) != PACKAGE_SHA256
    ):
        raise ValueError("T203 uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T203 package contains playground/.tmp")
    ...
    if install.returncode != 0:
        raise RuntimeError(f"T203 pinned install failed: {install.returncode}")
    versions = {
        name: importlib.metadata.version(name)
        for name in EXPECTED_VERSIONS
    }
    if versions != EXPECTED_VERSIONS:
        raise ValueError(f"T203 software versions changed: {versions}")
    ...
    preflight_source = "\n".join(
        (
            "from pathlib import Path",
            "import colab_t203_predicted_roll_risk_continuation as driver",
            ...
            ")",
            "assert cmd.count('--winner_t202_predicted_roll_risk') == 1",
            "assert cmd.count('--winner_t193_corrected_dynamic_reference_support') == 0",
            ...
            "assert 'T202_PREDICTED_ROLL_RISK=' in runner",
            "assert 'T98_HIDDEN_EXPERT_CONTINUATION=' in runner",
            "print('PASS_T203_HOSTED_FULL_PREFLIGHT')",
        )
    )
    ...
    if (
        ...
        or "PASS_T203_HOSTED_FULL_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T203 hosted preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    driver = bundle / "colab_t203_predicted_roll_risk_continuation.py"
    command = [
        sys.executable,
        str(driver),
        "--bundle-root",
    ...
    receipt = {
        "schema_version": "open_duck.t203_colab_launch_receipt.v1",
        "status": "STARTING_T203_HASH_FROZEN_GPU_CONTINUATION",
    ...
    receipt["status"] = (
        "COMPLETED_T203_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T203_COLAB_LAUNCH"
    )
    for name, path in (
