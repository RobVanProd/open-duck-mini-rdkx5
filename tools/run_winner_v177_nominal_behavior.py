#!/usr/bin/env python3
"""Run the preregistered V177 16-cell nominal physical gate on CPU."""
...
PREREG = ANALYSIS / "winner_v177_nominal_behavior_preregistration.json"
RESULT = ANALYSIS / "winner_v177_nominal_behavior_result.json"
MARKDOWN = ANALYSIS / "WINNER_V177_NOMINAL_BEHAVIOR_RESULT_20260725.md"
PREREG_SHA256 = (
    "0fac862fd44ca44276d3e05f8875c350ef02a2aca931e45a3fc1dae0d2303de0"
)
MATRIX_SHA256 = (
    "476edfa2eb8a7bb719fc7c558bd7e186e9d10caf5467ca657ff19f78cdaec485"
)
...
def main() -> int:
    ...
    result["schema_version"] = "winner_v177.nominal_behavior_result.v1"
    result["status"] = (
        "PASS_WINNER_V177_NOMINAL_BEHAVIOR_VALID_RESULT"
        if valid
        else "INVALID_WINNER_V177_NOMINAL_BEHAVIOR"
    )
    result["decision"] = {
        "status": (
            "ADVANCE_V177_TO_FULL_FROZEN_MATRIX"
            if valid and persistent
            else "REJECT_V175_NOMINAL_POLICY"
        ),
        "persistent_both_checkpoint_pass": persistent,
        "next_action": (
            "preregister the full frozen robustness matrix"
            if valid and persistent
            else "close V175 without retry and file failure attribution"
        ),
    }
    ...
    MARKDOWN.write_text(
        "# Winner V177 nominal behavior result\n\n"
