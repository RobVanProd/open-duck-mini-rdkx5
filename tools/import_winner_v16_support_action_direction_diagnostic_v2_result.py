#!/usr/bin/env python3
"""Strictly import one hosted Winner-v16 direction-diagnostic v2 result."""
...
WORKFLOW = (
    ROOT / ".github/workflows/winner-v16-support-action-direction-diagnostic-v2.yml"
)
OUTPUT_JSON = (
    ANALYSIS / "winner_v16_support_action_direction_diagnostic_v2_result.json"
)
OUTPUT_MD = (
    ANALYSIS / "WINNER_V16_SUPPORT_ACTION_DIRECTION_DIAGNOSTIC_V2_RESULT_20260721.md"
)
RAW_RESULT_NAME = "winner-v16-support-action-direction-v2-result.json"
RAW_RECEIPT_NAME = "winner-v16-support-action-direction-v2-result.sha256"
...
def read_result_artifact(path: Path) -> tuple[bytes, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v16 v2 artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v16 v2 artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v16 v2 artifact has unsafe member")
...
def repository_attribution(
    *,
    ...
    if (
        ...
        or artifact_id <= 0
        or artifact_name
        != f"winner-v16-support-action-direction-v2-{run_id}"
        or HEX64_RE.fullmatch(artifact_zip_sha256) is None
        or artifact_digest != f"sha256:{artifact_zip_sha256}"
    ):
        raise ValueError("Winner-v16 v2 workflow attribution changed")
    return {
...
def validate_intervention_result(
    ...
    if set(row) != expected_fields or row["intervention"] != intervention:
        raise ValueError("Winner-v16 v2 intervention schema changed")
    cells = row["cells"]
    ...
    if len(identities) != 12 or any(cell["condition"] is not None for cell in cells):
        raise ValueError("Winner-v16 v2 cell identity population changed")
    if any(cell["intervention"] != intervention for cell in cells):
        raise ValueError("Winner-v16 v2 cell intervention changed")
    if any(
        cell["source_previous_action_out_exact"] is not True
        ...
        raise ValueError("Winner-v16 v2 action contract failed")
    ...
    if (
        ...
        or row["baseline_matches_formal"]
        is not (True if intervention["id"] == "BASELINE" else None)
    ):
        raise ValueError("Winner-v16 v2 intervention summary is not rederived")
    expected_ticks = (
        None
        if not failed
        else [
    ...
    if row["failure_tick_range"] != expected_ticks or row["failure_pitch_rad_range"] != expected_pitch:
        raise ValueError("Winner-v16 v2 failure range is not rederived")
    if not math.isfinite(row["maximum_abs_action_delta_from_source"]) or not math.isfinite(
        ...
        raise ValueError("Winner-v16 v2 intervention summary is non-finite")
...
def validate_result(result: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("Winner-v16 v2 result decision changed")
    preregistration = json.loads(PREREGISTRATION.read_text(encoding="utf-8"))
    interventions = preregistration["frozen_screen"]["interventions"]
    baseline_reproduction = preregistration["baseline_reproduction"]
    if result["baseline_reproduction"] != baseline_reproduction:
    ...
    if (
        ...
        raise ValueError("Winner-v16 v2 validity or authority changed")
    expected_sources = {
    ...
    if result["sources"] != expected_sources:
        raise ValueError("Winner-v16 v2 sources changed")

    checkpoints = result["checkpoint_results"]
    if [row.get("label") for row in checkpoints] != ["half", "final"]:
        raise ValueError("Winner-v16 v2 checkpoint set changed")
    ...
    for checkpoint, label, update in zip(
        ...
        if (
            ...
            raise ValueError(f"Winner-v16 v2 {label} checkpoint changed")
        for row, intervention in zip(
            ...
            summaries[intervention["id"]].append(row)
            total_cells += len(row["cells"])
    if total_cells != 168:
        raise ValueError("Winner-v16 v2 total population changed")
    ...
    for intervention in interventions:
        ...
        if summary != derived or summary["passes_both_checkpoints"] is not False:
            raise ValueError("Winner-v16 v2 intervention summary changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v16 v2 result is already imported")
    ...
    if receipt_bytes != f"{raw_sha}  /tmp/{RAW_RESULT_NAME}\n".encode():
        raise ValueError("Winner-v16 v2 raw-result receipt changed")
    ...
    summary = payload["intervention_summary"]
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Winner-v16 support action-direction diagnostic v2 result",
                "",
                ...
                "- Baseline failures half / final: "
                f"`{summary['BASELINE']['half_failure_count']} / "
                f"{summary['BASELINE']['final_failure_count']}`",
                "- Best directional reduction (ankle positive) half / final: "
                f"`{summary['ANKLE_POS']['half_failure_count']} / "
                f"{summary['ANKLE_POS']['final_failure_count']}`",
                "- Full pass candidates: `0`",
                "- Optimizer / robot access: `0 / 0`",
                "",
                "The diagnostic is valid, but no single bilateral hip, knee, or ankle",
                "direction passes all 12 negative-X cells at both checkpoints. No closest",
                "failing intervention advances, and this result grants no robot clearance.",
                "",
            ]
        ),
        encoding="utf-8",
