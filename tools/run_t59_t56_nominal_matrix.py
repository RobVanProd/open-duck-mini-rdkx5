#!/usr/bin/env python3
"""Run T56's preregistered 16-cell corrected nominal matrix."""
...
RESULT_MD = ANALYSIS / "T59_T56_NOMINAL_MATRIX_RESULT_20260728.md"
CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t59_t56_nominal_matrix_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T59 preregistration")
    if prereg["status"] != "PREREGISTERED_T59_T56_NOMINAL_MATRIX":
        raise ValueError("T59 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T59 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T59 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T59 execution requires a clean worktree")
    ...
    RESULT_MD.write_text(
        "\n".join(
            [
                ...
                "",
                "A pass authorizes only robustness-matrix preregistration; "
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t59_t56_nominal_matrix_result.v1",
        "status": (
            "PASS_T59_T56_NOMINAL_MATRIX"
            if passed
            else "HOLD_T59_T56_NOMINAL_MATRIX"
        ),
        "decision": (
