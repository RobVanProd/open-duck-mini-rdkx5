#!/usr/bin/env python3
"""Run the preregistered V115 16-cell nominal physical gate on CPU."""
...
MARKDOWN = ANALYSIS / "WINNER_V115_NOMINAL_BEHAVIOR_RESULT_20260724.md"
PREREG_SHA256 = (
...
def main() -> int:
    ...
    evaluator.PREREG_STATUS = "PREREGISTERED_WINNER_V115_NOMINAL_BEHAVIOR"
    ...
    result["schema_version"] = "winner_v115.nominal_behavior_result.v1"
    result["status"] = (
        "PASS_WINNER_V115_NOMINAL_BEHAVIOR_VALID_RESULT"
        if valid
        else "INVALID_WINNER_V115_NOMINAL_BEHAVIOR"
    )
    result["decision"] = {
        "status": (
            "ADVANCE_V115_TO_FULL_FROZEN_MATRIX"
            if valid and persistent
            else "REJECT_V115_NOMINAL_POLICY"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner-v115 nominal behavior result\n\n"
