#!/usr/bin/env python3
"""Run T67's exact 16-cell R2 condition-7 causal screen."""
...
PREREG = ANALYSIS / "t70_t67_condition7_preregistration.json"
RESULT = ANALYSIS / "t70_t67_condition7_result.json"
RESULT_MD = ANALYSIS / "T70_T67_CONDITION7_RESULT_20260728.md"
CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t70_t67_condition7_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T70 preregistration")
    if prereg["status"] != "PREREGISTERED_T70_T67_CONDITION7":
        raise ValueError("T70 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T70 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T70 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T70 execution requires a clean worktree")
    ...
    RESULT_MD.write_text(
        "\n".join(
            [
                ...
                "",
                "A pass authorizes only the remaining R2 preregistration; "
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t70_t67_condition7_result.v1",
        "status": (
            "PASS_T70_T67_CONDITION7"
            if passed
            else "HOLD_T70_T67_CONDITION7"
        ),
        "decision": (
        ...
        "authority": {
            "r2_remainder_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
