#!/usr/bin/env python3
"""Freeze exact composition of T216 heads with T164 deployment repairs."""
...
OUTPUT = ANALYSIS / "t218_t216_postexport_composition_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T218_T216_POSTEXPORT_COMPOSITION_PREREGISTRATION_20260730.md"
)
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError("refusing to overwrite T218 preregistration")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T218 preregistration requires clean worktree")

    paths = {
        "t217": ANALYSIS / "t217_t216_recovered_training_validation.json",
        "t217b": ANALYSIS / "t217b_cost_init_backend_recovery_result.json",
    ...
    t217 = values["t217"]
    graph_by_step = {
        int(row["step"]): row for row in t217["exports"]["onnx"]
    }
    checkpoint_by_step = {
        int(row["step"]): row
        for row in t217["exports"]["policy_checkpoints"]
    }
    t164_final = next(
    ...
    frozen = {
        ...
        "test": receipt(
            ROOT / "tests/test_t218_t216_postexport_composition.py"
        ),
        **{name: receipt(path) for name, path in paths.items()},
    }
    checks = {
        "t217_recovery_green_via_t217b": (
            t217["status"]
            == "HOLD_T217_T216_RECOVERED_TRAINING_VALIDATION"
            and t217["failed_checks"]
            == ["step_zero_cost_tree_reproducible_bit_exact"]
            and values["t217b"]["status"]
            == "PASS_T217B_COST_INIT_BACKEND_RECOVERY"
            and not values["t217b"]["failed_checks"]
            and values["t217b"]["decision"]
            == (
    ...
    if failed:
        raise RuntimeError(f"T218 preregistration checks failed: {failed}")
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t218_t216_postexport_composition_"
            "preregistration.v1"
        ),
        "status": "PREREGISTERED_T218_T216_POSTEXPORT_COMPOSITION",
        "question": (
            "Can each T216 raw graph replace only T164-final's nominal "
        ...
        "composition": {
            ...
            "base": "T164_COMPOSED_FINAL_FOR_ALL_THREE_T216_EXPORTS",
        ...
        "decision_rule": {
            ...
            "fail": "HOLD_T216_BEHAVIOR_AND_AUDIT_COMPOSITION",
            "no_retry": True,
        },
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# T218 T216 post-export composition preregistration\n\n"
        f"- Status: `{value['status']}`\n"
        "- Three raw T216 graphs; one frozen T164-final deployment base\n"
