#!/usr/bin/env python3
"""Validate recovered T170 training on CPU before any behavior screen."""
...
CPU_AUTHORITY = ANALYSIS / "t169b_cpu_diagnostic_validity_result.json"
CPU_SOURCE = ANALYSIS / "t169_eight_stratum_head_continuation_cpu_result.json"
PACKAGE_CONTRACT = (
    ANALYSIS / "t170_eight_stratum_head_hosted_package_contract.json"
)
LAUNCH_CONTRACT = ANALYSIS / "t170_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t171_t170_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T171_T170_RECOVERED_TRAINING_VALIDATION_20260729.md"
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T171: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T171 validation requires clean worktree")

    recovery_root = args.recovery_root.resolve()
    work = (
        args.extracted_root.resolve()
        / "t170_eight_stratum_head_continuation"
    )
    training = work / "training"
    paths = {
        "hosted_result": recovery_root / "t170_result.json",
        "launch_receipt": recovery_root / "t170_launch_receipt.json",
        "recovery_archive": recovery_root / "t170_artifacts.tar.gz",
    }
    ...
    checks = {
        "t170_preregistration_green": (
            prereg["status"]
            == "PREREGISTERED_T170_EIGHT_STRATUM_HEAD_HOSTED_CONTINUATION"
            and prereg["decision"]
            == "AUTHORIZE_ONE_HASH_FROZEN_T170_L4_CONTINUATION"
            and prereg["failed_checks"] == []
        ),
        "t169b_cpu_authority_exact": (
            ...
            and hosted["validation"]["input_hashes"]["diagnostic_result"]
            == sha256(CPU_AUTHORITY)
        ),
        "cpu_topology_is_frozen_t169_remap": (
            directory_sha256(cpu_topology)
            == cpu_source["source_remap"]["cpu_remap"]["sha256"]
            and cpu_source["source_remap"]["source"]["sha256"]
        ...
        "package_and_launch_contracts_green": (
            ...
            and launch["status"] == "PASS_T170_COLAB_CLI_LAUNCH_CONTRACT"
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T170_COLAB_LAUNCH"
        ...
        "preregistration_hash_matches_hosted_and_receipt": (
            ...
            and receipt["preregistration_sha256"] == sha256(PREREGISTRATION)
        ),
        "exact_local_exports": (
        ...
        "exact_eight_stratum_readback": readback in log_text,
    ...
    result = {
        "schema_version": "open_duck.t171_t170_recovered_training_validation.v1",
        "status": (
            "PASS_T171_T170_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T171_T170_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T172_T170_POSTEXPORT_COMPOSITION_PREREGISTRATION_ONLY"
            if not failed
        ...
        "input_hashes": {
            **input_hashes,
            "t170_preregistration": sha256(PREREGISTRATION),
            "t169b_cpu_authority": sha256(CPU_AUTHORITY),
            "t169_cpu_source": sha256(CPU_SOURCE),
    ...
    MARKDOWN.write_text(
        "# T171 T170 recovered training validation\n\n"
