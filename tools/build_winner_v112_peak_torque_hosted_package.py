#!/usr/bin/env python3
"""Build the hash-frozen Winner-v112 hosted continuation bundle."""
...
CONTRACT = ANALYSIS / "winner_v112_peak_torque_hosted_package_contract.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V112_PEAK_TORQUE_HOSTED_PACKAGE_CONTRACT_20260724.md"
)
...
def file_manifest(root: Path) -> dict[str, dict[str, Any]]:
    return {
        ...
        if path.name != "winner_v112_package_manifest.json"
    }
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("Winner-v112 preregistration changed")
    observed = {
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError("Winner-v112 package inputs changed")

    bundle = staging / BUNDLE_NAME
    assets = bundle / "assets"
    assets.mkdir(parents=True)
    for path in (PREREGISTRATION, CPU_CORRECTION, DRIVER, PATCH):
    ...
    for relative, expected in json.loads(
        (
            bundle / "playground/WINNER_V111_COMPOSED_SOURCE_MANIFEST.json"
        ).read_text(encoding="utf-8")
    )["final_python_hashes"].items():
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_WINNER_V112_PACKAGE_IMPORT')"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_WINNER_V112_PACKAGE_IMPORT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            "Winner-v112 isolated import failed: "
            f"{preflight.stdout}\n{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "winner_v112.hosted_bundle_manifest.v1",
        "preregistration_sha256": PREREGISTRATION_SHA256,
        "input_hashes": observed,
        "peak_torque_patch_sha256": sha256(PATCH),
    ...
    manifest_path = bundle / "winner_v112_package_manifest.json"
    ...
    contract = {
        "schema_version": "winner_v112.peak_torque_hosted_package.v1",
        "status": "PASS_WINNER_V112_PEAK_TORQUE_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
