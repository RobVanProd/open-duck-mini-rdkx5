#!/usr/bin/env python3
"""Run T86's frozen full sequential R2 matrix."""
...
PREREG = ANALYSIS / "t86_rolling_midpoint_r2_preregistration.json"
RESULT = ANALYSIS / "t86_rolling_midpoint_r2_result.json"
MARKDOWN = ANALYSIS / "T86_ROLLING_MIDPOINT_R2_RESULT_20260728.md"
DEFAULT_CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t86_rolling_midpoint_r2_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T86 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T86 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T86 matrix plan changed")
    return value
...
def write_markdown(result: Mapping[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "A pass authorizes only deployment-readiness package "
            "preregistration. Gate 5 hardware, RDK-X5, robot access, "
            "torque, motion, and deployment remain closed.",
            "",
        ]
    )
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T86 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T86 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": (
            "open_duck.t86_rolling_midpoint_r2_result.v1"
        ),
        "status": (
            "PASS_T86_ROLLING_MIDPOINT_FULL_R2"
            if all_green
            else "HOLD_T86_ROLLING_MIDPOINT_R2"
        ),
        "decision": (
