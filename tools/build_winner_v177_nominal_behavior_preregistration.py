#!/usr/bin/env python3
"""Preregister the unchanged 16-cell V177 nominal physical gate."""
...
TRANSFORM = ANALYSIS / "winner_v176_deployment_transform_contract.json"
TRAINING = ANALYSIS / "winner_v175_recovered_training_validation.json"
BASE = ANALYSIS / "winner_v110_pitch_guard_behavior_preregistration.json"
OUTPUT = ANALYSIS / "winner_v177_nominal_behavior_preregistration.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V177_NOMINAL_BEHAVIOR_PREREGISTRATION_20260725.md"
)
EXPECTED = {
    "v176_transform_contract": (
        "888f87c77153d6039577f6ebdd50c141125858e2996a5267df6857a85b668da9"
    ),
    "v175_training_validation": (
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite V177: {path}")
    ...
    hashes = {
        "v176_transform_contract": sha256(TRANSFORM),
        "v175_training_validation": sha256(TRAINING),
    ...
    checks = {
        ...
        "transform_contract_exact": (
            ...
            == "PASS_WINNER_V176_DEPLOYMENT_TRANSFORM_CONTRACT"
    ...
    payload = {
        "schema_version": "winner_v177.nominal_behavior_preregistration.v1",
        "status": (
            "PREREGISTERED_WINNER_V177_NOMINAL_BEHAVIOR"
            if not failed
            else "HOLD_WINNER_V177_NOMINAL_BEHAVIOR_PREREGISTRATION"
        ),
        ...
        "decision_rule": {
            ...
            "any_behavior_current_or_torque_failure": (
                "close V175 without retry or optimizer adjustment"
            ),
    ...
    MARKDOWN.write_text(
        "# Winner V177 nominal behavior preregistration\n\n"
