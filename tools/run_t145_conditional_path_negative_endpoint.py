#!/usr/bin/env python3
"""Run T143C's frozen negative-COM endpoint matrix."""
...
PREREG = ANALYSIS / "t145_conditional_path_negative_endpoint_preregistration.json"
RESULT = ANALYSIS / "t145_conditional_path_negative_endpoint_result.json"
MARKDOWN = ANALYSIS / "T145_CONDITIONAL_PATH_NEGATIVE_ENDPOINT_RESULT_20260729.md"
CACHE = Path(
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T145 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T145 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t145_conditional_path_negative_endpoint_result.v1"
        ),
        "status": (
            "PASS_T145_CONDITIONAL_PATH_NEGATIVE_ENDPOINT_MATRIX"
            if passed
            else "HOLD_T145_CONDITIONAL_PATH_NEGATIVE_ENDPOINT_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T145 conditional-path negative endpoint result\n\n"
