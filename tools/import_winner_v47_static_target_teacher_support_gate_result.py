#!/usr/bin/env python3
"""Safely import the sole Winner-v47 static-target-teacher support gate."""
...
PREREGISTRATION = ANALYSIS / "winner_v47_static_target_teacher_support_gate_preregistration.json"
TRAINING_RESULT = ANALYSIS / "winner_v46_static_target_teacher_training_result.json"
TRAINING_PREREGISTRATION = ANALYSIS / "winner_v46_static_target_teacher_training_preregistration.json"
TRAINING_RUNNER = ROOT / "tools/run_winner_v46_static_target_teacher_training.py"
...
WORKFLOW = ROOT / ".github/workflows/winner-v47-static-target-teacher-support-gate.yml"
V15_IMPORTER = ROOT / "tools/import_winner_v15_pitch_margin_support_gate_result.py"
OUTPUT_JSON = ANALYSIS / "winner_v47_static_target_teacher_support_gate_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V47_STATIC_TARGET_TEACHER_SUPPORT_GATE_RESULT_20260722.md"
RAW_RESULT_NAME = "winner-v47-static-target-teacher-support-gate-result.json"
RAW_RECEIPT_NAME = "winner-v47-static-target-teacher-support-gate-result.sha256"
...
ATTRIBUTION_FIELDS = {
    ...
    "workflow_lf_sha256", "runner_lf_sha256", "v15_importer_lf_sha256",
    "importer_lf_sha256",
}


def _load_v15_importer():
    spec = importlib.util.spec_from_file_location("winner_v15_support_gate_import_helpers_v47", V15_IMPORTER)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load reviewed support-gate importer")
...
def read_result_artifact(path: Path) -> tuple[bytes, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v47 artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v47 artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v47 artifact has unsafe member")
...
def repository_attribution(
    *, run_id: int, run_attempt: int, run_head_sha: str, artifact_id: int,
    ...
    require_sha256(artifact_zip_sha256, "Winner-v47 artifact")
    if (
        run_id <= 0 or run_attempt != 1 or HEX40.fullmatch(run_head_sha) is None
        or artifact_id <= 0
        or artifact_name != f"winner-v47-static-target-teacher-support-gate-{run_id}"
        or artifact_digest != f"sha256:{artifact_zip_sha256}"
    ):
        raise ValueError("Winner-v47 workflow attribution changed")
    return {
...
def checkpoint_identities() -> dict[str, dict[str, str]]:
    ...
    if (
        source.get("status") != "PASS_WINNER_V46_STATIC_TARGET_TEACHER_TRAINING_ARTIFACT"
        or source.get("decision") != "AUTHORIZE_STATIC_TARGET_TEACHER_SUPPORT_GATE_PREREGISTRATION_ONLY"
        or source.get("failed_checks") != []
        or source.get("repository_attribution", {}).get("repository") != EXPECTED_REPOSITORY
    ):
        raise ValueError("Winner-v46 source is not verified")
    snapshots = source.get("snapshot_manifest")
    checkpoints = source.get("persistent_checkpoints")
    if (
        not isinstance(snapshots, list) or len(snapshots) != 100
        or not isinstance(checkpoints, list)
        or [(row.get("label"), row.get("completed_updates")) for row in checkpoints]
        != [("half", 302), ("final", 352)]
    ):
        raise ValueError("Winner-v46 checkpoint set changed")
    identities: dict[str, dict[str, str]] = {}
    for label, update, checkpoint in zip(("half", "final"), (302, 352), checkpoints, strict=True):
        snapshot = next(row for row in snapshots if row.get("completed_updates") == update)
        snapshot_sha = snapshot.get("sha256")
        graph_sha = checkpoint.get("graph", {}).get("sha256")
        require_sha256(snapshot_sha, f"Winner-v47 {label} snapshot")
        require_sha256(graph_sha, f"Winner-v47 {label} graph")
        identities[label] = {"checkpoint_sha256": str(snapshot_sha), "onnx_sha256": str(graph_sha)}
    return identities


def _validate_checkpoint_result(row: Mapping[str, Any], label: str, identity: Mapping[str, str]) -> None:
    expected_update = {"half": 302, "final": 352}[label]
    if row.get("update") != expected_update:
        raise ValueError(f"Winner-v47 {label} update changed")
    adapted = dict(row)
    adapted["update"] = {"half": 50, "final": 100}[label]
    V15.validate_checkpoint_result(adapted, label, identity)
...
def validate_result(result: Mapping[str, Any]) -> None:
    ...
    if frozenset(result) not in {frozenset(raw_fields), frozenset(raw_fields | {"repository_attribution"})}:
        raise ValueError("Winner-v47 result field inventory changed")
    if result.get("schema_version") != "winner_v47.static_target_teacher_support_gate_result.v1":
        raise ValueError("Winner-v47 result schema changed")
    if "repository_attribution" in result:
        ...
        if (
            ...
            or type(item.get("github_run_id")) is not int or item["github_run_id"] <= 0
            or HEX40.fullmatch(str(item.get("github_run_head_sha"))) is None
            or type(item.get("github_artifact_id")) is not int or item["github_artifact_id"] <= 0
            or item.get("github_artifact_name")
            != f"winner-v47-static-target-teacher-support-gate-{item['github_run_id']}"
            or item.get("github_artifact_digest") != f"sha256:{item.get('artifact_zip_sha256')}"
            or any(HEX64.fullmatch(str(item.get(name))) is None for name in (
            ...
            or item.get("v15_importer_lf_sha256") != lf_sha256(V15_IMPORTER)
            or item.get("importer_lf_sha256") != lf_sha256(Path(__file__))
        ):
            raise ValueError("Winner-v47 imported attribution changed")
    checks = result.get("checks")
    expected_checks = {"all_248_main_cells_pass", "both_checkpoints_evaluated", "formal_cell_count_exact"}
    if not isinstance(checks, Mapping) or set(checks) != expected_checks or not all(type(value) is bool for value in checks.values()):
        raise ValueError("Winner-v47 checks changed")
    if result.get("execution") != {
        ...
        raise ValueError("Winner-v47 execution or authority changed")
    ...
    derived = {
        ...
        "formal_cell_count_exact": sum(
            len(row["core_model_plant_cells"]) + len(row["sensor_transport_plant_cells"])
            for row in rows
        ) == 248,
    }
    if checks != derived:
        raise ValueError("Winner-v47 summary is not rederived")
    ...
    if (
        ...
        or result.get("status") != (
            ...
            if passed else "HOLD_WINNER_V47_STATIC_TARGET_TEACHER_SUPPORT_GATE"
        )
        ...
        raise ValueError("Winner-v47 decision changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v47 result is already imported")
    ...
    if receipt_bytes != f"{raw_sha}  /tmp/{RAW_RESULT_NAME}\n".encode():
        raise ValueError("Winner-v47 raw receipt changed")
    ...
    OUTPUT_MD.write_text("\n".join([
        "# Winner-v47 static-target teacher formal support-gate result", "",
        ...
        "Both fixed checkpoints must pass; no closest result is promoted.", "",
    ]), encoding="utf-8")
