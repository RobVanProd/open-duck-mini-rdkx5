#!/usr/bin/env python3
"""Freeze T74's sole 16-cell COM-x-negative behavior matrix."""
...
T74 = ANALYSIS / "t74_signed_core_secant_result.json"
...
RUNNER = ROOT / "tools" / "run_t75_signed_core_secant_condition7.py"
...
TEST = ROOT / "tests" / "test_t75_signed_core_secant_condition7.py"
OUTPUT = ANALYSIS / "t75_signed_core_secant_condition7_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T75_SIGNED_CORE_SECANT_CONDITION7_PREREGISTRATION_20260728.md"
)
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T75 preregistration requires a clean worktree")
    t74 = json.loads(T74.read_text(encoding="utf-8"))
    t73 = json.loads(T73.read_text(encoding="utf-8"))
    t70 = json.loads(T70_PREREG.read_text(encoding="utf-8"))
    t69 = json.loads(T69_PREREG.read_text(encoding="utf-8"))
    policies = [row["policy"] for row in t74["candidates"]]
    condition = t70["condition"]
    checks = {
        "t74_secant_contract_green": (
            t74["status"] == "PASS_T74_SIGNED_CORE_SECANT"
            and t74["decision"]
            == "EARN_T75_SIGNED_CORE_SECANT_CONDITION7_PREREGISTRATION"
            and not t74["failed_checks"]
        ),
        "t73_directional_basis_exact": (
            t73["status"] == "HOLD_T73_SIGNED_CORE_MIRROR_CONDITION7"
            and t73["condition"]["green_cells"] == 11
        ),
        "exact_two_persistent_secant_endpoints": (
            len(policies) == 2
            and {item["checkpoint_id"] for item in policies}
            == {
                "T74_SIGNED_CORE_SECANT_3M",
                "T74_SIGNED_CORE_SECANT_4M",
            }
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t75_signed_core_secant_condition7_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T75_SIGNED_CORE_SECANT_CONDITION7"
            if not failed
            else "HOLD_T75_SIGNED_CORE_SECANT_CONDITION7_PREREGISTRATION"
        ),
        "question": (
            "Do both fixed 3M/4M negative secant endpoints pass the exact "
            "16-cell torso-COM-x-negative boundary?"
        ),
        "condition": condition,
        ...
        "decision_rule": {
            ...
            "fail_decision": "CLOSE_SIGNED_CORE_SECANT_FAMILY",
            "no_retry": True,
            "both_checkpoints_required": True,
            "no_checkpoint_selection": True,
        ...
        "frozen_inputs": {
            "t74_transform_result": receipt(T74),
            "t73_mirror_behavior": receipt(T73),
            "t70_boundary_contract": receipt(T70_PREREG),
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T75 signed recurrent-core secant condition-7 preregistration",
                "",
                f"- Status: `{value['status']}`",
                "- Boundary: `TORSO_COM_X_NEG = -0.05 m`",
                "- Matrix: `2 endpoints x 2 fits x 4 commands = 16 cells`",
