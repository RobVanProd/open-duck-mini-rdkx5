#!/usr/bin/env python3
"""Validate recovered T194 training on CPU before any behavior."""
...
LAUNCH_CONTRACT = ANALYSIS / "t194b_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t195_t194_recovered_training_validation.json"
MARKDOWN = (
    ANALYSIS / "T195_T194_RECOVERED_TRAINING_VALIDATION_20260730.md"
)
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T195: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T195 validation requires clean worktree")
    ...
    paths = {
        "hosted_result": recovery_root / "t194_result.json",
        "launch_receipt": recovery_root / "t194_launch_receipt.json",
        "recovery_archive": recovery_root / "t194_artifacts.tar.gz",
    }
    ...
    reference_values = {
        tag: finite_values(events, tag) for tag in REFERENCE_TAGS
    }
    original_values = finite_values(events, ORIGINAL_REWARD_TAG)

    checks = {
        "t194_preregistration_green": (
            prereg["status"]
            == "PREREGISTERED_T194_CORRECTED_DYNAMIC_REFERENCE_SUPPORT_"
            "HOSTED_CONTINUATION"
            and prereg["decision"]
            == "AUTHORIZE_ONE_HASH_FROZEN_T194_L4_CONTINUATION"
            and prereg["failed_checks"] == []
        ),
        "t193_cpu_authority_exact": (
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T194_COLAB_LAUNCH"
        ...
        "exact_t193_and_t98_readbacks": (
            t193_readback in log_text and t98_readback in log_text
        ),
        "hosted_support_metrics_exercised_bilaterally": all(
            len(values) == 3 and min(values) > 0.0
            for values in support_values.values()
        ),
        "hosted_reference_metrics_exercised_bilaterally": all(
            len(values) == 3 and min(values) > 0.0
            for values in reference_values.values()
        ),
        "hosted_original_reward_preserved": (
            len(original_values) == 3 and min(original_values) > 0.0
        ),
    ...
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t195_t194_recovered_training_validation.v1"
        ),
        "status": (
            "PASS_T195_T194_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T195_T194_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T196_T194_POSTEXPORT_COMPOSITION_PREREGISTRATION_ONLY"
            if not failed
        ...
        "input_hashes": {
            **input_hashes,
            "t194_preregistration": sha256(PREREGISTRATION),
            "t193_cpu_authority": sha256(CPU_AUTHORITY),
            "t193_cpu_source": sha256(CPU_SOURCE),
        ...
        "support_metrics": {
            ...
            "reference_requests": reference_values,
            "original_reward": original_values,
        },
        "classification": {
            ...
            "reference_support_active": "all_locomotion",
    ...
    MARKDOWN.write_text(
        "# T195 T194 recovered training validation\n\n"
        f"- Status: `{value['status']}`\n"
        f"- Decision: `{value['decision']}`\n"
        f"- Failed checks: `{failed}`\n"
        "- Exports: `0 / 1,003,520 / 2,007,040`\n"
        "- T193 support/reference metrics: bilateral, finite, positive\n"
