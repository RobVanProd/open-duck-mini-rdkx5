#!/usr/bin/env python3
"""Execute the corrected zero-update Winner-v58 guard attribution."""
...
import build_winner_v58b_guard_failure_attribution_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v58b_guard_failure_attribution_preregistration.json"


def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        value.get("schema_version")
        != "winner_v58b.guard_failure_attribution_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V58B_GUARD_FAILURE_ATTRIBUTION"
        or value.get("decision")
        != "AUTHORIZE_ONE_ZERO_UPDATE_WINNER_V58_GUARD_ATTRIBUTION_ONLY"
        or value.get("diagnostic", {}).get("optimizer_updates") != 0
        or value.get("predecessor_failure", {}).get("optimizer_updates") != 0
        or value.get("authority", {}).get("diagnostic_authorized") is not True
        or value.get("authority", {}).get("retry_authorized") is not False
    ):
        raise ValueError("Winner-v58b contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v58b source manifest absent")
    for name, item in sources.items():
        if (
            item.get("hash_mode") != "lf"
            or spec.lf_sha256(ROOT / item["path"]) != item.get("sha256")
        ):
            raise ValueError(f"Winner-v58b source changed: {name}")
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v58b source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v58b transformed source changed")
    return transformed
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.guard_failure_attribution_authorized:
        raise PermissionError(
            "Winner-v58b requires --offline-cpu-only "
            "--guard-failure-attribution-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v58b evidence")
    ...
    namespace: dict[str, Any] = {
        ...
        "__name__": "winner_v58b_transformed_attribution",
    }
