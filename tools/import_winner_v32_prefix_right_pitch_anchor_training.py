#!/usr/bin/env python3
"""Strictly import the sole Winner-v32 prefix-anchor training artifact."""
...
OUTPUT_JSON = ANALYSIS / "winner_v32_prefix_right_pitch_anchor_training_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V32_PREFIX_RIGHT_PITCH_ANCHOR_TRAINING_RESULT_20260722.md"
RAW_RESULT = "winner-v32-prefix-right-pitch-anchor-training-result.json"
RAW_RECEIPT = "winner-v32-prefix-right-pitch-anchor-training-result.sha256"
WORK = "winner-v32-prefix-anchor-training-work"
...
NON_ANCHOR_GRADIENT_KEYS = GRADIENT_KEYS - ANCHOR_GRADIENT_KEYS
...
def artifact_members() -> set[str]:
    return {
        ...
        RAW_RECEIPT,
        *(
            f"{WORK}/snapshots/snapshot_prefix_anchor_update_{i:03d}.npz"
            for i in range(202, 302)
        ),
        f"{WORK}/graphs/winner_v32_half.onnx",
        f"{WORK}/graphs/winner_v32_final.onnx",
    }
...
def read_artifact(path: Path) -> dict[str, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v32 training artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v32 training artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v32 training artifact has unsafe member")
...
def expected_objective() -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ValueError("Winner-v32 training preregistration authority changed")
...
def _positive_tree(value: Any, label: str) -> bool:
    if not isinstance(value, Mapping) or set(value) != GRADIENT_KEYS:
        raise ValueError(f"Winner-v32 training {label} tree changed")
    if not all(
        ...
        raise ValueError(f"Winner-v32 training {label} is nonpositive")
    return True


def _anchor_tree(value: Any) -> bool:
    if not isinstance(value, Mapping) or set(value) != GRADIENT_KEYS:
        raise ValueError("Winner-v32 anchor gradient tree changed")
    if not all(
        type(value[key]) in {int, float}
        and math.isfinite(float(value[key]))
        and value[key] > 0
        for key in ANCHOR_GRADIENT_KEYS
    ) or not all(value[key] == 0 for key in NON_ANCHOR_GRADIENT_KEYS):
        raise ValueError("Winner-v32 anchor gradient support changed")
    return True
...
def validate_result(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "winner_v32.prefix_right_pitch_anchor_training_result.v1"
        or value.get("status")
        not in {
            "PASS_WINNER_V32_PREFIX_RIGHT_PITCH_ANCHOR_TRAINING_ARTIFACT",
            "HOLD_WINNER_V32_PREFIX_RIGHT_PITCH_ANCHOR_TRAINING_ARTIFACT",
        }
        ...
        raise ValueError("Winner-v32 training result schema changed")
    hold = json.loads(V30_HOLD.read_text(encoding="utf-8"))
    teacher_result = json.loads(V22_TRAINING.read_text(encoding="utf-8"))
    source = hold["snapshot"]
    teacher = teacher_result["snapshot_manifest"][99]
    if value.get("source_snapshot") != {
        ...
        raise ValueError("Winner-v32 training snapshot identities changed")
    if value.get("sources") != {
        "preregistration_lf_sha256": lf_sha256(PREREGISTRATION),
        "v31_result_lf_sha256": lf_sha256(V31_RESULT),
        "v30_hold_lf_sha256": lf_sha256(V30_HOLD),
        "v22_training_lf_sha256": lf_sha256(V22_TRAINING),
        "runner_lf_sha256": lf_sha256(RUNNER),
    }:
        raise ValueError("Winner-v32 training source identities changed")
    ...
    if (
        ...
        or [row.get("rollout_update_index") for row in metrics]
        != list(range(201, 301))
        or [row.get("completed_updates") for row in metrics]
        != list(range(202, 302))
        or [row.get("optimizer_count") for row in metrics]
        != list(range(202, 302))
        ...
        raise ValueError("Winner-v32 training sequence changed")
    for row in metrics:
        if (
            ...
            or row.get("prefix_anchor_loss", -1.0) < 0.0
            or row.get("maximum_selected_action_delta", -1.0) < 0.0
            or not (
                (
            ...
            or not _anchor_tree(row.get("anchor_gradient_max_abs"))
            or not _positive_tree(row.get("combined_gradient_max_abs"), "gradient")
            or not _positive_tree(row.get("leaf_max_abs_delta"), "delta")
        ):
            raise ValueError("Winner-v32 training metric changed")
    if not _positive_tree(
        value.get("cumulative_leaf_max_abs_delta"), "cumulative delta"
    ):
        raise ValueError("Winner-v32 training cumulative delta changed")
    for row in snapshots:
        if (
            Path(str(row.get("path"))).name
            != f"snapshot_prefix_anchor_update_{row['completed_updates']:03d}.npz"
            or row.get("bytes", 0) <= 0
            or len(str(row.get("sha256", ""))) != 64
        ):
            raise ValueError("Winner-v32 training snapshot receipt changed")
    for row in checkpoints:
        graph = row.get("graph", {})
        if (
            Path(str(graph.get("path"))).name
            != f"winner_v32_{row['label']}.onnx"
            ...
            raise ValueError("Winner-v32 training checkpoint graph changed")
    checks = {
        ...
        "optimizer_count_301_exact": metrics[-1]["optimizer_count"] == 301,
    ...
    if value.get("checks") != checks:
        raise ValueError("Winner-v32 training checks are not rederived")
    ...
    if value.get("failed_checks") != failed or (passed and failed):
        raise ValueError("Winner-v32 training classification changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v32 training result is already imported")
    ...
    if (
        ...
        != f"winner-v32-prefix-right-pitch-anchor-training-{args.run_id}"
        or args.artifact_digest != f"sha256:{zip_sha}"
    ):
        raise ValueError("Winner-v32 training repository attribution changed")
    ...
    if members[RAW_RECEIPT] != f"{raw_sha}  /tmp/{RAW_RESULT}\n".encode():
        raise ValueError("Winner-v32 training raw receipt changed")
    raw = json.loads(
    ...
    for row in raw["snapshot_manifest"]:
        ...
        if (
            sha256_bytes(members[name]) != row["sha256"]
            or len(members[name]) != row["bytes"]
        ):
            raise ValueError(f"Winner-v32 snapshot {row['completed_updates']} changed")
    for row in raw["persistent_checkpoints"]:
        name = f"{WORK}/graphs/winner_v32_{row['label']}.onnx"
        graph = row["graph"]
        if (
            sha256_bytes(members[name]) != graph["sha256"]
            or len(members[name]) != graph["bytes"]
        ):
            raise ValueError(f"Winner-v32 {row['label']} graph changed")
    result = dict(raw)
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Winner-v32 prefix right-pitch anchor training result",
                "",
                ...
                "- Source / final optimizer count: `201 / 301`",
