#!/usr/bin/env python3
"""Run T39's preregistered uniform-normalizer 16-cell nominal matrix."""
...
RESULT = ANALYSIS / "t39_uniform_normalizer_rollback_nominal_result.json"
RESULT_MD = (
    ANALYSIS
    / "T39_UNIFORM_NORMALIZER_ROLLBACK_NOMINAL_RESULT_20260728.md"
)
CACHE_ROOT = (
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T39 preregistration")
    if prereg["status"] != (
        "PREREGISTERED_T39_UNIFORM_NORMALIZER_ROLLBACK_NOMINAL_MATRIX"
    ):
        raise ValueError("T39 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T39 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T39 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T39 execution requires a clean worktree")
...
def write_markdown(result: Mapping[str, Any]) -> None:
    ...
    RESULT_MD.write_text(
        "\n".join(
            [
                "# T39 uniform normalizer rollback nominal result",
                "",
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t39_uniform_normalizer_rollback_nominal_result.v1"
        ),
        "status": (
            "PASS_T39_UNIFORM_NORMALIZER_ROLLBACK_NOMINAL_MATRIX"
            if passed
            else "HOLD_T39_UNIFORM_NORMALIZER_ROLLBACK_NOMINAL_MATRIX"
        ),
        "decision": (
