#!/usr/bin/env python3
"""Run T193's default-off, enabled, and 1,024-step CPU contract."""
...
MARKDOWN = (
    ANALYSIS
    / "T193_CORRECTED_DYNAMIC_REFERENCE_SUPPORT_CPU_RESULT_20260730.md"
)
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t193_corrected_dynamic_reference_support_cpu_v1"
)
...
def validate_prereg(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("T193 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T193 composed playground changed")
...
def run_worker(
    *,
    ...
    command = [
        sys.executable,
        str(TOOLS / "run_t193_environment_contract_worker.py"),
        "--mode",
        mode,
        "--playground",
        str(playground),
...
def training_command(
    *,
    ...
    command.insert(
        index, "--winner_t193_corrected_dynamic_reference_support"
    )
    return command
...
def main() -> int:
    ...
    if not args.cpu_contract_authorized:
        raise PermissionError("T193 requires --cpu-contract-authorized")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T193 execution requires clean worktree")
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T193 CPU training failed rc={completed.returncode}\n"
    ...
    if sorted(checkpoints) != [0, 1024] or sorted(graphs) != [0, 1024]:
        raise RuntimeError("T193 export steps changed")
    ...
    result: dict[str, Any] = {
        ...
        "status": (
            ...
            if passed
            else "HOLD_T193_CORRECTED_DYNAMIC_REFERENCE_SUPPORT_"
            "CPU_CONTRACT"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T193 corrected dynamic reference-support CPU result\n\n"
        f"- Status: `{result['status']}`\n"
        f"- Decision: `{result['decision']}`\n"
        f"- Failed checks: `{failed}`\n"
        f"- Reference requests left/right: "
        f"`{enabled['reference_requested_counts']}`\n"
        f"- Policy matches left/right: "
        f"`{enabled['matched_support_counts']}`\n"
