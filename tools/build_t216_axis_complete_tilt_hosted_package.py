#!/usr/bin/env python3
"""Build T216's hash-frozen hosted continuation bundle."""
...
CONTRACT = ANALYSIS / "t216_axis_complete_tilt_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T216_AXIS_COMPLETE_TILT_HOSTED_PACKAGE_20260730.md"
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T216 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T216 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T216 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T216 playground changed")
    if (
        ...
        raise RuntimeError("copied T216 assets changed")
    ...
    preflight_source = "\n".join(
        (
            ...
            "    print('PASS_T216_FULL_IDENTITY_PREFLIGHT')",
            "else:",
    ...
    if (
        ...
        or "PASS_T216_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T216 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t216_hosted_bundle_manifest.v1",
        "preregistration_sha256": sha256(PREREG),
        "cpu_result_sha256": sha256(CPU_RESULT),
        "t204_validation_sha256": sha256(T204),
        "t214b_selection_sha256": sha256(T214B),
    ...
    contract = {
        "schema_version": "open_duck.t216_hosted_package.v1",
        "status": "PASS_T216_AXIS_COMPLETE_TILT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            ...
            "t215b_and_t98_readbacks_frozen": True,
    ...
    MARKDOWN.write_text(
        "# T216 axis-complete tilt hosted package\n\n"
