#!/usr/bin/env python3
"""Evaluate one T16 support-coordinate policy/fit/COM block."""
...
def main() -> int:
    ...
    if args.seed != SEED or args.duration_s != 12.0:
        raise ValueError("T16 seed or duration changed")
    ...
    if sha256(calibrator) != CALIBRATOR_SHA256:
        raise RuntimeError("T16 calibrator changed")
        ...
        result = module.run_closed_loop_sim(
            module.ClosedLoopConfig(
                ...
                eval_dynamics_override={
                    "torso_com_offset_m": [args.com_x, 0.0, 0.0]
                },
                trace_jsonl=trace,
                trace_full_obs=True,
    ...
    payload = {
        "schema_version": "open_duck.t16_support_coordinate_block.v1",
        "status": "COMPLETE_T16_SUPPORT_COORDINATE_BLOCK",
        "inputs": {
