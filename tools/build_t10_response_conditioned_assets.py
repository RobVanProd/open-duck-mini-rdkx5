#!/usr/bin/env python3
"""Build the zero-update V121-half response-conditioned T10 assets."""
...
def expand_checkpoint(
    ...
    if added != EXPECTED_CONTEXT_KEYS or missing:
        raise RuntimeError(
            f"T10 actor expansion changed: added={added}, missing={missing}"
        )
    ...
    for key in EXPECTED_CONTEXT_KEYS:
        ...
        if not leaves or any(np.count_nonzero(np.asarray(leaf)) for leaf in leaves):
            raise RuntimeError(f"T10 context path not exact zero: {key}")
    ...
    for name, width in neutral.items():
        if name in normalizer["mean"]:
            raise RuntimeError(f"T10 normalizer key already exists: {name}")
...
def step_zero_parity(
    ...
    if describe_onnx(source_path) != expected_source:
        raise RuntimeError("V121-half source ABI changed")
    if describe_onnx(expanded_path) != expected_expanded:
        raise RuntimeError("T10 step-zero ABI changed")
...
def main() -> int:
    ...
    parser.add_argument("--v121-half-policy", type=Path, required=True)
    parser.add_argument("--calibrator", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--execute", action="store_true")
    args = parser.parse_args()
    if not args.execute:
        raise SystemExit("T10 asset build requires --execute")
    playground = args.playground_root.resolve()
    source_path = args.source_checkpoint.resolve()
    cpu_template = args.cpu_template.resolve()
    v121_policy = args.v121_half_policy.resolve()
    calibrator = args.calibrator.resolve()
    output = args.output_root.resolve()
    if output.exists():
        raise FileExistsError(f"refusing to reuse T10 assets: {output}")
    if sha256(calibrator) != (
        "cb3380ed99b3e9d7e9000904a210227aa397db2a064aa80d8f70e77c8339783b"
    ):
        raise RuntimeError("T10 calibrator changed")
    ...
    step_zero = output / "t10_v121_half_step_zero.onnx"
    ...
    parity = step_zero_parity(v121_policy, step_zero)
    checks = {
        "cpu_only": (
        ...
        "exact_two_context_parameter_families_added": (
    ...
    basis = {
        "schema_version": "open_duck.t10_response_assets.v1",
        "status": (
            "PASS_T10_RESPONSE_CONDITIONED_ASSETS"
            if not failed
            else "HOLD_T10_RESPONSE_CONDITIONED_ASSETS"
        ),
        ...
        "sources": {
            ...
            "v121_half_policy": receipt(v121_policy),
            "calibrator": receipt(calibrator),
            "t9_result": receipt(
                ANALYSIS
                / "t9_command_aware_prefix_bypass_result.json"
            ),
