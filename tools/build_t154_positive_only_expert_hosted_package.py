#!/usr/bin/env python3
"""Build the hash-frozen T154 exact-positive expert hosted bundle."""
...
MARKDOWN = (
    ANALYSIS / "T154_POSITIVE_ONLY_EXPERT_HOSTED_PACKAGE_20260729.md"
)
BUNDLE_NAME = "t154_positive_only_expert_bundle"
MANIFEST_NAME = "t154_package_manifest.json"
...
def main() -> int:
    ...
    for path in (staging, archive_path, CONTRACT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T154 package: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T154 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T154 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T154 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T154 playground changed")
    if (
        ...
        raise RuntimeError("copied T154 assets changed")
        ...
        [
            ...
            "\n".join(
                (
                    ...
                    "    print('PASS_T154_FULL_IDENTITY_PREFLIGHT')",
                    "else:",
    ...
    if (
        ...
        or "PASS_T154_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T154 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t154_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t154_hosted_package.v1",
        "status": "PASS_T154_POSITIVE_ONLY_EXPERT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            ...
            "exact_positive_randomizer_readback_frozen": True,
    ...
    MARKDOWN.write_text(
        "# T154 exact-positive-only expert hosted package\n\n"
