#!/usr/bin/env python3
"""Exercise the preregistered sparse exact-oracle schedule on 32 CPU ticks."""
...
PREREG = ANALYSIS / "winner_v126_sparse_oracle_schedule_preregistration.json"
PREREG_SHA256 = (
    "ec2ee73f44c3f078ac31a84e31938876d4c1a9dd259f564def6e695b6a8232ce"
)
BASE_PREREG = (
    ANALYSIS / "winner_v3_variable_configuration_replacement_preregistration.json"
)
OUTPUT = ANALYSIS / "winner_v126_sparse_oracle_schedule_cpu_contract.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V126_SPARSE_ORACLE_SCHEDULE_CPU_CONTRACT_20260724.md"
)
...
def main() -> int:
    ...
    if run_root.exists():
        raise FileExistsError(f"refusing to overwrite sparse contract: {run_root}")
    if OUTPUT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite sparse contract result")
    ...
    if (
        ...
        or prereg.get("authority", {}).get(
            "nonformal_schedule_contract_runs"
        )
        != 1
    ):
        raise ValueError("sparse-oracle schedule preregistration changed")
    spec = prereg["nonformal_schedule_contract"]
    row = next(
        row
    ...
    if [
        int(tick)
        for tick in row["oracle_schedule_ticks"]
        if int(tick) < int(spec["duration_ticks"])
    ] != spec["schedule_ticks"]:
        raise ValueError("sparse-oracle contract schedule changed")
    policy_spec = next(
        item
    ...
    if sha256(policy) != policy_spec["sha256"]:
        raise ValueError("sparse-oracle contract policy changed")
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        result = run_closed_loop_sim(
            ClosedLoopConfig(
                ...
                exact_torque_oracle_schedule_ticks=tuple(
                    int(tick) for tick in spec["schedule_ticks"]
                ),
            )
        )
    ...
    checks = {
        ...
        "duration_complete_32": mode.get("samples") == 32
        and mode.get("termination_reason") == "duration_complete"
        and len(rows) == 32,
        "schedule_exact": int(oracle.get("scheduled_ticks", -1)) == 3
        and int(oracle.get("schedule_bypass_ticks", -1)) == 29,
    ...
    payload = {
        "schema_version": "winner_v126.sparse_oracle_schedule_cpu_contract.v1",
        "status": (
            "PASS_WINNER_V126_SPARSE_ORACLE_SCHEDULE_CPU_CONTRACT"
            if not failed
            else "HOLD_WINNER_V126_SPARSE_ORACLE_SCHEDULE_CPU_CONTRACT"
        ),
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# Winner-v126 sparse-oracle schedule CPU contract\n\n"
        f"Status: `{payload['status']}`\n\n"
        f"Projected joint events: `{oracle.get('projected_joint_events')}`.\n\n"
        f"Prediction mismatches: `{oracle.get('prediction_mismatches')}`.\n\n"
        f"Unscheduled residual violations: "
        f"`{oracle.get('unscheduled_residual_violations')}`.\n\n"
        "A pass authorizes only the frozen 16-cell CPU screen. It authorizes "
        "no training, hosted compute, Gate 5, RDK-X5, robot, torque, or motion.\n",
        encoding="utf-8",
    )
    print(
        json.dumps(
