#!/usr/bin/env python3
"""Apply the exact V119 training-matched deployment hierarchy."""
...
CONTRACT = ANALYSIS / "winner_v120_deployment_transform_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V120_DEPLOYMENT_TRANSFORM_CONTRACT_20260724.md"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("V120 transform preregistration changed")
    ...
    selected_delta = np.asarray(
        transform["selected_normalized_action_delta"], dtype=np.float32
    )
    ...
    changed_indices = np.asarray(
        transform["affected_action_indices"], dtype=np.int64
    )
    expected_io = {
        "inputs": {
            "obs": [1, 115],
    ...
    for spec in prereg["sources"]:
        ...
        if (
            ...
            raise ValueError(f"raw V119 source changed: {spec['id']}")
        ...
        rows.append(
            {
                ...
                "preprojection_nodes_preserved": (
                    [
                        ...
                        for node in final.graph.node[
                            : len(deadbanded.graph.node)
                        ]
                    ]
                ...
                "source_train_rate_projection_exact": bool(
    ...
    checks = {
        ...
        "all_source_train_rate_projections_exact": all(
            row["source_train_rate_projection_exact"] for row in rows
        ),
        ...
        "v117_vector_source_exact": (
            transform["selected_normalized_action_delta"]
            == v117["projection"]["selected_normalized_action_delta"]
        ),
        "formal_behavior_cells_zero": True,
    }
    failed = sorted(name for name, passed in checks.items() if not passed)
    value = {
        "schema_version": "winner_v120.deployment_transform_contract.v1",
        "status": (
            "PASS_WINNER_V120_DEPLOYMENT_TRANSFORM_CONTRACT"
            if not failed
            else "HOLD_WINNER_V120_DEPLOYMENT_TRANSFORM_CONTRACT"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner-v120 deployment transform contract\n\n"
        f"Status: `{value['status']}`\n\n"
        "Both V119 post-update actor graphs preserve their trained nodes and "
        "initializers, then apply the exact train-matched G3 guard, x=0 "
        "deadband, final V117 rate projection, and restored x=0 state. No "
        "behavior, Gate 5, RDK-X5, or robot authority is granted.\n",
        encoding="utf-8",
    )
    print(value["status"])
    for row in rows:
