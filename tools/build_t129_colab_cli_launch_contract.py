#!/usr/bin/env python3
"""Freeze T129's one-session Colab CLI launch contract."""
...
PACKAGE = Path(
    ...
    "t129_negative_only_expert_bundle_20260729.tar.gz"
)
EXECUTOR = ROOT / "tools" / "execute_t129_colab_cli.py"
OUTPUT = ANALYSIS / "t129_colab_cli_launch_contract.json"
MARKDOWN = ANALYSIS / "T129_COLAB_CLI_LAUNCH_CONTRACT_20260729.md"
SESSION = "t129-negative-only-expert-20260729"
PACKAGE_SHA256 = (
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T129 launch: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T129 launch contract requires clean tree")
    ...
    checks = {
        ...
        "executor_uses_exact_driver": (
            "colab_t129_negative_only_expert.py" in text
            and "PASS_T129_HOSTED_FULL_PREFLIGHT" in text
        ),
    ...
    commands = [
        ...
        [
            ...
            SESSION,
            str(PACKAGE),
            "/content/t129-negative-only-expert-20260729.tar.gz",
        ],
        [
            "colab",
            "exec",
        ...
        [
            ...
            "--session",
            SESSION,
            "/content/t129_launch_receipt.json",
            "<LOCAL_RECEIPT>",
        ],
        ["colab", "stop", "--session", SESSION],
    ]
    value = {
        "schema_version": "open_duck.t129_colab_cli_launch_contract.v1",
        "status": (
            "PASS_T129_COLAB_CLI_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_T129_COLAB_CLI_LAUNCH_CONTRACT"
        ),
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# T129 Colab CLI launch contract\n\n"
