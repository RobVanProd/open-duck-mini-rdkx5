#!/usr/bin/env python3
"""Bind the unchanged reviewed support gate to Winner-v21 checkpoints."""
...
def _validate_snapshot(snapshot: Mapping[str, Any], *, expected_stage: str) -> None:
    if smoke is None or training is None or v21 is None:
        raise AssertionError("Winner-v21 gate dependencies were not loaded")
    if set(snapshot) != {"parameters", "optimizer", "metadata", "target_mean", "target_std"}:
        raise ValueError("Winner-v21 checkpoint state schema changed")
    ...
    if (
        ...
        or metadata.get("predictor_scale") != 8.393629541414427e-11
        ...
        raise ValueError("Winner-v21 checkpoint metadata changed")
    for name in ("target_mean", "target_std"):
        ...
        if (
            ...
            raise ValueError(f"Winner-v21 {name} schema changed")
    if bool(np.any(np.asarray(snapshot["target_std"]) <= 0.0)):
        raise ValueError("Winner-v21 target_std is not strictly positive")
    for tree in (
        snapshot["parameters"],
        ...
        if not all(bool(np.all(np.isfinite(np.asarray(value)))) for value in tree.values()):
            raise ValueError("Winner-v21 checkpoint contains nonfinite arrays")


def load_snapshot_for_reviewed_gate(path: Path) -> dict[str, Any]:
    if v21v2 is None:
        raise AssertionError("Winner-v21 mechanics were not loaded")
    exact = v21v2.load_snapshot(path)
    _validate_snapshot(exact, expected_stage="predictor_preserving_joint_stage2")
    adapted = dict(exact)
...
def checkpoint_paths(work_root: Path, label: str) -> tuple[Path, Path]:
    if _TRAINING is None or smoke is None:
        raise AssertionError("Winner-v21 training result was not loaded")
    mapping = {"half": 50, "final": 100}
    if label not in mapping:
        raise ValueError(f"unknown Winner-v21 checkpoint: {label}")
    update = mapping[label]
    snapshot = work_root / "snapshots" / f"snapshot_predictor_preserving_update_{update:03d}.npz"
    graph = work_root / "graphs" / f"winner_v21_{label}.onnx"
    ...
    for path, receipt in ((snapshot, snapshot_receipt), (graph, graph_receipt)):
        if (
            ...
            raise ValueError(f"Winner-v21 {label} artifact bytes changed: {path.name}")
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "winner_v21.predictor_preserving_support_gate_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V21_PREDICTOR_PRESERVING_SUPPORT_GATE"
        ...
        raise ValueError("Winner-v21 gate preregistration changed")
    ...
    if (
        ...
        raise ValueError("Winner-v21 gate dimensions changed")
    sources = value.get("sources")
    if not isinstance(sources, dict) or not sources:
        raise ValueError("Winner-v21 gate sources are absent")
    for name, item in sources.items():
        ...
        if (
            ...
            raise ValueError(f"Winner-v21 gate source changed: {name}")
    if reviewed_gate.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v21 gate source manifest changed")


def main() -> int:
    global reviewed_gate, smoke, training, v21, v21v2, _TRAINING
    ...
    import winner_v21_predictor_preserving_joint_support_v2 as v21v2_module
    ...
    if (
        ...
        raise ValueError("Winner-v21 training does not authorize the gate")
    ...
    reviewed["schema_version"] = "winner_v21.predictor_preserving_support_gate_result.v1"
    reviewed["status"] = (
