#!/usr/bin/env python3
"""Run the T120 joint soft-router frozen nominal matrix."""
...
PREREG = ANALYSIS / "t123_t120_nominal_preregistration.json"
RESULT = ANALYSIS / "t123_t120_nominal_result.json"
MARKDOWN = ANALYSIS / "T123_T120_NOMINAL_RESULT_20260729.md"
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T123 input: {path}")


def load() -> dict[str, Any]:
    value = json.loads(PREREG.read_text(encoding="utf-8"))
    if (
        value["status"] != "PREREGISTERED_T123_T120_NOMINAL_MATRIX"
        or value["failed_checks"]
        or canonical_sha256(value, "preregistered_contract_sha256")
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T123 preregistration changed")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T123 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T123 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t123_t120_nominal_result.v1",
        "status": (
            "PASS_T123_T120_NOMINAL_MATRIX"
            if passed
            else "HOLD_T123_T120_NOMINAL_MATRIX"
        ),
        "decision": (
        ...
        "authority": {
            "t124_negative_endpoint_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
