#!/usr/bin/env python3
"""Run T170's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t173_t170_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t173_t170_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T173_T170_TARGETED_Y_NEGATIVE_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t173_t170_targeted_y_negative_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T173 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T173 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T173 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T173 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t173_t170_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T173_T170_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T173_T170_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T173 T170 targeted Y-negative result\n\n"
