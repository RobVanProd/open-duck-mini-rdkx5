#!/usr/bin/env python3
"""Strictly import the sole Winner-v45 one-update CPU-proof artifact."""
...
PREREGISTRATION = ANALYSIS / "winner_v45_static_target_teacher_one_update_cpu_contract.json"
RUNNER = ROOT / "tools/run_winner_v45_static_target_teacher_one_update_cpu_proof.py"
WORKFLOW = ROOT / ".github/workflows/winner-v45-static-target-teacher-one-update-cpu.yml"
...
RAW_RESULT_NAME = "winner-v45-static-target-teacher-one-update-result.json"
RAW_RECEIPT_NAME = "winner-v45-static-target-teacher-one-update-result.sha256"
SNAPSHOT_MEMBER = "winner-v45-static-target-teacher-one-update-work/winner_v45_static_target_teacher_update_252.npz"
GRAPH_MEMBER = "winner-v45-static-target-teacher-one-update-work/winner_v45_static_target_teacher_update_252.onnx"
...
def read_artifact(path: Path) -> dict[str, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError(f"Winner-v45 artifact inventory changed: {names}")
        for item in members:
            ...
            if (
                ...
                raise ValueError("Winner-v45 artifact contains unsafe member")
        return {name: archive.read(name) for name in expected}


def validate_result(
    value: Mapping[str, Any], *, snapshot_bytes: bytes | None = None,
    graph_bytes: bytes | None = None,
) -> None:
    fields = {
        "authority", "checks", "decision", "environment", "execution",
        "failed_checks", "graph", "objective", "optimization", "rollout",
        "schema_version", "snapshot", "source_identity", "source_manifest_sha256",
        "sources", "status",
    }
    if set(value) not in (fields, fields | {"repository_attribution"}):
        raise ValueError("Winner-v45 result field inventory changed")
    if (
        value.get("schema_version")
        != "winner_v45.static_target_teacher_one_update_cpu_result.v1"
        or value.get("status")
        ...
        raise ValueError("Winner-v45 result did not pass the frozen contract")
    ...
    if (
        ...
        raise ValueError("Winner-v45 source or execution boundary changed")
    expected_identity = {
        "source_snapshot": prereg["artifact_inputs"]["winner_v32"]["half"]["snapshot"],
        "source_graph": prereg["artifact_inputs"]["winner_v32"]["half"]["graph"],
        "teacher_snapshot": prereg["artifact_inputs"]["winner_v22"]["snapshot"],
    }
    if value.get("source_identity") != expected_identity:
        raise ValueError("Winner-v45 source identity changed")
    rollout = value.get("rollout", {})
    if (
        rollout.get("update_index") != 251
        or rollout.get("episode_slots") != 80
        or rollout.get("scheduled_ticks") != 20000
        or rollout.get("selected_teacher_rows") != 22
        or int(rollout.get("selected_teacher_elements", 0)) <= 0
        or rollout.get("cross_worker_byte_hashes_are_not_pass_gates") is not True
        ...
        raise ValueError("Winner-v45 rollout changed")
    for name in (
        "observations_sha256", "previous_actions_sha256", "teacher_raw_targets_sha256",
        "teacher_bounded_targets_sha256", "teacher_mask_sha256",
    ):
        if HEX64.fullmatch(str(rollout.get(name))) is None:
            raise ValueError(f"Winner-v45 rollout digest changed: {name}")
    optimization = value.get("optimization", {})
    before = float(optimization.get("teacher_loss_before", math.nan))
    after = float(optimization.get("teacher_loss_after", math.nan))
    if (
        optimization.get("optimizer_count_before") != 251
        or optimization.get("optimizer_count_after") != 252
        or float(optimization.get("teacher_scale", math.nan)) != 58.436370849609375
        or not math.isfinite(before)
        or not math.isfinite(after)
        or not after < before
        or float(optimization.get("teacher_loss_delta", math.nan)) != after - before
        or float(optimization.get("teacher_scale_relative_error", math.inf)) > 2.0e-6
        ...
        or not all(float(item) == 0.0 for item in optimization.get("frozen_parameter_leaf_max_abs_delta", {}).values())
        ...
        raise ValueError("Winner-v45 optimization changed")
    snapshot = value.get("snapshot", {})
    graph = value.get("graph", {})
    contract = graph.get("contract", {})
    if (
        snapshot.get("completed_updates") != 252
        ...
        or graph.get("v45_forbidden_tokens_present") != []
        ...
        raise ValueError("Winner-v45 snapshot or graph receipt changed")
    if snapshot_bytes is not None and (
        len(snapshot_bytes) != snapshot["bytes"]
        or sha256_bytes(snapshot_bytes) != snapshot["sha256"]
    ):
        raise ValueError("Winner-v45 snapshot bytes changed")
    if graph_bytes is not None and (
        len(graph_bytes) != graph["bytes"]
        or sha256_bytes(graph_bytes) != graph["sha256"]
    ):
        raise ValueError("Winner-v45 graph bytes changed")
    if value.get("authority") != {
        ...
        raise ValueError("Winner-v45 authority changed")
    if "repository_attribution" in value:
        item = value["repository_attribution"]
        if (
            item.get("repository") != EXPECTED_REPOSITORY
            ...
            or item.get("contract_lf_sha256") != lf_sha256(PREREGISTRATION)
            ...
            raise ValueError("Winner-v45 repository attribution changed")
...
def main() -> int:
    ...
    expected_name = f"winner-v45-static-target-teacher-one-update-{args.run_id}"
    if (
        args.run_id <= 0
        ...
        raise ValueError("Winner-v45 workflow attribution changed")
    ...
    validate_result(raw, snapshot_bytes=members[SNAPSHOT_MEMBER], graph_bytes=members[GRAPH_MEMBER])
    result = dict(raw)
    result["repository_attribution"] = {
        ...
        "contract_lf_sha256": lf_sha256(PREREGISTRATION),
        "workflow_lf_sha256": lf_sha256(WORKFLOW),
        "runner_lf_sha256": lf_sha256(RUNNER),
        "importer_lf_sha256": lf_sha256(Path(__file__)),
    }
    validate_result(result, snapshot_bytes=members[SNAPSHOT_MEMBER], graph_bytes=members[GRAPH_MEMBER])
