#!/usr/bin/env python3
"""Build T228's hash-frozen hosted continuation bundle."""
...
PREREG = ANALYSIS / "t228_command_atom_hosted_preregistration.json"
...
CONTRACT = ANALYSIS / "t228_command_atom_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T228_COMMAND_ATOM_HOSTED_PACKAGE_20260730.md"
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T228 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T228 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T228 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T228 playground changed")
    if (
        ...
        raise RuntimeError("copied T228 assets changed")
    ...
    preflight_source = "\n".join(
        (
            ...
            "assert 'T227_COMMAND_ATOM_BANK=' in runner",
            ...
            "    print('PASS_T228_FULL_IDENTITY_PREFLIGHT')",
            "else:",
    ...
    if (
        ...
        or "PASS_T228_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T228 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t228_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t228_hosted_package.v1",
        "status": "PASS_T228_COMMAND_ATOM_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            "preregistration_exact": True,
            "cpu_selection_and_source_evidence_exact": True,
            ...
            "pure_v127_lagrangian_engine_frozen": True,
            "t227_command_atom_lattice_frozen": True,
            "hosted_cost_and_dual_exercise_checks_frozen": True,
            "fixed_price_and_legacy_reward_penalties_absent": True,
    ...
    MARKDOWN.write_text(
        "# T228 command-atom hosted package\n\n"
