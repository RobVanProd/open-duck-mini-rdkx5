#!/usr/bin/env python3
"""Run the frozen Winner-v94 stable universal-response readout contract."""
...
import build_winner_v94_stable_response_readout_preregistration as builder  # noqa: E402
import run_winner_v93_universal_response_readout as v93  # noqa: E402


PREREGISTRATION = ANALYSIS / "winner_v94_stable_response_readout_preregistration.json"
...
def validate_preregistration(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "winner_v94.stable_response_readout_preregistration.v1"
        or value.get("status") != "PREREGISTERED_WINNER_V94_STABLE_RESPONSE_READOUT"
        ...
        or readout.get("full_rank_required") != 65
        or readout.get("regularization") is not None
        or readout.get("hyperparameter_or_alternative_feature_search") is not False
        or readout.get("condition_number_times_float32_epsilon_at_most")
        != builder.FLOAT32_AMPLIFICATION_LIMIT
        or readout.get("heldout_float64_vs_float32_max_abs_at_most")
        != builder.FLOAT32_PREDICTION_DELTA_LIMIT
        ...
        raise ValueError("Winner-v94 preregistration identity changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v94 source manifest absent")
    for name, item in sources.items():
        if (
            ...
            raise ValueError(f"Winner-v94 source changed: {name}")
    if builder.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v94 source manifest changed")
...
def stable_arrays(
    ...
    full, target, frozen = v93.feature_target_arrays(
        cell, target_mean, target_std, auxiliary_indices
    )
    return np.concatenate((full[:, :64], full[:, -1:]), axis=1), target, frozen
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.formal_contract_authorized:
        raise PermissionError(
            "Winner-v94 requires --offline-cpu-only --formal-contract-authorized"
        )
    if args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v94 evidence")
    ...
    if len(fit_configurations) != 40 or len(heldout_configurations) != 16:
        raise ValueError("Winner-v94 configuration split changed")
    ...
    if (
        ...
        raise ValueError("Winner-v94 source artifact changed")
    ...
    raw_target = np.asarray(builder.ACTION, dtype=np.float32)
    ...
    for cell in fit_cells:
        features, target, _ = stable_arrays(
            cell, target_mean, target_std, auxiliary_indices
        )
    ...
    coefficients64, residuals, rank, singular_values = np.linalg.lstsq(
        design64, targets64, rcond=None
    )
    coefficients32 = coefficients64.astype(np.float32)
    condition_number = float(singular_values[0] / singular_values[-1])
    amplification = condition_number * float(np.finfo(np.float32).eps)
    heldout_rows = []
    maximum_precision_delta = 0.0
    for cell in heldout_cells:
        features, target, frozen = stable_arrays(
            cell, target_mean, target_std, auxiliary_indices
        )
        ...
        maximum_precision_delta = max(
            ...
            float(
                np.max(
                    np.abs(prediction64 - prediction32.astype(np.float64))
                )
            ),
        )
        heldout_rows.append(
            {
                ...
                "fitted_float32_normalized_prediction_mse": v93.mse(
                    prediction32, target
                ),
                "frozen_predictor_normalized_prediction_mse": v93.mse(
                    frozen, target
                ),
                "constant_normalized_prediction_mse": v93.mse(
    ...
    checks = {
        ...
        "design_is_full_rank": rank == 65,
        "condition_amplification_at_most_1e_minus_3": (
            amplification <= builder.FLOAT32_AMPLIFICATION_LIMIT
        ),
        "heldout_float64_float32_delta_at_most_1e_minus_4": (
            maximum_precision_delta <= builder.FLOAT32_PREDICTION_DELTA_LIMIT
        ),
    ...
    result = {
        "schema_version": "winner_v94.stable_response_readout_result.v1",
        "status": (
            "PASS_WINNER_V94_STABLE_RESPONSE_READOUT"
            if passed
            else "HOLD_WINNER_V94_STABLE_RESPONSE_READOUT"
        ),
        "decision": (
            "PREREGISTER_RESPONSE_CONDITIONED_LOCOMOTION_CPU_CONTRACT"
            if passed
            else "DO_NOT_SELECT_STABLE_RESPONSE_READOUT"
        ),
        ...
        "source_trace_cells": [v93.compact_cell(gate, cell) for cell in all_cells],
        "normalization": {
    ...
    print(f"rank={rank} condition={condition_number} amplification={amplification}")
