#!/usr/bin/env python3
"""Run the explicit-gradient two-update Winner-v21 CPU proof."""
...
import run_winner_v21_predictor_preserving_joint_cpu_contract as base_runner
...
SNAPSHOT_SHA256 = base_runner.SNAPSHOT_SHA256
SNAPSHOT_BYTES = base_runner.SNAPSHOT_BYTES
sha256 = base_runner.sha256
lf_sha256 = base_runner.lf_sha256
canonical_sha256 = base_runner.canonical_sha256
tree_equal = base_runner.tree_equal
arrays_equal = base_runner.arrays_equal
max_abs = base_runner.max_abs


def validate_source_manifest(contract: Mapping[str, Any]) -> None:
    sources = contract.get("sources")
    if not isinstance(sources, dict) or not sources:
        raise ValueError("Winner-v21 two-update source manifest is absent")
    for name, item in sources.items():
        ...
        if (
            ...
            raise ValueError(f"Winner-v21 two-update source changed: {name}")
    if canonical_sha256(sources) != contract.get("source_manifest_sha256"):
        raise ValueError("Winner-v21 two-update source-manifest digest changed")
...
def main() -> int:
    ...
    parser.add_argument(
        "--two-update-predictor-preserving-proof-authorized", action="store_true"
    )
    args = parser.parse_args()
    if (
        not args.offline_cpu_only
        or not args.two_update_predictor_preserving_proof_authorized
    ):
        raise PermissionError(
            "Winner-v21 two-update proof requires --offline-cpu-only "
            "--two-update-predictor-preserving-proof-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v21 two-update evidence")
    ...
    if jax.default_backend() != "cpu" or any(
        ...
        raise ValueError("Winner-v21 two-update proof requires CPU-only JAX")
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    if (
        contract.get("schema_version")
        != "winner_v21.predictor_preserving_two_update_cpu_contract.v1"
        or contract.get("status")
        != "FROZEN_WINNER_V21_PREDICTOR_PRESERVING_TWO_UPDATE_CPU_CONTRACT"
        or contract.get("decision")
        != "AUTHORIZE_EXACT_TWO_UPDATE_EXPLICIT_GRADIENT_PROOF_ONLY"
        ...
        raise ValueError("Winner-v21 two-update CPU contract changed")
    ...
    if (
        ...
        or attribution.get("frozen_predictor_scale", {}).get("value")
        != float(v21v2.FROZEN_PREDICTOR_SCALE)
        or attribution.get("authority", {}).get("optimizer_updates_authorized_now")
        != 0
    ):
        raise ValueError("Winner-v21 gradient-composition attribution changed")
    ...
    if (
        ...
        raise ValueError("Winner-v21 two-update Stage-1 source changed")
    ...
    if (
        ...
        raise ValueError("Winner-v21 two-update source metadata changed")
    ...
    if expected_versions.pop("platform") != "CPU only" or not smoke.validate_software_versions(
        ...
        raise ValueError("Winner-v21 two-update software environment changed")
    if smoke.sha256(args.canonical_fit) != smoke.P30_FIT_LF_SHA256:
        raise ValueError("Winner-v21 two-update canonical P30 fit changed")
    if smoke.git_output(args.playground_root, "rev-parse", "HEAD") != smoke.CONTROL_COMMIT:
        raise ValueError("Winner-v21 two-update Playground source changed")
    ...
    if smoke.sha256(scene) != smoke.SCENE_SHA256:
        raise ValueError("Winner-v21 two-update scene changed")
    ...
    if len(population) != 80:
        raise ValueError("Winner-v21 two-update population changed")
    ...
    for update_index in range(2):
        ...
        rollout_exact = (
            arrays_equal(v15_batch, batch_np, set(v15_batch))
        ...
        def predictor_objective(
            values: Mapping[str, Any], data: Mapping[str, Any]
        ):
        ...
        rollout_reports.append(
            {
                ...
                == stored_count,
                "reward_formula_exact_and_nonzero": reward[
                    "reward_formula_bit_exact"
                ]
                and reward["nonzero_penalty_count"] > 0,
                "action_boundary_exact": boundary[
        ...
        optimization_reports.append(
            {
                ...
                "ppo_gradient_max_abs": max_abs(ppo_gradients),
                "predictor_gradient_max_abs": max_abs(predictor_gradients),
        ...
        if not training.finite_tree(
            ...
            raise FloatingPointError(
                f"Winner-v21 two-update proof {update_index + 1} is nonfinite"
            )

    if chain_observations is None:
        raise ValueError("Winner-v21 two-update proof has no ONNX observations")
    ...
    graph = args.work_root / "winner_v21_predictor_preserving_update_002.onnx"
    ...
    snapshot = args.work_root / "winner_v21_predictor_preserving_update_002.npz"
    ...
    loaded = v21v2.load_snapshot(snapshot)
    all_updates_open = all(
    ...
    checks = {
        ...
        "both_winner_v15_rollouts_bit_exact": all(
            row["winner_v15_transition_exact"] for row in rollout_reports
        ),
        ...
        "both_updates_all_12_gradients_and_deltas_nonzero": all_updates_open,
        "cumulative_all_12_leaves_changed": all(
    ...
    result = {
        "schema_version": "winner_v21.predictor_preserving_two_update_cpu_result.v1",
        "status": (
        ...
        "optimization": {
            ...
            "cumulative_leaf_max_abs_delta": cumulative_deltas,
            "trainable_leaves": list(v21.JOINT_TRAINABLE_KEYS),
            "gradient_composition": "explicit g_ppo + frozen_scale * g_predictor",
        },
        "graph": {
            "path": str(graph),
            "sha256": sha256(graph),
