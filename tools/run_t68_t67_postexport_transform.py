#!/usr/bin/env python3
"""Apply T67's frozen deployment chain to all three exports."""
...
PREREGISTRATION = ANALYSIS / "t68_t67_postexport_preregistration.json"
RESULT = ANALYSIS / "t68_t67_postexport_result.json"
MARKDOWN = ANALYSIS / "T68_T67_POSTEXPORT_RESULT_20260728.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T68 exact transform requires --execute")
    ...
    if raw_steps != EXPECTED_STEPS or raw_hashes != prereg["raw_graphs"]:
        raise ValueError("T68 raw export inventory changed")
    ...
    value = {
        "schema_version": "open_duck.t68_t67_postexport_result.v1",
        "status": (
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T68 T67 post-export result",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                f"- Failed checks: `{failed}`",
                "- All three graphs use the identical frozen deployment chain",
