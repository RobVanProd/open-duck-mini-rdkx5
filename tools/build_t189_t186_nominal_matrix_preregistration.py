#!/usr/bin/env python3
"""Freeze T186's 16-cell nominal/default behavior matrix."""
...
OUTPUT = ANALYSIS / "t189_t186_nominal_matrix_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T189_T186_NOMINAL_MATRIX_PREREGISTRATION_20260730.md"
)
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t189_t186_nominal_matrix.py"
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T189: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T189 preregistration requires clean worktree")
    ...
    if failed:
        raise RuntimeError(f"T189 preregistration checks failed: {failed}")
    copied = {
        name: basis[name]
        for name in (
            "fits",
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t189_t186_nominal_matrix_preregistration.v1"
        ),
        "status": "PREREGISTERED_T189_T186_NOMINAL_MATRIX",
        **copied,
        "condition": condition,
        "policies": policies,
        "matrix": {
    ...
    MARKDOWN.write_text(
        "# T189 T186 nominal matrix preregistration\n\n"
