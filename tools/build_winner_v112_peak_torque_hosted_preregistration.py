#!/usr/bin/env python3
"""Preregister one Winner-v112 peak-torque hosted continuation."""
...
CPU_CORRECTION = ANALYSIS / "winner_v111_peak_torque_cpu_correction.json"
REFERENCE = ANALYSIS / "ground_up_projected_reference_feature_table.npz"
DRIVER = ROOT / "tools/colab_winner_v112_peak_torque_continuation.py"
OUTPUT = ANALYSIS / "winner_v112_peak_torque_hosted_preregistration.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V112_PEAK_TORQUE_HOSTED_PREREGISTRATION_20260724.md"
)
...
COMPOSED_MANIFEST_SHA256 = (
    "5c16eb0d8b66ed428e427913ca776a22e5320ebbe5aab93b584e6b6800363bed"
)
...
def main() -> int:
    ...
    correction = json.loads(CPU_CORRECTION.read_text(encoding="utf-8"))
    input_hashes = {
        "driver": sha256(DRIVER),
        "cpu_correction": sha256(CPU_CORRECTION),
    ...
    if input_hashes != expected_hashes:
        raise ValueError(f"Winner-v112 input identity changed: {input_hashes}")
    failed = sorted(name for name, passed in checks.items() if not passed)
    payload = {
        "schema_version": "winner_v112.peak_torque_hosted_preregistration.v1",
        "status": (
            "PREREGISTERED_WINNER_V112_PEAK_TORQUE_HOSTED_CONTINUATION"
            if not failed
            else "HOLD_WINNER_V112_PEAK_TORQUE_HOSTED_PREREGISTRATION"
        ),
        "failed_checks": failed,
        "checks": checks,
        "date": "2026-07-24",
        "decision": "AUTHORIZE_ONE_HASH_FROZEN_GPU_CONTINUATION_WITHOUT_RETRY",
        ...
        "training": {
            ...
            "discounting": 0.97,
            "entropy_cost": 0.005,
            "peak_torque_exceedance_threshold_nm": 1.91229675,
            "peak_torque_exceedance_scale": -1000.0,
    ...
    MARKDOWN.write_text(
        "# Winner-v112 peak-torque hosted preregistration\n\n"
        f"Status: `{payload['status']}`\n\n"
        "One no-retry GPU continuation is frozen from the exact recurrent "
        "Winner-v109 source. It adds only the CPU-proven all-joint peak-torque "
        "exceedance objective and exports step 0, half, and final artifacts. "
        "No behavior gate runs during training. A valid artifact still grants "
        "no Gate 5, deployment, robot, torque, or motion authority.\n",
        encoding="utf-8",
    )
