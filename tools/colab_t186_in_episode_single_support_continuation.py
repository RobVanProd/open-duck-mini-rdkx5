#!/usr/bin/env python3
"""Run T186's one frozen same-episode single-support continuation."""
...
CPU_RESULT_NAME = "t185c_in_episode_single_support_cpu_result.json"
RECOVERY_RESULT_NAME = "t185f_metric_runner_path_recovery_result.json"
...
def runner_command(
    ...
    """Add only T185's frozen same-episode prefix to T170's recipe."""
    ...
    command.insert(
        anchor, "--winner_t185_in_episode_single_support_prefix"
    )
    return command
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T186 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T186 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T186 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(
            work,
            arcname="t186_in_episode_single_support_continuation",
        )
...
def run(
    ...
    value["schema_version"] = (
        "open_duck.t186_in_episode_single_support_hosted_result.v1"
    )
    value["status"] = (
        "PASS_T186_TRAINING_ARTIFACT_PENDING_CPU_VALIDATION"
        if value["status"]
        == "PASS_T170_TRAINING_ARTIFACT_PENDING_CPU_VALIDATION"
        else "HOLD_T186_IN_EPISODE_SINGLE_SUPPORT_CONTINUATION"
    )
    value["mechanism"] = {
        "source": "exact_T170_half_checkpoint_1003520",
        "prefix_ticks": 27,
        "support_phase_anchors": {"left": 2, "right": 15},
        "support_side_sampling": "Bernoulli(0.5) at every reset",
        "prefix_reward": "single_support_balance_only",
        "same_episode_transition": (
            "resume unchanged T170 locomotion phase and reward after prefix"
        ),
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T186 requires hosted GPU authorization")
    return run(
