#!/usr/bin/env python3
"""Preregister T194's one earned corrected dynamic support continuation."""
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T194 preregistration requires clean worktree")
    ...
    checks = {
        "t193_substantive_cpu_contract_green": (
            cpu["status"]
        ...
        "t193b_metric_recovery_green_and_earned_exactly": (
            recovery["status"]
    ...
    if failed:
        raise RuntimeError(f"T194 preregistration checks failed: {failed}")
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t194_corrected_dynamic_reference_support_"
            "hosted_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T194_CORRECTED_DYNAMIC_REFERENCE_SUPPORT_"
            "HOSTED_CONTINUATION"
        ),
        "decision": "AUTHORIZE_ONE_HASH_FROZEN_T194_L4_CONTINUATION",
        "repository_commit": subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True
        ).strip(),
        "question": (
            "Does one exact T170-half continuation with the corrected "
            "reference-contact support objective active throughout "
            "locomotion produce two persistent checkpoints that pass the "
            "nominal and frozen robustness gates?"
        ),
        "causal_basis": {
            "t190_failure": (
                "T186 nominal passed 16/16 but targeted Y-negative passed "
                "15/16 with a late single-support fall"
            ),
            "t192b": (
                "late checkpoint switching changed zero policy, target, "
                "state, or dynamics fields after tick 520"
            ),
            "old_support_family_defect": (
                "T55 phase-sign mapping assigned both reference support "
                "anchors to the opposite side"
            ),
            "reset_only_limit": (
                "T185/T186 used the corrected side labels only for a "
                "27-tick reset prefix"
            ),
            "t193_distinction": (
                "actual reference contact channels drive support credit on "
                "every applicable locomotion tick"
            ),
        ...
        "training": {
            ...
            "support_active": "all_reference_single_support_ticks",
            "perfect_match_reward_per_tick": 0.4,
            "support_scale_derivation": "alive_20_times_dt_0.02",
        ...
        "post_training": {
            ...
            "validate_tree_raw_onnx_and_bilateral_metrics_before_behavior": (
                True
            ),
    ...
    MARKDOWN.write_text(
        "# T194 corrected dynamic reference-support hosted preregistration\n\n"
        f"- Status: `{value['status']}`\n"
        "- Source: exact T170 half checkpoint\n"
        "- Run: one L4, 2,007,040 steps, no retry/resume\n"
        "- Objective: unchanged locomotion plus corrected reference-contact "
        "support throughout gait\n"
