#!/usr/bin/env python3
"""Run T52's frozen four-condition, 64-cell qualification."""
...
RESULT = ANALYSIS / "t52_uniform_half_head_qualification_result.json"
MARKDOWN = (
    ANALYSIS / "T52_UNIFORM_HALF_HEAD_QUALIFICATION_RESULT_20260728.md"
)
DEFAULT_CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t52_uniform_half_head_qualification_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T52 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T52 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T52 matrix plan changed")
    return value


def write_markdown(result: Mapping[str, Any]) -> None:
    lines = [
        "# T52 uniform half-head qualification result",
        "",
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T52 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T52 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": (
            "open_duck.t52_uniform_half_head_qualification_result.v1"
        ),
        "status": (
            "PASS_T52_UNIFORM_HALF_HEAD_QUALIFICATION"
            if all_green
            else "HOLD_T52_UNIFORM_HALF_HEAD_QUALIFICATION"
        ),
        "decision": (
