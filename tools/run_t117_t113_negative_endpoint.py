#!/usr/bin/env python3
"""Run T113's frozen negative-COM endpoint matrix."""
...
PREREG = ANALYSIS / "t117_t113_negative_endpoint_preregistration.json"
RESULT = ANALYSIS / "t117_t113_negative_endpoint_result.json"
MARKDOWN = ANALYSIS / "T117_T113_NEGATIVE_ENDPOINT_RESULT_20260729.md"
CACHE = Path("D:/CodexArtifacts/open-duck-policy/t117_t113_negative_endpoint_v1")
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T117 input: {path}")


def load() -> dict[str, Any]:
    value = json.loads(PREREG.read_text(encoding="utf-8"))
    if (
        value["status"] != "PREREGISTERED_T117_T113_NEGATIVE_ENDPOINT"
        or value["failed_checks"]
        or canonical_sha256(value, "preregistered_contract_sha256")
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T117 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T117 matrix plan changed")
    return value
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T117 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T117 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t117_t113_negative_endpoint_result.v1"
        ),
        "status": (
            "PASS_T117_T113_NEGATIVE_ENDPOINT"
            if passed
            else "HOLD_T117_T113_NEGATIVE_ENDPOINT"
        ),
        "decision": (
        ...
        "authority": {
            "t118_full_r2_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
    ...
    MARKDOWN.write_text(
        "# T117 T113 negative endpoint result\n\n"
