#!/usr/bin/env python3
"""Compose the default-off T55 curriculum playground from T31."""
...
MODULE = (
    ROOT / "patches" / "t55_dynamic_single_support_curriculum.py"
)
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to reuse T55 tree: {output}")
    required = (
    ...
    manifest = {
        "schema_version": "open_duck.t55_composed_source.v1",
        "base": {
            "path": str(base),
    ...
    manifest_path = output / "T55_COMPOSED_SOURCE_MANIFEST.json"
