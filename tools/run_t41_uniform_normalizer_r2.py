#!/usr/bin/env python3
"""Run T41's frozen sequential R2 matrix for both transformed checkpoints."""
...
PREREG = ANALYSIS / "t41_uniform_normalizer_r2_preregistration.json"
RESULT = ANALYSIS / "t41_uniform_normalizer_r2_result.json"
MARKDOWN = ANALYSIS / "T41_UNIFORM_NORMALIZER_R2_RESULT_20260728.md"
DEFAULT_CACHE_ROOT = (
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T41 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T41 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T41 matrix plan changed")
    return value
...
def write_markdown(result: Mapping[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "Gate 5 hardware, RDK-X5, robot access, torque, motion, and "
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T41 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T41 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": "open_duck.t41_uniform_normalizer_r2_result.v1",
        "status": (
            "PASS_T41_UNIFORM_NORMALIZER_FULL_R2"
            if all_green
            else "HOLD_T41_UNIFORM_NORMALIZER_R2"
        ),
        "decision": (
