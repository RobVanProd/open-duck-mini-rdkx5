#!/usr/bin/env python3
"""Run T227A's instrumentation-corrected T227 CPU contract."""
...
RESULT = ANALYSIS / "t227a_command_atom_cpu_result.json"
MARKDOWN = ANALYSIS / "T227A_COMMAND_ATOM_CPU_RESULT_20260730.md"
WORK = Path(
...
def validate_prereg(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("T227A preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T227A composed playground changed")
...
def transform_result(return_code: int) -> int:
    ...
    value["schema_version"] = (
        "open_duck.t227a_command_atom_cpu_result.v1"
    )
    value["status"] = (
        "PASS_T227A_COMMAND_ATOM_CPU_CONTRACT"
        if passed
        else "HOLD_T227A_COMMAND_ATOM_CPU_CONTRACT"
    )
    ...
    MARKDOWN.write_text(
        "# T227A command-atom CPU result\n\n"
        f"- Status: `{value['status']}`\n"
        f"- Decision: `{value['decision']}`\n"
        f"- Failed checks: `{value['failed_checks']}`\n"
        "- Cartesian reset: 32/32 configuration-command pairs\n"
        "- Retry scope: audit instrumentation only; scientific mechanism unchanged\n"
