#!/usr/bin/env python3
"""Safely import one Winner-v22 normalized-predictor support-gate result."""
...
OUTPUT_JSON = ANALYSIS / "winner_v22_normalized_predictor_support_gate_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V22_NORMALIZED_PREDICTOR_SUPPORT_GATE_RESULT_20260721.md"
RAW_RESULT_NAME = "winner-v22-normalized-predictor-support-gate-result.json"
RAW_RECEIPT_NAME = "winner-v22-normalized-predictor-support-gate-result.sha256"
...
def read_result_artifact(path: Path) -> tuple[bytes, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v22 support-gate artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v22 support-gate artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v22 support-gate artifact has unsafe member")
...
def repository_attribution(
    *,
    ...
    require_sha256(artifact_zip_sha256, "Winner-v22 support-gate artifact")
    if (
        run_id <= 0
        or run_attempt != 1
        or HEX40_RE.fullmatch(run_head_sha) is None
        or artifact_id <= 0
        or artifact_name != f"winner-v22-normalized-predictor-support-gate-{run_id}"
        or artifact_digest != f"sha256:{artifact_zip_sha256}"
    ):
        raise ValueError("Winner-v22 support-gate workflow attribution changed")
    return {
...
def checkpoint_identities() -> dict[str, dict[str, str]]:
    ...
    if (
        ...
        raise ValueError("Winner-v22 training source is not verified")
    ...
    if (
        ...
        raise ValueError("Winner-v22 checkpoint set changed")
    ...
    for label, update, checkpoint in zip(
        ...
        require_sha256(snapshot_sha, f"Winner-v22 {label} snapshot")
        require_sha256(graph_sha, f"Winner-v22 {label} graph")
        identities[label] = {
...
def validate_result(result: Mapping[str, Any]) -> None:
    ...
    if frozenset(result) not in {
        ...
        "winner_v22.normalized_predictor_support_gate_result.v1"
    ):
        raise ValueError("Winner-v22 formal gate result schema changed")
    if "repository_attribution" in result:
        ...
        if (
            ...
            != f"winner-v22-normalized-predictor-support-gate-{attribution['github_run_id']}"
            ...
            raise ValueError("Winner-v22 imported repository attribution changed")
    ...
    if (
        ...
        raise ValueError("Winner-v22 formal gate checks changed")
    if result.get("execution") != {
        ...
        raise ValueError("Winner-v22 formal gate execution or authority changed")
    if result.get("sources") != {
        ...
        raise ValueError("Winner-v22 formal gate sources changed")
    rows = result.get("checkpoint_results")
    if not isinstance(rows, list) or [row.get("label") for row in rows] != [
        "half",
        "final",
    ]:
        raise ValueError("Winner-v22 formal gate checkpoint set changed")
    ...
    if checks != derived:
        raise ValueError("Winner-v22 formal gate summary is not rederived")
    ...
    if (
        ...
        raise ValueError("Winner-v22 formal gate decision changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v22 support-gate result is already imported")
    ...
    if receipt_bytes != f"{raw_sha}  /tmp/{RAW_RESULT_NAME}\n".encode():
        raise ValueError("Winner-v22 support-gate raw-result receipt changed")
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Winner-v22 normalized-predictor formal support-gate result",
                "",
