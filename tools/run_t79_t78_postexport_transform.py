#!/usr/bin/env python3
"""Apply T78's frozen deployment chain to all three exports."""
...
PREREGISTRATION = ANALYSIS / "t79_t78_postexport_preregistration.json"
RESULT = ANALYSIS / "t79_t78_postexport_result.json"
MARKDOWN = ANALYSIS / "T79_T78_POSTEXPORT_RESULT_20260728.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T79 exact transform requires --execute")
    ...
    if raw_steps != EXPECTED_STEPS or raw_hashes != prereg["raw_graphs"]:
        raise ValueError("T79 raw export inventory changed")
    ...
    value = {
        "schema_version": "open_duck.t79_t78_postexport_result.v1",
        "status": (
            "PASS_T79_T78_POSTEXPORT_TRANSFORM"
            if not failed
