#!/usr/bin/env python3
"""Run T216's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t220_t216_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t220_t216_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T220_T216_TARGETED_Y_NEGATIVE_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t220_t216_targeted_y_negative_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T220 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T220 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T220 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T220_T216_TARGETED_Y_NEGATIVE"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T220 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t220_t216_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T220_T216_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T220_T216_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T220 T216 targeted Y-negative result\n\n"
