#!/usr/bin/env python3
"""Reuse T102 nominal evidence through T139's exact nominal identity."""
...
PREREG = ANALYSIS / "t140_nominal_identity_reuse_preregistration.json"
RESULT = ANALYSIS / "t140_nominal_identity_reuse_result.json"
MARKDOWN = ANALYSIS / "T140_NOMINAL_IDENTITY_REUSE_RESULT_20260729.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T140 requires --execute")
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("formal T140 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T140 preregistration changed")
    for name, item in prereg["frozen_inputs"].items():
        verify(item, name)
    t139 = json.loads(
        Path(prereg["frozen_inputs"]["t139_result"]["path"]).read_text(
            encoding="utf-8"
        )
    )
    t102 = json.loads(
    ...
    source_identity = {
        step: (
            t139["graphs"][step]["nominal_source"]
            == t102_policies[step]
        )
        for step in sorted(t139["graphs"])
    }
    nominal_contract = {
        step: [
            row
            for row in t139["contracts"][step]["rows"]
            if row["population"] == "nominal"
        ]
        for step in sorted(t139["contracts"])
    }
    checks = {
        ...
        "nominal_sources_are_exact_t102_policies": all(
            source_identity.values()
        ),
        "both_fit_contexts_select_t100c_bit_exact": all(
            len(rows) == 2
            and {row["fit_id"] for row in rows} == {"p30", "p31_34"}
            and all(
                row["selected_source"] == "T100C_nominal_expert"
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t140_nominal_identity_reuse_result.v1",
        "status": (
            "PASS_T140_NOMINAL_IDENTITY_REUSE"
            if passed
            else "HOLD_T140_NOMINAL_IDENTITY_REUSE"
        ),
        "decision": (
