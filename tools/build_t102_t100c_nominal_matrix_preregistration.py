#!/usr/bin/env python3
"""Freeze T100C's 16-cell corrected nominal behavior matrix."""
...
TRANSFORM = ANALYSIS / "t101_t100c_postexport_result.json"
T27_PREREG = ANALYSIS / "t27_t23_robustness_matrix_preregistration.json"
T27_RUNNER_CONTRACT = ANALYSIS / "t27_t23_robustness_runner_contract.json"
RUNNER = ROOT / "tools" / "run_t102_t100c_nominal_matrix.py"
WORKER = ROOT / "tools" / "evaluate_t27_t23_robustness_condition.py"
ADAPTER = ROOT / "tools" / "t27_state_coherent_eval_adapter.py"
TEST = ROOT / "tests" / "test_t102_t100c_nominal_matrix.py"
OUTPUT = ANALYSIS / "t102_t100c_nominal_matrix_preregistration.json"
OUTPUT_MD = ANALYSIS / "T102_T100C_NOMINAL_MATRIX_PREREGISTRATION_20260728.md"
...
def main() -> int:
    for path in (OUTPUT, OUTPUT_MD):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T102: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T102 preregistration requires a clean worktree")
    ...
    for checkpoint_id, step_value in (
        ...
        if sha256(path) != wrapped["sha256"]:
            raise ValueError(f"changed T102 policy: {path}")
        policies.append(
            {
    ...
    checks = {
        "t101_transform_green": (
            transform["status"] == "PASS_T101_T100C_POSTEXPORT_TRANSFORM"
            and transform["decision"]
            == "EARN_T102_NOMINAL_MATRIX_PREREGISTRATION_ONLY"
        ),
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t102_t100c_nominal_matrix_preregistration.v1",
        "status": (
            "PREREGISTERED_T102_T100C_NOMINAL_MATRIX"
            if not failed
            else "HOLD_T102_T100C_NOMINAL_MATRIX_PREREGISTRATION"
        ),
        "question": (
            "Do both hidden-expert checkpoints preserve every nominal command "
            "under both measured actuator fits?"
        ),
        ...
        "matrix": {
            ...
            "maximum_cells": 16,
            "execution_order": (
                "default condition -> hidden-expert half/final -> fit p30/"
        ...
        "decision_rule": {
            "pass": (
                "All 16 cells pass unchanged behavior, duration protection, "
                "handoff, readback, margin, torque/current, and rate checks."
            ),
            "pass_decision": (
        ...
        "frozen_inputs": {
            "t101_transform": receipt(TRANSFORM),
        ...
        "authority": {
            ...
            "negative_endpoint_execution": False,
            "training": False,
            "colab": False,
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# T102 T100C nominal matrix preregistration",
                "",
                f"- Status: `{payload['status']}`",
                "- Cells: `16`",
                "- Policies: hidden-expert half + final, both required",
                "- Fits: `P30 / P31-34`",
