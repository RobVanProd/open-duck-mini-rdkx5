#!/usr/bin/env python3
"""Safely import the sole Winner-v33 prefix-anchor support-gate result."""
...
PREREGISTRATION = ANALYSIS / "winner_v33_prefix_right_pitch_anchor_support_gate_preregistration.json"
TRAINING_RESULT = ANALYSIS / "winner_v32_prefix_right_pitch_anchor_training_result.json"
TRAINING_PREREGISTRATION = ANALYSIS / "winner_v32_prefix_right_pitch_anchor_training_preregistration.json"
TRAINING_RUNNER = ROOT / "tools/run_winner_v32_prefix_right_pitch_anchor_training.py"
...
WORKFLOW = ROOT / ".github/workflows/winner-v33-prefix-right-pitch-anchor-support-gate.yml"
V15_IMPORTER = ROOT / "tools/import_winner_v15_pitch_margin_support_gate_result.py"
OUTPUT_JSON = ANALYSIS / "winner_v33_prefix_right_pitch_anchor_support_gate_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V33_PREFIX_RIGHT_PITCH_ANCHOR_SUPPORT_GATE_RESULT_20260722.md"
RAW_RESULT_NAME = "winner-v33-prefix-right-pitch-anchor-support-gate-result.json"
RAW_RECEIPT_NAME = "winner-v33-prefix-right-pitch-anchor-support-gate-result.sha256"
...
def _load_v15_importer():
    spec = importlib.util.spec_from_file_location(
        "winner_v15_support_gate_import_helpers_v33", V15_IMPORTER
    )
...
def read_result_artifact(path: Path) -> tuple[bytes, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v33 support-gate artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v33 support-gate artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v33 support-gate artifact has unsafe member")
...
def repository_attribution(
    *,
    ...
    require_sha256(artifact_zip_sha256, "Winner-v33 support-gate artifact")
    if (
        run_id <= 0
        or run_attempt != 1
        ...
        or artifact_id <= 0
        or artifact_name
        != f"winner-v33-prefix-right-pitch-anchor-support-gate-{run_id}"
        or artifact_digest != f"sha256:{artifact_zip_sha256}"
    ):
        raise ValueError("Winner-v33 support-gate workflow attribution changed")
    return {
...
def checkpoint_identities() -> dict[str, dict[str, str]]:
    ...
    if (
        ...
        raise ValueError("Winner-v32 training source is not verified")
    ...
    if (
        ...
        raise ValueError("Winner-v32 checkpoint set changed")
    identities: dict[str, dict[str, str]] = {}
    for label, update, checkpoint in zip(
        ("half", "final"), (251, 301), checkpoints, strict=True
    ):
        snapshot = next(
        ...
        require_sha256(snapshot_sha, f"Winner-v33 {label} snapshot")
        require_sha256(graph_sha, f"Winner-v33 {label} graph")
        identities[label] = {
...
def _validate_checkpoint_result(
    ...
    expected_update = {"half": 251, "final": 301}[label]
    if row.get("update") != expected_update:
        raise ValueError(f"Winner-v33 formal {label} update changed")
    adapted = dict(row)
...
def validate_result(result: Mapping[str, Any]) -> None:
    ...
    if frozenset(result) not in {
        ...
        "winner_v33.prefix_right_pitch_anchor_support_gate_result.v1"
    ):
        raise ValueError("Winner-v33 formal gate result schema changed")
    if "repository_attribution" in result:
        ...
        if (
            ...
            != f"winner-v33-prefix-right-pitch-anchor-support-gate-{attribution['github_run_id']}"
            ...
            raise ValueError("Winner-v33 imported repository attribution changed")
    ...
    if (
        ...
        raise ValueError("Winner-v33 formal gate checks changed")
    if result.get("execution") != {
        ...
        raise ValueError("Winner-v33 formal gate execution or authority changed")
    if result.get("sources") != {
        ...
        raise ValueError("Winner-v33 formal gate sources changed")
    rows = result.get("checkpoint_results")
    if not isinstance(rows, list) or [row.get("label") for row in rows] != [
        "half",
        "final",
    ]:
        raise ValueError("Winner-v33 formal gate checkpoint set changed")
    ...
    if checks != derived:
        raise ValueError("Winner-v33 formal gate summary is not rederived")
    ...
    if (
        ...
        != (
            "PASS_WINNER_V33_PREFIX_RIGHT_PITCH_ANCHOR_SUPPORT_GATE"
            if passed
            else "HOLD_WINNER_V33_PREFIX_RIGHT_PITCH_ANCHOR_SUPPORT_GATE"
        )
        ...
        raise ValueError("Winner-v33 formal gate decision changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v33 support-gate result is already imported")
    ...
    if receipt_bytes != f"{raw_sha}  /tmp/{RAW_RESULT_NAME}\n".encode():
        raise ValueError("Winner-v33 support-gate raw-result receipt changed")
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Winner-v33 prefix right-pitch anchor formal support-gate result",
                "",
