#!/usr/bin/env python3
"""Run the 64-tick V126 all-tick exact-oracle CPU contract."""
...
RECOVERY_AMENDMENT = (
    ANALYSIS / "winner_v126_all_tick_contract_recovery_amendment.json"
)
RECOVERY_AMENDMENT_SHA256 = (
    "1f92dce6daca451bed0c5acb81ec707a92b75d47728d439ed90d03b5b6706e85"
)
BASE_PREREG = (
    ANALYSIS / "winner_v3_variable_configuration_replacement_preregistration.json"
)
OUTPUT = ANALYSIS / "winner_v126_all_tick_oracle_cpu_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V126_ALL_TICK_ORACLE_CPU_CONTRACT_20260724.md"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("all-tick preregistration changed")
    recovery = json.loads(RECOVERY_AMENDMENT.read_text(encoding="utf-8"))
    if (
        sha256(RECOVERY_AMENDMENT) != RECOVERY_AMENDMENT_SHA256
        or recovery.get("status")
        != "PREREGISTERED_WINNER_V126_ALL_TICK_CONTRACT_RECOVERY"
        or recovery.get("authorized_recovery_executions") != 1
        or not all(recovery.get("checks", {}).values())
    ):
        raise ValueError("all-tick recovery amendment changed")
    ...
    payload = {
        "schema_version": "winner_v126.all_tick_oracle_cpu_contract.v1",
        "status": (
            "PASS_WINNER_V126_ALL_TICK_ORACLE_CPU_CONTRACT"
            if not failed
            else "HOLD_WINNER_V126_ALL_TICK_ORACLE_CPU_CONTRACT"
        ),
        "checks": checks,
        ...
        "input_hashes": {
            ...
            "recovery_amendment": sha256(RECOVERY_AMENDMENT),
    ...
    MARKDOWN.write_text(
        "# V126 all-tick exact-oracle CPU contract\n\n"
