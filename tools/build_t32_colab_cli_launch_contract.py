#!/usr/bin/env python3
"""Freeze T32's one-session Colab CLI launch contract."""
...
OUTPUT = ANALYSIS / "t32_colab_cli_launch_contract.json"
MARKDOWN = ANALYSIS / "T32_COLAB_CLI_LAUNCH_CONTRACT_20260727.md"
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T32 launch: {path}")
    ...
    checks = {
        ...
        "executor_single_t32_driver_exact": (
    ...
    value = {
        "schema_version": "open_duck.t32_colab_cli_launch_contract.v1",
        "status": (
            "PASS_T32_COLAB_CLI_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_T32_COLAB_CLI_LAUNCH_CONTRACT"
        ),
        "checks": checks,
