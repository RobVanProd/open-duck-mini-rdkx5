#!/usr/bin/env python3
"""Run the one preregistered A1 hard-vector continuation on a Colab T4."""
...
ASSETS = Path("/content")
ROOT = ASSETS / "ground_up_hard_vector_playground"
SOURCE_ROOT = ASSETS / "ground_up_hard_vector_source"
OUTPUT = ASSETS / "ground_up_hard_vector_outputs" / "A1_HARD_VECTOR_COMMAND_SUPPORT"
ARTIFACT = ASSETS / "A1_HARD_VECTOR_COMMAND_SUPPORT_artifacts.tar.gz"
MANIFEST = ASSETS / "A1_HARD_VECTOR_COMMAND_SUPPORT_manifest.json"
CONTROL_COMMIT = "b9be205ac64488c23504ca42e5ec790337adeec3"
SOURCE_CHECKPOINT = (
    SOURCE_ROOT
    / "A1_REFERENCE_ANCHORED_RESIDUAL"
    / "2026_07_14_141621_4014080"
)
PATCHES = (
...
EXPECTED_HASHES = {
    ...
    "A1_REFERENCE_ANCHORED_RESIDUAL_ground_up_recipe_artifacts.tar.gz": "7cccf38d1bea2596d0fd44c8e9eeabc3248f0321e39adaa4707a5a9e59788564",
}
...
def run(
    ...
    *,
    ...
    print("CONTINUATION_COMMAND=" + json.dumps(command), flush=True)
...
def main() -> int:
    ...
    with tarfile.open(
        ASSETS / "A1_REFERENCE_ANCHORED_RESIDUAL_ground_up_recipe_artifacts.tar.gz",
        "r:gz",
    ) as archive:
    ...
    run(
        [sys.executable, "-m", "pip", "install", "-q", "--no-deps", "tf2onnx==1.17.0"]
    )
    run(
    ...
    print("CONTINUATION_DEVICE_CONTRACT=" + versions, flush=True)
    ...
    metadata = {
        "schema_version": "ground_up_hard_vector_command_support_colab.v1",
        ...
        "artifacts": {
            "checkpoints": [
                {"name": path.name, "directory": True} for path in checkpoints
            ],
            "onnx": [
                {"name": path.name, "sha256": sha256(path)} for path in onnx_files
            ],
        },
    ...
    print(
        "GROUND_UP_HARD_VECTOR_CONTINUATION_RESULT="
        + json.dumps(
            {
