#!/usr/bin/env python3
"""Preregister T67's exact frozen deployment transforms."""
...
VALIDATION = ANALYSIS / "t67_recovered_training_validation.json"
T31_PREREGISTRATION = (
...
OUTPUT = ANALYSIS / "t68_t67_postexport_preregistration.json"
MARKDOWN = ANALYSIS / "T68_T67_POSTEXPORT_PREREGISTRATION_20260728.md"
RAW_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t67_extracted_20260728/"
    "t67_endpoint_core_continuation/training"
)
EXPECTED_STEPS = [0, 1_003_520, 2_007_040]
SOURCE_PATHS = (
    "tools/run_t68_t67_postexport_transform.py",
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T68: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"],
        cwd=ROOT,
        text=True,
    ).strip():
        raise RuntimeError("T68 preregistration requires a clean worktree")
    ...
    checks = {
        "t67_recovered_training_validation_green": (
            validation["status"]
        ...
        "exact_three_t67_exports": steps == EXPECTED_STEPS,
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t68_t67_postexport_preregistration.v1",
        "status": (
            "PREREGISTERED_T68_T67_POSTEXPORT_TRANSFORM"
            if not failed
            else "HOLD_T68_T67_POSTEXPORT_TRANSFORM"
        ),
        "failed_checks": failed,
        "checks": checks,
        "input_hashes": {
            "t67_validation": sha256(VALIDATION),
        ...
        "golden_requirement": (
            ...
            "every deployment sub-contract on every T67 export"
        ),
        "postupdate_steps": EXPECTED_STEPS[1:],
        "decision": (
            "RUN_ONE_CPU_ONLY_T68_POSTEXPORT_TRANSFORM"
            if not failed
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T68 T67 post-export preregistration",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                "- Three raw exports must match T67 CPU validation",
