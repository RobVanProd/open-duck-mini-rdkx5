#!/usr/bin/env python3
"""Run the frozen T136 static-router nominal persistence matrix."""
...
PREREG = ANALYSIS / "t137_static_router_nominal_preregistration.json"
RESULT = ANALYSIS / "t137_static_router_nominal_result.json"
MARKDOWN = ANALYSIS / "T137_STATIC_ROUTER_NOMINAL_RESULT_20260729.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t137_static_router_nominal_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T137 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T137 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T137 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T137 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t137_static_router_nominal_result.v1"
        ),
        "status": (
            "PASS_T137_STATIC_ROUTER_NOMINAL_MATRIX"
            if passed
            else "HOLD_T137_STATIC_ROUTER_NOMINAL_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T137 static-router nominal result\n\n"
