#!/usr/bin/env python3
"""Freeze the one archive-root-corrected V127d Colab launch."""
...
PREREG = ANALYSIS / "winner_v127d_hosted_preregistration.json"
...
PACKAGE = Path(
    ...
    "winner-v127d-hosted-20260724.tar.gz"
)
CHUNK_ROOT = Path(
    "D:/CodexArtifacts/open-duck-mini-rdkx5/"
    "winner-v127d-upload-chunks-20260724"
)
LAUNCHER = ROOT / "tools/launch_winner_v127_constrained_colab.py"
EXECUTOR = ROOT / "tools/execute_winner_v127d_chunked_colab_cli.py"
OUTPUT = ANALYSIS / "winner_v127d_colab_launch_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V127D_COLAB_LAUNCH_CONTRACT_20260724.md"
SESSION = "winner-v127d-constrained-20260724"
...
def main() -> int:
    ...
    checks = {
        "v127d_preregistration_green": prereg.get("status")
        == "PREREGISTERED_WINNER_V127D_HOSTED_CONTINUATION",
        "v127d_package_green": package_contract.get("status")
        == "PASS_WINNER_V127_HOSTED_PACKAGE",
        "pretraining_correction_green": pretraining.get("status")
        == "PASS_WINNER_V127_PRETRAINING_LAUNCH_CORRECTION",
        "archive_root_correction_green": archive_root.get("status")
        ...
        "archive_has_exact_selected_root": roots == [BUNDLE_NAME],
        "four_chunks_exact": len(chunks) == 4,
        "chunks_reconstruct_package": (
            reconstructed.hexdigest() == sha256(PACKAGE)
        ),
        "launcher_uses_selected_bundle": BUNDLE_NAME in launcher_text,
        "launcher_pins_package": sha256(PACKAGE) in launcher_text,
        "launcher_pins_both_corrections": (
            sha256(PRETRAINING_CORRECTION) in launcher_text
            and sha256(ARCHIVE_ROOT_CORRECTION) in launcher_text
        ),
        "executor_pins_package_and_launcher": (
            sha256(PACKAGE) in executor_text
            and sha256(LAUNCHER) in executor_text
        ),
        "both_prior_launches_pre_optimizer": (
    ...
    payload = {
        "schema_version": "winner_v127d.colab_launch_contract.v1",
        "status": (
            "PASS_WINNER_V127D_COLAB_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_WINNER_V127D_COLAB_LAUNCH_CONTRACT"
        ),
        ...
        "commands": commands,
        "authority": {
            "one_archive_root_corrected_exact_cli_launch": not failed,
    ...
    MARKDOWN.write_text(
        "# Winner V127d Colab launch\n\n"
        f"- Status: `{payload['status']}`\n"
        f"- Archive root: `{BUNDLE_NAME}`\n"
        "- Four chunks reconstruct the exact package.\n"
        "- Both prior launches stopped before simulator or optimizer steps.\n"
        "- Authority: one L4 launch; no retry, resume, behavior, or robot.\n",
        encoding="utf-8",
    )
