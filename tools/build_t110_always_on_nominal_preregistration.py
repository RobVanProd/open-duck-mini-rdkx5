#!/usr/bin/env python3
"""Freeze the always-on expert's 16-cell nominal matrix."""
...
T109_RESULT = ANALYSIS / "t109_always_on_expert_result.json"
BASIS = ANALYSIS / "t107_soft_gate_nominal_preregistration.json"
BUILDER = ROOT / "tools" / Path(__file__).name
RUNNER = ROOT / "tools" / "run_t110_always_on_nominal.py"
TEST = ROOT / "tests" / "test_t110_always_on_nominal.py"
OUTPUT = ANALYSIS / "t110_always_on_nominal_preregistration.json"
MARKDOWN = ANALYSIS / "T110_ALWAYS_ON_NOMINAL_PREREGISTRATION_20260729.md"
...
def main() -> int:
    ...
    if not args.read_only_authorized:
        raise PermissionError("T110 preregistration requires authorization")
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T110: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T110 preregistration requires clean worktree")
    t109 = json.loads(T109_RESULT.read_text(encoding="utf-8"))
    basis = json.loads(BASIS.read_text(encoding="utf-8"))
    policies = []
    for checkpoint_id, step in (
        ("T109_ALWAYS_ON_HALF", "1003520"),
        ("T109_ALWAYS_ON_FINAL", "2007040"),
    ):
        policies.append(
            {
                "checkpoint_id": checkpoint_id,
                "step": int(step),
                **t109["transforms"][step]["output"],
            }
        )
    inputs = {
        "builder": BUILDER,
        "runner": RUNNER,
        "test": TEST,
        "t109_result": T109_RESULT,
        "basis": BASIS,
    }
    checks = {
        "t109_green": (
            t109["status"] == "PASS_T109_ALWAYS_ON_EXPERT_TRANSFORM"
            and t109["failed_checks"] == []
        ),
        "policies_exact": all(
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t110_always_on_nominal_preregistration.v1",
        "status": (
            "PREREGISTERED_T110_ALWAYS_ON_NOMINAL_MATRIX"
            if not failed
            else "HOLD_T110_ALWAYS_ON_NOMINAL_PREREGISTRATION"
        ),
    ...
    MARKDOWN.write_text(
        "# T110 always-on nominal preregistration\n\n"
