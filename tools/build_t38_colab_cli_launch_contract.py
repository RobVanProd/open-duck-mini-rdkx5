#!/usr/bin/env python3
"""Freeze T38's one-session Colab CLI launch contract."""
...
PACKAGE = Path(
    ...
    "t38-frozen-normalizer-hosted-20260727.tar.gz"
)
EXECUTOR = ROOT / "tools" / "execute_t38_colab_cli.py"
OUTPUT = ANALYSIS / "t38_colab_cli_launch_contract.json"
MARKDOWN = ANALYSIS / "T38_COLAB_CLI_LAUNCH_CONTRACT_20260727.md"
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T38 launch: {path}")
    ...
    checks = {
        ...
        "executor_single_t38_driver_exact": (
            "colab_t38_frozen_normalizer_continuation.py"
            in executor_text
        ),
        "executor_checks_frozen_normalizer_source": (
            "winner_t37_freeze_observation_normalizer" in executor_text
        ),
    ...
    commands = [
        ...
        [
            ...
            "--session",
            SESSION,
            "/content/t38_launch_receipt.json",
            "<LOCAL_RECEIPT>",
        ],
        ["colab", "stop", "--session", SESSION],
    ]
    value = {
        "schema_version": "open_duck.t38_colab_cli_launch_contract.v1",
        "status": (
            "PASS_T38_COLAB_CLI_LAUNCH_CONTRACT"
            if not failed
            else "HOLD_T38_COLAB_CLI_LAUNCH_CONTRACT"
        ),
        "checks": checks,
