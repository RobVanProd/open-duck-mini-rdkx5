#!/usr/bin/env python3
"""Run one frozen T56 balance-first then gait-transfer GPU continuation."""
...
CPU_RESULT_NAME = "t55_dynamic_single_support_cpu_result.json"
REFERENCE_NAME = "ground_up_projected_reference_feature_table.npz"
SOURCE_CHECKPOINT_NAME = "t52_half_materialized_checkpoint"
BALANCE_STEPS = 1_003_520
TRANSFER_STEPS = 2_007_040
EXPECTED_BALANCE_EXPORTS = [0, BALANCE_STEPS]
EXPECTED_TRANSFER_EXPORTS = [0, BALANCE_STEPS, TRANSFER_STEPS]
...
def runner_command(
    *,
    ...
    if stage == "balance":
        ...
        raise ValueError(f"unknown T56 stage: {stage}")
    return [
        sys.executable,
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T56 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T56 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T56 requires one GPU process: {devices}")
    return {
...
def run_stage(
    *,
    ...
    if output.exists():
        raise FileExistsError(f"T56 {stage} output exists: {output}")
    output.mkdir()
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T56 {stage} runner returned {completed.returncode}"
        )
    ...
    checks = {
        ...
        "stage_flag_exact": (
            f"--winner_t55_{stage}_stage" in command
            and (
                "--winner_t55_transfer_stage"
                if stage == "balance"
                else "--winner_t55_balance_stage"
            )
            not in command
        ),
    }
    ...
    if failed:
        raise ValueError(f"T56 {stage} artifact checks failed: {failed}")
    final_checkpoint = checkpoints[-1]
    return (
        {
            "status": f"PASS_T56_{stage.upper()}_TRAINING_ARTIFACT",
            "command": command,
            "checks": checks,
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(
            work,
            arcname="t56_dynamic_single_support_continuation",
        )
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t56_hosted_result.v1",
        "status": "RUNNING_T56_DYNAMIC_SINGLE_SUPPORT_HOSTED_CONTINUATION",
    ...
    try:
        balance, balance_final = run_stage(
            stage="balance",
            playground=playground,
            output=work / "balance_training",
            source=source,
            reference=reference,
            work=work,
        )
        payload["balance"] = balance
        payload["status"] = "RUNNING_T56_GAIT_TRANSFER"
        run_state.write_text(
        ...
        checks = {
            ...
            "balance_exact": balance["failed_checks"] == [],
            "transfer_exact": transfer["failed_checks"] == [],
            "transfer_restores_balance_final": (
                transfer["command"][
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T56_DYNAMIC_SINGLE_SUPPORT_CONTINUATION"
                ),
                "checks": checks,
                "failed_checks": failed,
                "wall_seconds": elapsed,
        ...
        if failed:
            raise ValueError(f"T56 hosted checks failed: {failed}")
        ...
        payload["status"] = "HOLD_T56_DYNAMIC_SINGLE_SUPPORT_CONTINUATION"
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T56 requires hosted GPU authorization")
    return run(
