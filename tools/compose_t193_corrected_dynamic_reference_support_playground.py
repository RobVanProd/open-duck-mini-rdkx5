#!/usr/bin/env python3
"""Compose T193's default-off playground from the frozen T170 base."""
...
MODULE = (
    ROOT / "patches" / "t193_corrected_dynamic_reference_support.py"
)
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to reuse T193 tree: {output}")
    required = (
    ...
    destination = (
        ...
        / "playground"
        / "common"
        / "t193_corrected_dynamic_reference_support.py"
    )
    ...
    run(
        [
            ...
            "-m",
            "py_compile",
            "playground/common/t193_corrected_dynamic_reference_support.py",
    ...
    manifest = {
        "schema_version": "open_duck.t193_composed_source.v1",
        "base": {
            "path": str(base),
    ...
    manifest_path = output / "T193_COMPOSED_SOURCE_MANIFEST.json"
