#!/usr/bin/env python3
"""Freeze the unchanged reviewed support gate for Winner-v24 half/final."""
...
OUTPUT = ANALYSIS / "winner_v24_baseline_anchored_support_gate_preregistration.json"
MARKDOWN = ANALYSIS / "WINNER_V24_BASELINE_ANCHORED_SUPPORT_GATE_PREREGISTRATION_20260722.md"
...
SOURCES = {
    "builder": Path("tools/build_winner_v24_baseline_anchored_support_gate_preregistration.py"),
    "gate_runner": Path("tools/run_winner_v24_baseline_anchored_support_gate.py"),
    "gate_tests": Path("tests/test_winner_v24_baseline_anchored_support_gate.py"),
    "preregistration_tests": Path("tests/test_winner_v24_baseline_anchored_support_gate_preregistration.py"),
    "result_importer": Path("tools/import_winner_v24_baseline_anchored_support_gate_result.py"),
    "result_importer_tests": Path("tests/test_winner_v24_baseline_anchored_support_gate_import.py"),
    "workflow": Path(".github/workflows/winner-v24-baseline-anchored-support-gate.yml"),
    "training_result": Path("outputs/analysis/winner_v24_baseline_anchored_training_result.json"),
    "training_preregistration": Path("outputs/analysis/winner_v24_baseline_anchored_training_preregistration.json"),
    "training_runner": Path("tools/run_winner_v24_baseline_anchored_training.py"),
    "training_importer": Path("tools/import_winner_v24_baseline_anchored_training.py"),
...
def main() -> int:
    ...
    if args.output.exists() or args.markdown.exists():
        raise FileExistsError("refusing to overwrite Winner-v24 support-gate contract")
    ...
    if (
        ...
        raise ValueError("Winner-v24 training does not authorize a gate")
    ...
    if (
        ...
        raise ValueError("Winner-v24 training attribution changed")
    checkpoints = training.get("persistent_checkpoints", [])
    if [row.get("label") for row in checkpoints] != ["half", "final"]:
        raise ValueError("Winner-v24 checkpoints changed")
    if [row.get("completed_updates") for row in checkpoints] != [150, 200]:
        raise ValueError("Winner-v24 checkpoint update identities changed")
    ...
    payload = {
        "schema_version": "winner_v24.baseline_anchored_support_gate_preregistration.v1",
        ...
        "binding_change_only": (
            ...
            "Winner-v24 count-150/count-200 paths are bound read-only and their normalized "
            "auxiliary head is projected exactly into the raw scoring coordinates."
        ),
        "training_artifact": {
            "repository_attribution": attribution,
            "source_snapshot": training["source_snapshot"],
            "objective": training["objective"],
            "half": _checkpoint(training, "half", 150),
            "final": _checkpoint(training, "final", 200),
        },
