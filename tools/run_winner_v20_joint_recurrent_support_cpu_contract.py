...
def main() -> int:
    ...
    if (
        ...
        != "winner_v20.joint_recurrent_support_cpu_contract.v2"
    ...
    if (
        ...
        raise ValueError("Winner-v20 CPU hold attribution changed")
    ...
    optimizer = training.adam_initialize(v20.joint_trainable_parameters(parameters))
        ...
        rollout_reports.append(
            {
                ...
                "observation_capture_exact": observation_capture_exact,
                ...
                "sampled_hidden_replay_max_abs_error": float(
                    loss_metrics["sampled_hidden_replay_max_abs_error"]
                ),
            }
        )
    ...
    if chain_observations is None:
        raise ValueError("Winner-v20 produced no ONNX chain observations")
    auxiliary_after = v20.frozen_auxiliary_parameters(parameters)
    cumulative_deltas = v20.leaf_max_abs_delta(
        v20.joint_trainable_parameters(source_parameters),
        v20.joint_trainable_parameters(parameters),
    )

    args.work_root.mkdir(parents=True, exist_ok=False)
    graph = args.work_root / "winner_v20_joint_recurrent_update_002.onnx"
    ...
    snapshot = args.work_root / "winner_v20_joint_recurrent_update_002.npz"
    snapshot_receipt = full.save_snapshot(
        snapshot,
        parameters,
        optimizer,
        {
            "schema_version": "winner_v20.joint_recurrent_support_cpu_snapshot.v2",
    ...
    loaded = v20.load_joint_snapshot(
        snapshot,
        expected_schema_version="winner_v20.joint_recurrent_support_cpu_snapshot.v2",
    )
    recurrent = set(v20.RECURRENT_CORE_KEYS)
    update_1_gradients = optimization_reports[0]["gradient_max_abs"]
    update_1_deltas = optimization_reports[0]["leaf_max_abs_delta"]
    update_2_gradients = optimization_reports[1]["gradient_max_abs"]
    update_2_deltas = optimization_reports[1]["leaf_max_abs_delta"]
    checks = {
        "source_stage1_snapshot_exact": True,
        "exact_two_80_episode_populations": len(rollout_reports) == 2,
        "both_winner_v15_rollouts_bit_exact": all(
            row["winner_v15_transition_exact"] for row in rollout_reports
        ),
        "both_observation_captures_exact": all(
            row["observation_capture_exact"] for row in rollout_reports
        ),
        ...
        "both_reward_formulas_exact_and_nonzero": all(
            row["reward"]["reward_formula_bit_exact"]
            and row["reward"]["penalty_formula_bit_exact"]
            and row["reward"]["nonzero_penalty_count"] > 0
        ...
        "update_1_recurrent_gradients_exact_zero": all(
            update_1_gradients[name] == 0.0 for name in recurrent
        ),
        "update_1_recurrent_deltas_exact_zero": all(
            update_1_deltas[name] == 0.0 for name in recurrent
        ),
        "update_1_other_gradients_nonzero": all(
            value > 0.0
            for name, value in update_1_gradients.items()
            if name not in recurrent
        ),
        "update_1_other_deltas_nonzero": all(
            value > 0.0
            for name, value in update_1_deltas.items()
            if name not in recurrent
        ),
        "update_2_all_joint_gradients_nonzero": all(
            value > 0.0 for value in update_2_gradients.values()
        ),
        "update_2_all_joint_leaves_changed": all(
            value > 0.0 for value in update_2_deltas.values()
        ),
        "cumulative_all_joint_leaves_changed": all(
    ...
    result = {
        "schema_version": "winner_v20.joint_recurrent_support_cpu_result.v2",
        "status": (
