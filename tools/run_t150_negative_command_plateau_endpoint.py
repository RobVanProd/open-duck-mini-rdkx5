#!/usr/bin/env python3
"""Run T149B's frozen negative-COM command-plateau endpoint matrix."""
...
PREREG = (
    ANALYSIS / "t150_negative_command_plateau_endpoint_preregistration.json"
)
RESULT = ANALYSIS / "t150_negative_command_plateau_endpoint_result.json"
MARKDOWN = (
    ANALYSIS
    / "T150_NEGATIVE_COMMAND_PLATEAU_ENDPOINT_RESULT_20260729.md"
)
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t150_negative_command_plateau_endpoint_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T145 input: {path}")
...
def main() -> int:
    ...
    if (
        ...
        raise FileExistsError("refusing to overwrite T150 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T150 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T150 preregistration changed")
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t150_negative_command_plateau_endpoint_result.v1"
        ),
        "status": (
            "PASS_T150_NEGATIVE_COMMAND_PLATEAU_ENDPOINT"
            if passed
            else "HOLD_T150_NEGATIVE_COMMAND_PLATEAU_ENDPOINT"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T150 negative-command plateau endpoint result\n\n"
