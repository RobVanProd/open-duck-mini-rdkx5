#!/usr/bin/env python3
"""Freeze the Winner-v13 half/final support-controller gate."""
...
OUTPUT = ANALYSIS / "winner_v13_support_controller_gate_preregistration.json"
MARKDOWN = ANALYSIS / "WINNER_V13_SUPPORT_CONTROLLER_GATE_PREREGISTRATION_20260721.md"
...
SOURCES = {
    "builder": Path("tools/build_winner_v13_support_controller_gate_preregistration.py"),
    "gate_runner": Path("tools/run_winner_v13_support_controller_gate.py"),
    "gate_tests": Path("tests/test_winner_v13_support_controller_gate.py"),
    "preregistration_tests": Path("tests/test_winner_v13_support_controller_gate_preregistration.py"),
    "workflow": Path(".github/workflows/winner-v13-support-controller-gate.yml"),
    "training_result": Path("outputs/analysis/winner_v13_support_controller_training_result.json"),
    "training_preregistration": Path("outputs/analysis/winner_v13_support_controller_training_preregistration.json"),
    "training_runner": Path("tools/run_winner_v13_support_controller_training.py"),
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("Winner-v13 support training does not authorize a gate")
    ...
    if (
        ...
        raise ValueError("Winner-v13 support-training attribution changed")
    checkpoints = training.get("persistent_checkpoints", [])
    if [row.get("label") for row in checkpoints] != ["half", "final"]:
        raise ValueError("Winner-v13 support-training checkpoints changed")
    ...
    payload = {
        "schema_version": "winner_v13.support_controller_gate_preregistration.v1",
        "status": "PREREGISTERED_WINNER_V13_SUPPORT_CONTROLLER_GATE",
        "decision": "AUTHORIZE_ONE_FROZEN_248_CELL_GATE_ONLY",
        "binding_change_only": (
            "The reviewed Winner-v12 evaluator, populations, thresholds, seeds, "
            "and all-or-nothing half/final rule are unchanged. Only the independently "
            "verified Winner-v13 snapshot and graph paths are bound."
        ),
        "training_artifact": {
        ...
        "pass_rule": (
            ...
            "both half and final. No nearest checkpoint, reward selection, or "
    ...
    args.markdown.write_text(
        "\n".join(
            [
                "# Winner-v13 support-controller gate preregistration",
                "",
                ...
                "independently imported Winner-v13 half/final artifacts.",
                "",
            ]
        ),
        encoding="utf-8",
