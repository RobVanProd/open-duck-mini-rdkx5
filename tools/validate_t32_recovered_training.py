#!/usr/bin/env python3
"""Validate recovered T32 outputs on an explicit CPU topology."""
...
PREEXECUTION_ATTRIBUTION = (
    ANALYSIS / "t32_preexecution_identity_hold_attribution.json"
)
PACKAGE_CONTRACT = (
    ANALYSIS / "t32_action_margin_trainthrough_hosted_package_contract.json"
)
LAUNCH_CONTRACT = ANALYSIS / "t32b_colab_cli_recovery_contract.json"
OUTPUT = ANALYSIS / "t32_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T32_RECOVERED_TRAINING_VALIDATION_20260727.md"
...
PREEXECUTION_ATTRIBUTION_SHA256 = (
    "0d2b293b53405366c762a9437abd8746a2a53dbba063792064a6fa5130cbba15"
)
PACKAGE_CONTRACT_SHA256 = (
    "ba7733d4f4406bd5149dd4f6e8cac1343165c89906925297577ac25785c225c5"
)
LAUNCH_CONTRACT_SHA256 = (
    "d08f7f39cddbe3b7c9b7bb9bb6f28450ffa212e0a0480b76754c035bb8a99724"
)
SOURCE_CHECKPOINT_SHA256 = (
    "7a570f670813b0405615d4e253869402129183f7f18d14625fb56f74569db39b"
)
CPU_TEMPLATE_SHA256 = (
...
def main() -> int:
    ...
    if OUTPUT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T32 validation")
    recovery = args.recovery_root.resolve()
    work = (
        args.extracted_root.resolve()
        / "t32_action_margin_trainthrough_continuation"
    )
    training = work / "training"
    source = args.source_checkpoint.resolve()
    cpu_template = args.cpu_template.resolve()
    paths = {
        "hosted_result": recovery / "t32_result.json",
        "launch_receipt": recovery / "t32_launch_receipt.json",
        "recovery_archive": recovery / "t32_artifacts.tar.gz",
    }
    ...
    attribution = json.loads(
        PREEXECUTION_ATTRIBUTION.read_text(encoding="utf-8")
    )
    ...
    checks = {
        ...
        "t31_authority_exact": (
            sha256(T31_RESULT) == T31_RESULT_SHA256
            and t31.get("status")
            == "PASS_T31_ACTION_MARGIN_TRAINTHROUGH_CPU_SMOKE"
            and t31.get("decision")
            == "EARN_T31_HOSTED_CONTINUATION_PREREGISTRATION"
        ),
        "preexecution_identity_hold_exact_zero_weight": (
            sha256(PREEXECUTION_ATTRIBUTION)
            == PREEXECUTION_ATTRIBUTION_SHA256
        ...
        "hosted_artifact_passed_pending_cpu_validation": (
            hosted.get("status")
            == "PASS_T32_TRAINING_ARTIFACT_PENDING_CPU_VALIDATION"
        ...
        "launch_receipt_complete": (
            receipt.get("status") == "COMPLETED_T32_COLAB_LAUNCH"
    ...
    result = {
        "schema_version": "open_duck.t32_recovered_training_validation.v1",
        "status": (
            "PASS_T32_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T32_RECOVERED_TRAINING_VALIDATION"
        ),
        ...
        "input_hashes": {
            **input_hashes,
            ...
            "preexecution_attribution": sha256(PREEXECUTION_ATTRIBUTION),
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T32 recovered training validation",
                "",
