#!/usr/bin/env python3
"""Run the T113 always-on train-through frozen nominal matrix."""
...
PREREG = ANALYSIS / "t116_t113_nominal_preregistration.json"
RESULT = ANALYSIS / "t116_t113_nominal_result.json"
MARKDOWN = ANALYSIS / "T116_T113_NOMINAL_RESULT_20260729.md"
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T116 input: {path}")


def load() -> dict[str, Any]:
    value = json.loads(PREREG.read_text(encoding="utf-8"))
    if (
        value["status"] != "PREREGISTERED_T116_T113_NOMINAL_MATRIX"
        or value["failed_checks"]
        or canonical_sha256(value, "preregistered_contract_sha256")
        != value["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T116 preregistration changed")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T116 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T116 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t116_t113_nominal_result.v1",
        "status": (
            "PASS_T116_T113_NOMINAL_MATRIX"
            if passed
            else "HOLD_T116_T113_NOMINAL_MATRIX"
        ),
        "decision": (
