#!/usr/bin/env python3
"""Run T153's exact-positive-only linear-expert CPU contract."""
...
PREREG = ANALYSIS / "t153_positive_only_expert_cpu_preregistration.json"
RESULT = ANALYSIS / "t153_positive_only_expert_cpu_result.json"
MARKDOWN = ANALYSIS / "T153_POSITIVE_ONLY_EXPERT_CPU_RESULT_20260729.md"
WORK = Path(
    "D:/CodexArtifacts/open-duck-policy/t153_positive_only_expert_cpu_v1"
)
CPU_SOURCE = WORK / "t100c_half_cpu_remap"
OUTPUT = WORK / "smoke"
INSPECTION = WORK / "inspection"
EXPECTED = WORK / "t100c_half_expected_always_on.onnx"
EXPERT_SLOT = "negative_adapter_location"
...
def validate_prereg(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("T153 preregistration identity changed")
    for name, item in value["sources"].items():
        t20.verify_receipt(item, name)
    for name, item in value["assets"].items():
        t20.verify_receipt(item, name)
    for name, item in value["preserved_deployments"].items():
        path = Path(item["path"])
        if (
            not path.is_file()
            ...
            raise RuntimeError(f"T153 preserved graph changed: {name}")
    ...
    if (
        ...
        raise RuntimeError("T153 composed source inventory changed")
...
def mechanism_contract(playground: Path, population: int) -> dict[str, Any]:
    ...
    spec = importlib.util.spec_from_file_location("t153_mechanism", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot import T153 mechanism")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    offsets = np.asarray(module.positive_only_offsets(population))
    expected = np.tile(
    ...
    return {
        ...
        "training_slot": module.TRAINABLE_ACTOR_NAME,
    }


def preserved_deployments_exact(prereg: dict[str, Any]) -> bool:
    for item in prereg["preserved_deployments"].values():
        path = Path(item["path"])
        if (
            not path.is_file()
            or path.stat().st_size != item["bytes"]
...
def main() -> int:
    ...
    if not args.cpu_contract_authorized:
        raise PermissionError("T153 requires --cpu-contract-authorized")
    for path in (RESULT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T153 result: {path}")
    if WORK.exists():
        raise FileExistsError(f"refusing to reuse T153 work root: {WORK}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T153 execution requires a clean worktree")
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T153 CPU training failed rc={completed.returncode}\n"
    ...
    if sorted(checkpoints) != [0, 1024] or sorted(graphs) != [0, 1024]:
        raise RuntimeError("T153 export steps changed")
    ...
    t128_prereg = json.loads(
        Path(prereg["sources"]["t128_cpu_contract"]["path"])
        .with_name("t128_negative_only_expert_cpu_preregistration.json")
        .read_text(encoding="utf-8")
    )
    source_is_same = all(
        prereg["assets"][name] == t128_prereg["assets"][name]
        for name in (
            "source_checkpoint",
            "source_raw_onnx",
    ...
    checks = {
        "source_is_same_as_t128": source_is_same,
        "command_exact": (
        ...
        "exact_positive_model_contract": (
            ...
            and model_contract["training_slot"] == EXPERT_SLOT
        ),
        ...
        "only_positive_expert_slot_actor_changed": (
            bool(expert_deltas)
        ...
        "original_nominal_negative_graphs_unchanged": (
            preserved_deployments_exact(prereg)
        ),
    ...
    result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t153_positive_only_expert_cpu_result.v1"
        ),
        "status": (
            "PASS_T153_POSITIVE_ONLY_EXPERT_CPU_CONTRACT"
            if passed
            else "HOLD_T153_POSITIVE_ONLY_EXPERT_CPU_CONTRACT"
        ),
        "decision": (
        ...
        "tree_contract": {
            "positive_expert_slot_actor_leaf_deltas": expert_deltas,
        ...
        "preserved_deployments": prereg["preserved_deployments"],
        "checks": checks,
    ...
    MARKDOWN.write_text(
        "# T153 exact-positive-only expert CPU result\n\n"
        f"- Status: `{result['status']}`\n"
        f"- Decision: `{result['decision']}`\n"
        f"- Failed checks: `{failed}`\n"
        "- Physical population exact positive COM: "
