#!/usr/bin/env python3
"""Run the preregistered one-cell T28 margin-causality A/B."""
...
OUTPUT = ANALYSIS / "t30_t28_margin_causality_result.json"
OUTPUT_MD = ANALYSIS / "T30_T28_MARGIN_CAUSALITY_RESULT_20260726.md"
CACHE_ROOT = (
    Path(r"D:\CodexArtifacts\open-duck-policy")
    / "t30_t28_margin_causality_v1"
)
...
def verify(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T30 preregistration")
    if prereg["status"] != "PREREGISTERED_T30_T28_MARGIN_CAUSAL_AB":
        raise ValueError("T30 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T30 frozen input: {path}")
    for item in [
        prereg["unwrapped_policy"],
        prereg["fit"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T30 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T30 execution requires a clean worktree")
...
def main() -> int:
    ...
    policy = prereg["unwrapped_policy"]
    cell = prereg["cell"]
    ...
    if (
        ...
        raise RuntimeError(
            f"T30 worker failed: {completed.returncode}; {stdout_path}"
        )
    evaluation = json.loads(evaluation_path.read_text(encoding="utf-8"))
    if evaluation.get("formal") is not True or len(evaluation["runs"]) != 1:
        raise ValueError("T30 worker returned the wrong formal matrix")
    ...
    if not valid_result:
        status = "INVALID_T30_T28_MARGIN_CAUSAL_AB"
        decision = "ZERO_POLICY_DECISION_WEIGHT"
    elif source_green:
        status = "PASS_T30_T28_MARGIN_IS_CAUSAL"
        decision = "CLOSE_T28_ACTION_MARGIN_TRANSFORM"
    else:
        status = "PASS_T30_T28_MARGIN_NOT_SUFFICIENT_CAUSE"
        decision = "RETAIN_T28_CONTINUE_FAILURE_ATTRIBUTION"
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t30_t28_margin_causality_result.v1",
        "status": status,
        "decision": decision,
        ...
        "wrapped_cell": prereg["wrapped_failure"],
        "causal_comparison": {
            ...
            "unwrapped_cell_green": source_green,
        },
        "execution": {
            "cpu_only": True,
            "command": command,
        ...
        "authority": {
            "continue_failure_attribution": (
                valid_result and not source_green
            ),
            "close_t28": valid_result and source_green,
            "training": False,
            "colab": False,
            "gate5": False,
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# T30 T28 margin-causality result",
                "",
                f"status: `{status}`",
                "",
                f"decision: `{decision}`",
                "",
                (
                    f"- unwrapped samples: `{behavior['samples']}`; wrapped "
                    f"samples: `{prereg['wrapped_failure']['samples']}`"
                ),
                (
                    "- pre-intervention state/action equality: "
                    f"`{pre_intervention_exact}`"
                ),
                "",
                "No training, Gate 5, RDK-X5, robot, torque, or motion is "
                "authorized.",
                "",
            ]
        ),
    ...
    print(f"source_green={source_green}")
    print(f"source_samples={behavior['samples']}")
