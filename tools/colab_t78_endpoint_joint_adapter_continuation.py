#!/usr/bin/env python3
"""Run the one frozen T78 endpoint-stratified joint-adapter continuation."""
...
PREREGISTRATION_NAME = (
    "t78_endpoint_joint_adapter_hosted_preregistration.json"
)
CPU_RESULT_NAME = "t77_endpoint_joint_adapter_cpu_result.json"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    step_zero = bundle / "assets" / STEP_ZERO_NAME
    ...
    if (
        ...
        raise ValueError("T78 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T78 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T78 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(
            work,
            arcname="t78_endpoint_joint_adapter_continuation",
        )
...
def run(
    ...
    for path in (work, output_json, output_archive):
        if path.exists():
            raise FileExistsError(f"T78 no-retry path exists: {path}")
    ...
    payload: dict[str, Any] = {
        "schema_version": (
            "open_duck.t78_endpoint_joint_adapter_hosted_result.v1"
        ),
        "status": (
            "RUNNING_T78_ENDPOINT_JOINT_ADAPTER_HOSTED_CONTINUATION"
        ),
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"T78 runner returned {completed.returncode}")
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T78_ENDPOINT_JOINT_ADAPTER_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"T78 artifact checks failed: {failed}")
        ...
        payload["status"] = (
            "HOLD_T78_ENDPOINT_JOINT_ADAPTER_CONTINUATION"
        )
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T78 requires hosted GPU authorization")
    return run(
