#!/usr/bin/env python3
"""Freeze T210's 16-cell nominal/default behavior matrix."""
...
OUTPUT = ANALYSIS / "t213_t210_nominal_matrix_preregistration.json"
MARKDOWN = ANALYSIS / "T213_T210_NOMINAL_MATRIX_PREREGISTRATION_20260730.md"
BUILDER = Path(__file__).resolve()
RUNNER = ROOT / "tools" / "run_t213_t210_nominal_matrix.py"
TEST = ROOT / "tests" / "test_t213_t210_nominal_matrix.py"


def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T213: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T213 preregistration requires clean worktree")
    ...
    if failed:
        raise RuntimeError(f"T213 preregistration checks failed: {failed}")
    copied = {
        name: basis[name]
        for name in (
            "fits",
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t213_t210_nominal_matrix_preregistration.v1"
        ),
        "status": "PREREGISTERED_T213_T210_NOMINAL_MATRIX",
        **copied,
        "condition": condition,
        "policies": policies,
        "matrix": {
    ...
    MARKDOWN.write_text(
        "# T213 T210 nominal matrix preregistration\n\n"
