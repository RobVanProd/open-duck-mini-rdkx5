#!/usr/bin/env python3
"""Run the preregistered T10 response-conditioned V121 CPU contract."""
...
MARKDOWN = (
    ANALYSIS
    / "T10_RESPONSE_CONDITIONED_CONTINUATION_CPU_RESULT_20260726.md"
)
...
def validate_receipt(item: dict[str, Any], *, directory: bool) -> None:
    ...
    if observed != item["sha256"]:
        raise RuntimeError(f"T10 frozen input changed: {path}")
    if not directory and path.stat().st_size != item["bytes"]:
        raise RuntimeError(f"T10 frozen input size changed: {path}")
...
def validate_preregistration(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("T10 preregistration identity changed")
...
def run_training(
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"T10 CPU smoke failed rc={completed.returncode}; "
    ...
    if checkpoint_steps != [0, 1024] or onnx_steps != [0, 1024]:
        raise RuntimeError(
            f"T10 exports changed: {checkpoint_steps}, {onnx_steps}"
        )
    return {
        "command": command,
...
def graph_contract(path: Path, *, seed: int, cases: int) -> dict[str, Any]:
    ...
    hierarchy = {
        "context_hidden_path": producer.get(
            "context_hidden_projected"
        )
        == (
            "MatMul",
            ["calibration_context", "context_hidden_weight"],
        ),
        "context_action_path": producer.get("context_location")
        == (
            "MatMul",
...
def context_sensitivity(
    ...
    *,
    ...
    for _ in range(cases):
        ...
        observation[:, 6] = np.float32(0.077)
        observation[:, 101:115] = rng.uniform(
            -0.8,
            0.8,
            size=(1, 14),
        ...
        maximum_action_delta = max(
            ...
            float(np.max(np.abs(outputs[0][0] - outputs[1][0]))),
        )
        maximum_hidden_delta = max(
            maximum_hidden_delta,
            float(np.max(np.abs(outputs[0][1] - outputs[1][1]))),
        )
    return {
        "cases": cases,
        "maximum_action_delta": maximum_action_delta,
        "maximum_hidden_delta": maximum_hidden_delta,
        "context_changes_state": maximum_hidden_delta > 1.0e-8,
        "context_changes_action": maximum_action_delta > 1.0e-8,
    }
...
def main() -> int:
    ...
    if not args.offline_cpu_only or not args.cpu_smoke_authorized:
        raise PermissionError(
            "T10 requires --offline-cpu-only --cpu-smoke-authorized"
        )
    playground = args.playground_root.resolve()
    assets_root = args.assets_root.resolve()
    work = args.work_root.resolve()
    if work.exists():
        raise FileExistsError(f"refusing to reuse T10 work root: {work}")
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T10 formal result")
    ...
    if (
        ...
        raise RuntimeError("T10 execution roots differ from preregistration")
    ...
    context_deltas = {
        ...
        if "/context_hidden_projection/" in name
        or "/context_location/" in name
    }
    ...
    checks = {
        ...
        "trained_graph_uses_context_state": sensitivity[
            "context_changes_state"
        ],
        "trained_graph_uses_context_action": sensitivity[
    ...
    basis = {
        "schema_version": "open_duck.t10_response_cpu_result.v1",
        "status": (
    ...
    MARKDOWN.write_text(
        "# T10 response-conditioned V121 CPU contract\n\n"
