#!/usr/bin/env python3
"""Build the hash-frozen Winner-v119 hosted continuation bundle."""
...
CONTRACT = ANALYSIS / "winner_v119_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "WINNER_V119_HOSTED_PACKAGE_CONTRACT_20260724.md"
PREREGISTRATION_SHA256 = (
    "187062db82ef635b895b09e6354189f57f7d19067a9df16be9ed4e1d48ab3b2d"
)
BUNDLE_NAME = "winner_v119_transition_bundle"
MANIFEST_NAME = "winner_v119_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("Winner-v119 preregistration changed")
    observed = {
    ...
    if observed != prereg["input_hashes"]:
        raise ValueError("Winner-v119 package inputs changed")
    ...
    composed_manifest = json.loads(
        (
            bundle / "playground/WINNER_V119_COMPOSED_SOURCE_MANIFEST.json"
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_WINNER_V119_PACKAGE_IMPORT')"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_WINNER_V119_PACKAGE_IMPORT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            "Winner-v119 isolated import failed: "
            f"{preflight.stdout}\n{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "winner_v119.hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "winner_v119.hosted_package.v1",
        "status": "PASS_WINNER_V119_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
