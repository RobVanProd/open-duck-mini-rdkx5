#!/usr/bin/env python3
"""Preregister T113's exact frozen deployment transforms."""
...
VALIDATION = ANALYSIS / "t114_t113_recovered_training_validation.json"
ATTRIBUTION = ANALYSIS / "t114c_protobuf_presence_recovery_result.json"
T31_PREREGISTRATION = (
...
OUTPUT = ANALYSIS / "t115_t113_postexport_preregistration.json"
MARKDOWN = ANALYSIS / "T115_T113_POSTEXPORT_PREREGISTRATION_20260729.md"
RAW_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t113_colab_extracted_20260729/"
    "t78_endpoint_joint_adapter_continuation/training"
)
EXPECTED_STEPS = [0, 1_003_520, 2_007_040]
SOURCE_PATHS = (
    "tools/run_t115_t113_postexport_transform.py",
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T115 preregistration requires clean worktree")
    ...
    checks = {
        "t113_validation_single_metadata_hold_attributed": (
            validation["status"]
            == "HOLD_T114_T113_RECOVERED_TRAINING_VALIDATION"
            and validation["failed_checks"] == ["step_zero_raw_onnx_byte_exact"]
            and all(
                passed
        ...
        "exact_three_t113_exports": steps == EXPECTED_STEPS,
        ...
        "always_on_architecture_cpu_contract_green": (
            t112b["status"] == "PASS_T112B_READ_ONLY_CPU_RECOVERY"
            and t112b["failed_checks"] == []
            and t112b["checks"]["step_zero_trace_exact"]
            and t112b["checks"]["step_zero_random_chain_exact"]
        ),
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t115_t113_postexport_preregistration.v1",
        "status": (
            "PREREGISTERED_T115_T113_POSTEXPORT_TRANSFORM"
            if not failed
        ...
        "checks": checks,
        "input_hashes": {
            "t114_validation": sha256(VALIDATION),
            "t114c_attribution": sha256(ATTRIBUTION),
            "t31_preregistration": sha256(T31_PREREGISTRATION),
            "t31_result": sha256(T31_RESULT),
            "t112b_result": sha256(T112B_RESULT),
        },
        ...
        "decision": (
            "RUN_ONE_CPU_ONLY_T115_POSTEXPORT_TRANSFORM"
            if not failed
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T115 T113 post-export preregistration",
                "",
                f"- Status: `{value['status']}`",
                "- Three raw exports must match recovered T113 validation",
