#!/usr/bin/env python3
"""Run T50's frozen four-condition, 64-cell qualification."""
...
RESULT = ANALYSIS / "t50_uniform_half_core_qualification_result.json"
MARKDOWN = (
    ANALYSIS / "T50_UNIFORM_HALF_CORE_QUALIFICATION_RESULT_20260728.md"
)
DEFAULT_CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t50_uniform_half_core_qualification_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T50 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T50 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T50 matrix plan changed")
    return value


def write_markdown(result: Mapping[str, Any]) -> None:
    lines = [
        "# T50 uniform half-core qualification result",
        "",
        f"- Status: `{result['status']}`",
        f"- Decision: `{result['decision']}`",
        (
            "- Conditions: "
            f"`{result['summary']['completed_conditions']}/4`"
        ),
        (
            "- Green cells: "
    ...
    lines.extend(
        [
            "",
            "A pass authorizes only preregistration of R2 conditions 5-20. "
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T50 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T50 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": (
            "open_duck.t50_uniform_half_core_qualification_result.v1"
        ),
        "status": (
            "PASS_T50_UNIFORM_HALF_CORE_QUALIFICATION"
            if all_green
            else "HOLD_T50_UNIFORM_HALF_CORE_QUALIFICATION"
        ),
        "decision": (
        ...
        "summary": {
            ...
            "all_four_conditions_green": all_green,
