#!/usr/bin/env python3
"""Run T203's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t207_t203_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t207_t203_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T207_T203_TARGETED_Y_NEGATIVE_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t207_t203_targeted_y_negative_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T207 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T207 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T207 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T207 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t207_t203_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T207_T203_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T207_T203_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T207 T203 targeted Y-negative result\n\n"
