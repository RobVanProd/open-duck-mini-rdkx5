#!/usr/bin/env python3
"""Run T234B's frozen nominal/default persistence matrix."""
...
PREREG = ANALYSIS / "t235_exact_low_command_nominal_preregistration.json"
RESULT = ANALYSIS / "t235_exact_low_command_nominal_result.json"
MARKDOWN = ANALYSIS / "T235_EXACT_LOW_COMMAND_NOMINAL_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/"
    "t235_exact_low_command_nominal_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T235 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T235 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T235 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T235_EXACT_LOW_COMMAND_NOMINAL"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T235 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t235_exact_low_command_nominal_result.v1"
        ),
        "status": (
            "PASS_T235_EXACT_LOW_COMMAND_NOMINAL"
            if passed
            else "HOLD_T235_EXACT_LOW_COMMAND_NOMINAL"
        ),
        "decision": (
        ...
        "authority": {
            "upper_z_preregistration": passed,
            "training": False,
            "hosted": False,
            "gate5": False,
    ...
    MARKDOWN.write_text(
        "# T235 exact low-command nominal result\n\n"
