#!/usr/bin/env python3
"""V126 all-tick closed-loop eval with optional actuator bridge.
...
from exact_torque_oracle_all_tick import (
    ProjectionConstants as ExactTorqueProjectionConstants,
    project_action as project_exact_torque_action,
    supreme_action_bounds as exact_torque_action_bounds,
)
...
class ClosedLoopConfig:
    ...
    exact_torque_oracle_enabled: bool = False
    exact_torque_oracle_limit_nm: float = 1.91229675
    exact_torque_oracle_guard_rad: float = 0.165
    exact_torque_oracle_action_delta: tuple[float, ...] = ()
    exact_torque_oracle_force_tolerance_nm: float = 5.0e-6
    exact_torque_oracle_monotonicity_tolerance_nm: float = 5.0e-6
    exact_torque_oracle_grid_points: int = 9
    exact_torque_oracle_bisection_iterations: int = 32
    exact_torque_oracle_maximum_coordinate_passes: int = 14
...
def run_closed_loop_sim(config: ClosedLoopConfig) -> dict:
    ...
    if response_prefix_enabled:
        ...
        schedule = config.exact_torque_oracle_schedule_ticks
        if schedule is not None and (
            len(set(schedule)) != len(schedule)
            or any(int(tick) < 0 for tick in schedule)
        ):
            return {
                "status": "HOLD_EXACT_TORQUE_ORACLE_CONTRACT",
                "error": (
                    "exact torque oracle schedule ticks must be unique and "
                    "nonnegative"
                ),
            }
    ...
    if config.exact_torque_oracle_enabled:
        if (
            not config.policy_graph_authoritative_output
            or config.bridge_mode != "fitted"
            or config.oracle_phase_com_controller_json is not None
            or config.policy_action_rate_limit_rad_s is not None
            or config.response_calibrator_path is not None
            or config.eval_push_enable
            or config.winner_v3_additional_action_delay_ticks != 0
            or config.winner_v3_imu_delay_ticks != 0
            or config.winner_v3_native_quantization
            or config.winner_v3_sensor_noise_scales is not None
            or len(config.exact_torque_oracle_action_delta)
            != config.expected_action_dim
        ):
            return {
                "status": "HOLD_EXACT_TORQUE_ORACLE_CONTRACT",
                "error": (
                    "exact torque oracle requires the graph-authoritative, fitted, "
                    "unconditioned nominal CPU transport with a complete action-delta "
                    "vector and no host controller, push, delay, noise, or quantization"
                ),
            }
    if (
    ...
    for mode in available_modes(config.bridge_mode):
        ...
        exact_oracle_pending: dict[int, list[dict[str, Any]]] = {}
        exact_oracle_totals = {
            ...
            "projected_ticks": 0,
            "projected_joint_events": 0,
            "empty_intersection_events": 0,
            "prehistory_committed_violations": 0,
            ...
            "max_clip_linf": 0.0,
            "max_prediction_error_nm": 0.0,
        }
        exact_oracle_constants = ExactTorqueProjectionConstants(
            torque_limit_nm=float(config.exact_torque_oracle_limit_nm),
            force_tolerance_nm=float(
                config.exact_torque_oracle_force_tolerance_nm
            ),
            monotonicity_tolerance_nm=float(
                config.exact_torque_oracle_monotonicity_tolerance_nm
            ),
            monotonicity_grid_points=int(
                config.exact_torque_oracle_grid_points
            ),
            bisection_iterations=int(
                config.exact_torque_oracle_bisection_iterations
            ),
            maximum_coordinate_passes=int(
                config.exact_torque_oracle_maximum_coordinate_passes
            ),
        )
        ...
        for tick in range(sim_steps):
            ...
            exact_oracle_tick: dict[str, Any] | None = None
            if config.exact_torque_oracle_enabled:
                exact_oracle_totals["ticks"] += 1
                exact_oracle_schedule_active = (
                    config.exact_torque_oracle_schedule_ticks is None
                    or tick in config.exact_torque_oracle_schedule_ticks
                )
                if abs(float(config.command_x)) < 0.01:
                    if np.count_nonzero(action) != 0:
                        return {
                            "status": "HOLD_EXACT_TORQUE_ORACLE_CONTRACT",
                            "error": "x=0 graph output is not exact zero",
                            "tick": tick,
                        }
                    exact_oracle_tick = {
                        "status": "BYPASS_EXACT_X0_DEADBAND",
                        "base_action": action.astype(float).tolist(),
                        "final_action": action.astype(float).tolist(),
                        "predicted_force_nm": None,
                        "projected_joint_indices": [],
                        "empty_intersection_joint_indices": [],
                        "rollout_evaluations": 0,
                        "clip_linf": 0.0,
                    }
                elif not exact_oracle_schedule_active:
                    exact_oracle_totals["schedule_bypass_ticks"] += 1
                    exact_oracle_tick = {
                        "status": "BYPASS_EXACT_ORACLE_NOT_SCHEDULED",
                        "base_action": action.astype(float).tolist(),
                        "final_action": action.astype(float).tolist(),
                        "predicted_force_nm": None,
                        "projected_joint_indices": [],
                    ...
                    exact_oracle_totals["scheduled_ticks"] += 1
                    horizons = np.asarray(
                        [int(item.delay_ticks) for item in params],
                        dtype=np.int64,
                    )
                    maximum_horizon = int(np.max(horizons))
                    previous_policy_action = np.asarray(
                        policy_state_input["previous_action"][0],
                        dtype=np.float64,
                    )
                    low_action, high_action = exact_torque_action_bounds(
                        actual_position_rad=pre_actual_position,
                        home_position_rad=np.asarray(
                            env._default_actuator, dtype=np.float64
                        ),
                        previous_action=previous_policy_action,
                        action_scale_rad=float(env._config.action_scale),
                        action_delta=np.asarray(
                            config.exact_torque_oracle_action_delta,
                            dtype=np.float64,
                        ),
                        guard_joint_indices=WINNER_V3_PITCH_CHAIN_INDICES,
                        guard_rad=float(config.exact_torque_oracle_guard_rad),
                    )
                    ...
                    def exact_oracle_rollout(
                        ...
                        branch_bridge = copy.deepcopy(bridge)
                        branch_policy_observer_bridge = (
                            None
                            if policy_observer_bridge is None
                            else copy.deepcopy(policy_observer_bridge)
                        )
                        branch_hidden = {
                        ...
                        for offset in range(maximum_horizon + 1):
                            ...
                            branch_policy_applied_np = (
                                branch_applied_np.copy()
                                if branch_policy_observer_bridge is None
                                else branch_policy_observer_bridge.step(
                                    branch_sent_np, float(env.dt)
                                )
                            )
                            branch_state = apply_motor_target_runner(
                                ...
                                jp.asarray(branch_policy_applied_np),
                                branch_push,
                                branch_push_impulse,
                            )
                            force_rows.append(
                    ...
                    exact_oracle_totals["rollout_evaluations"] += int(
                        exact_oracle_tick["rollout_evaluations"]
                    )
                    exact_oracle_totals["max_clip_linf"] = max(
                        float(exact_oracle_totals["max_clip_linf"]),
                        float(exact_oracle_tick["clip_linf"]),
                    )
                    projected_joints = set(
                        exact_oracle_tick["projected_joint_indices"]
                    )
                    empty_joints = set(
                        exact_oracle_tick[
                            "empty_intersection_joint_indices"
                    ...
                    if projected_joints:
                        exact_oracle_totals["projected_ticks"] += 1
                        exact_oracle_totals["projected_joint_events"] += len(
                            projected_joints
                        )
                    exact_oracle_totals[
                        "empty_intersection_events"
                    ] += len(empty_joints)
                    for joint, horizon in enumerate(horizons):
                        exact_oracle_pending.setdefault(
                            tick + int(horizon), []
                        ).append(
                            {
                                "source_tick": tick,
                                "joint_index": joint,
                                "predicted_force_nm": float(
                                    exact_oracle_tick[
                                        "predicted_force_nm"
                                    ][joint]
                                ),
                                "empty_intersection": joint
                                in empty_joints,
                            }
                        )
                    if "previous_action" in hidden_state:
                        hidden_state["previous_action"] = action[
                            None, :
                        ].astype(np.float32)
            ...
            exact_oracle_matured: list[dict[str, Any]] = []
            if config.exact_torque_oracle_enabled:
                exact_oracle_matured = exact_oracle_pending.pop(tick, [])
                exact_oracle_totals["matured_predictions"] += len(
                    exact_oracle_matured
                )
                matured_by_joint = {
                    int(item["joint_index"]): item
                    for item in exact_oracle_matured
                }
                for item in exact_oracle_matured:
                    joint = int(item["joint_index"])
                    item["actual_force_nm"] = float(actuator_force[joint])
                    item["prediction_error_nm"] = abs(
                        float(actuator_force[joint])
                        - float(item["predicted_force_nm"])
                    )
                    exact_oracle_totals["max_prediction_error_nm"] = max(
                        float(
                            exact_oracle_totals[
                                "max_prediction_error_nm"
                            ]
                        ),
                        float(item["prediction_error_nm"]),
                    )
                    item["prediction_exact"] = bool(
                        float(item["prediction_error_nm"])
                        <= float(
                            config.exact_torque_oracle_force_tolerance_nm
                        )
                    )
                    if not item["prediction_exact"]:
                        exact_oracle_totals["prediction_mismatches"] += 1
                for joint, force_nm in enumerate(actuator_force):
                    if (
                        abs(float(force_nm))
                        <= float(config.exact_torque_oracle_limit_nm)
                        + float(
                            config.exact_torque_oracle_force_tolerance_nm
                        )
                    ):
                        continue
                    provenance = matured_by_joint.get(joint)
                    if provenance is None:
                        if tick < int(params[joint].delay_ticks):
                            exact_oracle_totals[
                                "prehistory_committed_violations"
                            ] += 1
                        else:
                            exact_oracle_totals[
                                "unscheduled_residual_violations"
                            ] += 1
                    elif not bool(provenance["empty_intersection"]):
                        exact_oracle_totals[
                            "nonempty_residual_violations"
                        ] += 1
            ...
            record = {
                ...
                "exact_torque_oracle": exact_oracle_tick,
        ...
        modes[mode] = {
            ...
            "exact_torque_oracle": {
                **exact_oracle_totals,
                "pending_predictions_after_episode": sum(
                    len(items) for items in exact_oracle_pending.values()
                ),
                "prediction_exact": (
                    int(exact_oracle_totals["prediction_mismatches"]) == 0
                ),
                "only_empty_or_prehistory_residuals": (
                    int(
                        exact_oracle_totals[
                            "nonempty_residual_violations"
                        ]
    ...
    result = {
        ...
        "exact_torque_oracle": {
            ...
            "torque_limit_nm": float(
                config.exact_torque_oracle_limit_nm
            ),
            "guard_rad": float(config.exact_torque_oracle_guard_rad),
            "action_delta": list(
                config.exact_torque_oracle_action_delta
            ...
            "schedule_ticks": (
                None
                if config.exact_torque_oracle_schedule_ticks is None
                else list(config.exact_torque_oracle_schedule_ticks)
            ),
        },
