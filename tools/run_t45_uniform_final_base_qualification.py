#!/usr/bin/env python3
"""Run T45's frozen three-condition, 48-cell qualification."""
...
PREREG = (
    ANALYSIS / "t45_uniform_final_base_qualification_preregistration.json"
)
RESULT = ANALYSIS / "t45_uniform_final_base_qualification_result.json"
MARKDOWN = (
    ANALYSIS / "T45_UNIFORM_FINAL_BASE_QUALIFICATION_RESULT_20260728.md"
)
DEFAULT_CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t45_uniform_final_base_qualification_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T45 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T45 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T45 matrix plan changed")
    return value


def write_markdown(result: Mapping[str, Any]) -> None:
    lines = [
        "# T45 uniform final-base qualification result",
        "",
        f"- Status: `{result['status']}`",
        f"- Decision: `{result['decision']}`",
        (
            "- Conditions: "
            f"`{result['summary']['completed_conditions']}/3`"
        ),
        (
            "- Green cells: "
    ...
    lines.extend(
        [
            "",
            "A pass authorizes only preregistration of R2 conditions 4-20. "
            "Gate 5 hardware and robot access remain closed.",
            "",
        ]
    )
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T45 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T45 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": (
            "open_duck.t45_uniform_final_base_qualification_result.v1"
        ),
        "status": (
            "PASS_T45_UNIFORM_FINAL_BASE_QUALIFICATION"
            if all_green
            else "HOLD_T45_UNIFORM_FINAL_BASE_QUALIFICATION"
        ),
        "decision": (
        ...
        "summary": {
            ...
            "all_three_conditions_green": all_green,
        ...
        "authority": {
            "later_robustness_preregistration": all_green,
