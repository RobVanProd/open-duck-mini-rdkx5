#!/usr/bin/env python3
"""Run T186's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t190_t186_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t190_t186_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T190_T186_TARGETED_Y_NEGATIVE_RESULT_20260730.md"
CACHE = Path(
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T190 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T190 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T190 execution requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T190 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t190_t186_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T190_T186_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T190_T186_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T190 T186 targeted Y-negative result\n\n"
