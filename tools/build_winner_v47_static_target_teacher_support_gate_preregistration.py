#!/usr/bin/env python3
"""Bind the unchanged reviewed support gate to Winner-v46 half/final."""
...
OUTPUT = ANALYSIS / "winner_v47_static_target_teacher_support_gate_preregistration.json"
MARKDOWN = ANALYSIS / "WINNER_V47_STATIC_TARGET_TEACHER_SUPPORT_GATE_PREREGISTRATION_20260722.md"
...
SOURCES = {
    "builder": Path("tools/build_winner_v47_static_target_teacher_support_gate_preregistration.py"),
    "gate_runner": Path("tools/run_winner_v47_static_target_teacher_support_gate.py"),
    "gate_tests": Path("tests/test_winner_v47_static_target_teacher_support_gate.py"),
    "preregistration_tests": Path("tests/test_winner_v47_static_target_teacher_support_gate_preregistration.py"),
    "result_importer": Path("tools/import_winner_v47_static_target_teacher_support_gate_result.py"),
    "result_importer_tests": Path("tests/test_winner_v47_static_target_teacher_support_gate_import.py"),
    "reviewed_result_importer": Path("tools/import_winner_v15_pitch_margin_support_gate_result.py"),
    "workflow": Path(".github/workflows/winner-v47-static-target-teacher-support-gate.yml"),
    "training_result": Path("outputs/analysis/winner_v46_static_target_teacher_training_result.json"),
    "training_preregistration": Path("outputs/analysis/winner_v46_static_target_teacher_training_preregistration.json"),
    "training_runner": Path("tools/run_winner_v46_static_target_teacher_training.py"),
    "training_importer": Path("tools/import_winner_v46_static_target_teacher_training.py"),
    "teacher_mechanics": Path("patches/winner_v43_static_target_teacher.py"),
    ...
    "baseline_mechanics": Path("patches/winner_v24_symmetric_support_failure_v2.py"),
    "explicit_gradient": Path("patches/winner_v22_normalized_predictor_v2.py"),
...
def _checkpoint(training: dict[str, Any], label: str, update: int) -> dict[str, Any]:
    snapshot = next(row for row in training["snapshot_manifest"] if row["completed_updates"] == update)
    persistent = next(
...
def main() -> int:
    ...
    if args.output.exists() or args.markdown.exists():
        raise FileExistsError("refusing to overwrite Winner-v47 contract")
    training = json.loads(TRAINING_RESULT.read_text(encoding="utf-8"))
    if (
        training.get("status") != "PASS_WINNER_V46_STATIC_TARGET_TEACHER_TRAINING_ARTIFACT"
        or training.get("decision") != "AUTHORIZE_STATIC_TARGET_TEACHER_SUPPORT_GATE_PREREGISTRATION_ONLY"
        ...
        raise ValueError("Winner-v46 does not authorize Winner-v47")
    ...
    if (
        ...
        raise ValueError("Winner-v46 attribution changed")
    if [row.get("label") for row in training.get("persistent_checkpoints", [])] != ["half", "final"]:
        raise ValueError("Winner-v46 checkpoint labels changed")
    if [row.get("completed_updates") for row in training["persistent_checkpoints"]] != [302, 352]:
        raise ValueError("Winner-v46 checkpoint updates changed")
    ...
    payload = {
        "schema_version": "winner_v47.static_target_teacher_support_gate_preregistration.v1",
        "status": "PREREGISTERED_WINNER_V47_STATIC_TARGET_TEACHER_SUPPORT_GATE",
        "decision": "AUTHORIZE_ONE_FROZEN_WINNER_V47_248_CELL_GATE_ONLY",
        "binding_change_only": (
            "The reviewed Winner-v12 physical evaluator, populations, thresholds, seeds, "
            "constant baseline, and all-or-nothing half/final rule are unchanged. The verified "
            "Winner-v46 count-302/count-352 paths are bound read-only and the normalized "
            "auxiliary head is projected exactly into raw scoring coordinates."
        ),
        "training_artifact": {
            ...
            "half": _checkpoint(training, "half", 302),
            "final": _checkpoint(training, "final", 352),
        },
    ...
    args.markdown.write_text("\n".join([
        "# Winner-v47 static-target teacher support-gate preregistration", "",
        ...
        "- Checkpoints: exact Winner-v46 count `302 / 352`",
        "- Selection: every check at both checkpoints; no closest result",
        "- Locomotion / robot: `0 / 0`", "",
        "This binds only frozen artifacts to the unchanged reviewed evaluator.",
        "It is not checkpoint selection or robot clearance.", "",
    ]), encoding="utf-8")
