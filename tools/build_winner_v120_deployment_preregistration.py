#!/usr/bin/env python3
"""Preregister the exact V119 train-matched deployment transform."""
...
TRANSFORM = ROOT / "tools/build_winner_v120_deployment_policies.py"
OUTPUT = ANALYSIS / "winner_v120_deployment_transform_preregistration.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V120_DEPLOYMENT_TRANSFORM_PREREGISTRATION_20260724.md"
)
EXPECTED = {
    "v119_training_validation": (
        "456121dc7680df6e66915fa67e59f088f718e08db1468d3523795f8dd56abb14"
    ),
    "v117_transform_contract": (
    ...
    "transform_tool": (
        "571c1d0cca36a2a0a4bb185baa5f5a77cb57818ebb72438714e94bb502737b33"
    ),
}
...
def main() -> int:
    ...
    v117 = json.loads(V117_CONTRACT.read_text(encoding="utf-8"))
    ...
    hashes = {
        ...
        "v117_transform_contract": sha256(V117_CONTRACT),
    ...
    for step in (1_003_520, 2_007_040):
        ...
        sources.append(
            {
                "id": (
                    "V120_TRAIN_MATCHED_HALF"
                    if step == 1_003_520
                    else "V120_TRAIN_MATCHED_FINAL"
                ),
                "step": step,
                "filename": path.name,
                "sha256": sha256(path),
                "bytes": path.stat().st_size,
                "output_filename": f"V120_DEPLOYMENT_{step}.onnx",
            }
        )
    projection = v117["projection"]
    selected_delta = projection["selected_normalized_action_delta"]
    current_delta = projection["current_normalized_action_delta"]
    affected_indices = [
        index
        for index, (before, after) in enumerate(
            zip(current_delta, selected_delta, strict=True)
        )
        if before != after
    ]
    selected_guard = next(
    ...
    transform = {
        "order": [
            ...
            "x=0 deadband",
            "V117 final previous-action-centered rate projection",
            "restored x=0 deadband and final-action state feedback",
        ],
        "observation_dim": 115,
        "action_dim": 14,
        "home_target_rad": guard["guard_contract"]["home_target_rad"],
        "measured_joint_offset_indices": guard["guard_contract"][
            "measured_joint_offset_indices"
        ],
        "pitch_chain_action_indices": guard["guard_contract"][
            "pitch_chain_action_indices"
        ],
        "action_scale_rad": guard["guard_contract"]["action_scale_rad"],
        ...
        "affected_action_indices": affected_indices,
        "selected_normalized_action_delta": selected_delta,
        "selected_rate_limit_rad_s": projection[
    ...
    checks = {
        ...
        "v117_transform_contract_exact": (
            v117.get("status")
            == "PASS_WINNER_V117_POSTGUARD_RATE_PROJECTION_CONTRACT"
            and v117.get("failed_checks") == []
        ),
        ...
        "exact_frozen_hierarchy_only": len(transform["order"]) == 5,
    ...
    value = {
        "schema_version": (
            "winner_v120.deployment_transform_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_WINNER_V120_DEPLOYMENT_TRANSFORM"
            if not failed
            else "HOLD_WINNER_V120_DEPLOYMENT_TRANSFORM_PREREGISTRATION"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner-v120 deployment transform preregistration\n\n"
        f"Status: `{value['status']}`\n\n"
        "The exact V119 half and final actor graphs receive one unchanged "
        "deployment hierarchy matching their train-time transition. No "
        "training, behavior, Gate 5, RDK-X5, robot, torque, or motion work "
        "occurs here.\n",
        encoding="utf-8",
    )
    print(value["status"])
