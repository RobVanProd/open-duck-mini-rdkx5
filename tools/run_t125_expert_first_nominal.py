#!/usr/bin/env python3
"""Run the frozen expert-first nominal mechanism screen."""
...
PREREG = ANALYSIS / "t125_expert_first_nominal_preregistration.json"
RESULT = ANALYSIS / "t125_expert_first_nominal_result.json"
MARKDOWN = ANALYSIS / "T125_EXPERT_FIRST_NOMINAL_RESULT_20260729.md"
CACHE = Path("D:/CodexArtifacts/open-duck-policy/t125_expert_first_nominal_v1")
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T125 input: {path}")
...
def load() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T125 preregistration changed")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T125 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T125 execution requires clean worktree")
    prereg = load()
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t125_expert_first_nominal_result.v1",
        "status": (
            "PASS_T125_EXPERT_FIRST_NOMINAL_SCREEN"
            if passed
            else "HOLD_T125_EXPERT_FIRST_NOMINAL_SCREEN"
        ),
        "decision": (
        ...
        "authority": {
            "t126_negative_endpoint_preregistration": passed,
            "training": False,
            "colab": False,
            "gate5": False,
    ...
    MARKDOWN.write_text(
        "# T125 expert-first nominal result\n\n"
