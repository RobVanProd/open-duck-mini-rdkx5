#!/usr/bin/env python3
"""Build the hash-frozen T113 always-on train-through hosted bundle."""
...
CONTRACT = ANALYSIS / "t113_always_on_trainthrough_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T113_ALWAYS_ON_TRAINTHROUGH_HOSTED_PACKAGE_20260729.md"
BUNDLE_NAME = "t113_always_on_trainthrough_bundle"
MANIFEST_NAME = "t113_package_manifest.json"
...
def main() -> int:
    ...
    for path in (staging, archive_path, CONTRACT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T113 package: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T113 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T113 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T113 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T113 playground changed")
    if (
        ...
        raise RuntimeError("copied T113 assets changed")
        ...
        [
            ...
            "\n".join(
                (
                    ...
                    "    print('PASS_T113_FULL_IDENTITY_PREFLIGHT')",
                    "else:",
    ...
    if (
        ...
        or "PASS_T113_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T113 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t113_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t113_hosted_package.v1",
        "status": "PASS_T113_ALWAYS_ON_TRAINTHROUGH_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T113 always-on train-through hosted package",
                "",
