#!/usr/bin/env python3
"""Run T203's frozen nominal/default persistence matrix."""
...
PREREG = ANALYSIS / "t206_t203_nominal_matrix_preregistration.json"
RESULT = ANALYSIS / "t206_t203_nominal_matrix_result.json"
MARKDOWN = ANALYSIS / "T206_T203_NOMINAL_MATRIX_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t206_t203_nominal_matrix_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T206 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T206 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T206 execution requires clean worktree")
    ...
    if (
        prereg["status"] != "PREREGISTERED_T206_T203_NOMINAL_MATRIX"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T206 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": "open_duck.t206_t203_nominal_matrix_result.v1",
        "status": (
            "PASS_T206_T203_NOMINAL_MATRIX"
            if passed
            else "HOLD_T206_T203_NOMINAL_MATRIX"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T206 T203 nominal matrix result\n\n"
