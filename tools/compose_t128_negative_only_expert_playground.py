#!/usr/bin/env python3
"""Compose T128 from the frozen T112 always-on-expert Playground."""
...
def replace_exact(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"T128 {label} replacement count changed: {count}")
...
"""
    new = """                if args.winner_t98_hidden_expert_continuation:
                    from playground.common.t98_hidden_expert_continuation import (
                        make_negative_only_randomizer,
                    )

                    self.randomizer = make_negative_only_randomizer(
                        self.randomizer,
                        torso_body_id=torso_body_id,
                    )
                    print(
...
"""
    return replace_exact(text, old, new, "negative-only randomizer")
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to reuse T128 Playground: {output}")
    ...
    value = {
        "schema_version": "open_duck.t128_composed_playground.v1",
    ...
    manifest = output / "T128_COMPOSED_SOURCE_MANIFEST.json"
    manifest.write_text(
