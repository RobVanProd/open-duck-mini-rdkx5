#!/usr/bin/env python3
"""Build T210's hash-frozen hosted continuation bundle."""
...
CONTRACT = ANALYSIS / "t210_dual_roll_cost_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T210_DUAL_ROLL_COST_HOSTED_PACKAGE_20260730.md"
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T210 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T210 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T210 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T210 playground changed")
    if (
        ...
        raise RuntimeError("copied T210 assets changed")
    ...
    preflight_source = "\n".join(
        (
            ...
            "    print('PASS_T210_FULL_IDENTITY_PREFLIGHT')",
            "else:",
    ...
    if (
        ...
        or "PASS_T210_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T210 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t210_hosted_bundle_manifest.v1",
        "preregistration_sha256": sha256(PREREG),
        "cpu_result_sha256": sha256(CPU_RESULT),
        "t204_validation_sha256": sha256(T204),
        "t208_selection_sha256": sha256(T208),
    ...
    contract = {
        "schema_version": "open_duck.t210_hosted_package.v1",
        "status": "PASS_T210_DUAL_ROLL_COST_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            ...
            "t209_and_t98_readbacks_frozen": True,
    ...
    MARKDOWN.write_text(
        "# T210 dual roll-cost hosted package\n\n"
