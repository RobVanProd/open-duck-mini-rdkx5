#!/usr/bin/env python3
"""Run T46's frozen sequential R2 conditions 4-20."""
...
MARKDOWN = (
    ANALYSIS / "T46_UNIFORM_FINAL_BASE_R2_REMAINDER_RESULT_20260728.md"
)
DEFAULT_CACHE_ROOT = (
    Path("D:/CodexArtifacts/open-duck-policy")
    / "t46_uniform_final_base_r2_remainder_v1"
)
...
def verify_receipt(value: Mapping[str, Any], label: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"T46 frozen receipt changed: {label}={path}")
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("T46 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T46 matrix plan changed")
    return value


def write_markdown(result: Mapping[str, Any]) -> None:
    lines = [
        "# T46 uniform final-base R2 remainder result",
        "",
        f"- Status: `{result['status']}`",
        f"- Decision: `{result['decision']}`",
        (
            "- Conditions: "
            f"`{result['summary']['completed_conditions']}/17`"
        ),
        (
            "- Green cells: "
    ...
    lines.extend(
        [
            ...
            "",
            "A pass authorizes only Gate 5 package preregistration. "
            "Hardware, RDK-X5, robot access, torque, motion, and deployment "
            "remain closed.",
            "",
        ]
    )
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T46 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T46 formal execution requires a clean worktree")
    ...
    result = {
        "schema_version": (
            "open_duck.t46_uniform_final_base_r2_remainder_result.v1"
        ),
        "status": (
            "PASS_T46_UNIFORM_FINAL_BASE_R2_REMAINDER"
            if all_green
            else "HOLD_T46_UNIFORM_FINAL_BASE_R2_REMAINDER"
        ),
        "decision": (
        ...
        "summary": {
            ...
            "all_seventeen_conditions_green": all_green,
