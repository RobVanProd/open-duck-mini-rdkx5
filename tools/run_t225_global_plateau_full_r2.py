#!/usr/bin/env python3
"""Run T222B's sequential full R2 matrix one condition per invocation."""
...
PREREG = ANALYSIS / "t225_global_plateau_full_r2_preregistration.json"
RESULT = ANALYSIS / "t225_global_plateau_full_r2_result.json"
MARKDOWN = ANALYSIS / "T225_GLOBAL_PLATEAU_FULL_R2_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t225_global_plateau_full_r2_v1"
)
...
def load_preregistration() -> dict[str, Any]:
    ...
    if (
        value.get("status")
        != "PREREGISTERED_T225_GLOBAL_PLATEAU_FULL_R2"
        or value.get("failed_checks")
        or canonical_sha256(basis)
        != value.get("preregistered_contract_sha256")
    ):
        raise RuntimeError("T225 preregistration changed")
    ...
    if (
        ...
        raise RuntimeError("T225 matrix plan changed")
    return value
...
def write_progress(
    ...
    *,
    ...
    value = {
        "schema_version": "open_duck.t225_global_plateau_full_r2_progress.v1",
        "status": "IN_PROGRESS_T225_GLOBAL_PLATEAU_FULL_R2",
...
def write_markdown(value: dict[str, Any]) -> None:
    lines = [
        "# T225 global plateau full R2 result",
        "",
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists():
        raise FileExistsError("refusing to overwrite T225 result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T225 execution requires clean worktree")
    ...
    if paused and first_failed is None:
        ...
        print("IN_PROGRESS_T225_GLOBAL_PLATEAU_FULL_R2")
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t225_global_plateau_full_r2_result.v1",
        "status": (
            "PASS_T225_GLOBAL_PLATEAU_FULL_R2"
            if all_green
            else "HOLD_T225_GLOBAL_PLATEAU_FULL_R2"
        ),
        "decision": (
