#!/usr/bin/env python3
"""Preregister the frozen 16-cell V117 nominal physical gate."""
...
OUTPUT = (
    ANALYSIS / "winner_v117_nominal_behavior_preregistration.json"
)
MARKDOWN = (
    ANALYSIS
    / "WINNER_V117_NOMINAL_BEHAVIOR_PREREGISTRATION_20260724.md"
)
EXPECTED = {
...
def main() -> int:
    ...
    hashes = {
        "v117_transform_contract": sha256(TRANSFORM),
        "v115_nominal_result": sha256(V115_RESULT),
    ...
    checks = {
        ...
        "v115_persistence_failure_exact": (
            v115_result.get("status")
            == "PASS_WINNER_V115_NOMINAL_BEHAVIOR_VALID_RESULT"
            and v115_result.get("decision", {}).get("status")
            == "REJECT_V115_NOMINAL_POLICY"
            and v115_result.get("decision", {}).get(
    ...
    payload = {
        "schema_version": (
            "winner_v117.nominal_behavior_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_WINNER_V117_NOMINAL_BEHAVIOR"
            if not failed
            else "HOLD_WINNER_V117_NOMINAL_BEHAVIOR_PREREGISTRATION"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner-v117 nominal behavior preregistration\n\n"
        f"Status: `{payload['status']}`\n\n"
        f"Matrix SHA-256: `{matrix_hash}`\n\n"
        "The unchanged frozen 16-cell gate covers both checkpoints, both "
        "measured actuator plants, and x=0/.074/.077/.080 for 600 ticks. "
        "Both checkpoints must pass all behavior, current, and torque checks. "
        "No training, full matrix, Gate 5, RDK, or robot access is granted.\n",
        encoding="utf-8",
    )
