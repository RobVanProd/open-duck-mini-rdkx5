#!/usr/bin/env python3
"""Safely import one Winner-v21 predictor-preserving support-gate result."""
...
PREREGISTRATION = ANALYSIS / "winner_v21_predictor_preserving_support_gate_preregistration.json"
TRAINING_RESULT = ANALYSIS / "winner_v21_predictor_preserving_training_result.json"
TRAINING_PREREGISTRATION = ANALYSIS / "winner_v21_predictor_preserving_training_preregistration.json"
TRAINING_RUNNER = ROOT / "tools/run_winner_v21_predictor_preserving_training.py"
BASE_GATE_RUNNER = ROOT / "tools/run_winner_v12_calibrator_support_gate.py"
RUNNER = ROOT / "tools/run_winner_v21_predictor_preserving_support_gate.py"
WORKFLOW = ROOT / ".github/workflows/winner-v21-predictor-preserving-support-gate.yml"
V15_IMPORTER = ROOT / "tools/import_winner_v15_pitch_margin_support_gate_result.py"
OUTPUT_JSON = ANALYSIS / "winner_v21_predictor_preserving_support_gate_result.json"
OUTPUT_MD = ANALYSIS / "WINNER_V21_PREDICTOR_PRESERVING_SUPPORT_GATE_RESULT_20260721.md"
RAW_RESULT_NAME = "winner-v21-predictor-preserving-support-gate-result.json"
RAW_RECEIPT_NAME = "winner-v21-predictor-preserving-support-gate-result.sha256"
...
def read_result_artifact(path: Path) -> tuple[bytes, bytes]:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if len(names) != len(set(names)) or set(names) != expected:
            raise ValueError("Winner-v21 support-gate artifact inventory changed")
        if sum(item.file_size for item in infos) > 100_000_000:
            raise ValueError("Winner-v21 support-gate artifact exceeds size ceiling")
        for info in infos:
            ...
            if (
                ...
                raise ValueError("Winner-v21 support-gate artifact has unsafe member")
...
def repository_attribution(
    *,
    ...
    require_sha256(artifact_zip_sha256, "Winner-v21 support-gate artifact")
    if (
        run_id <= 0
        or run_attempt != 1
        or HEX40_RE.fullmatch(run_head_sha) is None
        or artifact_id <= 0
        or artifact_name != f"winner-v21-predictor-preserving-support-gate-{run_id}"
        or artifact_digest != f"sha256:{artifact_zip_sha256}"
    ):
        raise ValueError("Winner-v21 support-gate workflow attribution changed")
    return {
...
def checkpoint_identities() -> dict[str, dict[str, str]]:
    ...
    if (
        ...
        raise ValueError("Winner-v21 training source is not verified")
    ...
    if (
        ...
        raise ValueError("Winner-v21 checkpoint set changed")
    ...
    for label, update, checkpoint in zip(
        ...
        require_sha256(snapshot_sha, f"Winner-v21 {label} snapshot")
        require_sha256(graph_sha, f"Winner-v21 {label} graph")
        identities[label] = {
...
def validate_result(result: Mapping[str, Any]) -> None:
    ...
    if frozenset(result) not in {
        ...
        "winner_v21.predictor_preserving_support_gate_result.v1"
    ):
        raise ValueError("Winner-v21 formal gate result schema changed")
    if "repository_attribution" in result:
        ...
        if (
            ...
            != f"winner-v21-predictor-preserving-support-gate-{attribution['github_run_id']}"
            ...
            raise ValueError("Winner-v21 imported repository attribution changed")
    ...
    if (
        ...
        raise ValueError("Winner-v21 formal gate checks changed")
    if result.get("execution") != {
        ...
        raise ValueError("Winner-v21 formal gate execution or authority changed")
    if result.get("sources") != {
        ...
        raise ValueError("Winner-v21 formal gate sources changed")
    rows = result.get("checkpoint_results")
    if not isinstance(rows, list) or [row.get("label") for row in rows] != [
        "half",
        "final",
    ]:
        raise ValueError("Winner-v21 formal gate checkpoint set changed")
    ...
    if checks != derived:
        raise ValueError("Winner-v21 formal gate summary is not rederived")
    ...
    if (
        ...
        != (
            "PASS_WINNER_V21_PREDICTOR_PRESERVING_SUPPORT_GATE"
            if passed
            else "HOLD_WINNER_V21_PREDICTOR_PRESERVING_SUPPORT_GATE"
        )
        ...
        raise ValueError("Winner-v21 formal gate decision changed")
...
def main() -> int:
    ...
    if OUTPUT_JSON.exists() or OUTPUT_MD.exists():
        raise FileExistsError("Winner-v21 support-gate result is already imported")
    ...
    if receipt_bytes != f"{raw_sha}  /tmp/{RAW_RESULT_NAME}\n".encode():
        raise ValueError("Winner-v21 support-gate raw-result receipt changed")
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Winner-v21 predictor-preserving formal support-gate result",
                "",
                ...
                "",
                "A pass requires every support cell plus the existing predictor-versus-constant",
                "and context checks at both checkpoints. It is not robot clearance.",
                "",
            ]
        ),
        encoding="utf-8",
