#!/usr/bin/env python3
"""Build the hash-frozen T100B pre-execution replacement bundle."""
...
PREREG = ANALYSIS / "t100b_preexecution_recovery_preregistration.json"
...
CONTRACT = ANALYSIS / "t100b_hidden_expert_hosted_package_contract.json"
MARKDOWN = ANALYSIS / "T100B_HIDDEN_EXPERT_HOSTED_PACKAGE_20260728.md"
BUNDLE_NAME = "t100b_hidden_expert_bundle"
MANIFEST_NAME = "t100b_package_manifest.json"
...
def main() -> int:
    ...
    for path in (staging, archive_path, CONTRACT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T100B package: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T100B packaging requires clean worktree")
    ...
    if (
        prereg["status"] != "PREREGISTERED_T100B_PREEXECUTION_RECOVERY"
        or prereg["failed_checks"]
        or canonical_sha256(basis) != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T100B preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T100B package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T100B playground changed")
    if (
        ...
        raise RuntimeError("copied T100B assets changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T100B_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T100B_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T100B isolated import failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t100b_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t100b_hosted_package.v1",
        "status": "PASS_T100B_HIDDEN_EXPERT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            "preexecution_attribution_exact": True,
            "recovery_preregistration_exact": True,
            "input_hashes_exact": True,
            "only_dispatch_fix_present": True,
        ...
        "authority": {
            "one_hash_exact_preexecution_replacement": True,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T100B hidden-expert hosted package",
                "",
                f"- Status: `{contract['status']}`",
                "- Classification: `pre-optimizer replacement, not retry`",
