#!/usr/bin/env python3
"""Preregister the single no-retry T78 joint-adapter continuation."""
...
CPU_PREREG = (
    ANALYSIS / "t77_endpoint_joint_adapter_cpu_preregistration.json"
)
CPU_RESULT = ANALYSIS / "t77_endpoint_joint_adapter_cpu_result.json"
...
MARKDOWN = (
    ANALYSIS
    / "T78_ENDPOINT_JOINT_ADAPTER_HOSTED_PREREGISTRATION_20260728.md"
)
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T78: {path}")
    ...
    checks = {
        ...
        "actor_update_scope_exact": (
            cpu_result["checks"]["every_joint_adapter_leaf_updated"]
            and cpu_result["checks"]["every_base_actor_leaf_bit_exact"]
    ...
    basis = {
        "schema_version": (
            "open_duck.t78_endpoint_joint_adapter_hosted_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T78_ENDPOINT_JOINT_ADAPTER_HOSTED_CONTINUATION"
            if not failed
            else "HOLD_T78_ENDPOINT_JOINT_ADAPTER_HOSTED_PREREGISTRATION"
        ),
        "decision": (
            "AUTHORIZE_ONE_HASH_FROZEN_T78_L4_CONTINUATION_WITHOUT_RETRY"
            if not failed
            else "NO_T78_HOSTED_EXECUTION"
        ),
        "question": (
            "Does one continuation from the exact T52-half source, with "
            "nominal/broad/all-six isolated COM endpoint strata and the "
            "complete recurrent adapter trainable, produce two persistent "
            "nominal checkpoints without sacrificing the green gait?"
        ),
        ...
        "causal_basis": {
            ...
            "t67_result": (
                "core-only endpoint continuation was nominally clean but "
                "failed condition-7 persistence"
            ),
            "t71_result": (
                "COM information is present and causally used in hidden state"
            ),
            "t76_result": (
                "COM response is distributed rather than stable rank-one"
            ),
            "mechanism": (
                "co-adapt recurrent representation and adapter output head"
            ),
        },
        "training": {
            ...
            "all_base_actor_groups_frozen": True,
        ...
        "post_training": {
            ...
            "only_complete_adapter_and_critic_may_change": True,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T78 endpoint joint-adapter hosted preregistration",
                "",
                f"- Status: `{value['status']}`",
                "- Run budget: `2,007,040 steps; one L4; no retry/resume`",
                "- Exports: `0 / 1,003,520 / 2,007,040`",
                "- Population: `8 exact strata x 32 environments`",
                "- Actor updates: `complete recurrent adapter only`",
                "- Base actor / normalizer: `frozen / frozen`",
