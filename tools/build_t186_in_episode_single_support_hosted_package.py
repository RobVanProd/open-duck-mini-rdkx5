#!/usr/bin/env python3
"""Build T186's hash-frozen hosted continuation bundle."""
...
MARKDOWN = (
    ANALYSIS / "T186_IN_EPISODE_SINGLE_SUPPORT_HOSTED_PACKAGE_20260730.md"
)
BUNDLE_NAME = "t186_in_episode_single_support_bundle"
MANIFEST_NAME = "t186_package_manifest.json"
...
def main() -> int:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("T186 packaging requires clean worktree")
    ...
    if (
        ...
        raise RuntimeError("T186 preregistration identity changed")
    ...
    if (
        ...
        raise RuntimeError("T186 package inputs changed")
    ...
    if copied_inventory != inventory:
        raise RuntimeError("copied T186 playground changed")
    if (
        ...
        raise RuntimeError("copied T186 assets changed")
    ...
    preflight_source = "\n".join(
        (
            ...
            "    print('PASS_T186_FULL_IDENTITY_PREFLIGHT')",
            "else:",
    ...
    if (
        ...
        or "PASS_T186_FULL_IDENTITY_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T186 identity preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t186_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t186_hosted_package.v1",
        "status": "PASS_T186_IN_EPISODE_SINGLE_SUPPORT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
            ...
            "t185_and_eight_stratum_readback_frozen": True,
    ...
    MARKDOWN.write_text(
        "# T186 in-episode single-support hosted package\n\n"
