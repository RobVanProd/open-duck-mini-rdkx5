#!/usr/bin/env python3
"""Run T100C's preregistered 16-cell corrected nominal matrix."""
...
PREREG = ANALYSIS / "t102_t100c_nominal_matrix_preregistration.json"
RESULT = ANALYSIS / "t102_t100c_nominal_matrix_result.json"
RESULT_MD = ANALYSIS / "T102_T100C_NOMINAL_MATRIX_RESULT_20260728.md"
CACHE_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t102_t100c_nominal_matrix_v1"
)
...
def verify_preregistration(prereg: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError("changed T102 preregistration")
    if prereg["status"] != "PREREGISTERED_T102_T100C_NOMINAL_MATRIX":
        raise ValueError("T102 preregistration is not green")
    for item in prereg["frozen_inputs"].values():
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed frozen T102 input: {path}")
    for item in [
        *prereg["policies"],
        *prereg["fits"],
        prereg["calibrator"],
        ...
        path = Path(item["path"])
        if sha256(path) != item["sha256"]:
            raise ValueError(f"changed T102 evaluated input: {path}")
...
def require_clean_worktree() -> str:
    ...
    if status.strip():
        raise RuntimeError("formal T102 execution requires a clean worktree")
...
def write_markdown(result: Mapping[str, Any]) -> None:
    ...
    RESULT_MD.write_text(
        "\n".join(
            [
                "# T102 T100C nominal matrix result",
                "",
                ...
                "",
                "A pass authorizes only negative-endpoint preregistration; "
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "open_duck.t102_t100c_nominal_matrix_result.v1",
        "status": (
            "PASS_T102_T100C_NOMINAL_MATRIX"
            if passed
            else "HOLD_T102_T100C_NOMINAL_MATRIX"
        ),
        "decision": (
