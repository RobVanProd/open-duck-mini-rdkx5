#!/usr/bin/env python3
"""Execute the frozen Winner-v60 integrated numeric-guard continuation."""
...
import build_winner_v60_integrated_numeric_guard_training_preregistration as spec  # noqa: E402


CONTRACT = ANALYSIS / "winner_v60_integrated_numeric_guard_training_preregistration.json"


def validate_contract(value: Mapping[str, Any]) -> str:
    if (
        value.get("schema_version")
        != "winner_v60.integrated_first_tick_training_preregistration.v1"
        or value.get("status")
        != "PREREGISTERED_WINNER_V60_INTEGRATED_FIRST_TICK_TEACHER_TRAINING"
        or value.get("decision")
        != "AUTHORIZE_ONE_100_UPDATE_INTEGRATED_FIRST_TICK_TEACHER_ARM_ONLY"
        or value.get("frozen_training", {}).get("source_completed_updates") != 454
        or value.get("frozen_training", {}).get("continuation_optimizer_updates")
        != 100
        or value.get("frozen_training", {}).get("persistent_checkpoints")
        != {"half": 504, "final": 554}
        or value.get("frozen_training", {}).get("hidden_replay_numeric_guard")
        != spec.NUMERIC_REPLAY_GUARD
        or value.get("authority", {}).get("training_authorized") is not True
        or value.get("authority", {}).get("formal_support_gate_authorized")
        is not False
    ):
        raise ValueError("Winner-v60 contract changed")
    sources = value.get("sources")
    if not isinstance(sources, Mapping) or not sources:
        raise ValueError("Winner-v60 source manifest absent")
    for name, item in sources.items():
        if (
            item.get("hash_mode") != "lf"
            or spec.lf_sha256(ROOT / item["path"]) != item.get("sha256")
        ):
            raise ValueError(f"Winner-v60 source changed: {name}")
    if spec.canonical_sha256(sources) != value.get("source_manifest_sha256"):
        raise ValueError("Winner-v60 source-manifest digest changed")
    ...
    if (
        ...
        raise ValueError("Winner-v60 transformed source changed")
    return transformed
...
def main() -> int:
    ...
    parser.add_argument(
        "--integrated-first-tick-teacher-training-authorized", action="store_true"
    )
    args = parser.parse_args()
    if (
        not args.offline_cpu_only
        or not args.integrated_first_tick_teacher_training_authorized
    ):
        raise PermissionError(
            "Winner-v60 requires --offline-cpu-only "
            "--integrated-first-tick-teacher-training-authorized"
        )
    if args.work_root.exists() or args.output.exists():
        raise FileExistsError("refusing to overwrite Winner-v60 evidence")
    ...
    namespace: dict[str, Any] = {
        ...
        "__name__": "winner_v60_transformed_training",
    }
    ...
    result["numeric_guard_evidence"] = contract["numeric_guard_evidence"]
