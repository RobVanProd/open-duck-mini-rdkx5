#!/usr/bin/env python3
"""Build the hash-frozen T56 hosted continuation bundle."""
...
from colab_t56_dynamic_single_support_continuation import (
    CPU_RESULT_NAME,
...
MARKDOWN = (
    ANALYSIS
    / "T56_DYNAMIC_SINGLE_SUPPORT_HOSTED_PACKAGE_20260728.md"
)
BUNDLE_NAME = "t56_dynamic_single_support_bundle"
MANIFEST_NAME = "t56_package_manifest.json"
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("T56 preregistration changed")
    ...
    if (
        ...
        raise ValueError("T56 package inputs changed")
    ...
    if copied_inventory != observed_inventory:
        raise ValueError("copied T56 playground changed")
    if (
        directory_sha256(assets / SOURCE_CHECKPOINT_NAME)
        != observed["source_checkpoint"]
    ):
        raise ValueError("copied T56 source checkpoint changed")
    ...
    preflight = subprocess.run(
        [
            ...
            (
                ...
                "print('PASS_T56_PACKAGE_IMPORT', Joystick.__name__)"
            ),
        ],
        cwd=bundle,
        env=environment,
    ...
    if (
        ...
        or "PASS_T56_PACKAGE_IMPORT Joystick"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T56 isolated import failed: {preflight.stdout}\n"
            f"{preflight.stderr}"
        )

    members = file_manifest(bundle)
    manifest = {
        "schema_version": "open_duck.t56_hosted_bundle_manifest.v1",
    ...
    contract = {
        "schema_version": "open_duck.t56_hosted_package.v1",
        "status": "PASS_T56_DYNAMIC_SINGLE_SUPPORT_HOSTED_PACKAGE",
        "failed_checks": [],
        "checks": {
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T56 dynamic single-support hosted package",
                "",
