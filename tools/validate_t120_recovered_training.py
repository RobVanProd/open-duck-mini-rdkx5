#!/usr/bin/env python3
"""Validate recovered T120 training on CPU before behavior evaluation."""
...
RECOVERY_RESULT = ANALYSIS / "t119b_random_binding_recovery_result.json"
PACKAGE_CONTRACT = (
    ANALYSIS / "t120_joint_soft_router_hosted_package_contract.json"
)
LAUNCH_CONTRACT = ANALYSIS / "t120_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t121_t120_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T121_T120_RECOVERED_TRAINING_VALIDATION_20260729.md"
...
def split_deltas(source: Any, target: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "expert_actor_leaf_deltas": expert,
        "router_actor_leaf_deltas": router,
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T121: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T121 validation requires clean worktree")

    recovery_root = args.recovery_root.resolve()
    work = (
        args.extracted_root.resolve()
        / "t78_endpoint_joint_adapter_continuation"
    )
    training = work / "training"
    paths = {
        "hosted_result": recovery_root / "t120_result.json",
        "launch_receipt": recovery_root / "t120_launch_receipt.json",
        "recovery_archive": recovery_root / "t120_artifacts.tar.gz",
    }
    ...
    checks = {
        ...
        "package_and_launch_contracts_green": (
            ...
            and launch["status"] == "PASS_T120_COLAB_CLI_LAUNCH_CONTRACT"
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T120_COLAB_LAUNCH"
        ...
        "joint_soft_router_readback_exact": readback in log_text,
        "all_checkpoint_trees_finite": all(tree_finite(tree) for tree in trees),
        "both_exports_only_update_router_expert_and_critic": all(
            row["structure_exact"]
            and row["tree_finite"]
    ...
    result = {
        "schema_version": "open_duck.t121_t120_recovered_training_validation.v1",
        "status": (
            "PASS_T121_T120_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T121_T120_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T122_JOINT_SOFT_ROUTER_NOMINAL_PREREGISTRATION_ONLY"
            if not failed
        ...
        "input_hashes": {
            **input_hashes,
            "t120_preregistration": sha256(PREREGISTRATION),
        ...
        "classification": {
            ...
            "behavior_cells": 0,
            "actor_update_scope": (
                "negative_adapter_location_plus_soft_router_deltas"
            ),
            "forward_path": "joint_soft_router_times_negative_expert",
            "endpoint_strata": 8,
            "hosted_wall_seconds": hosted["wall_seconds"],
        },
        "authority": {
            "nominal_preregistration_authorized": not failed,
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T121 T120 recovered training validation",
                "",
                f"- Status: `{result['status']}`",
                f"- Decision: `{result['decision']}`",
                f"- Failed checks: `{failed}`",
                "- Exports: 0 / 1,003,520 / 2,007,040",
                "- Actor scope: negative expert + two router deltas",
