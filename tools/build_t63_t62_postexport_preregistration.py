#!/usr/bin/env python3
"""Preregister T62's exact transfer-stage deployment transforms."""
...
VALIDATION = ANALYSIS / "t62_recovered_training_validation.json"
T31_PREREGISTRATION = (
...
OUTPUT = ANALYSIS / "t63_t62_postexport_preregistration.json"
MARKDOWN = ANALYSIS / "T63_T62_POSTEXPORT_PREREGISTRATION_20260728.md"
RAW_ROOT = Path(
    "D:/CodexArtifacts/open-duck-policy/t62_extracted_20260728/"
    "t62_midpoint_gait_transfer_continuation/transfer_training"
)
EXPECTED_STEPS = [0, 1_003_520, 2_007_040]
SOURCE_PATHS = (
    "tools/run_t63_t62_postexport_transform.py",
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T63: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"],
        cwd=ROOT,
        text=True,
    ).strip():
        raise RuntimeError("T63 preregistration requires a clean worktree")
    ...
    validation_graphs = {
        ...
        for row in validation["stages"]["transfer"]["onnx"]
    }
    source_hashes = {name: sha256(ROOT / name) for name in SOURCE_PATHS}
    checks = {
        "t62_recovered_training_validation_green": (
            validation["status"]
        ...
        "exact_three_transfer_exports": steps == EXPECTED_STEPS,
    ...
    value: dict[str, Any] = {
        "schema_version": (
            "open_duck.t63_t62_postexport_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T63_T62_POSTEXPORT_TRANSFORM"
            if not failed
            else "HOLD_T63_T62_POSTEXPORT_TRANSFORM"
        ),
        "failed_checks": failed,
        "checks": checks,
        "input_hashes": {
            "t62_validation": sha256(VALIDATION),
        ...
        "golden_requirement": (
            ...
            "every deployment sub-contract on every T62 transfer export"
        ),
        "postupdate_steps": EXPECTED_STEPS[1:],
        "decision": (
            "RUN_ONE_CPU_ONLY_T63_POSTEXPORT_TRANSFORM"
            if not failed
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T63 T62 post-export preregistration",
                "",
                f"- Status: `{value['status']}`",
                f"- Decision: `{value['decision']}`",
                "- Three transfer exports are hash-exact to T62 CPU validation",
