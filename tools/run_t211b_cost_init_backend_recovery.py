#!/usr/bin/env python3
"""Resolve T211's sole cost-init hold at frozen float32 resolution."""
...
PREREG = ANALYSIS / "t211b_cost_init_backend_recovery_preregistration.json"
RESULT = ANALYSIS / "t211b_cost_init_backend_recovery_result.json"
MARKDOWN = ANALYSIS / "T211B_COST_INIT_BACKEND_RECOVERY_RESULT_20260730.md"
...
def main() -> int:
    ...
    for path in (RESULT, MARKDOWN):
        if path.exists():
            raise FileExistsError("refusing to overwrite T211B result")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T211B execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T211B_COST_INIT_BACKEND_RECOVERY"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T211B preregistration changed")
    for name in ("builder", "runner", "test", "t211_hold", "t209_cpu_result"):
    ...
    if (
        ...
        raise RuntimeError("T211B cost checkpoint identity changed")
    t211 = json.loads(
        Path(prereg["frozen_inputs"]["t211_hold"]["path"]).read_text(
            encoding="utf-8"
        )
    )
    ...
    checks = {
        ...
        "t211_exact_single_hold_preserved": (
            t211["status"]
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t211b_cost_init_backend_recovery_result.v1"
        ),
        "status": (
            "PASS_T211B_COST_INIT_BACKEND_RECOVERY"
            if not failed
            else "HOLD_T211B_COST_INIT_BACKEND_RECOVERY"
        ),
        "decision": (
            "RECOVER_T211_AND_EARN_T212_T210_POSTEXPORT_"
            "COMPOSITION_PREREGISTRATION_ONLY"
            if not failed
            else "HOLD_T210_WITHOUT_BEHAVIOR"
        ),
        ...
        "recovered_t211": {
            ...
            "original_result_sha256": t211["result_sha256"],
        },
        "execution": {
    ...
    MARKDOWN.write_text(
        "# T211B cost-init backend recovery result\n\n"
