#!/usr/bin/env python3
"""Freeze T62's 16-cell corrected nominal behavior matrix."""
...
TRANSFORM = ANALYSIS / "t63_t62_postexport_result.json"
T27_PREREG = ANALYSIS / "t27_t23_robustness_matrix_preregistration.json"
T27_RUNNER_CONTRACT = (
    ANALYSIS / "t27_t23_robustness_runner_contract.json"
)
RUNNER = ROOT / "tools" / "run_t64_t62_nominal_matrix.py"
WORKER = ROOT / "tools" / "evaluate_t27_t23_robustness_condition.py"
ADAPTER = ROOT / "tools" / "t27_state_coherent_eval_adapter.py"
TEST = ROOT / "tests" / "test_t64_t62_nominal_matrix.py"
OUTPUT = ANALYSIS / "t64_t62_nominal_matrix_preregistration.json"
OUTPUT_MD = (
    ANALYSIS / "T64_T62_NOMINAL_MATRIX_PREREGISTRATION_20260728.md"
)
...
def main() -> int:
    for path in (OUTPUT, OUTPUT_MD):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T64: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"],
        cwd=ROOT,
        text=True,
    ).strip():
        raise RuntimeError("T64 preregistration requires a clean worktree")
    ...
    checks = {
        "t63_transform_green": (
            transform["status"] == "PASS_T63_T62_POSTEXPORT_TRANSFORM"
            and transform["decision"]
            == "EARN_T64_NOMINAL_MATRIX_PREREGISTRATION"
        ),
    ...
    payload: dict[str, Any] = {
        "schema_version": (
            "open_duck.t64_t62_nominal_matrix_preregistration.v1"
        ),
        "status": (
            "PREREGISTERED_T64_T62_NOMINAL_MATRIX"
            if not failed
            else "HOLD_T64_T62_NOMINAL_MATRIX_PREREGISTRATION"
        ),
        "question": (
            "Do both T62 full-transfer checkpoints pass every command and "
        ...
        "matrix": {
            ...
            "maximum_cells": 16,
            "execution_order": (
                "default condition -> transfer half/final -> fit p30/"
        ...
        "frozen_inputs": {
            "t63_transform": receipt(TRANSFORM),
