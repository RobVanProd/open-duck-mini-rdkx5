#!/usr/bin/env python3
"""Execute T210's exact package in one fresh Colab L4 session."""
...
PACKAGE = Path("/content/t210-dual-roll-cost-20260730.tar.gz")
PACKAGE_BYTES = 4_349_714
PACKAGE_SHA256 = (
    "ac17c7469ae74f240bfc66390aa45b081ea59f368300d6564c683bcf690f1737"
)
BUNDLE_NAME = "t210_dual_roll_cost_bundle"
MANIFEST_NAME = "t210_package_manifest.json"
MANIFEST_SHA256 = (
    "c400ae4459e96685c4bf59b400f8b3f530884b0ea1756e2e529946bb5963c051"
)
WORK = Path("/content/t210_work")
EXTRACT = Path("/content/t210_launch")
OUTPUT_JSON = Path("/content/t210_result.json")
OUTPUT_ARCHIVE = Path("/content/t210_artifacts.tar.gz")
RECEIPT = Path("/content/t210_launch_receipt.json")
EXPECTED_VERSIONS = {
    "brax": "0.14.2",
    "flax": "0.11.2",
    "jax": "0.7.2",
...
def verify_manifest(bundle: Path) -> dict[str, Any]:
    ...
    if sha256(manifest_path) != MANIFEST_SHA256:
        raise ValueError("T210 extracted manifest changed")
    ...
    if (
        ...
        raise ValueError("T210 manifest identity changed")
    for relative, expected in members.items():
        ...
        if (
            ...
            raise ValueError(f"T210 bundle member changed: {relative}")
    return manifest
...
def main() -> int:
    ...
    if gpu.returncode != 0 or "L4" not in gpu.stdout:
        raise RuntimeError(f"T210 requires L4: {gpu.stdout}{gpu.stderr}")
    for path in (WORK, EXTRACT, OUTPUT_JSON, OUTPUT_ARCHIVE, RECEIPT):
        if path.exists():
            raise FileExistsError(f"T210 no-retry path exists: {path}")
    if (
        PACKAGE.stat().st_size != PACKAGE_BYTES
        or sha256(PACKAGE) != PACKAGE_SHA256
    ):
        raise ValueError("T210 uploaded package changed")
    ...
    with tarfile.open(PACKAGE, "r:gz") as archive:
        if any(
            ...
            raise ValueError("T210 package contains playground/.tmp")
    ...
    if install.returncode != 0:
        raise RuntimeError(f"T210 pinned install failed: {install.returncode}")
    versions = {
        name: importlib.metadata.version(name)
        for name in EXPECTED_VERSIONS
    }
    if versions != EXPECTED_VERSIONS:
        raise ValueError(f"T210 software versions changed: {versions}")
    ...
    preflight_source = "\n".join(
        (
            "from pathlib import Path",
            "import colab_t210_dual_roll_cost_continuation as driver",
            ...
            "assert cmd.count('--winner_t209_dual_roll_cost') == 1",
    ...
    if (
        ...
        or "PASS_T210_HOSTED_FULL_PREFLIGHT"
        not in preflight.stdout.splitlines()
    ):
        raise RuntimeError(
            f"T210 hosted preflight failed:\n{preflight.stdout}\n"
            f"{preflight.stderr}"
        )
    driver = bundle / "colab_t210_dual_roll_cost_continuation.py"
    command = [
        sys.executable,
        str(driver),
        "--bundle-root",
    ...
    receipt = {
        "schema_version": "open_duck.t210_colab_launch_receipt.v1",
        "status": "STARTING_T210_HASH_FROZEN_GPU_CONTINUATION",
    ...
    receipt["status"] = (
        "COMPLETED_T210_COLAB_LAUNCH"
        if result.returncode == 0
        else "HOLD_T210_COLAB_LAUNCH"
    )
    for name, path in (
