#!/usr/bin/env python3
"""Run the single frozen T129 exact-negative-only expert continuation."""
...
PREREGISTRATION_NAME = (
    "t129_negative_only_expert_hosted_preregistration.json"
)
CPU_RESULT_NAME = "t128_negative_only_expert_cpu_result.json"
...
def runner_command(
    ...
    """Use the frozen recipe with T128's exact-negative expert path."""
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T129 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T129 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T129 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(work, arcname="t129_negative_only_expert_continuation")
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T129 requires hosted GPU authorization")
    return run(
