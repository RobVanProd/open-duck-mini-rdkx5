#!/usr/bin/env python3
"""Freeze exact composition of T186 heads with T164 deployment repairs."""
...
OUTPUT = ANALYSIS / "t188_t186_postexport_composition_preregistration.json"
MARKDOWN = (
    ANALYSIS / "T188_T186_POSTEXPORT_COMPOSITION_PREREGISTRATION_20260730.md"
)
...
def main() -> int:
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError("refusing to overwrite T188 preregistration")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T188 preregistration requires clean worktree")
    paths = {
    ...
    t187_graphs = {
        int(row["step"]): row for row in values["t187"]["exports"]["onnx"]
    }
    t187_checkpoints = {
        int(row["step"]): row
        for row in values["t187"]["exports"]["checkpoints"]
    }
    t164_final = next(
    ...
    graphs = [
        {
            "step": step,
            "raw": {
                "path": f"{t187_checkpoints[step]['path']}.onnx",
                "bytes": Path(
                    f"{t187_checkpoints[step]['path']}.onnx"
                ).stat().st_size,
                "sha256": t187_graphs[step]["sha256"],
            },
            "base": t164_final,
        }
    ...
    frozen = {
        ...
        "runner": receipt(
            ROOT / "tools" / "run_t188_t186_postexport_composition.py"
        ),
        "test": receipt(
            ROOT / "tests" / "test_t172_t170_postexport_composition.py"
        ),
    ...
    checks = {
        "t187_recovery_green": (
            ...
            and not values["t187"]["failed_checks"]
            and values["t187"]["decision"]
        ...
        "three_raw_exports_exact": (
            sorted(t187_graphs) == [0, 1_003_520, 2_007_040]
            and all(Path(row["raw"]["path"]).is_file() for row in graphs)
        ),
        "single_frozen_t164_final_base": (
            all(row["base"]["sha256"] == t164_final["sha256"] for row in graphs)
    ...
    basis: dict[str, Any] = {
        "schema_version": (
            "open_duck.t188_t186_postexport_composition_"
            "preregistration.v1"
        ),
        "status": "PREREGISTERED_T188_T186_POSTEXPORT_COMPOSITION",
        "question": (
            "Can each T186 raw checkpoint replace only T164-final's nominal "
        ...
        "composition": {
            ...
            "base": "T164_COMPOSED_FINAL_FOR_ALL_THREE_T186_EXPORTS",
    ...
    MARKDOWN.write_text(
        "# T188 T186 post-export composition preregistration\n\n"
        f"- Status: `{value['status']}`\n"
        f"- Failed checks: `{failed}`\n"
        "- Three raw T186 graphs; one frozen T164-final deployment base\n"
