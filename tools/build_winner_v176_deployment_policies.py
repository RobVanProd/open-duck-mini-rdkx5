#!/usr/bin/env python3
"""Apply the exact V121 deployment hierarchy to both V175 checkpoints."""
...
PREREG = ANALYSIS / "winner_v176_deployment_transform_preregistration.json"
CONTRACT = ANALYSIS / "winner_v176_deployment_transform_contract.json"
MARKDOWN = (
    ANALYSIS / "WINNER_V176_DEPLOYMENT_TRANSFORM_CONTRACT_20260725.md"
)
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("V176 transform preregistration changed")
    ...
    for spec in prereg["sources"]:
        ...
        if (
            ...
            raise ValueError(f"raw V175 source changed: {spec['id']}")
    ...
    payload = {
        "schema_version": "winner_v176.deployment_transform_contract.v1",
        "status": (
            "PASS_WINNER_V176_DEPLOYMENT_TRANSFORM_CONTRACT"
            if not failed
            else "HOLD_WINNER_V176_DEPLOYMENT_TRANSFORM_CONTRACT"
        ),
    ...
    MARKDOWN.write_text(
        "# Winner V176 deployment transform\n\n"
        f"- Status: `{payload['status']}`\n"
        "- Both V175 post-update graphs use the exact V121 hierarchy.\n"
