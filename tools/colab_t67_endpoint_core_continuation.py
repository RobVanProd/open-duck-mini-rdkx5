#!/usr/bin/env python3
"""Run the one frozen T67 endpoint-stratified recurrent-core continuation."""
...
PREREGISTRATION_NAME = "t67_endpoint_core_hosted_preregistration.json"
CPU_RESULT_NAME = "t66_endpoint_core_cpu_result.json"
...
def validate_inputs(bundle: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T67 prerequisite status or identity changed")
    ...
    if (
        ...
        raise ValueError("T67 frozen inputs changed")
    ...
    if (
        ...
        raise RuntimeError(f"T67 requires one GPU process: {devices}")
    return {
...
def make_artifact(work: Path, destination: Path) -> dict[str, Any]:
    ...
    with tarfile.open(temporary, "w:gz") as archive:
        archive.add(work, arcname="t67_endpoint_core_continuation")
...
def run(
    ...
    payload: dict[str, Any] = {
        "schema_version": "open_duck.t67_endpoint_core_hosted_result.v1",
        "status": "RUNNING_T67_ENDPOINT_CORE_HOSTED_CONTINUATION",
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"T67 runner returned {completed.returncode}")
        ...
        payload.update(
            {
                "status": (
                    ...
                    if not failed
                    else "HOLD_T67_ENDPOINT_CORE_CONTINUATION"
                ),
                "checks": checks,
        ...
        if failed:
            raise ValueError(f"T67 artifact checks failed: {failed}")
        ...
        payload["status"] = "HOLD_T67_ENDPOINT_CORE_CONTINUATION"
...
def main() -> int:
    ...
    if not args.hosted_gpu_authorized:
        raise PermissionError("T67 requires hosted GPU authorization")
    return run(
