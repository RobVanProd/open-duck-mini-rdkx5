#!/usr/bin/env python3
"""Validate recovered T113 training on CPU before behavior evaluation."""
...
LAUNCH_CONTRACT = ANALYSIS / "t113_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t114_t113_recovered_training_validation.json"
MARKDOWN = ANALYSIS / "T114_T113_RECOVERED_TRAINING_VALIDATION_20260729.md"
...
def split_deltas(source: Any, target: Any) -> dict[str, Any]:
    ...
    frozen = {
        name: value for name, value in policy.items() if name not in expert
    }
    normalizer = {
    ...
    return {
        ...
        "every_expert_leaf_updated": bool(expert)
        and all(value > 0.0 for value in expert.values()),
        "every_mature_actor_leaf_exact": bool(frozen)
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T114: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T114 validation requires clean worktree")
    ...
    paths = {
        "hosted_result": recovery / "t113_result.json",
        "launch_receipt": recovery / "t113_launch_receipt.json",
        "recovery_archive": recovery / "t113_artifacts.tar.gz",
    }
    ...
    checks = {
        ...
        "package_and_launch_contracts_green": (
            ...
            and launch["status"] == "PASS_T113_COLAB_CLI_LAUNCH_CONTRACT"
        ...
        "launch_receipt_complete": (
            receipt["status"] == "COMPLETED_T113_COLAB_LAUNCH"
    ...
    result = {
        "schema_version": "open_duck.t114_t113_recovered_training_validation.v1",
        "status": (
            "PASS_T114_T113_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T114_T113_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
        ...
        "input_hashes": {
            **input_hashes,
            "t113_preregistration": sha256(PREREGISTRATION),
    ...
    MARKDOWN.write_text(
        "\n".join(
            [
                "# T114 T113 recovered training validation",
                "",
