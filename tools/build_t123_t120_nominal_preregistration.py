#!/usr/bin/env python3
"""Freeze the T120 joint soft-router 16-cell nominal matrix."""
...
T122_RESULT = ANALYSIS / "t122_t120_postexport_result.json"
BASIS = ANALYSIS / "t107_soft_gate_nominal_preregistration.json"
BUILDER = ROOT / "tools" / Path(__file__).name
RUNNER = ROOT / "tools" / "run_t123_t120_nominal.py"
TEST = ROOT / "tests" / "test_t123_t120_nominal.py"
OUTPUT = ANALYSIS / "t123_t120_nominal_preregistration.json"
MARKDOWN = ANALYSIS / "T123_T120_NOMINAL_PREREGISTRATION_20260729.md"
...
def main() -> int:
    ...
    if not args.read_only_authorized:
        raise PermissionError("T123 preregistration requires authorization")
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T123: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T123 preregistration requires clean worktree")
    t122 = json.loads(T122_RESULT.read_text(encoding="utf-8"))
    basis = json.loads(BASIS.read_text(encoding="utf-8"))
    policies = []
    for checkpoint_id, step in (
        ("T120_JOINT_SOFT_ROUTER_HALF", "1003520"),
        ("T120_JOINT_SOFT_ROUTER_FINAL", "2007040"),
    ):
        policies.append(
            {
                "checkpoint_id": checkpoint_id,
                "step": int(step),
                **t122["deployments"][step]["wrapped"],
            }
        )
    inputs = {
        "builder": BUILDER,
    ...
    value: dict[str, Any] = {
        "schema_version": "open_duck.t123_t120_nominal_preregistration.v1",
        "status": (
            "PREREGISTERED_T123_T120_NOMINAL_MATRIX"
            if not failed
            else "HOLD_T123_T120_NOMINAL_PREREGISTRATION"
        ),
        ...
        "decision_rule": {
            ...
            "fail_decision": "CLOSE_T120_JOINT_SOFT_ROUTER_TRAINTHROUGH",
            "no_retry": True,
    ...
    MARKDOWN.write_text(
        "# T123 T120 nominal preregistration\n\n"
