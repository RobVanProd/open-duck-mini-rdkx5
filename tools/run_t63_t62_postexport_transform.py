#!/usr/bin/env python3
"""Apply T62's frozen deployment chain to all transfer exports."""
...
PREREGISTRATION = ANALYSIS / "t63_t62_postexport_preregistration.json"
RESULT = ANALYSIS / "t63_t62_postexport_result.json"
MARKDOWN = ANALYSIS / "T63_T62_POSTEXPORT_RESULT_20260728.md"
...
def main() -> int:
    ...
    if not args.execute:
        raise PermissionError("T63 exact transform requires --execute")
    ...
    if raw_steps != EXPECTED_STEPS or raw_hashes != prereg["raw_graphs"]:
        raise ValueError("T63 raw export inventory changed")
    ...
    value = {
        "schema_version": "open_duck.t63_t62_postexport_result.v1",
        "status": (
            "PASS_T63_T62_POSTEXPORT_TRANSFORM"
            if not failed
