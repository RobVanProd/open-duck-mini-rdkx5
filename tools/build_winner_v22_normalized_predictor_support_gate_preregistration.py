#!/usr/bin/env python3
"""Freeze the corrected-coordinate support/context gate for Winner-v22."""
...
OUTPUT = ANALYSIS / "winner_v22_normalized_predictor_support_gate_preregistration.json"
MARKDOWN = ANALYSIS / "WINNER_V22_NORMALIZED_PREDICTOR_SUPPORT_GATE_PREREGISTRATION_20260721.md"
...
SOURCES = {
    "builder": Path("tools/build_winner_v22_normalized_predictor_support_gate_preregistration.py"),
    "gate_runner": Path("tools/run_winner_v22_normalized_predictor_support_gate.py"),
    "gate_tests": Path("tests/test_winner_v22_normalized_predictor_support_gate.py"),
    "preregistration_tests": Path("tests/test_winner_v22_normalized_predictor_support_gate_preregistration.py"),
    "result_importer": Path("tools/import_winner_v22_normalized_predictor_support_gate_result.py"),
    "result_importer_tests": Path("tests/test_winner_v22_normalized_predictor_support_gate_import.py"),
    "workflow": Path(".github/workflows/winner-v22-normalized-predictor-support-gate.yml"),
    "training_result": Path("outputs/analysis/winner_v22_normalized_predictor_training_result.json"),
    "training_preregistration": Path("outputs/analysis/winner_v22_normalized_predictor_training_preregistration.json"),
    "training_runner": Path("tools/run_winner_v22_normalized_predictor_training.py"),
    "training_importer": Path("tools/import_winner_v22_normalized_predictor_training.py"),
    "predictor_mechanics": Path("patches/winner_v22_normalized_predictor.py"),
    ...
    "semantics_attribution": Path("outputs/analysis/winner_v21_normalized_semantics_attribution.json"),
...
def main() -> int:
    ...
    if args.output.exists() or args.markdown.exists():
        raise FileExistsError("refusing to overwrite Winner-v22 support-gate contract")
    ...
    if (
        ...
        or training.get("predictor_scale") != 380.9135437011719
    ):
        raise ValueError("Winner-v22 training does not authorize a gate")
    ...
    if (
        ...
        raise ValueError("Winner-v22 training attribution changed")
    checkpoints = training.get("persistent_checkpoints", [])
    if [row.get("label") for row in checkpoints] != ["half", "final"]:
        raise ValueError("Winner-v22 checkpoints changed")
    ...
    payload = {
        "schema_version": "winner_v22.normalized_predictor_support_gate_preregistration.v1",
        ...
        "binding_change_only": (
            ...
            "Winner-v22 paths are bound read-only and its normalized auxiliary head is projected "
            "exactly into the raw coordinates expected by the reviewed scoring formula."
        ),
        "training_artifact": {
