#!/usr/bin/env python3
"""Validate recovered T129 training on CPU before any behavior screen."""
...
PACKAGE_CONTRACT = (
    ANALYSIS / "t129_negative_only_expert_hosted_package_contract.json"
)
LAUNCH_CONTRACT = ANALYSIS / "t129_colab_cli_launch_contract.json"
OUTPUT = ANALYSIS / "t130_t129_recovered_training_validation.json"
MARKDOWN = (
    ANALYSIS / "T130_T129_RECOVERED_TRAINING_VALIDATION_20260729.md"
)
...
def main() -> int:
    ...
    for path in (OUTPUT, MARKDOWN):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite T130: {path}")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("formal T130 validation requires clean worktree")

    recovery_root = args.recovery_root.resolve()
    work = (
        args.extracted_root.resolve()
        / "t129_negative_only_expert_continuation"
    )
    training = work / "training"
    paths = {
        "hosted_result": recovery_root / "t129_result.json",
        "launch_receipt": recovery_root / "t129_launch_receipt.json",
        "recovery_archive": recovery_root / "t129_artifacts.tar.gz",
    }
    ...
    checks = {
        ...
        "exact_negative_readback": readback in log_text,
    ...
    result = {
        "schema_version": (
            "open_duck.t130_t129_recovered_training_validation.v1"
        ),
        "status": (
            "PASS_T130_T129_RECOVERED_TRAINING_VALIDATION"
            if not failed
            else "HOLD_T130_T129_RECOVERED_TRAINING_VALIDATION"
        ),
        "decision": (
            "EARN_T131_T129_HARD_GATE_POSTEXPORT_PREREGISTRATION_ONLY"
            if not failed
        ...
        "input_hashes": {
            **input_hashes,
            "t129_preregistration": sha256(PREREGISTRATION),
    ...
    MARKDOWN.write_text(
        "# T130 T129 recovered training validation\n\n"
        f"- Status: `{result['status']}`\n"
        f"- Decision: `{result['decision']}`\n"
        f"- Failed checks: `{failed}`\n"
        "- Exports: 0 / 1,003,520 / 2,007,040\n"
        "- Actor scope: existing linear negative expert only\n"
