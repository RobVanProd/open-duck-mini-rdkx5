#!/usr/bin/env python3
"""Compose V171's read-only actor-gradient geometry audit source."""
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to reuse V171 tree: {output}")
    base_manifest = base / "WINNER_V127_COMPOSED_SOURCE_MANIFEST.json"
    ...
    destination = (
        output / "playground/common/winner_v171_gradient_geometry.py"
    )
    ...
    manifest = {
        "schema_version": "winner_v171.gradient_geometry_source.v1",
        "base": {
            "path": str(base),
    ...
    manifest_path = output / "WINNER_V171_COMPOSED_SOURCE_MANIFEST.json"
