#!/usr/bin/env python3
"""Run T210's frozen Y-negative persistence matrix."""
...
PREREG = ANALYSIS / "t214_t210_targeted_y_negative_preregistration.json"
RESULT = ANALYSIS / "t214_t210_targeted_y_negative_result.json"
MARKDOWN = ANALYSIS / "T214_T210_TARGETED_Y_NEGATIVE_RESULT_20260730.md"
CACHE = Path(
    "D:/CodexArtifacts/open-duck-policy/t214_t210_targeted_y_negative_v1"
)
...
def verify(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"changed T214 input: {path}")
...
def main() -> int:
    ...
    if RESULT.exists() or MARKDOWN.exists() or CACHE.exists():
        raise FileExistsError("refusing to overwrite T214 output")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).strip():
        raise RuntimeError("T214 execution requires clean worktree")
    ...
    if (
        prereg["status"]
        != "PREREGISTERED_T214_T210_TARGETED_Y_NEGATIVE"
        or prereg["failed_checks"]
        or canonical_sha256(basis)
        != prereg["preregistered_contract_sha256"]
    ):
        raise RuntimeError("T214 preregistration changed")
    ...
    basis_result: dict[str, Any] = {
        "schema_version": (
            "open_duck.t214_t210_targeted_y_negative_result.v1"
        ),
        "status": (
            "PASS_T214_T210_TARGETED_Y_NEGATIVE"
            if passed
            else "HOLD_T214_T210_TARGETED_Y_NEGATIVE"
        ),
        "decision": (
    ...
    MARKDOWN.write_text(
        "# T214 T210 targeted Y-negative result\n\n"
