# Ground-Up Policy Evaluation

status: `PASS_GAIT_EMERGENCE_CHECKPOINT`
execution: `CPU_ONLY`

## Aggregate

- runs: `4`
- moving_runs: `3`
- zero_command_runs: `1`
- moving_emergence_pass: `True`
- zero_command_finite_recorded: `True`
- checkpoint_emergence_pass: `True`

## Runs

| command x | seed | evaluator | emergence | termination | dx m | transitions | reasons |
|---:|---:|---|---|---|---:|---:|---|
| 0.0 | 167931544 | `PASS_CANDIDATE_SIM_GATE` | `True` | `duration_complete` | -0.007895044769157736 | 0 | none |
| 0.074 | 167931544 | `HOLD_CANDIDATE_TARGET_VELOCITY` | `True` | `duration_complete` | 1.0264224316645414 | 158 | none |
| 0.077 | 167931544 | `HOLD_CANDIDATE_TARGET_VELOCITY` | `True` | `duration_complete` | 1.1603939991025254 | 145 | none |
| 0.08 | 167931544 | `HOLD_CANDIDATE_TARGET_VELOCITY` | `True` | `duration_complete` | 1.2469716114550828 | 126 | none |
