# Phase-Modulated BC Student

status: `PASS_PHASE_MODULATED_BC_FIT_SMOKE`

Offline behavior cloning with a shared trunk and command/phase modulation.
No robot tests, SSH, deploy, runtime behavior changes, or PPO training were performed.

## Inputs

- manifest: `outputs/analysis/phase2_z0075_iter10_spike_local_merged_manifest.json`
- samples: `52924`
- context indices: `[6, 99, 100]`
- trunk hidden sizes: `[512, 256]`
- context hidden sizes: `[64]`
- modulation scale: `0.5`

## Outputs

- NPZ: `outputs/analysis/phase2_z0075_spike_local_rate150_candidate/candidate_mlp.npz`
- ONNX: `policy/candidates/phase2_z0075_spike_local_rate150_20260704/candidate.onnx`

## Fit Metrics

- MAE: `0.009697`
- p95 abs error: `0.031047`
- max abs error: `0.417011`
- target-rate p95: `1.364341` rad/s
- target-rate max: `3.030199` rad/s

## ONNX Verification

- samples checked: `64`
- p95 abs error: `0.00000012`
- max abs error: `0.00000027`
